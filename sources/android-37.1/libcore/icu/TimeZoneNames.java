    ...
    private static class ZoneStringsCache extends BasicLruCache<Locale, String[][]> {
        ...
        private void addOffsetStrings(String[][] result) {
            for (int i = 0; i < result.length; ++i) {
                // Using ICU's TimeZone as it has a dedicated getOffset method which returns both
                // standard and DST (if there is any) offsets for a given timestamp.
                // j.u.TimeZone getDSTSavings() might return different value. See its javadoc.
                android.icu.util.TimeZone tz = android.icu.util.TimeZone.getTimeZone(result[i][0]);
                ...
                for (int j = 1; j < NAME_COUNT; ++j) {
                    ...
                    tz.getOffset(/*date=*/ now, /*local time=*/ false, offsets);
                    int offsetMillis = offsets[0];
                    if (j == LONG_NAME_DST || j == SHORT_NAME_DST) {
                        offsetMillis += offsets[1];
                    }
