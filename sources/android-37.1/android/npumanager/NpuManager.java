...
public final class NpuManager {
    ...
    class ModelLoadCallbackWrapper extends IModelLoadCallback.Stub {
        ...
        /**
         * The app should unload the model corresponding to this request.
         */
        @Hide
        @RequiresNoPermission
         ...
         * The app should unload the model corresponding to this request.
         *
         * @param reason The reason for the unload request.
         */
        @Hide
        @RequiresNoPermission
        public void onRequestUnloadModelWithReason(@NpuModelUnloadReason int reason) {
            mExecutor.execute(
                    () -> {
                        Trace.beginAsyncSection(
                                "NpuManager#onRequestUnloadModelWithReason", mRequest.getId());
                        mCallback.onRequestUnloadModel(mRequest, reason);
                        mCallback.onRequestUnloadModel(mRequest);
                    });
        }
    ...
    @IntDef(
            ...
            value = {
                ...
                NPU_MODEL_PRIORITY_OPPORTUNISTIC,
            })
    @Retention(RetentionPolicy.SOURCE)
    ...
    /**
     * Opportunistic priority models requested for jobs that should either be run right away or not
     * at all.
     */
    @SystemApi
    @FlaggedApi(com.android.npumanager.Flags.FLAG_OPPORTUNISTIC_PRIORITY)
    public static final int NPU_MODEL_PRIORITY_OPPORTUNISTIC =
            ModelLoadRequestParcelable.PRIORITY_OPPORTUNISTIC;
    ...
    @SystemApi
    @FlaggedApi(com.android.npumanager.Flags.FLAG_REQUEST_UNLOAD_REASON)
    @IntDef(
            prefix = {"NPU_MODEL_UNLOAD_REASON_"},
            value = {
                NPU_MODEL_UNLOAD_REASON_PREEMPTED,
                NPU_MODEL_UNLOAD_REASON_YIELD_IF_IDLE,
            })
    @Retention(RetentionPolicy.SOURCE)
    @interface NpuModelUnloadReason {}

    /**
     * The model is being preempted because a higher priority model is being loaded.
     */
    @SystemApi
    @FlaggedApi(com.android.npumanager.Flags.FLAG_REQUEST_UNLOAD_REASON)
    public static final int NPU_MODEL_UNLOAD_REASON_PREEMPTED = 0;

    /**
     * The model is being asked to unload if it is currently idle.
     */
    @SystemApi
    @FlaggedApi(com.android.npumanager.Flags.FLAG_REQUEST_UNLOAD_REASON)
    public static final int NPU_MODEL_UNLOAD_REASON_YIELD_IF_IDLE = 1;
