...
public final class MediaStore {
     ...
     * For {@link MediaStore#ACTION_PICK_IMAGES}, using this intent extra does not guarantee that
     ...
     * <p>
     * <strong>Note:</strong> This method only returns deleted items for external volumes.
     * Items deleted from the internal volume are not tracked or returned by this API.
     * <p>
     * <strong>Note:</strong> If an external volume (such as an SD card) is removed from the device,
     * its associated deleted items are no longer tracked or returned by this API.
     * <p>
     * The returned cursor includes columns such as {@link DeletedFiles.DeletedFilesColumns#_ID},
     * {@link DeletedFiles.DeletedFilesColumns#MEDIA_TYPE},
     * {@link DeletedFiles.DeletedFilesColumns#DELETED_GENERATION}, and
     * {@link DeletedFiles.DeletedFilesColumns#VOLUME_NAME}.
     *
     * @param resolver The ContentResolver to use for the call.
     * @param queryArgs Arguments for the query. Supported arguments include
     ...
     *            {@link ContentResolver#QUERY_ARG_LIMIT}, and
     *            {@link ContentResolver#QUERY_ARG_OFFSET}. The selection and sort order can
     *            filter and sort on any of the columns defined in
     *            {@link DeletedFiles.DeletedFilesColumns}.
     * @param signal A cancellation signal to cancel the operation in progress.
     * @return A cursor of deleted items, or null if the operation failed.
     * @see DeletedFiles.DeletedFilesColumns
     */
    @FlaggedApi(Flags.FLAG_ENABLE_DELETED_FILES_API)
    public static @Nullable Cursor queryDeletedFiles(@NonNull ContentResolver resolver,
            @Nullable Bundle queryArgs, @Nullable CancellationSignal signal) {
        return resolver.query(DeletedFiles.getContentUri(VOLUME_EXTERNAL),
                /* projection= */ null, queryArgs, signal);
    }
     ...
     */
    @FlaggedApi(Flags.FLAG_ENABLE_DELETED_FILES_API)
    public static final class DeletedFiles {
        private DeletedFiles() {}

        @Hide
        public static final String TABLE = "deleted_files";

        /**
         * Get the content:// style URI for the deleted_files table on the
         * given volume.
         *
         * @param volumeName the name of the volume to get the URI for
         * @return the URI to the deleted files table on the given volume
         */
         ...
         */
        @FlaggedApi(Flags.FLAG_ENABLE_DELETED_FILES_API)
        public interface DeletedFilesColumns extends BaseColumns {
            /** The media type of the deleted file. */
            @Column(value = Cursor.FIELD_TYPE_INTEGER, readOnly = true)
            String MEDIA_TYPE = Files.FileColumns.MEDIA_TYPE;

            /** The generation at which the file was deleted. */
            @Column(value = Cursor.FIELD_TYPE_INTEGER, readOnly = true)
            String DELETED_GENERATION = "deleted_generation";

            /** The volume name of the deleted file. */
            @Column(value = Cursor.FIELD_TYPE_STRING, readOnly = true)
            String VOLUME_NAME = Files.FileColumns.VOLUME_NAME;
        }
    }
    ...
    public static final class Images {
        ...
        public interface ImageColumns extends MediaColumns {
            ...
            /**
             * Flag indicating if the image file contains a gain map or not
             * (e.g., Ultra HDR or ISO 21496-1).
             */
            @FlaggedApi(com.android.providers.media.flags.Flags.FLAG_ENABLE_GAINMAP_DETECTION)
            @Column(value = Cursor.FIELD_TYPE_INTEGER, readOnly = true)
            public static final String HAS_GAIN_MAP = "has_gain_map";
        }
