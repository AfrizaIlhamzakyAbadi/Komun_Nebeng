...
public abstract class DocumentsProvider extends ContentProvider {
    ...
    private static final int MATCH_SEARCH_DOCUMENT = 10;
    private static final int MATCH_SEARCH_TREE = 11;
    ...
    private void registerAuthority(String authority) {
        ...
        mMatcher.addURI(mAuthority, "document/*/search", MATCH_SEARCH_DOCUMENT);
        mMatcher.addURI(mAuthority, "tree/*/document/*", MATCH_DOCUMENT_TREE);
        mMatcher.addURI(mAuthority, "tree/*/document/*/children", MATCH_CHILDREN_TREE);
        mMatcher.addURI(mAuthority, "tree/*/document/*/search", MATCH_SEARCH_TREE);
    }
    ...
    private void enforceSearchByDocumentArguments(String documentId) throws FileNotFoundException {
        try (Cursor cursor =
                queryDocument(
                        documentId,
                        new String[] {Document.COLUMN_MIME_TYPE, Document.COLUMN_FLAGS})) {
            if (cursor != null && cursor.moveToFirst()) {
                ...
                if ((flags & Document.FLAG_SUPPORTS_SUBTREE_SEARCH) == 0) {
                    throw new IllegalArgumentException("Parent must support subtree search");
                }
            } else {
                throw new FileNotFoundException("Failed to find document " + documentId);
            }
        }
    }
     ...
     *
     * <p>This method is called for root-level searches. It requires the root to advertise {@link
     * Root#FLAG_SUPPORTS_SEARCH}.
     *
     * <p>If your provider is cloud-based, and you have some data cached or pinned locally, you may
     * return the local data immediately, setting {@link DocumentsContract#EXTRA_LOADING} on the
     * Cursor to indicate that you are still fetching additional data. Then, when the network data
     * is available, you can send a change notification to trigger a requery and return the complete
     * contents.
     *
     * <p>To support change notifications, you must {@link
     * Cursor#setNotificationUri(ContentResolver, Uri)} with a relevant Uri, such as {@link
     * DocumentsContract#buildSearchDocumentsUri(String, String, String)}. Then you can call {@link
     * ContentResolver#notifyChange(Uri, android.database.ContentObserver, boolean)} with that Uri
     * to send change notifications.
     *
     * @param rootId the root to search under.
     * @param projection list of {@link Document} columns to put into the cursor. If {@code null}
     *     all supported columns should be included.
     * @param queryArgs the query arguments. {@link DocumentsContract#QUERY_ARG_EXCLUDE_MEDIA},
     *     {@link DocumentsContract#QUERY_ARG_DISPLAY_NAME}, {@link
     *     DocumentsContract#QUERY_ARG_MIME_TYPES}, {@link
     *     DocumentsContract#QUERY_ARG_FILE_SIZE_OVER}, {@link
     ...
     * @see DocumentsContract#EXTRA_INFO
     * @see DocumentsContract#EXTRA_ERROR
     * @see Root#FLAG_SUPPORTS_SEARCH
     */
    @SuppressWarnings("unused")
    @Nullable
    public Cursor querySearchDocuments(
            @NonNull String rootId, @Nullable String[] projection, @NonNull Bundle queryArgs)
            throws FileNotFoundException {
        Preconditions.checkNotNull(rootId, "rootId can not be null");
    ...
    /**
     * Return documents that match the given query under the requested directory. The returned
     * documents should be sorted by relevance in descending order. How documents are matched
     * against the query string is an implementation detail left to each provider, but it's
     * suggested that at least {@link Document#COLUMN_DISPLAY_NAME} be matched in a case-insensitive
     * fashion.
     *
     * <p>If your provider is cloud-based, and you have some data cached or pinned locally, you may
     * return the local data immediately, setting {@link DocumentsContract#EXTRA_LOADING} on the
     ...
     * <p>To support change notifications, you must {@link
     * Cursor#setNotificationUri(ContentResolver, Uri)} with a relevant Uri, such as {@link
     * DocumentsContract#buildSearchDocumentsUri(Uri, String)}. Then you can call {@link
     * ContentResolver#notifyChange(Uri, android.database.ContentObserver, boolean)} with that Uri
     * to send change notifications.
     *
     * @param parentDocumentId the directory to search under.
     * @param query string to match documents against.
     * @param projection list of {@link Document} columns to put into the cursor. If {@code null}
     *     all supported columns should be included.
     * @param signal used by the caller to signal if the request should be cancelled. May be null.
     * @see DocumentsContract#buildSearchDocumentsUri(Uri, String)
     */
    @FlaggedApi(Flags.FLAG_ENABLE_DOCUMENTSPROVIDER_SEARCH_BY_DOCUMENT_API)
    @Nullable
    public Cursor querySearchDocuments(
            ...
            @Nullable CancellationSignal signal)
            throws FileNotFoundException {
        throw new UnsupportedOperationException("Search by document not supported");
    }
     ...
     * fashion.
     *
     * <p>This method is called for subtree-level searches. It requires the specific directory to
     * advertise {@link Document#FLAG_SUPPORTS_SUBTREE_SEARCH}.
     *
     * <p>If your provider is cloud-based, and you have some data cached or pinned locally, you may
     * return the local data immediately, setting {@link DocumentsContract#EXTRA_LOADING} on the
     * Cursor to indicate that you are still fetching additional data. Then, when the network data
     ...
     * @see DocumentsContract#EXTRA_INFO
     * @see DocumentsContract#EXTRA_ERROR
     * @see Document#FLAG_SUPPORTS_SUBTREE_SEARCH
     */
    @FlaggedApi(Flags.FLAG_ENABLE_DOCUMENTSPROVIDER_SEARCH_BY_DOCUMENT_API)
    @Nullable
    public Cursor querySearchDocuments(
            ...
            @Nullable CancellationSignal signal)
            throws FileNotFoundException {
        Preconditions.checkNotNull(parentDocumentId, "parentDocumentId can not be null");
        Preconditions.checkNotNull(queryArgs, "queryArgs can not be null");
        return querySearchDocuments(
                parentDocumentId,
                DocumentsContract.getSearchDocumentsQuery(queryArgs),
                projection,
                signal);
    }
    ...
    public final Cursor query(
        ...
        try {
            switch (mMatcher.match(uri)) {
                ...
                case MATCH_SEARCH_DOCUMENT:
                case MATCH_SEARCH_TREE:
                    enforceTree(uri);
                    if (Flags.enableDocumentsproviderSearchByDocumentApi()) {
                        final String documentId = getDocumentId(uri);
                        enforceSearchByDocumentArguments(documentId);
                        return querySearchDocuments(
                                documentId, projection, queryArgs, cancellationSignal);
                    } else {
                        throw new UnsupportedOperationException("Search by document not supported");
                    }
                case MATCH_DOCUMENT:
