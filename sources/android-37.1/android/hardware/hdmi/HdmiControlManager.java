...
public final class HdmiControlManager {
    ...
    // --- Configuration setting for HDMI CEC source devices.
    // This setting governs whether a source device, upon confirming it is the active source
    // in the wakeUpIfActiveSource() method, should still trigger a system wakeup
    // even if the device is already interactive (i.e., PowerManager.isInteractive() returns true).
    // Enabling the "avoid wakeup" mode prevents redundant wakeUp() calls in this scenario. ---

    /**
     * Skips waking up the device in wakeUpIfActiveSource() if it is already interactive.
     *
     * @see HdmiControlManager#CEC_SETTING_NAME_SOURCE_AVOID_WAKEUP_IF_INTERACTIVE
     */
    @Hide
    public static final int SOURCE_AVOID_WAKEUP_IF_INTERACTIVE_ENABLED = 1;

    /**
     * Standard behavior: wake up the device in wakeUpIfActiveSource().
     * (Default)
     *
     * @see HdmiControlManager#CEC_SETTING_NAME_SOURCE_AVOID_WAKEUP_IF_INTERACTIVE
     */
    @Hide
    public static final int SOURCE_AVOID_WAKEUP_IF_INTERACTIVE_DISABLED = 0;

    /**
     * @see HdmiControlManager#CEC_SETTING_NAME_SOURCE_AVOID_WAKEUP_IF_INTERACTIVE
     */
    @Hide
    @IntDef(prefix = { "SOURCE_AVOID_WAKEUP_IF_INTERACTIVE_" }, value = {
            SOURCE_AVOID_WAKEUP_IF_INTERACTIVE_ENABLED,
            SOURCE_AVOID_WAKEUP_IF_INTERACTIVE_DISABLED
    })
    @Retention(RetentionPolicy.SOURCE)
    public @interface SourceAvoidWakeupIfInteractive {}
    ...
    /**
     * Name of a setting deciding whether a source device should avoid waking up via
     * wakeUpIfActiveSource() if the device is already interactive.
     */
    @Hide
    public static final String CEC_SETTING_NAME_SOURCE_AVOID_WAKEUP_IF_INTERACTIVE =
            "source_avoid_wakeup_if_interactive";
    ...
    @StringDef(value = {
        ...
        CEC_SETTING_NAME_SOURCE_AVOID_WAKEUP_IF_INTERACTIVE,
    ...
    /**
     * Controls whether a source device should avoid waking up via wakeUpIfActiveSource()
     * if the device is already interactive.
     *
     * @param value target state of avoiding wakeup.
     */
    @Hide
    @RequiresPermission(android.Manifest.permission.HDMI_CEC)
    public void setSourceAvoidWakeupIfInteractive(
            @SourceAvoidWakeupIfInteractive int value) {
        try {
            mService.setCecSettingIntValue(CEC_SETTING_NAME_SOURCE_AVOID_WAKEUP_IF_INTERACTIVE,
                    value);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    ...
    /**
     * Returns whether a source device should avoid waking up via wakeUpIfActiveSource()
     * if the device is already interactive.
     */
    @Hide
    @RequiresPermission(android.Manifest.permission.HDMI_CEC)
    @SourceAvoidWakeupIfInteractive
    public int getSourceAvoidWakeupIfInteractive() {
        try {
            return mService.getCecSettingIntValue(CEC_SETTING_NAME_SOURCE_AVOID_WAKEUP_IF_INTERACTIVE);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
