...
public class BiometricManager {
    ...
    /**
     * Sets the last authentication time returned by {@link #getLastAuthenticationTime(int)}
     * for testing scenarios.
     *
     * @param timeMillis The last authentication time in milliseconds or a negative value to
     *                   resume using the real last authentication time.
     */
    @TestApi
    @RequiresPermission(TEST_BIOMETRIC)
    @FlaggedApi(FLAG_AGENT_AUTH_XDEVICE_TOKEN)
    public void setLastAuthenticationTimeForTest(@ElapsedRealtimeLong long timeMillis) {
        try {
            mService.setLastAuthenticationTimeForTest(timeMillis);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
