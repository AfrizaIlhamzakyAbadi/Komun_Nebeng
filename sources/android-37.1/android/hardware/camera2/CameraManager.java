...
import android.hardware.TorchStatus;
...
public final class CameraManager {
     ...
     *
     * <p>Many apps do not handle landscape camera sensors and display stretched or sideways camera
     * previews.
     *
     * <p>This is a per-app override. See
     * <a href="https://developer.android.com/about/versions/14/features/manufacturer-per-app-overrides">this documentation</a>
     * for more details.
     *
     * <p>This override is enabled by default, but it needs a
     * {@link CameraManager#LANDSCAPE_TO_PORTRAIT_PROP} system property to be set to {@code true}. This
     * approach has no heuristic, and should be used only if the issues are so widespread that an
     * opt-in approach is not preferable. See {@link
     * CameraManager#OVERRIDE_CAMERA_ALLOW_LANDSCAPE_TO_PORTRAIT_OPT_IN_ONLY} for an opt-in variant.
     *
     * <p>Only one of {@link CameraManager#OVERRIDE_CAMERA_ALLOW_LANDSCAPE_TO_PORTRAIT_OPT_IN_ONLY}
     * and this override should be used: this override will be checked if {@link
     * CameraManager#LANDSCAPE_TO_PORTRAIT_PROP} is true.
     *
     * <p>Setting this override to false will disable cropping the camera feed to portrait.
     */
    @ChangeId
    @Overridable
    ...
    /**
     * Override the lens facing of front camera to back camera.
     *
     * <p>Some devices only have a single front camera, which does not sit well with apps that
     * expect to find a back camera.</p>
     */
    @Hide
    @ChangeId
    @Overridable
    ...
    /**
     * System property for allowing the {@link CameraManager#OVERRIDE_CAMERA_LANDSCAPE_TO_PORTRAIT}
     * treatment for cropping landscape feed to portrait.
     */
    @TestApi
     ...
     *
     * <p>Many apps do not handle landscape camera sensors and display stretched or sideways camera
     * previews. As opposed to {@link CameraManager#OVERRIDE_CAMERA_LANDSCAPE_TO_PORTRAIT}, this
     * override is disabled by default. It does not need {@link
     * CameraManager#LANDSCAPE_TO_PORTRAIT_PROP} system property in order to work. This approach is
     * often preferable if the majority of apps of interest handle landscape sensors well.
     *
     * <p>This is a per-app override. See
     * <a href="https://developer.android.com/about/versions/14/features/manufacturer-per-app-overrides">this documentation</a>
     * for more details.
     *
     * <p>Only one of {@link CameraManager#OVERRIDE_CAMERA_LANDSCAPE_TO_PORTRAIT} and this override
     * should be used: this override will be checked if {@link
     * CameraManager#LANDSCAPE_TO_PORTRAIT_PROP} is false.
     */
    @ChangeId
    @Overridable
    @Disabled
    ...
    public CameraCharacteristics getCameraCharacteristics(@NonNull String cameraId)
            throws CameraAccessException {
        return getCameraCharacteristics(cameraId, getCameraCompatibilityInfo(mContext));
    }
    ...
    public CameraCharacteristics getCameraCharacteristics(@NonNull String cameraId,
        ...
        return getCameraCharacteristics(cameraId,
                getCameraCompatibilityInfo(overrideToPortrait));
    }
    ...
    public void openCamera(@NonNull String cameraId,
        ...
        openCameraImpl(cameraId, callback, CameraDeviceImpl.checkAndWrapHandler(handler),
                /*oomScoreOffset*/0, getCameraCompatibilityInfo(mContext), /*sharedMode*/false);
    }
    ...
    public void openCamera(@NonNull String cameraId,
        ...
        openCameraImpl(cameraId, callback, executor, /*oomScoreOffset*/0,
                getCameraCompatibilityInfo(mContext), /*sharedMode*/false);
    }
    ...
    public void openSharedCamera(@NonNull String cameraId,
        ...
        openCameraImpl(cameraId, callback, executor, /*oomScoreOffset*/0,
                getCameraCompatibilityInfo(mContext), /*sharedMode*/true);
    }
    ...
    @Hide
    public static CameraCompatibilityInfo getCameraCompatibilityInfo(@Nullable Context context) {
    ...
    public static CameraCompatibilityInfo getRotationOverride(@Nullable Context context,
        ...
        // Disabled by default
        boolean lensFacingFrontToBackOverride = shouldOverrideLensFacingFrontToBack(
                context, packageManager, packageName);
        if (isCameraCompatibilityInfoRequested()) {
            // Keep the WindowManagers requested camera compatibility info intact.
            return getRequestedCameraCompatibilityInfo(
                    CompatibilityInfo.getCameraCompatibilityInfo(),
                    lensFacingFrontToBackOverride);
        }

        return getStaticLandscapeToPortraitOverride(packageManager,
                packageName, lensFacingFrontToBackOverride);
    }

    private static CameraCompatibilityInfo getStaticLandscapeToPortraitOverride(
            @Nullable PackageManager packageManager, @Nullable String packageName,
            boolean lensFacingFrontToBackOverride) {
        if (packageManager != null && packageName != null) {
            try {
                return getCameraCompatibilityInfo(packageManager.getProperty(
                        PackageManager.PROPERTY_COMPAT_OVERRIDE_LANDSCAPE_TO_PORTRAIT, packageName)
                        .getBoolean(), lensFacingFrontToBackOverride);
            } catch (PackageManager.NameNotFoundException e) {
                // No such property
            }
        }

        boolean landscapeToPortraitOverride = CameraManagerGlobal.sLandscapeToPortrait
                ? CompatChanges.isChangeEnabled(OVERRIDE_CAMERA_LANDSCAPE_TO_PORTRAIT)
                : (com.android.window.flags.Flags.cameraCompatLandscapeOptIn() && CompatChanges
                        .isChangeEnabled(OVERRIDE_CAMERA_ALLOW_LANDSCAPE_TO_PORTRAIT_OPT_IN_ONLY));
        return getCameraCompatibilityInfo(landscapeToPortraitOverride,
                lensFacingFrontToBackOverride);
    }

    private static CameraCompatibilityInfo getRequestedCameraCompatibilityInfo(
            @NonNull CameraCompatibilityInfo compatInfo,
            boolean shouldOverrideLensFacingFrontToBack) {
        CameraCompatibilityInfo.Builder builder = new CameraCompatibilityInfo.Builder()
                .setRotateAndCropRotation(compatInfo.getRotateAndCropRotation())
                .setShouldOverrideSensorOrientation(compatInfo.shouldOverrideSensorOrientation())
                .setShouldLetterboxForCameraCompat(compatInfo.shouldLetterboxForCameraCompat())
                .setDisplayRotationSandbox(compatInfo.getDisplayRotationSandbox())
                .setShouldAllowTransformInverseDisplay(
                        compatInfo.shouldAllowTransformInverseDisplay());

        if (com.android.window.flags.Flags.cameraCompatSingleFrontCameraTreatment()) {
            builder.setShouldOverrideLensFacingFrontToBack(shouldOverrideLensFacingFrontToBack);
        }
        return builder.build();
    }

    private CameraCompatibilityInfo getCameraCompatibilityInfo(
        ...
        boolean shouldOverrideLensFacingFrontToBack = shouldOverrideLensFacingFrontToBack(
                mContext, packageManager, packageName);
        return getCameraCompatibilityInfo(
                shouldOverrideRotation,
                shouldOverrideLensFacingFrontToBack);
    }

    private static boolean shouldOverrideLensFacingFrontToBack(
            @Nullable Context context, @Nullable PackageManager packageManager,
        ...
        if (context != null && !context.getResources().getBoolean(
                        R.bool.config_isCameraCompatOverrideLensFacingTreatmentEnabled)) {
            return false;
        }

        if (!CompatChanges.isChangeEnabled(OVERRIDE_CAMERA_LENS_FACING_FRONT)) {
            return false;
        }
        if (packageManager != null && packageName != null) {
            try {
                return packageManager.getProperty(
                        PackageManager.PROPERTY_COMPAT_OVERRIDE_LENS_FACING_FRONT, packageName)
                        .getBoolean();
            } catch (PackageManager.NameNotFoundException e) {
                // No such property, default to true because override is enabled
            }
        }
        return true;
    }

    private static CameraCompatibilityInfo getCameraCompatibilityInfo(
            boolean shouldOverrideRotation,
            boolean shouldOverrideLensFacingFrontToBack) {
        CameraCompatibilityInfo.Builder builder =  new CameraCompatibilityInfo.Builder()
                .setRotateAndCropRotation(shouldOverrideRotation ? ROTATION_90 : ROTATION_UNDEFINED)
                .setShouldOverrideSensorOrientation(shouldOverrideRotation);
    ...
    private static final class CameraManagerGlobal extends ICameraServiceListener.Stub
        ...
        private void connectCameraServiceLocked(boolean resetListener) {
            ...
            try {
                ...
                updateAllCameraStatusLocked(cameraStatuses);
        ...
        public String[] getCameraIdListNoLazy(int deviceId, int devicePolicy) {
            ...
            ICameraServiceListener.Stub testListener = new ICameraServiceListener.Stub() {
                ...
                @Override
                public void onStatusChangedBatched(CameraStatus[] cameraStatuses,
                        TorchStatus[] torchStatuses) {
                }
            };
        ...
        private void onStatusChangedLocked(int status, DeviceCameraInfo info) {
            onStatusChangedLocked(status, info, false);
        }

        private void onStatusChangedLocked(int status, DeviceCameraInfo info,
                boolean isStatusChangeBatched) {
            if (DEBUG) {
                Log.v(TAG,
            ...
            for (int i = 0; i < callbackCount; i++) {
                ...
                // Send the NOT_PRESENT state for unavailable physical cameras
                if (!isStatusChangeBatched && isAvailable(status)
                        && mUnavailablePhysicalDevices.containsKey(info)) {
                    List<String> unavailableIds = mUnavailablePhysicalDevices.get(info);
                    for (String unavailableId : unavailableIds) {
                        postSingleUpdate(callback, executor, info.mCameraId, unavailableId,
        ...
        @Override
        public void onStatusChangedBatched(
                CameraStatus[] cameraStatuses, TorchStatus[] torchStatuses) {
            synchronized (mLock) {
                addDeviceStatusHistoryLocked(TextUtils.formatSimple(
                        "onStatusChangedBatched(E): tid(%d): mDeviceStatus size %d",
                        Thread.currentThread().getId(), mDeviceStatus.size()));
                // first check if there are DeviceCameraInfo missing, which means camera devices
                // removed.
                for (DeviceCameraInfo key :  mDeviceStatus.keySet()) {
                    boolean keyFound = false;
                    for (int i = 0; i < cameraStatuses.length; i++) {
                        if (key.mCameraId.equals(cameraStatuses[i].cameraId)
                                && key.mDeviceId == cameraStatuses[i].deviceId) {
                            keyFound = true;
                            break;
                        }
                    }
                    if (!keyFound) {
                        onStatusChangedLocked(ICameraServiceListener.STATUS_NOT_PRESENT, key);
                    }
                }

                updateAllCameraStatusLocked(cameraStatuses);
                for (int i = 0; i < torchStatuses.length; i++) {
                    DeviceCameraInfo info = new DeviceCameraInfo(torchStatuses[i].cameraId,
                            torchStatuses[i].deviceId);
                    onTorchStatusChangedLocked(torchStatuses[i].status, info);
                }
                addDeviceStatusHistoryLocked(TextUtils.formatSimple(
                        "onStatusChangedBatched(X): tid(%d): mDeviceStatus size %d",
                        Thread.currentThread().getId(), mDeviceStatus.size()));
            }

        }

        private void updateAllCameraStatusLocked(CameraStatus[] cameraStatuses) {
            for (CameraStatus cameraStatus : cameraStatuses) {
                DeviceCameraInfo info = new DeviceCameraInfo(cameraStatus.cameraId,
                        cameraStatus.deviceId);
                // update camera status
                onStatusChangedLocked(cameraStatus.status, info, true);

                // Update physical camera availability in the batch update case.
                List<String> formerUnavailablePhysicalDevices =
                        mUnavailablePhysicalDevices.get(info) == null ? Collections.emptyList()
                                : mUnavailablePhysicalDevices.get(info);
                List<String> currentUnavailablePhysicalDevices =
                        (cameraStatus.unavailablePhysicalCameras == null) ? Collections.emptyList()
                                : Arrays.stream(cameraStatus.unavailablePhysicalCameras).toList();

                for (String formerUnavailablePhysicalCamera :
                        formerUnavailablePhysicalDevices) {
                    if (!currentUnavailablePhysicalDevices.contains(
                            formerUnavailablePhysicalCamera)) {
                        onPhysicalCameraStatusChangedLocked(
                                ICameraServiceListener.STATUS_PRESENT,
                                info, formerUnavailablePhysicalCamera);
                    }
                }
                for (String unavailablePhysicalCamera :
                        currentUnavailablePhysicalDevices) {
                    onPhysicalCameraStatusChangedLocked(
                            ICameraServiceListener.STATUS_NOT_PRESENT,
                            info, unavailablePhysicalCamera);
                }

                // update camera open close status
                if (mHasOpenCloseListenerPermission) {
                    if (cameraStatus.status == ICameraServiceListener.STATUS_NOT_AVAILABLE
                            && !cameraStatus.clientPackage.isEmpty()) {
                        if (!mOpenedDevices.containsKey(info)) {
                            onCameraOpenedLocked(info, cameraStatus.clientPackage);
                        } else if (!Objects.equals(mOpenedDevices.get(info),
                                cameraStatus.clientPackage)) {
                            onCameraClosedLocked(info);
                            onCameraOpenedLocked(info, cameraStatus.clientPackage);
                        }
                    } else if (cameraStatus.status == ICameraServiceListener.STATUS_PRESENT) {
                        if (mOpenedDevices.containsKey(info)) {
                            onCameraClosedLocked(info);
                        }
                    }
