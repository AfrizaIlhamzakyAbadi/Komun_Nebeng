...
public final class DataFlowSink implements AutoCloseable {
    ...
    @RequiresPermission(android.Manifest.permission.ACCESS_CONTEXT_HUB)
    public void close() {
        closeInternal(/* notifySource= */ true);
    }
     ...
     * @throws IllegalArgumentException if the element count is less than 1
    ...
    public void seekToSource(@IntRange(from = 0) int offsetFromSource) {
        try (ApiGuard guard = acquireApiGuard()) {
            ...
            mEndpoint.sinkSyncToSource(this, offsetFromSource);
        }
    }
    ...
    public boolean canSourceOverwriteReadPosition() {
        try (ApiGuard guard = acquireApiGuard()) {
            return mEndpoint.sinkSourceCanOverwriteReadPosition(this);
        }
    }
    ...
    /**
     * Returns the size of the data flow with respect to this sink's read position, in elements for
     * flows of fixed-size elements and in bytes for flows of variable-size elements.
     *
     * @throws IllegalStateException if this sink has been stopped
     * @throws ConcurrentModificationException if another API is being called concurrently or an
     *     async operation is in progress
    ...
    public int size() {
        try (ApiGuard guard = acquireApiGuard()) {
            return mEndpoint.sinkSize(this);
        }
    }
     ...
     *
     * @param bool notifySource If true, notify the source that this sink is closed.
     */
    @Hide
    /* package */ void closeInternal(boolean notifySource) {
        if (mClosed) {
            return;
        }
        mClosed = true;
        ...
        mEndpoint.removeSink(mContext.id, notifySource);
    }
    ...
    /** Returns the id of the data flow this sink reads from. */
    /* package */ DataFlowId getDataFlowId() {
        return mContext.id;
    }
    ...
    private void checkCancelled() {
        ...
        if (mCancellationStarted.compareAndSet(/* expectedValue= */ false, /* newValue= */ true)) {
            closeInternal(/* notifySource= */ false);
        }
    ...
    private DataFlowData requestDataInternal(int elementCount, boolean allOrNothing) {
        return mEndpoint.sinkRequestData(this, elementCount, allOrNothing);
    }
