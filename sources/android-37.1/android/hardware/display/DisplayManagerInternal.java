...
import static android.hardware.display.DisplayManager.BRIGHTNESS_UNIT_RAW;
...
public abstract class DisplayManagerInternal {
    ...
    /**
     * Checks whether the brightness is within the valid brightness range.
     */
    public static boolean isValidBrightnessValue(float value,
            @DisplayManager.BrightnessUnit int unit) {
        if (Float.isNaN(value)) {
            return false;
        }
        return switch (unit) {
            case BRIGHTNESS_UNIT_PERCENTAGE -> value >= 0 && value <= 100;
            case BRIGHTNESS_UNIT_NITS -> value >= 0;
            case BRIGHTNESS_UNIT_RAW ->
                    value >= PowerManager.BRIGHTNESS_MIN && value <= PowerManager.BRIGHTNESS_MAX;
            default -> throw new IllegalArgumentException("Invalid brightness unit: " + unit);
        };
    }
    ...
    /**
     * Registers multi-display update listener which will be informed of the addition, removal, or
     * change of any display. Whenever fired it reports a snapshot of all displays available
     * in the system.
     *
     * @param listener The listener to register.
     * @param handler The handler to deliver the events.
     */
    public abstract void registerMultiDisplayListener(MultiDisplayListener listener,
            Handler handler);

    /**
     * Unregisters multi-display update listener.
     *
     * @param listener The listener to unregister.
     */
    public abstract void unregisterMultiDisplayListener(MultiDisplayListener listener);
    ...
    /**
     * Returns the number of connected external displays in the topology.
     */
    ...
    */
     ...
     *
     * @param brightness     An override of the screen brightness. Set to
     *                       {@link PowerManager#BRIGHTNESS_INVALID} if there's no override.
     * @param tag            Tag used to identify the app window requesting the brightness override.
     * @param brightnessUnit The unit of the brightness override.
     */
    public record DisplayBrightnessOverrideRequest(
            float brightness,
            CharSequence tag,
            @DisplayManager.BrightnessUnit int brightnessUnit) {
    }
    ...
    /**
     * Called when there are changes to any display in the system
     */
    public interface MultiDisplayListener {
        /**
         * Called when there was a change in the basic display properties or a display was added
         * or removed. This is intended to be used only by WindowManager.
         *
         * @param displayInfos new snapshot of all displays in the system
         * @param transaction a surface control transaction that must be applied by WindowManager
         *                    together with the respective window updates (such as resize)
         */
        void onDisplaysChanged(@NonNull DisplayInfos displayInfos,
