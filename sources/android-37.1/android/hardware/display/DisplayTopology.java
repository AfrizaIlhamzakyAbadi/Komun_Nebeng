...
public final class DisplayTopology implements Parcelable {
    ...
    private @Position int mDefaultPositionOfSecondDisplay = POSITION_TOP;

    @Hide
    public DisplayTopology() {}

    /**
     * Set the default position of the external display.
     */
    @Hide
    public void setDefaultPosition(@Position int defaultPosition) {
        mDefaultPositionOfSecondDisplay = defaultPosition;
    }
    ...
    private DisplayTopology(@Nullable TreeNode root, int primaryDisplayId,
            @Position int defaultPosition) {
        this(root, primaryDisplayId);
        mDefaultPositionOfSecondDisplay = defaultPosition;
    }

    @Hide
    public DisplayTopology(Parcel source) {
        this(source.readTypedObject(TreeNode.CREATOR), source.readInt(), source.readInt());
    }
    ...
    public void addDisplay(
        ...
        if (mRoot == null) {
            ...
            // or the left and right edges, depending on the default position.
            float offset;
            if (mDefaultPositionOfSecondDisplay == POSITION_TOP
                    || mDefaultPositionOfSecondDisplay == POSITION_BOTTOM) {
                float width = pxToDp(logicalWidth, logicalDensity);
                offset = mRoot.getWidth() / 2 - width / 2;
            } else {
                float height = pxToDp(logicalHeight, logicalDensity);
                offset = mRoot.getHeight() / 2 - height / 2;
            }
            TreeNode display = new TreeNode(
                    displayId, logicalWidth, logicalHeight, logicalDensity,
                    mDefaultPositionOfSecondDisplay, offset);
    ...
    public DisplayTopology copy() {
        ...
        return new DisplayTopology(
                rootCopy, mPrimaryDisplayId, mDefaultPositionOfSecondDisplay);
    }
    ...
    public void writeToParcel(@NonNull Parcel dest, int flags) {
        ...
        dest.writeInt(mDefaultPositionOfSecondDisplay);
    }
