...
public class UserInfo implements Parcelable {
     ...
     *
     * Does not evaluate whether the given userType is a valid profile type or not; userTypes
     * that are not recognized (including non-profile types) will be treated as generic profiles.
     */
    @Hide
    public boolean canHaveProfile(String userType) {
        ...
        // Restricted profiles aren't real profiles, but they're still handled by this method, so
        // let's get them out of the way first.
        if (UserManager.isUserTypeRestricted(userType)) {
            return android.multiuser.Flags.restrictedProfilesForAdmins()
                    ? isAdmin()
                    : isMainUnlogged();
        }

        // Now on to actual profiles (not restricted profiles).
        if (isMainUnlogged()) {
            return true;
        }

        if (UserManager.isUserTypeManagedProfile(userType)) {
            // We aren't necessarily ready for managed profiles for non-MainUsers yet. So guard it
            // behind an additional master config for now.
            if (!(android.multiuser.Flags.profilesForAll() && Resources.getSystem().getBoolean(
                    com.android.internal.R.bool.config_supportManagedProfileOnNonMainUser))) {
                return false;
            }
            if (!android.multiuser.Flags.privateProfilesForAll()) {
                // privateProfilesForAll change introduces an additional restriction evaluated below
                return true;
            }
        }
        ...
        if (android.multiuser.Flags.privateProfilesForAll()) {
            if (!Resources.getSystem().getBoolean(
                    com.android.internal.R.bool.config_isMainUserPermanentAdmin)) {
                // If the device is MainUserless, then profiles aren't restricted to the MainUser.
                return true;
            }
