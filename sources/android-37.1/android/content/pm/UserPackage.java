...
public final class UserPackage implements Parcelable {
    private static final boolean ENABLE_CACHING = true;

    /**
     * The maximum number of entries to keep in the cache per user ID. The value should ideally be
     * high enough to cover all packages on an end-user device, but low enough that stale or invalid
     * packages would eventually (probably) get removed. This should benefit components that loop
     * through all packages on a device and use this class, since being able to cache the objects
     * for all packages on the device means we don't have to keep recreating the objects.
     */
    @VisibleForTesting static final int MAX_NUM_CACHED_ENTRIES_PER_USER = 1000;
    ...
    /**
     * Set of userIDs to cache objects for. We start off with an empty set, so there's no caching by
     * default. The system will override with a valid set of userIDs in its process so that caching
     * becomes active in the system process.
     */
    @GuardedBy("sCacheLock")
    private static int[] sUserIds = EmptyArray.INT;

    private UserPackage(int userId, @NonNull String packageName) {
        this.userId = userId;
    ...
    public static final Parcelable.Creator<UserPackage> CREATOR =
            new Parcelable.Creator<UserPackage>() {
                @Override
                public UserPackage createFromParcel(@NonNull Parcel in) {
                    int userId = in.readInt();
                    String packageName = in.readString8();
                    return UserPackage.of(userId, checkNotNull(packageName));
                }
