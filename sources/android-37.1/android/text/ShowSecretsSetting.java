...
public final class ShowSecretsSetting {
     ...
     *
     * <p>See {@link #registerCallback(Context, Executor, Runnable)} regarding context lifetime
     * management.
     *
    ...
    @NonNull
    public static Runnable registerCallback(@NonNull Context context, @NonNull Runnable callback) {
     ...
     *
     * <p>Note: To avoid unintentionally extending the lifetime of the provided context, the actual
     * observer registration is performed using the {@link Context#getApplicationContext()
     * application context} (or a user-specific context derived from it), rather than holding a
     * strong reference to the provided {@code context}. However, the user ID of the provided {@code
     * context} is preserved to ensure the correct user's settings are monitored. As a fallback, if
    ...
    public static Runnable registerCallback(
        ...
        Context userAppContext = context;
        if (appContext != null) {
            if (appContext.getUser().equals(context.getUser())) {
                userAppContext = appContext;
            } else {
                userAppContext = appContext.createContextAsUser(context.getUser(), 0);
            }
        }

        ContentResolver resolver = userAppContext.getContentResolver();
