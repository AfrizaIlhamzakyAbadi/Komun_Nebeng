...
public class TelephonyFrameworkInitializer {
    ...
    // Cached vendor API level. Non-final to allow reflective override in unit tests.
    private static int sVendorApiLevel = SystemProperties.getInt(
            "ro.vendor.api_level", Build.VERSION.DEVICE_INITIAL_SDK_INT);

    // Cached early forced phone globals config. Non-final to allow reflective override in tests.
    private static volatile Boolean sForcePhoneGlobalsCreation = null;
    ...
    private static boolean shouldForcePhoneGlobalsCreation(Context context) {
        Boolean forcePhoneGlobals = sForcePhoneGlobalsCreation;
        if (forcePhoneGlobals == null) {
            synchronized (TelephonyFrameworkInitializer.class) {
                forcePhoneGlobals = sForcePhoneGlobalsCreation;
                if (forcePhoneGlobals == null) {
                    forcePhoneGlobals = context.getResources().getBoolean(
                            com.android.internal.R.bool.config_force_phone_globals_creation);
                    sForcePhoneGlobalsCreation = forcePhoneGlobals;
                }
            }
        }
        return forcePhoneGlobals;
    }
    ...
    private static boolean hasSystemFeature(Context context, String feature) {
        if (shouldForcePhoneGlobalsCreation(context)) {
            return true;
        }

        // Check SDK version of the vendor partition.
        if (sVendorApiLevel < Build.VENDOR_API_2024_Q2) return true;
