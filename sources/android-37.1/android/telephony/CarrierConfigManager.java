...
public class CarrierConfigManager {
    ...
    /**
     * Specifies whether call barring requires a password or not.
     * If true, the user will be prompted for a password when enabling or disabling call barring.
     * If false, the user will not be prompted for a password.
     *
     * Enabled by default.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_ALLOW_SKIP_PASSWORD_VALIDATION_FOR_CALL_BARRING)
    public static final String KEY_CALL_BARRING_REQUIRES_PASSWORD_BOOL =
            "call_barring_requires_password_bool";
    ...
    /**
     * A string array indicating the network capabilities that are metered by the carrier.
     *
     * The default value is {@code {"PRIORITIZE_BANDWIDTH", "PRIORITIZE_LATENCY",
     * "PRIORITIZE_UNIFIED_COMMUNICATIONS"}}.
     */
    @Hide
    public static final String KEY_CARRIER_METERED_NETWORK_CAPABILITIES_STRINGS =
            "carrier_metered_network_capabilities_strings";

    /**
     * A string array indicating the network capabilities that are roaming-metered by the carrier.
     *
     * The default value is {@code {"PRIORITIZE_BANDWIDTH", "PRIORITIZE_LATENCY",
     * "PRIORITIZE_UNIFIED_COMMUNICATIONS"}}.
     */
    @Hide
    public static final String KEY_CARRIER_METERED_ROAMING_NETWORK_CAPABILITIES_STRINGS =
            "carrier_metered_roaming_network_capabilities_strings";
    ...
    /**
     * Boolean indicating if default data account should always show LTE plus when in LTE. If both
     * this key and {@link #KEY_HIDE_LTE_PLUS_DATA_ICON_BOOL} are set to true, this key takes
     * precedence and the LTE+ icon will be shown.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_CUSTOMIZE_LTE_PLUS_ICON)
    ...
    /**
     * List of 4 customized 5G SS reference signal received power (SSRSRP) thresholds
     * to be used when a RSRP boost offset is active (e.g. for selective boosting on certain bands).
     *
     * <p>
     * When a 5G NR RSRP boost is active (configured via
     * {@link #KEY_NRARFCNS_RSRP_BOOST_INT_ARRAY} and matched by ARFCN range in
     * {@link #KEY_BOOSTED_NRARFCNS_STRING_ARRAY}), the AP-side signal strength leveling will use
     * this customized threshold array instead of the standard
     * {@link #KEY_5G_NR_SSRSRP_THRESHOLDS_INT_ARRAY}.
     *
     * <p>
     * Crucially, when these boosted thresholds are applied:
     * <ul>
     * <li>The Application Processor (AP) signal level evaluation will use these values directly
     * as the boosted signal boundaries, matching the virtual RSRP calculation
     * (measured RSRP + boost).</li>
     * <li>The Telephony framework will dynamically calculate and register corresponding raw
     * reporting thresholds with the modem by subtracting the active RSRP boost:
     * <pre>Modem Registered Threshold = Boosted Threshold - Active RSRP Boost</pre>
     * This ensures the modem and the AP are perfectly aligned and trigger level transitions at the
     * exact same physical RF boundaries, preventing signal bar sticking at low signals while
     * securing the Out-of-Service (OOS) floor.</li>
     * </ul>
     *
     * <p>
     * 4 threshold integers must be within the boundaries [-140 dBm, -44 dBm] and ordered from
     * lowest to highest. Default is {@code null} (no dynamic boosted thresholds, uses standard
     * thresholds even when boost is active).
     */
    @Hide
    public static final String KEY_5G_NR_SSRSRP_BOOSTED_THRESHOLDS_INT_ARRAY =
            "5g_nr_ssrsrp_boosted_thresholds_int_array";
    ...
    /**
     * List of 4 customized LTE reference signal received power (RSRP) thresholds
     * to be used when a RSRP boost offset is active (e.g. for selective boosting on certain bands).
     *
     * <p>
     * When an LTE RSRP boost is active (configured via
     * {@link #KEY_LTE_EARFCNS_RSRP_BOOST_INT} and matched by EARFCN range in
     * {@link #KEY_BOOSTED_LTE_EARFCNS_STRING_ARRAY}), the AP-side signal strength leveling will use
     * this customized threshold array instead of the standard
     * {@link #KEY_LTE_RSRP_THRESHOLDS_INT_ARRAY}.
     *
     * <p>
     * Crucially, when these boosted thresholds are applied:
     * <ul>
     ...
     */
    @Hide
    public static final String KEY_LTE_RSRP_BOOSTED_THRESHOLDS_INT_ARRAY =
            "lte_rsrp_boosted_thresholds_int_array";
    ...
    /**
     * Whether device resets NR advanced grace period timers when device is in NR advanced state and
     * RRC is currently in IDLE state.
     *
     ...
     */
    @Hide
    public static final String KEY_NR_TIMERS_RESET_IF_NR_ADVANCED_AND_RRC_IDLE_BOOL =
            "nr_timers_reset_if_nr_advanced_and_rrc_idle_bool";
    ...
    public static final class Ims {
         ...
         * <li>4: {@link #SA_DISABLE_POLICY_VONR_UNPROVISIONED  }</li>
         * </ul>
         */
        @Hide
        ...
        @IntDef({
                ...
                NR_SA_DISABLE_POLICY_VONR_UNPROVISIONED
        })
        ...
        /**
         * Disables NR SA mode when VoNR is unprovisioned (not provisioned).
         */
        @Hide
    ...
    /**
     * Data failure causes for which the network-suggested retry timer should be ignored.
     *
     * When data setup for the default internet data profile fails with one of these causes,
     * the framework will ignore any suggested retry timer from the network and instead
     * follow its own internal retry rules, provided that there are multiple suitable
     * data profiles (APNs) available. This allows the framework to proactively fall back
     * to other available data profiles rather than being stuck retrying the same profile.
     *
     * For non-default data profiles (e.g. IMS, SOS) or scenarios where only one suitable
     * data profile exists, the network-suggested retry timer will still be honored.
     *
     * <p>
     * <b>Note:</b> When configuring a cause in this array, you must also ensure there is a
     * corresponding {@link #KEY_TELEPHONY_DATA_SETUP_RETRY_RULES_STRING_ARRAY} (or a wildcard rule)
     * matching that cause in the carrier config. Otherwise, if the network-suggested retry
     * timer is ignored, the framework will fail to match any retry rule and will not schedule
     * any subsequent attempt, resulting in a silent retry drop.
     *
     * <p>
     * An example configuration in carrier config XML:
     * <pre>{@code
     * <int-array name=
     *     "telephony_data_setup_ignore_network_suggested_retry_causes_int_array"
     *     num="2">
     *     <item value="33"/> <!-- DataFailCause.USER_AUTHENTICATION -->
     *     <item value="38"/> <!-- DataFailCause.NETWORK_FAILURE -->
     * </int-array>
     * }</pre>
     *
     * The values should be one of the error codes defined in {@link DataFailCause}.
     */
    @Hide
    public static final String
            KEY_TELEPHONY_DATA_SETUP_IGNORE_NETWORK_SUGGESTED_RETRY_CAUSES_INT_ARRAY =
            "telephony_data_setup_ignore_network_suggested_retry_causes_int_array";
    ...
    /**
     * Controls whether the telephony framework initiates a legacy fallback mechanism using
     * SIM Forbidden Public Land Mobile Network (FPLMN) records when modern satellite
     * enablement APIs are not supported by the modem.
     *
     * <p>When this configuration is set to {@code true}, and a modem returns
     * {@link SatelliteManager#SATELLITE_RESULT_REQUEST_NOT_SUPPORTED} during an
     * attempt to enable satellite services, the framework will manually manage the
     * SIM's forbidden network list. It will temporarily remove specific satellite-capable
     * networks from the FPLMN list and trigger a manual network scan to force the modem
     * to recognize and attach to the satellite network.
     *
     * <p>This fallback is intended for older hardware that lacks native support for
     * carrier-orchestrated satellite network selection.
     *
     * <p>Note: The original state of the SIM's forbidden network list is restored
     * automatically by the framework when the satellite session ends.
     *
     * <p>The default value is {@code false}.
     */
    @Hide
    public static final String KEY_SATELLITE_NETWORK_ATTACH_CONTROLLED_WITH_FPLMN_BOOL =
            "satellite_network_attach_controlled_with_fplmn_bool";
    ...
    /**
     * An interval in dB for NG-RAN reference signal received power (SSRSRP) measurement
     * type defining the required magnitude change between reports when connected to a
     * non-terrestrial NR network.
     *
     * <p>The default value is 2 and the minimum allowed value is 0. If no value or negative value
     * is set, the default value 2 is used.
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_NGRAN_SSRSRP_HYSTERESIS_DB_INT =
            "ntn_ngran_ssrsrp_hysteresis_db_int";

    /**
     * An interval in dB for NG-RAN reference signal received quality (SSRSRQ) measurement
     * type defining the required magnitude change between reports when connected to a
     * non-terrestrial NR network.
     *
     * <p>The default value is 2 and the minimum allowed value is 0. If no value or negative value
     ...
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_NGRAN_SSRSRQ_HYSTERESIS_DB_INT =
            "ntn_ngran_ssrsrq_hysteresis_db_int";

    /**
     * An interval in dB for NG-RAN reference signal to noise and interference ratio (SSSINR)
     * measurement type defining the required magnitude change between reports when connected to a
     * non-terrestrial NR network.
     *
     * <p>The default value is 2 and the minimum allowed value is 0. If no value or negative value
     * is set, the default value 2 is used.
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_NGRAN_SSSINR_HYSTERESIS_DB_INT =
            "ntn_ngran_sssinr_hysteresis_db_int";

    /**
     * An interval in dB for E-UTRAN reference signal received power (RSRP) measurement
     * type defining the required magnitude change between reports when connected to a
     * non-terrestrial LTE network.
     *
     * <p>The default value is 2 and the minimum allowed value is 0. If no value or negative value
     * is set, the default value 2 is used.
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_EUTRAN_RSRP_HYSTERESIS_DB_INT =
            "ntn_eutran_rsrp_hysteresis_db_int";

    /**
     * An interval in dB for E-UTRAN reference signal received quality (RSRQ) measurement
     * type defining the required magnitude change between reports when connected to a
     * non-terrestrial LTE network.
     *
     * <p>The default value is 2 and the minimum allowed value is 0. If no value or negative value
     ...
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_EUTRAN_RSRQ_HYSTERESIS_DB_INT =
            "ntn_eutran_rsrq_hysteresis_db_int";

    /**
     * An interval in dB for E-UTRAN reference signal to noise and interference ratio (RSSNR)
     * measurement type defining the required magnitude change between reports when connected to a
     * non-terrestrial LTE network.
     *
     * <p>The default value is 2 and the minimum allowed value is 0. If no value or negative value
     ...
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_EUTRAN_RSSNR_HYSTERESIS_DB_INT =
            "ntn_eutran_rssnr_hysteresis_db_int";

    /**
     * An interval in milliseconds for NG-RAN signal reporting measurement
     * type defining the required time change between reports when connected to a
     * non-terrestrial NR network.
     *
     * <p>The default value is 5000 and the minimum allowed value is 0. If no value or negative
     * value is set, the default value 5000 is used.
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_NGRAN_SIGNAL_REPORTING_HYSTERESIS_MS_INT =
            "ntn_ngran_signal_reporting_hysteresis_ms_int";

    /**
     * An interval in milliseconds for E-UTRAN signal reporting measurement
     * type defining the required time change between reports when connected to a
     * non-terrestrial LTE network.
     *
     * <p>The default value is 5000 and the minimum allowed value is 0. If no value or negative
     ...
     */
    @FlaggedApi(Flags.FLAG_SATELLITE_SIGNAL_HYSTERESIS_THRESHOLDS)
    public static final String KEY_NTN_EUTRAN_SIGNAL_REPORTING_HYSTERESIS_MS_INT =
            "ntn_eutran_signal_reporting_hysteresis_ms_int";
    ...
    @IntDef({
        ...
        SATELLITE_DATA_SUPPORT_PURCHASE_ONLY,
    })
    ...
    /**
     * Support purchase mode satellite network that serves only purchase app and telephony traffic.
     * Note: It is not valid to set this supported data mode through Carrier Config. It can only be
     * set through Entitlement Server.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_SATELLITE_UPSELL_26Q4)
    public static final int SATELLITE_DATA_SUPPORT_PURCHASE_ONLY = 3;

    /**
     * Indicates what kind of traffic an {@link NetworkCapabilities#NET_CAPABILITY_NOT_RESTRICTED}
     * satellite network can possibly support. The network may be subject to further
     * restrictions (or upgrades such as bandwidth level) based on entitlement etc.
    ...
    /**
     * Search interval in milliseconds for prioritized network scan.
     *
     * <p>This bundle contains a list of key-value pairs, where the keys are reason strings (e.g.
     * {@link SatelliteManager#SATELLITE_ENABLEMENT_REQUEST_REASON_PURCHASE}) and the values are the
     * search intervals in milliseconds.
     *
     * <p>The search interval determines how often the modem should perform a network scan when
     * attempting to establish a connection.
     *
     * <p>This value is used when {@link
     * EnableRequestAttributes.Builder#setPrioritizedScanningRequired(boolean)} is set to {@code
     * true}.
     *
     * <p>The default value for {@link
     * SatelliteManager#SATELLITE_ENABLEMENT_REQUEST_REASON_PURCHASE} is 10000 milliseconds (10
     * seconds).
     *
     * @see EnableRequestAttributes.Builder#setPrioritizedScanningRequired(boolean)
     */
    @Hide
    public static final String KEY_SATELLITE_PRIORITIZED_SCAN_SEARCH_INTERVAL_MS_BUNDLE =
            "satellite_prioritized_scan_search_interval_ms_bundle";

    /**
     * Valid duration in seconds for prioritized network scan.
     *
     * <p>This bundle contains a list of key-value pairs, where the keys are reason strings (e.g.
     * {@link SatelliteManager#SATELLITE_ENABLEMENT_REQUEST_REASON_PURCHASE}) and the values are the
     * valid durations in seconds.
     *
     * <p>The valid duration determines how long the prioritized network scan remains active before
     ...
     *
     * <p>The default value for {@link
     * SatelliteManager#SATELLITE_ENABLEMENT_REQUEST_REASON_PURCHASE} is 300 seconds (5 minutes).
     *
     * @see EnableRequestAttributes.Builder#setPrioritizedScanningRequired(boolean)
     */
    @Hide
    public static final String KEY_SATELLITE_PRIORITIZED_SCAN_VALID_DURATION_SEC_BUNDLE =
            "satellite_prioritized_scan_valid_duration_sec_bundle";
    ...
    /**
     * A boolean indicating whether the carrier website contains the list of supported apps.
     *
     * The default value is {@code false}.
     */
    @Hide
    public static final String KEY_SATELLITE_CARRIER_WEBSITE_CONTAIN_SUPPORTED_APPS_BOOL =
            "satellite_carrier_website_contain_supported_apps_bool";
    ...
    /**
     * Boolean indicating whether 2G is re-enabled within Android for the carrier for emergency
     * calls. Intended for use by carriers who cannot consistently support emergency calls without
     * a 2G fallback being available.
     *
     * <p>When {@code true}, 2G will be enabled upon an emergency call being placed even if the
     * user has disabled 2G in settings. Normally, this setting is not needed, as the modem handles
     * re-enabling 2G as needed during emergencies, but for certain carrier configurations this may
     * not be possible. This setting does not impact the user’s ability to change the 2G toggle
     * state in their device settings.
     *
     * <p>The default value is {@code false}.
     */
    @Hide
    public static final String KEY_ENABLE_2G_FOR_EMERGENCY_CALL = "enable_2g_for_emergency_call";
   ...
   @Hide
    ...
    /**
     * Control whether users can reach the automatic SIM PIN management settings.
     * When hidden, the user can only manage their SIM PIN manually.
     */
    @Hide
    @FlaggedApi(android.security.Flags.FLAG_AUTO_SIM_PIN_MANAGEMENT)
    public static final String KEY_HIDE_AUTOMATIC_SIM_PIN_MANAGEMENT_BOOL =
            "hide_automatic_sim_pin_management_bool";
    ...
    static {
        ...
        sDefaults.putBoolean(KEY_HIDE_AUTOMATIC_SIM_PIN_MANAGEMENT_BOOL, false);
        ...
        sDefaults.putBoolean(KEY_CALL_BARRING_REQUIRES_PASSWORD_BOOL, true);
        ...
        sDefaults.putStringArray(KEY_CARRIER_METERED_ROAMING_APN_TYPES_STRINGS,
                new String[]{"default", "mms", "dun", "supl", "enterprise"});
        sDefaults.putStringArray(KEY_CARRIER_METERED_NETWORK_CAPABILITIES_STRINGS,
                new String[]{"PRIORITIZE_BANDWIDTH", "PRIORITIZE_LATENCY",
                        "PRIORITIZE_UNIFIED_COMMUNICATIONS"});
        sDefaults.putStringArray(KEY_CARRIER_METERED_ROAMING_NETWORK_CAPABILITIES_STRINGS,
                new String[]{"PRIORITIZE_BANDWIDTH", "PRIORITIZE_LATENCY",
                        "PRIORITIZE_UNIFIED_COMMUNICATIONS"});
        ...
        sDefaults.putIntArray(KEY_LTE_RSRP_BOOSTED_THRESHOLDS_INT_ARRAY, null);
        ...
        sDefaults.putIntArray(KEY_5G_NR_SSRSRP_BOOSTED_THRESHOLDS_INT_ARRAY, null);
        ...
        sDefaults.putInt(KEY_NTN_NGRAN_SSRSRP_HYSTERESIS_DB_INT, 2);
        sDefaults.putInt(KEY_NTN_NGRAN_SSRSRQ_HYSTERESIS_DB_INT, 2);
        sDefaults.putInt(KEY_NTN_NGRAN_SSSINR_HYSTERESIS_DB_INT, 2);
        sDefaults.putInt(KEY_NTN_EUTRAN_RSRP_HYSTERESIS_DB_INT, 2);
        sDefaults.putInt(KEY_NTN_EUTRAN_RSRQ_HYSTERESIS_DB_INT, 2);
        sDefaults.putInt(KEY_NTN_EUTRAN_RSSNR_HYSTERESIS_DB_INT, 2);
        sDefaults.putInt(KEY_NTN_NGRAN_SIGNAL_REPORTING_HYSTERESIS_MS_INT, 5000);
        sDefaults.putInt(KEY_NTN_EUTRAN_SIGNAL_REPORTING_HYSTERESIS_MS_INT, 5000);
        ...
        sDefaults.putBoolean(KEY_NR_TIMERS_RESET_IF_NR_ADVANCED_AND_RRC_IDLE_BOOL, false);
        ...
        sDefaults.putBoolean(KEY_SATELLITE_NETWORK_ATTACH_CONTROLLED_WITH_FPLMN_BOOL, false);
        ...
        sDefaults.putBoolean(KEY_SATELLITE_CARRIER_WEBSITE_CONTAIN_SUPPORTED_APPS_BOOL, false);
        ...
        PersistableBundle prioritizedScanSearchIntervalMsBundle = new PersistableBundle();
        prioritizedScanSearchIntervalMsBundle.putInt(
                String.valueOf(
                        SatelliteManager.SATELLITE_ENABLEMENT_REQUEST_REASON_PURCHASE), 10000);
        prioritizedScanSearchIntervalMsBundle.putInt(
                String.valueOf(
                        SatelliteManager.SATELLITE_ENABLEMENT_REQUEST_REASON_UNKNOWN), 10000);
        sDefaults.putPersistableBundle(
                KEY_SATELLITE_PRIORITIZED_SCAN_SEARCH_INTERVAL_MS_BUNDLE,
                prioritizedScanSearchIntervalMsBundle);

        PersistableBundle prioritizedScanValidDurationSecBundle = new PersistableBundle();
        prioritizedScanValidDurationSecBundle.putInt(
                String.valueOf(
                        SatelliteManager.SATELLITE_ENABLEMENT_REQUEST_REASON_PURCHASE), 300);
        prioritizedScanValidDurationSecBundle.putInt(
                String.valueOf(
                        SatelliteManager.SATELLITE_ENABLEMENT_REQUEST_REASON_UNKNOWN), 300);
        sDefaults.putPersistableBundle(
                KEY_SATELLITE_PRIORITIZED_SCAN_VALID_DURATION_SEC_BUNDLE,
                prioritizedScanValidDurationSecBundle);
        ...
        sDefaults.putInt(KEY_SATELLITE_ROAMING_ESOS_INACTIVITY_TIMEOUT_SEC_INT, 300);
        ...
        sDefaults.putBoolean(KEY_ENABLE_2G_FOR_EMERGENCY_CALL, false);
