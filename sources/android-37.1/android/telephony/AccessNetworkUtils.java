...
public class AccessNetworkUtils {
    ...
    /**
     * Gets the operating band for the given frequency and access network type.
     *
     * @param accessNetworkType The access network type (EUTRAN or NGRAN)
     * @param freq The frequency (EARFCN or NRARFCN)
     * @return The operating band, or INVALID_BAND if invalid
     */
    @android.annotation.Hide
    public static int getOperatingBand(int accessNetworkType, int freq) {
        if (accessNetworkType == AccessNetworkConstants.AccessNetworkType.EUTRAN) {
            return getOperatingBandForEarfcn(freq);
        } else if (accessNetworkType == AccessNetworkConstants.AccessNetworkType.NGRAN) {
            return getOperatingBandForNrarfcn(freq);
        } else if (accessNetworkType == AccessNetworkConstants.AccessNetworkType.GERAN) {
            return getOperatingBandForArfcn(freq);
        } else if (accessNetworkType == AccessNetworkConstants.AccessNetworkType.UTRAN) {
            return getOperatingBandForUarfcn(freq);
        }
        return INVALID_BAND;
    }
}
