...
public class LinkAddress implements Parcelable {
    ...
    private static boolean isValid(int flags) {
        ...
        return (flags & IFA_F_DADFAILED) == 0L
                && ((flags & IFA_F_TENTATIVE) == 0L || (flags & IFA_F_OPTIMISTIC) != 0L);
    }

    private static boolean isPreferred(int flags) {
        return isValid(flags) && (flags & IFA_F_DEPRECATED) == 0L;
    }

    private boolean isGlobal() {
        return scope == RT_SCOPE_UNIVERSE && !isIpv6ULA();
    }

    /**
     * Returns true if this {@code LinkAddress} is global scope and is valid (i.e., is not doing DAD
     * and has not failed DAD).
     */
    @Hide
    public boolean isGlobalValid() {
        return isGlobal() && isValid(getFlags());
    }
    ...
    public boolean isGlobalPreferred() {
        return isGlobal() && isPreferred(getFlags());
    }
