...
public class SharedConnectivityManager {
            ...
            if (mUserManager != null && !mUserManager.isUserUnlocked()) {
                ...
                if (Flags.fixMainThreadBlockingOnFirstUnlock()) {
                    mContext.registerReceiver(mBroadcastReceiver, intentFilter,
                            /* broadcastPermission = */ null, mBackgroundHandler);
                } else {
                    mContext.registerReceiver(mBroadcastReceiver, intentFilter);
                }
            } else {
    ...
    private void registerCallbackInternal(SharedConnectivityClientCallback callback,
        ...
        if (Flags.fixMainThreadBlockingOnFirstUnlock()) {
            ...
            mBackgroundHandler.post(() -> {
                try {
                    if (service != null) {
                        service.registerCallback(proxy);
                    }
    ...
    public boolean unregisterCallback(
        ...
        if (Flags.fixMainThreadBlockingOnFirstUnlock()) {
            ...
            boolean shouldUnbind;
            synchronized (mProxyDataLock) {
                proxy = mProxyMap.remove(callback);
            ...
            if (proxy != null) {
                ...
                mBackgroundHandler.post(() -> {
                    try {
                        if (service != null) {
                            service.unregisterCallback(proxy);
                        }
                    } catch (RemoteException e) {
    ...
    private void checkPermissions() {
        if (mContext.checkSelfPermission(android.Manifest.permission.NETWORK_SETTINGS)
