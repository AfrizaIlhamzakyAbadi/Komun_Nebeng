...
public class WifiP2pManager {
    ...
    /**
     * Extra for transporting a peer external gateway IPv4 address.
     */
    @Hide
    public static final String EXTRA_PARAM_KEY_GATEWAY_IPV4_ADDRESS =
            "android.net.wifi.p2p.EXTRA_PARAM_KEY_GATEWAY_IPV4_ADDRESS";
    ...
    /**
     * The lookup key for an {@link Inet4Address} object representing the external
     * gateway IPv4 address associated with the P2P network.
     * Retrieve with {@link android.content.Intent#getSerializableExtra(String, Class)}.
     */
    @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
    @FlaggedApi(Flags.FLAG_P2P_GC_NETWORK_AGENT)
    public static final String EXTRA_GATEWAY_IPV4_ADDRESS =
            "android.net.wifi.p2p.extra.GATEWAY_IPV4_ADDRESS";
    ...
    @Hide
    public static final int SET_AGENT_EXTERNAL_GATEWAY                = BASE + 125;
    @Hide
    public static final int SET_AGENT_EXTERNAL_GATEWAY_FAILED         = BASE + 126;
    @Hide
    public static final int SET_AGENT_EXTERNAL_GATEWAY_SUCCEEDED      = BASE + 127;
   ...
   public interface DnsSdTxtRecordListener {
        ...
        class P2pHandler extends Handler {
            ...
            public void handleMessage(Message message) {
                ...
                switch (message.what) {
                    ...
                    case SET_AGENT_EXTERNAL_GATEWAY_FAILED:
                        if (listener != null) {
                    ...
                    case SET_AGENT_EXTERNAL_GATEWAY_SUCCEEDED:
                        if (listener != null) {
    ...
    /**
     * Configure external gateway for an established P2P group.
     *
     * <p>This API allows a device (either Group Owner or Group Client) to use the Internet
     * connection of another peer device in the group. The caller should use an out-of-band
     * protocol to identify the peer device providing Internet access and ensure that the peer
     * P2P device is actively routing traffic from the P2P interface to its own Internet-active
     * interface before calling this API (e.g., by enabling packet forwarding via
     * {@link android.net.TetheringManager#startTethering}).
     *
     * <p><b>Warning:</b> When an external gateway is enabled, the P2P network transitions from
     * a local-only network ({@link android.net.NetworkCapabilities#NET_CAPABILITY_LOCAL_NETWORK})
     * to an Internet-capable network (
     * {@link android.net.NetworkCapabilities#NET_CAPABILITY_INTERNET}).
     * Consequently, the P2P network may become the system default network.
     *
     * @param channel is the channel created at {@link #initialize}
     * @param gatewayAddress the IPv4 address of the peer device within the P2P group that is
     *                       providing Internet access (acting as the gateway). This IP address must
     *                       be the peer's address on the local P2P interface subnet. Pass
     *                       {@code null} to clear the external gateway.
     * @param listener for callbacks on success or failure. Pass {@code null} if not needed.
     */
    @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
    ...
    public void setGroupExternalGateway(@NonNull Channel channel,
            @Nullable Inet4Address gatewayAddress, @Nullable ActionListener listener) {
        checkChannel(channel);
        Bundle extras = prepareExtrasBundle(channel);
        if (gatewayAddress != null) {
            extras.putSerializable(EXTRA_PARAM_KEY_GATEWAY_IPV4_ADDRESS, gatewayAddress);
        }
        channel.mAsyncChannel.sendMessage(prepareMessage(SET_AGENT_EXTERNAL_GATEWAY, 0,
                channel.putListener(listener), extras, channel.mContext));
    }
}
