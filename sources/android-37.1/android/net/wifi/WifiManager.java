...
public class WifiManager {
    ...
    @Hide
    public static final int CALLBACK_ERROR_UNKNOWN = 0;
    @Hide
    public static final int CALLBACK_ERROR_WIFI_DISABLED = 1;
    @Hide
    public static final int CALLBACK_ERROR_NOT_CONNECTED = 2;
    ...
    @Hide
    @IntDef(flag = false, prefix = { "SAP_" }, value = {
        SAP_START_FAILURE_GENERAL,
        SAP_START_FAILURE_NO_CHANNEL,
        SAP_START_FAILURE_UNSUPPORTED_CONFIGURATION,
        SAP_START_FAILURE_USER_REJECTED,
        SAP_DISABLE_REASON_CODE_GENERAL,
        SAP_DISABLE_REASON_CODE_INACTIVE_SESSION,
        SAP_DISABLE_REASON_CODE_IDLE_SESSION,
        SAP_DISABLE_REASON_CODE_INTERFACE_ERROR
    })
    @Retention(RetentionPolicy.SOURCE)
    public @interface SapStateReason {}

    /**
     * Soft AP disabled for a general reason.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_SOFTAP_TRAFFIC_MONITOR)
    public static final int SAP_DISABLE_REASON_CODE_GENERAL = 4;

    /**
     * Soft AP disabled due to inactive session.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_SOFTAP_TRAFFIC_MONITOR)
    public static final int SAP_DISABLE_REASON_CODE_INACTIVE_SESSION = 5;

    /**
     * Soft AP disabled due to idle session.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_SOFTAP_TRAFFIC_MONITOR)
    public static final int SAP_DISABLE_REASON_CODE_IDLE_SESSION = 6;

    /**
     * Soft AP disabled due to interface error.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_SOFTAP_TRAFFIC_MONITOR)
    public static final int SAP_DISABLE_REASON_CODE_INTERFACE_ERROR = 7;
    ...
    /**
     * Set whether scan MAC randomization is enabled.
     * <p>
     * MAC randomization for Wi-Fi scans helps improve user privacy by using a random MAC address
     * when the device is scanning for new networks. Disabling this feature means the device will
     * use its real hardware MAC address during Wi-Fi scans.
     *
     * @param enabled true to enable, false to disable.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_SCAN_MAC_RANDOMIZATION_SETTING)
    @RequiresPermission(android.Manifest.permission.NETWORK_SETTINGS)
    public void setScanMacRandomizationEnabled(boolean enabled) {
        try {
            mService.setScanMacRandomizationEnabled(enabled);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    private static class SignalPollCallbackProxy extends ISignalPollCallback.Stub {
        private final Executor mExecutor;
        private final OutcomeReceiver<WifiSignalPollResults, Exception> mReceiver;

        SignalPollCallbackProxy(
                Executor executor, OutcomeReceiver<WifiSignalPollResults, Exception> receiver) {
            mExecutor = executor;
            mReceiver = receiver;
        }
        ...
        public void onResult(WifiSignalPollResults results) {
            Binder.clearCallingIdentity();
            mExecutor.execute(() -> mReceiver.onResult(results));
        }
        ...
        public void onFailure(int errorCode) {
            ...
            Exception ex;
            switch (errorCode) {
                case CALLBACK_ERROR_WIFI_DISABLED -> {
                    ex = new IllegalStateException("Wi-Fi is disabled");
                }
                case CALLBACK_ERROR_NOT_CONNECTED -> {
                    ex = new IllegalStateException("Wi-Fi is not connected");
                }
                default -> {
                    ex = new Exception("Unknown internal error");
                }
            }
            mExecutor.execute(() -> mReceiver.onError(ex));
        }
    }

    /**
     * Query the current Wi-Fi signal poll results immediately.
     * <p>
     * This triggers an immediate signal poll to the hardware and returns the results.
     * If Wi-Fi is disabled or not connected, {@link IllegalStateException} will be reported to the
     * callback's {@link OutcomeReceiver#onError(Exception)}.
     *
     * @param executor The Executor on which to run the callback.
     * @param callback The OutcomeReceiver to receive the results.
     */
    @SystemApi
    @RequiresPermission(anyOf = {
            android.Manifest.permission.NETWORK_SETTINGS,
    ...
    public void querySignalPoll(
            @NonNull @CallbackExecutor Executor executor,
            @NonNull OutcomeReceiver<WifiSignalPollResults, Exception> callback) {
        Objects.requireNonNull(executor, "executor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");
        if (sVerboseLoggingEnabled) {
            Log.d(TAG, "querySignalPoll: callback=" + callback + ", executor=" + executor);
        }
        try {
            mService.querySignalPoll(new SignalPollCallbackProxy(executor, callback));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }

    private static class TxPacketCountersCallbackProxy extends ITxPacketCountersCallback.Stub {
        private final Executor mExecutor;
        private final OutcomeReceiver<TxPacketCounters, Exception> mReceiver;

        TxPacketCountersCallbackProxy(
                Executor executor, OutcomeReceiver<TxPacketCounters, Exception> receiver) {
            mExecutor = executor;
            mReceiver = receiver;
        }
        ...
        @Override
        @RequiresNoPermission
        public void onResult(TxPacketCounters counters) {
            Binder.clearCallingIdentity();
            mExecutor.execute(() -> mReceiver.onResult(counters));
        }
    ...
    /**
     * Query packet transmission counters for the primary interface immediately.
     * <p>
     * This returns the count of successfully transmitted packets and transmission failures.
     * These counters are cumulative since the current association (connection to the current AP).
     * If Wi-Fi is disabled or not connected, {@link IllegalStateException} will be reported to the
     * callback's {@link OutcomeReceiver#onError(Exception)}.
     *
     * @param executor The Executor on which to run the callback.
    ...
    @FlaggedApi(Flags.FLAG_WIFICOND_REPLACEMENT_APIS)
    public void queryTxPacketCounters(
            @NonNull @CallbackExecutor Executor executor,
            @NonNull OutcomeReceiver<TxPacketCounters, Exception> callback) {
        Objects.requireNonNull(executor, "executor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");
        if (sVerboseLoggingEnabled) {
            Log.d(TAG, "queryTxPacketCounters: callback=" + callback + ", executor=" + executor);
        }
        try {
            mService.queryTxPacketCounters(new TxPacketCountersCallbackProxy(executor, callback));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    private static class BooleanListenerWithFailureProxy
            extends IBooleanListenerWithFailure.Stub {
        private final Executor mExecutor;
        private final OutcomeReceiver<Boolean, Exception> mReceiver;

        BooleanListenerWithFailureProxy(
                Executor executor, OutcomeReceiver<Boolean, Exception> receiver) {
            mExecutor = executor;
            mReceiver = receiver;
        }
        ...
        public void onResult(boolean value) {
            Binder.clearCallingIdentity();
            mExecutor.execute(() -> mReceiver.onResult(value));
        }
    ...
    /**
     * Query whether a specific channel width is supported by the device's Wi-Fi chip.
     * <p>
     * If Wi-Fi is disabled, {@link IllegalStateException} will be reported to the
     * callback's {@link OutcomeReceiver#onError(Throwable)}.
     *
     * @param channelWidth The channel width to check, one of
     *                     {@link ScanResult#CHANNEL_WIDTH_20MHZ},
     *                     {@link ScanResult#CHANNEL_WIDTH_40MHZ},
     *                     {@link ScanResult#CHANNEL_WIDTH_80MHZ},
     *                     {@link ScanResult#CHANNEL_WIDTH_160MHZ},
    ...
    @FlaggedApi(Flags.FLAG_WIFICOND_REPLACEMENT_APIS)
    public void queryChannelWidthSupported(
        ...
        Objects.requireNonNull(callback, "callback cannot be null");
        if (sVerboseLoggingEnabled) {
            Log.d(TAG, "queryChannelWidthSupported: channelWidth=" + channelWidth
                    + ", callback=" + callback + ", executor=" + executor);
        }
        try {
            mService.queryChannelWidthSupported(channelWidth,
                    new BooleanListenerWithFailureProxy(executor, callback));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }

    private static class IntegerListenerWithFailureProxy
            extends IIntegerListenerWithFailure.Stub {
        private final Executor mExecutor;
        private final OutcomeReceiver<Integer, Exception> mReceiver;

        IntegerListenerWithFailureProxy(
                Executor executor, OutcomeReceiver<Integer, Exception> receiver) {
            mExecutor = executor;
            mReceiver = receiver;
        }
    ...
    /**
     * Query the maximum number of transmit spatial streams supported by the device's Wi-Fi chip.
     * <p>
     * If Wi-Fi is disabled, {@link IllegalStateException} will be reported to the
     * callback's {@link OutcomeReceiver#onError(Throwable)}.
     *
    ...
    @FlaggedApi(Flags.FLAG_WIFICOND_REPLACEMENT_APIS)
    public void queryMaxNumberTxSpatialStreams(
            @NonNull @CallbackExecutor Executor executor,
            @NonNull OutcomeReceiver<Integer, Exception> callback) {
        Objects.requireNonNull(executor, "executor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");
        if (sVerboseLoggingEnabled) {
            Log.d(TAG, "queryMaxNumberTxSpatialStreams: callback=" + callback
                    + ", executor=" + executor);
        }
        try {
            mService.queryMaxNumberTxSpatialStreams(
                    new IntegerListenerWithFailureProxy(executor, callback));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Query the maximum number of receive spatial streams supported by the device's Wi-Fi chip.
     * <p>
     * If Wi-Fi is disabled, {@link IllegalStateException} will be reported to the
     * callback's {@link OutcomeReceiver#onError(Throwable)}.
     *
    ...
    @FlaggedApi(Flags.FLAG_WIFICOND_REPLACEMENT_APIS)
    public void queryMaxNumberRxSpatialStreams(
            @NonNull @CallbackExecutor Executor executor,
            @NonNull OutcomeReceiver<Integer, Exception> callback) {
        Objects.requireNonNull(executor, "executor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");
        if (sVerboseLoggingEnabled) {
            Log.d(TAG, "queryMaxNumberRxSpatialStreams: callback=" + callback
                    + ", executor=" + executor);
        }
        try {
            mService.queryMaxNumberRxSpatialStreams(
                    new IntegerListenerWithFailureProxy(executor, callback));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    ...
    /**
     * Query the maximum number of AKM suites supported in connection requests by the Wi-Fi chip.
     * <p>
     * If Wi-Fi is disabled, {@link IllegalStateException} will be reported to the
     * callback's {@link OutcomeReceiver#onError(Throwable)}.
     *
    ...
    @FlaggedApi(Flags.FLAG_WIFICOND_REPLACEMENT_APIS)
    public void queryMaxNumberAkms(
            @NonNull @CallbackExecutor Executor executor,
            @NonNull OutcomeReceiver<Integer, Exception> callback) {
        Objects.requireNonNull(executor, "executor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");
        if (sVerboseLoggingEnabled) {
            Log.d(TAG, "queryMaxNumberAkms: callback=" + callback + ", executor=" + executor);
        }
        try {
            mService.queryMaxNumberAkms(
                    new IntegerListenerWithFailureProxy(executor, callback));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
   ...
   /**
    ...
    public interface SoftApCallback {
         ...
         *               {@link #WIFI_AP_STATE_DISABLING}, {@link #WIFI_AP_STATE_ENABLED},
         *               {@link #WIFI_AP_STATE_ENABLING}, {@link #WIFI_AP_STATE_FAILED}
         * @param reason reason when in failed/disabled/disabling state. One of
         *               {@link #SAP_START_FAILURE_GENERAL},
         *               {@link #SAP_START_FAILURE_NO_CHANNEL},
         *               {@link #SAP_START_FAILURE_UNSUPPORTED_CONFIGURATION},
         *               {@link #SAP_START_FAILURE_USER_REJECTED},
         *               {@link #SAP_DISABLE_REASON_CODE_GENERAL},
         *               {@link #SAP_DISABLE_REASON_CODE_INACTIVE_SESSION},
         *               {@link #SAP_DISABLE_REASON_CODE_IDLE_SESSION},
         *               {@link #SAP_DISABLE_REASON_CODE_INTERFACE_ERROR}
         */
        default void onStateChanged(@WifiApState int state, @SapStateReason int reason) {}
        ...
        default void onStateChanged(@NonNull SoftApState state) {
            onStateChanged(state.mState, state.mStateChangedReason);
        }
    ...
    private void updateVerboseLoggingEnabledFromService() {
        sVerboseLoggingEnabled = isVerboseLoggingEnabled();
    }
    ...
    */
    ...
    private static class WifiVerboseLoggingStatusChangedListenerProxy
            extends IWifiVerboseLoggingStatusChangedListener.Stub {
        private final Executor mExecutor;
        private final WifiVerboseLoggingStatusChangedListener mListener;

        WifiVerboseLoggingStatusChangedListenerProxy(@NonNull Executor executor,
        ...
        public void onStatusChanged(boolean enabled) {
            ...
            Binder.clearCallingIdentity();
            mExecutor.execute(() -> mListener.onWifiVerboseLoggingStatusChanged(enabled));
        }
    }
        ...
        try {
            synchronized (sWifiVerboseLoggingStatusChangedListenerMap) {
                IWifiVerboseLoggingStatusChangedListener.Stub binderCallback =
                        new WifiVerboseLoggingStatusChangedListenerProxy(executor, listener);
    ...
    /**
     * Extra string indicating the name of a custom id to set the positive button view to.
     * Used for UI test scripts.
     */
    @Hide
    public static final String EXTRA_DIALOG_POSITIVE_BUTTON_ID =
            "android.net.wifi.extra.DIALOG_POSITIVE_BUTTON_ID";
    ...
    /**
     * When the device is connected to a network suggested by the app specified by
     * {@code packageName}, this API provides a way to avoid the current connection as if it was
     * requested by that app.
     * <p>
     * This is a privileged system API intended for use by system-level components (such as the
     * Virtual Carrier Network (VCN) or NetworkStack) that monitor network connection quality
     * and need to trigger network avoidance when a suggested network performs poorly.
     * <p>
     * By proxying this request on behalf of the suggesting app (using {@code packageName}), the
     * Wi-Fi framework correctly attributes the failure to that specific suggestion. This allows
     * the framework to apply suggestion-specific heuristics, such as temporarily downvoting
     * suggestions from this package, avoiding auto-joining them, or notifying the suggesting
     * app of the failure, rather than applying a brute-force global block that would affect
     * unrelated network connections.
     * <p>
     * Example:
     * <pre>{@code
     * // VCN detects high packet loss on a carrier-suggested Wi-Fi network.
     * // It uses this API to temporarily disallow the network on behalf of the carrier app,
     * // allowing the device to fallback to cellular data while attributing the Wi-Fi failure
     * // to the carrier app's suggestion.
     * wifiManager.disallowCurrentSuggestedNetworkAsPackage(
     *         "com.example.carrier.app",
     *         new BlockingOption.Builder(60)
     *                 .setBlockingBssidOnly(true)
     *                 .build());
     * }</pre>
     *
     * @param packageName Package name of the app which suggested the current network.
     * @param blockingOption Option to change for the network blocking {@link BlockingOption}
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_DISALLOW_SUGGESTED_NETWORK_AS_PACKAGE)
    @RequiresPermission(android.Manifest.permission.NETWORK_STACK)
    public void disallowCurrentSuggestedNetworkAsPackage(@NonNull String packageName,
            @NonNull BlockingOption blockingOption) {
        Objects.requireNonNull(packageName, "packageName cannot be null");
        Objects.requireNonNull(blockingOption, "blockingOption cannot be null");
        try {
            mService.disallowCurrentSuggestedNetworkAsPackage(blockingOption, packageName);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
