...
public class WifiAwareManager {
    ...
    /**
     * Get all paired devices alias configured for all apps.
     * This is for Settings app use to display paired devices to the user.
     * <p>
     * Note: This method is not {@link android.annotation.UserHandleAware} and always returns the
     * pairing configuration corresponding to the currently active user, regardless of the user
     * associated with the calling context.
     *
     * @param executor The executor on which callback will be invoked.
     * @param resultsCallback An asynchronous callback that will return a map of package names to
     *                        the list of their paired devices' alias.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_AWARE_AUTO_PAIRING)
    @RequiresPermission(android.Manifest.permission.NETWORK_SETTINGS)
    public void getPairedDevicesPrivileged(@NonNull @CallbackExecutor Executor executor,
            @NonNull Consumer<Map<String, List<String>>> resultsCallback) {
        Objects.requireNonNull(executor, "executor cannot be null");
        Objects.requireNonNull(resultsCallback, "resultsCallback cannot be null");
        try {
            mService.getPairedDevicesPrivileged(
                    new IMapListener.Stub() {
                        @Override
                        public void onResult(Map value) {
                            Binder.clearCallingIdentity();
                            executor.execute(() -> resultsCallback.accept(
    ...
    /**
     * Remove the target paired device for a specific package. This is for Settings app use to
     * remove paired devices for a specific package on behalf of the user.
     * <p>
     * Note: This method is not {@link android.annotation.UserHandleAware} and always operates on
     * the pairing configuration corresponding to the currently active user, regardless of the user
     * associated with the calling context.
     *
     * @param packageName The package name of the application.
     * @param alias The alias set by the application or null to remove all paired devices. If the
     *     alias is non-null but not found, the call will be no-op.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_AWARE_AUTO_PAIRING)
    @RequiresPermission(android.Manifest.permission.NETWORK_SETTINGS)
    public void removePairedDevicePrivileged(@NonNull String packageName, @Nullable String alias) {
        Objects.requireNonNull(packageName, "packageName cannot be null");
        try {
            mService.removePairedDevicePrivileged(packageName, alias);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
