...
public final class MacAddress implements Parcelable {
    ...
    public static @NonNull String stringAddrFromByteAddr(byte[] addr) {
        ...
        return stringAddrFromBytes(addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
    }
    ...
    private static @NonNull String stringAddrFromBytes(
            byte b0, byte b1, byte b2, byte b3, byte b4, byte b5) {
        final StringBuilder sb = new StringBuilder(17);
        appendHexDigits(sb, b0);
        sb.append(':');
        appendHexDigits(sb, b1);
        sb.append(':');
        appendHexDigits(sb, b2);
        sb.append(':');
        appendHexDigits(sb, b3);
        sb.append(':');
        appendHexDigits(sb, b4);
        sb.append(':');
        appendHexDigits(sb, b5);
        return sb.toString();
    }

    /**
     * Appends two hexadecimal characters for the byte value to the {@link StringBuilder}.
     *
     * @param sb a StringBuilder to append to, non-null
     * @param b a byte value
     */
    private static void appendHexDigits(@NonNull StringBuilder sb, byte b) {
        sb.append(toLowHexDigit((byte) (b >> 4)));
        sb.append(toLowHexDigit(b));
    }

    /**
     * Returns the hexadecimal character for the low 4 bits of the byte value. The values in the
     * range {@code 10-15} are returned as {@code "a-f"}, The values in the range {@code 0-9} are
     * returned as {@code "0-9"}.
     *
     * @param b a value, only the low 4 bits {@code 0-3} of the value are used
     * @return the hexadecimal character for the low 4 bits {@code 0-3} of the value
     */
    private static char toLowHexDigit(byte b) {
        int i = 0xf & b;
        if (i < 10) {
            return (char) ('0' + i);
        }
