...
class BidirectionalStreamWrapper extends android.net.http.BidirectionalStream {
    private final String TAG = BidirectionalStreamWrapper.class.getSimpleName();
    ...
    private static boolean sLoggedWarningForEmptyMetrics;

    BidirectionalStreamWrapper(org.chromium.net.BidirectionalStream backend) {
        // Downcast to CronetBidirectionalStream to access impl only methods (e.g.
        // getRequestFinishedTimings)
    ...
    @Override
    public FinishedRequestTimings getFinishedRequestTimings() {
        if (backend instanceof CronetBidirectionalStream cronetBidirectionalStream) {
            // There's cases where the bidirectional stream is not CronetBidirectionalStream
            // (e.g. CronetAdaptiveNetworkBidirectionalStream). In this case, we'll return no
            // metrics.
            // TODO(b/498898784): Add support for getFinishedRequestTimings for
            // CronetAdaptiveNetworkBidirectionalStream.
            return new FinishedRequestTimings(
                    cronetBidirectionalStream.getFinishedRequestTimings());
        }
        if (!sLoggedWarningForEmptyMetrics) {
            sLoggedWarningForEmptyMetrics = true;
            Log.w(
                    TAG,
                    String.format(
                            "[%s] Returning empty metrics as the underlying implementation (%s)"
                                    + " does not support getFinishedRequestTimings",
                            BidirectionalStream.class.getName(), backend.getClass().getName()));
        }
        return new FinishedRequestTimings(CronetMetrics.empty());
    }
}
