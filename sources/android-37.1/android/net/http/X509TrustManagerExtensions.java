...
public class X509TrustManagerExtensions {
    ...
    /**
     * Attach a {@link ConscryptNetworkSecurityPolicy} object to the TrustManager.
     *
     * @param policy the network security policy
     */
    @Hide
    //TODO(b/453608356): merge this method with the public one below.
    public void setNetworkSecurityPolicy(@NonNull ConscryptNetworkSecurityPolicy policy) {
        if (com.android.org.conscrypt.net.flags.Flags.dynamicNetworkSecurityPolicy()) {
            if (mTrustManager instanceof ConscryptX509TrustManager tm) {
                tm.setNetworkSecurityPolicy(policy);
            }
            if (mDelegate != null) {
                mDelegate.setNetworkSecurityPolicy(policy);
            }
        } else {
            Log.w(TAG, "setNetworkSecurityPolicy was requested but not enabled in conscrypt.");
        }
    }

    /**
     * Sets the {@link NetworkSecurityPolicy} to be used by this TrustManager.
     *
     * <p>This method allows an application to associate a specific security policy with this {@link
     * X509TrustManager} instance. When a policy is set, it overrides the default process-wide
     * policy returned by {@link NetworkSecurityPolicy#getInstance()} for all certificate
     * verifications performed by this TrustManager.
     *
     * <p>The TrustManager will query the provided policy for security decisions, including:
     *
     * <ul>
     *   <li>Whether cleartext traffic is permitted for a specific hostname.
     *   <li>Whether Certificate Transparency (CT) verification is required for a hostname.
     *   <li>The domain encryption mode (e.g., Encrypted Client Hello) to be used.
     * </ul>
     *
     * <p>By using a custom {@link NetworkSecurityPolicy}, applications can programmatically define
     * granular security constraints that differ from the global application configuration. This is
     * useful for specialized network stacks or for enforcing stricter security on specific
     * sensitive connections.
     *
     * @param policy the {@link NetworkSecurityPolicy} to associate with this TrustManager. Must not
     *     be {@code null}.
     * @see NetworkSecurityPolicy
     * @see NetworkSecurityPolicy#getInstance()
     */
    @FlaggedApi(android.security.Flags.FLAG_DYNAMIC_NETWORK_SECURITY_POLICY)
    public void setNetworkSecurityPolicy(@NonNull NetworkSecurityPolicy policy) {
        Objects.requireNonNull(policy);
        if (com.android.org.conscrypt.net.flags.Flags.dynamicNetworkSecurityPolicy()) {
            setNetworkSecurityPolicy(policy.toConscryptNetworkSecurityPolicy());
        }
    }
}
