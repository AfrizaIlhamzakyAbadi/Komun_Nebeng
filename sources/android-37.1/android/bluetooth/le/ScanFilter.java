...
public final class ScanFilter implements Parcelable {
    ...
    public boolean matchesWithoutAddress(ScanResult scanResult) {
        ...
        if (mAdvertisingDataType > 0) {
            // If packet doesn't contain this type, it means no match
            if (!scanRecord.getAdvertisingDataMap().containsKey(mAdvertisingDataType)) {
                return false;
            }

            // If and only if we provided a specific data to match, check it
            if (mAdvertisingData != null) {
                byte[] advertisingData =
                        scanRecord.getAdvertisingDataMap().get(mAdvertisingDataType);
            ...
            // If we didn't provide a specific data to match, it means we are only interested in
            // advertising data type match only. Therefore, regardless of the actual data, we have a
            // match.
        }
    ...
    public static final class Builder {
        ...
        @SystemApi
        @RequiresNoPermission
        @FlaggedApi(Flags.FLAG_REMOVE_SCAN_FILTER_BUILDER_EXCEPTION)
