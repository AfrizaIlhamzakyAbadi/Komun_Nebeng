...
public final class BluetoothGattServer implements BluetoothProfile {
    ...
    private void executeCallback(Consumer<BluetoothGattServerCallback> consumer) {
        executeFromBinder(
                () -> {
                    final BluetoothGattServerCallback callback = mCallback;
                    if (callback == null) {
                        Log.v(TAG, "executeCallback(): mCallback is null");
                        return;
                    }
    ...
    private class GattServerCallback extends IBluetoothGattServerCallback.Stub {
        ...
        public void onServiceAdded(int status, BluetoothGattService service) {
            ...
            executeCallback(cb -> cb.onServiceAdded((int) status, tmp));
        }
        ...
        public void onCharacteristicReadRequest(
            ...
            executeCallback(
                    cb -> cb.onCharacteristicReadRequest(device, transId, offset, characteristic));
        }
        ...
        public void onDescriptorReadRequest(
            ...
            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onDescriptorReadRequest(device, transId, offset, descriptor));
        }
        ...
        public void onCharacteristicWriteRequest(
            ...
            executeCallback(
                    cb ->
                            cb.onCharacteristicWriteRequest(
                                    device,
                                    transId,
                                    characteristic,
                                    isPrep,
        ...
        public void onExecuteWrite(BluetoothDevice device, int transId, boolean execWrite) {
            ...
            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onExecuteWrite(device, transId, execWrite));
        }
        ...
        public void onNotificationSent(BluetoothDevice device, int status) {
            ...
            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onNotificationSent(device, status));
        }
        ...
        void onCharacteristicsOffloaded(
            ...
            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onCharacteristicsOffloaded(device, session, status));
        }
        ...
        public void onCharacteristicsUnoffloaded(
            ...
            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onCharacteristicsUnoffloaded(device, sessionId, status));
        }
    }

    private final ILeConnectionCallback mLeConnectionCallback = new LeConnectionCallback();

    private class LeConnectionCallback extends ILeConnectionCallback.Stub {
        @Override
        @RequiresNoPermission // Callback to app
        public void onConnectionState(int status, boolean connected, BluetoothDevice device) {
            Log.d(
                    TAG,
                    ("onConnectionState(): status=" + status)
                            + (", connected=" + connected)
                            + (", device=" + device));
        ...
        public void onPhyUpdate(BluetoothDevice device, int txPhy, int rxPhy, int status) {
            Log.d(
                    TAG,
                    ("onPhyUpdate(): device=" + device)
                            + (", txPhy=" + txPhy)
                            + (", rxPhy=" + rxPhy)
                            + (", status=" + status));

            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onPhyUpdate(device, txPhy, rxPhy, status));
        }
        ...
        public void onPhyRead(BluetoothDevice device, int txPhy, int rxPhy, int status) {
            Log.d(
                    TAG,
                    ("onPhyRead(): device=" + device)
                            + (", txPhy=" + txPhy)
                            + (", rxPhy=" + rxPhy)
                            + (", status=" + status));

            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onPhyRead(device, txPhy, rxPhy, status));
        }
        ...
        public void onConnectionUpdated(
            ...
            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(
                    cb -> cb.onConnectionUpdated(device, interval, latency, timeout, status));
        }
        ...
        public void onSubrateChange(BluetoothDevice device, int subrateMode, int status) {
            ...
            Attributable.setAttributionSource(device, mAttributionSource);
            executeCallback(cb -> cb.onSubrateChange(device, subrateMode, status));
        }

        @Override
        @RequiresNoPermission
        public void onReadRemoteRssi(BluetoothDevice device, int rssi, int status) {
            // Not supported on server role
        }
    }
    ...
    private BluetoothGattCharacteristic getCharacteristicByHandle(int handle) {
    ...
    private BluetoothGattDescriptor getDescriptorByHandle(int handle) {
        for (BluetoothGattService svc : mServices) {
            for (BluetoothGattCharacteristic charac : svc.getCharacteristics()) {
    ...
    public void setPreferredPhy(
            ...
            int phyOptions) {
        try {
            mService.setPreferredPhy(
                    mLeConnectionCallback, device, txPhy, rxPhy, phyOptions, mAttributionSource);
        } catch (RemoteException e) {
            Log.e(TAG, "", e);
        }
    }
    ...
    public void readPhy(BluetoothDevice device) {
        try {
            mService.readPhy(mLeConnectionCallback, device, mAttributionSource);
        } catch (RemoteException e) {
            Log.e(TAG, "", e);
        }
    }
