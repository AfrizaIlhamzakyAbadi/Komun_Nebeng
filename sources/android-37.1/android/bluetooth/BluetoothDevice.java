...
public final class BluetoothDevice implements Parcelable, Attributable {
    ...
    @SystemApi
    @Deprecated
    @FlaggedApi(Flags.FLAG_A2DP_ACTION_SWITCH_BUFFER_SIZE_DEPRECATION)
    ...
    public boolean equals(@Nullable Object o) {
        if (o instanceof BluetoothDevice other) {
            return mAddress.equals(other.getAddress());
        }
        return false;
    }
    ...
    /**
     * This API returns the bond state of the remote device.
     *
     * <p>Note: The bond state returned by this API may not correctly represent the actual bond
     * state for all transports. To get the correct bond status for a specific transport, use {@link
     * #getBondStatus(int)}
     *
     * <p>Possible values for the bond state are: {@link #BOND_NONE}, {@link #BOND_BONDING}, {@link
     * #BOND_BONDED}.
     *
     * @return the bond state of the remote device
     */
    @RequiresLegacyBluetoothPermission
    @RequiresBluetoothConnectPermission
    @RequiresPermission(BLUETOOTH_CONNECT)
     ...
     *
     * @return {@link BluetoothStatusCodes#SUCCESS} upon successful initiation of the operation,
     ...
     *
     * <p>If {@code transport} is {@link #TRANSPORT_AUTO}, it disconnects all ACL links for the
     * device. Otherwise, it disconnects the ACL link for the specified transport.
     *
     * <p>Disconnection is asynchronous, so apps should listen to {@link #ACTION_ACL_DISCONNECTED}
     * to verify whether disconnection was successful.
     *
     * <p>This method requires the calling app to have the {@link
     * android.Manifest.permission#BLUETOOTH_CONNECT} permission. Additionally, an app must either
     * have {@link android.Manifest.permission#BLUETOOTH_PRIVILEGED} or have a Companion Device
     * Manager association with this device (see {@link
     * android.companion.CompanionDeviceManager#associate(AssociationRequest,
     * android.companion.CompanionDeviceManager.Callback, Handler)}).
     *
     * @param transport the transport to disconnect (e.g. {@link #TRANSPORT_AUTO}, {@link
     *     #TRANSPORT_BREDR}, {@link #TRANSPORT_LE})
     ...
     */
    @CheckResult
    @FlaggedApi(Flags.FLAG_ADD_FORCE_DISCONNECT_API)
    @RequiresBluetoothConnectPermission
    @RequiresPermission(
            allOf = {BLUETOOTH_CONNECT, BLUETOOTH_PRIVILEGED},
            conditional = true)
    public @ConnectionReturnValues int disconnectAcl(@Transport int transport) {
        if (DBG) log("disconnectAcl(transport=" + transport + ")");
        ...
        return callServiceIfEnabled(
                s -> s.disconnectAcl(this, transport, mAttributionSource),
                BluetoothStatusCodes.ERROR_BLUETOOTH_NOT_ENABLED);
    }
