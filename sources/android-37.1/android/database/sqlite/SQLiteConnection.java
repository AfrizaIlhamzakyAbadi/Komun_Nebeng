...
public final class SQLiteConnection implements CancellationSignal.OnCancelListener {
    ...
    private static native void nativeFinalizeStatement(long statementPtr);
    ...
    private PreparedStatement acquirePreparedStatementLI(String sql) {
        ...
        final long statementPtr = mPreparedStatementCache.createStatement(mConnectionPtr, sql);
        ...
        try {
            ...
            if (statement == null || !statement.mInCache) {
                nativeFinalizeStatement(statementPtr);
            }
            throw ex;
        }
        statement.mInUse = true;
    ...
    private PreparedStatement obtainPreparedStatement(String sql, long statementPtr,
        ...
        final PreparedStatement statement = new PreparedStatement();
        statement.mSql = sql;
    ...
    static final class PreparedStatement {
        ...
        // Close this statement.  The native statement is finalized and the local pointer is
        // cleared.
        void close() {
            if (mStatementPtr != 0) {
                nativeFinalizeStatement(mStatementPtr);
                mStatementPtr = 0;
            }
        }
    }
    ...
    /**
     * This class must be static so that the SQLiteConnection can become unreachable even if this
     * object is reachable.
     */
    private static final class PreparedStatementCache extends LruCache<String, PreparedStatement> {
        ...
        // preparation step.
        synchronized long createStatement(long connectionPtr, String sql) {
            mLastSeqNum = mDatabaseSeqNum;
            return nativePrepareStatement(connectionPtr, sql);
        }
