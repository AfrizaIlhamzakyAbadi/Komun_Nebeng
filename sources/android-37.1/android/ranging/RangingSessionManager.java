...
public final class RangingSessionManager extends IRangingCallbacks.Stub {
    ...
    @Override
    public void onRangingRoundsUpdateDtTagStatus(SessionHandle session,
            byte[] rangingRoundIndexes) {
        Log.v(TAG, "onRangingRoundsUpdateDtTagStatus - sessionHandle: " + session);
        if (!mSessions.containsKey(session)) {
            Log.e(TAG, "SessionHandle not found");
            return;
        }
        mSessions.get(session).onDtRangingRoundsUpdated(rangingRoundIndexes);
    }
