...
public abstract class HostApduService extends Service {
    ...
    public static final class MsgHandler extends Handler {
        ...
        @Hide
        @VisibleForTesting
        public final WeakReference<HostApduService> mService;

        @Hide
        @VisibleForTesting
        public MsgHandler(HostApduService service) {
            mService = new WeakReference<>(service);
        }

        @Override
        public void handleMessage(@NonNull Message msg) {
            HostApduService service = mService.get();
            if (service == null) {
                return;
            }
            switch (msg.what) {
                case MSG_COMMAND_APDU:
                    ...
                    if (service.getNfcService() == null) service.setNfcService(msg.replyTo);
                    ...
                    if (apdu != null) {
                        ...
                        if (Flags.nfcHceLatencyEvents()) {
                            try {
                                ...
                                ackMsg.replyTo = service.getMessenger();
                        ...
                        if (responseApdu != null) {
                            ...
                            int ackCookie = 0;
                            if (Flags.nfcHceLatencyEvents()) {
                                ackCookie = service.generateApduAckCookie();
                                msg.arg1 = ackCookie;
                            ...
                            responseMsg.replyTo = service.getMessenger();
                            try {
                                service.getNfcService().send(responseMsg);
                            } catch (RemoteException e) {
                                if (Flags.nfcHceLatencyEvents()) {
                ...
                case MSG_RESPONSE_APDU:
                    ...
                    try {
                        msg.replyTo = service.getMessenger();
                        service.getNfcService().send(msg);
                    } catch (RemoteException e) {
