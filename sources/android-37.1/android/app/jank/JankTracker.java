...
public class JankTracker {
    ...
    public JankTracker(Choreographer choreographer, View decorView) {
        ...
        mStateTracker = new StateTracker(choreographer, getLayerId());
    ...
    private void initializeJankTrackingComponents() {
        ...
        if (mStateTracker == null) {
            mStateTracker = new StateTracker(viewRoot.getChoreographer(), getLayerId());
        } else {
            mStateTracker.setLayerId(getLayerId());
        }
    ...
    /**
     * Returns current layer ID (which can change if the view gets re-attached from window to
     * window).
     *
     * @return layerId or {@link SurfaceControl#INVALID_LAYER_ID} if unknown.
     */
    private int getLayerId() {
        ViewRootImpl viewRootImpl = mDecorView.getViewRootImpl();
        if (viewRootImpl != null) {
            SurfaceControl surfaceControl = viewRootImpl.getSurfaceControl();
            if (surfaceControl.isValid()) {
                return surfaceControl.getLayerId();
            }
        }

        return SurfaceControl.INVALID_LAYER_ID;
    }
}
