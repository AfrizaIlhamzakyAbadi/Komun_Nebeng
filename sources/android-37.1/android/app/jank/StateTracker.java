...
public class StateTracker {
    ...
    // Layer ID of the owning surface control for accurate traces.
    private int mLayerId = SurfaceControl.INVALID_LAYER_ID;
    ...
    public StateTracker(@NonNull Choreographer choreographer, int layerId) {
    ...
    /** Updates layerId if the view gets re-attached. */
    ...
    private void traceStateStart(StateData stateData) {
        ...
        if (mLayerId != SurfaceControl.INVALID_LAYER_ID) {
            Trace.instantForTrack(
                    TRACE_TAG_APP, stateData.mStateTraceKey, "FT#layerId#" + mLayerId);
        }
        Trace.instantForTrack(
    ...
    private void traceStateEnd(StateData stateData) {
        ...
        // #0 - REASON_END_NORMAL
        Trace.instantForTrack(TRACE_TAG_APP, stateData.mStateTraceKey, "FT#end#0");
        // Ordering here is important - the async track needs to contain the instants and has to
        // complete last.
