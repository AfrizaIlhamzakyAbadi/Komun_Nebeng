...
public final class ApplicationExitInfo implements Parcelable {
    ...
    /**
     * Application process was killed because it exceeded the system memory limits.
     */
    @FlaggedApi(Flags.FLAG_MEMORY_LIMITER_APP_EXITINFO)
    public static final int REASON_MEMORY_LIMITER = 17;
    // LINT.ThenChange(:ReasonToString, :ReasonIntDef)

    // LINT.IfChange(SubReasons)
    ...
    // LINT.ThenChange(:SubReasonToString, :SubReasonIntDef)

    /**
     * The process was killed because the storage volume it was on was being mounted; this
     * would be set only when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    public static final int SUBREASON_VOLUME_MOUNT = 50;

    /**
     * The process was killed because the storage volume it was on was being unmounted; this
     * would be set only when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    ...
    /**
     * The process was killed because its package was hidden (via
     * {@link PackageManager#setApplicationHiddenSettingAsUser}); this would be set only
     * when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    public static final int SUBREASON_PACKAGE_HIDDEN = 52;

    /**
     * The process was killed because its package was deleted (e.g., uninstalled by the
     * user or the system); this would be set only when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    public static final int SUBREASON_PACKAGE_DELETED = 53;

    /**
     * The process was killed because its package was moved between storage volumes; this would
     * be set only when the reason is {@link #REASON_USER_REQUESTED}.
     */
    @Hide
    public static final int SUBREASON_PACKAGE_MOVED = 54;

    /**
     * The process was killed because its data was being cleared; this would
     * be set only when the reason is {@link #REASON_USER_REQUESTED}.
     */
    @Hide
    ...
    /**
     * The process was killed because the storage session for the user was being reset. This is
     * done as a last resort if a storage session fails to exit (typically when vold resets or the
     * system needs to forget volumes) gracefully; this would be set only when the reason
     * is {@link #REASON_OTHER}.
     */
    @Hide
    public static final int SUBREASON_STORAGE_SESSION_RESET = 56;

    /**
     * The process was killed because it failed to launch an activity twice; this
     * would be set only when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    ...
    /**
     * The process was killed because its activity is being restarted; this would be
     * set only when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    public static final int SUBREASON_RESTART_ACTIVITY_PROCESS_TIMEOUT = 58;

    /**
     * The process was killed because it was launching activities too rapidly; this
     * would be set only when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    ...
    /**
     * The process was killed because it needed to be restarted due to a configuration change; this
     * would be set only when the reason is {@link #REASON_OTHER}.
     */
    @Hide
    ...
    /**
     * The process was killed because the client app interacting with the content provider
     * process noticed it is not available and informed the system.
     */
    @Hide
    ...
    // LINT.ThenChange(:ReasonCodes)

    // LINT.IfChange(SubReasonIntDef)
    ...
    @IntDef(prefix = { "SUBREASON_" }, value = {
        ...
        SUBREASON_VOLUME_MOUNT,
        SUBREASON_VOLUME_UNMOUNT,
        SUBREASON_PACKAGE_HIDDEN,
        SUBREASON_PACKAGE_DELETED,
        SUBREASON_PACKAGE_MOVED,
        SUBREASON_APP_DATA_CLEARED,
        SUBREASON_STORAGE_SESSION_RESET,
        SUBREASON_ACTIVITY_LAUNCH_FAILURE,
        SUBREASON_RESTART_ACTIVITY_PROCESS_TIMEOUT,
        SUBREASON_RAPID_ACTIVITY_LAUNCH,
        SUBREASON_CONFIG_RESET_RESTART,
        SUBREASON_PROVIDER_CLIENT_REPORTED_DEATH,
    })
    @Retention(RetentionPolicy.SOURCE)
    public @interface SubReason {
    }
    // LINT.ThenChange(:SubReasons)
    ...
    public static String reasonCodeToString(@Reason int reason) {
        switch (reason) {
            ...
            case REASON_MEMORY_LIMITER:
                return "SYSTEM MEMORY LIMIT EXCEEDED";
            case REASON_UNKNOWN:
            default:
                return "UNKNOWN";
        }
    ...
    @Hide
    // LINT.IfChange(SubReasonToString)
    @NonNull
    public static String subreasonToString(@SubReason int subreason) {
        switch (subreason) {
            ...
            case SUBREASON_VOLUME_MOUNT:
                return "VOLUME MOUNT";
            case SUBREASON_VOLUME_UNMOUNT:
                return "VOLUME UNMOUNT";
            case SUBREASON_PACKAGE_HIDDEN:
                return "PACKAGE HIDDEN";
            case SUBREASON_PACKAGE_DELETED:
                return "PACKAGE DELETED";
            case SUBREASON_PACKAGE_MOVED:
                return "PACKAGE MOVED";
            case SUBREASON_APP_DATA_CLEARED:
                return "APP DATA CLEARED";
            case SUBREASON_STORAGE_SESSION_RESET:
                return "STORAGE SESSION RESET";
            case SUBREASON_ACTIVITY_LAUNCH_FAILURE:
                return "ACTIVITY LAUNCH FAILURE";
            case SUBREASON_RESTART_ACTIVITY_PROCESS_TIMEOUT:
                return "RESTART ACTIVITY PROCESS TIMEOUT";
            case SUBREASON_RAPID_ACTIVITY_LAUNCH:
                return "RAPID ACTIVITY LAUNCH";
            case SUBREASON_CONFIG_RESET_RESTART:
                return "CONFIG CHANGE RESTART";
            case SUBREASON_PROVIDER_CLIENT_REPORTED_DEATH:
                return "PROVIDER CLIENT REPORTED DEATH";
