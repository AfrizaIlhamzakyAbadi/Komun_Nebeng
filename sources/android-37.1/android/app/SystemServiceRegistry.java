...
import static android.permission.flags.Flags.appInteractionAccessControlEnabled;
...
import android.app.contentsafety.IContentRestrictionService;
...
import android.hardware.hid.HidManager;
...
import android.security.IKeyChainManager;
...
public final class SystemServiceRegistry {
    ...
    private static void registerServices() {
                    ...
                    new CachedServiceFetcher<ContentSafetyManager>() {
                        ...
                        public ContentSafetyManager createService(ContextImpl ctx)
                            ...
                            IContentSafetyManager service =
                                    IContentSafetyManager.Stub.asInterface(b);
                            if (android.app.contentsafety.flags.Flags
                                    .contentRestrictionApi()) {
                                IBinder b2 =
                                        ServiceManager.getService(
                                                Context.CONTENT_RESTRICTION_SERVICE);
                                IContentRestrictionService restrictionService =
                                        IContentRestrictionService.Stub.asInterface(b2);
                                return new ContentSafetyManager(ctx, service, restrictionService);
                            } else {
                                return new ContentSafetyManager(ctx, service, null);
                            }
                        }
                    });
        }
        ...
        if (android.app.Flags.refactorModesCode()) {
                        ...
                        return new NotificationManager(
                                new ContextThemeWrapper(outerContext,
                                        Resources.selectSystemTheme(0,
                                                outerContext.getApplicationInfo()
                                                        .targetSdkVersion,
        ...
        registerService(Context.THEME_SERVICE, ThemeManager.class,
                new CachedServiceFetcher<ThemeManager>() {
                    ...
                    public ThemeManager createService(ContextImpl ctx)
                        ...
                        return b == null ? null : new ThemeManager(
                                IThemeManager.Stub.asInterface(b));
                    }
                });
        ...
        if (appInteractionAccessControlEnabled()) {
            registerService(Context.APP_INTERACTION_SERVICE, AppInteractionManager.class,
                    new CachedServiceFetcher<>() {
                        @Override
                        public AppInteractionManager createService(ContextImpl ctx)
                                throws ServiceNotFoundException {
                            IBinder b = ServiceManager.getServiceOrThrow(
                                    Context.APP_INTERACTION_SERVICE);
                            IAppInteractionManager service =
                                    IAppInteractionManager.Stub.asInterface(b);
                            return new AppInteractionManager(service);
                        }
                    });
        }
      ...
      if (enableLskfResetManager()) {
        ...
        registerService(
                ...
                new CachedServiceFetcher<KeyChainManager>() {
                    @Override
                    public KeyChainManager createService(ContextImpl ctx)
                            throws ServiceNotFoundException {
                        IBinder b =
                                ServiceManager.getServiceOrThrow(Context.KEYCHAIN_SERVICE);
                        return new KeyChainManager(IKeyChainManager.Stub.asInterface(b));
                    }
                });
        ...
        if (com.android.hardware.input.Flags.hidApi()) {
            registerService(
                    Context.HID_SERVICE,
                    HidManager.class,
                    new CachedServiceFetcher<HidManager>() {
                        @Override
                        public HidManager createService(ContextImpl ctx) {
                            return new HidManager(ctx.getOuterContext());
                        }
                    });
        }
            ...
            registerService(Context.ATTENTION_SERVICE, AttentionManager.class,
                    new CachedServiceFetcher<>() {
                        ...
                        public AttentionManager createService(ContextImpl ctx)
                                throws ServiceNotFoundException {
                            IBinder binder = ServiceManager.getService(Context.ATTENTION_SERVICE);
                            if (binder == null) {
                                return null;
                            }
                            return new AttentionManager(
                                    IAttentionManager.Stub.asInterface(binder));
                        }
                    });
        }
        ...
        if (sEnableServiceNotFoundWtf && ret == null) {
            ...
            switch (name) {
                ...
                case Context.WIFI_AWARE_SERVICE:
                    ...
                    break;
                case Context.ATTENTION_SERVICE:
                    // Avoid WTF on devices that don't support AttentionService (e.g., Wear OS)
                    if (hasSystemFeatureOpportunistic(ctx, PackageManager.FEATURE_WATCH)) {
                        return null;
                    }
                    break;
