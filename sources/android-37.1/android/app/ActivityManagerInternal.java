...
public abstract class ActivityManagerInternal {
     ...
     *
     * @param subReason The sub-reason for the kill to be reported in
     *                  {@link ApplicationExitInfo#getSubReason}.
     */
    public abstract void killProcess(String processName, int uid, String reason,
            @SubReason int subReason);

    /** Kill the process immediately, by pid, with the specified exit reasons. */
    public abstract void killProcess(int pid, int reason, int subReason, @NonNull String reasonStr);
    ...
    */
    ...
    /** Returns true if {@code uid} is the device DPC. */
    public abstract boolean isDeviceDpc(int uid);

    /**
     * Called by DevicePolicyManagerService to set the uid of the device DPC.
     */
    public abstract void setDeviceDpcUid(int deviceDpcUid);

    /** Returns true if {@code uid} is a profile owner. */
    ...
    /**
     * It is similar to {@link IActivityManager#killApplication(String, int, int, String, int, int)}
     * but it immediately stops the package.
     *
     * <p>Note: Do not call this method from inside PMS's lock, otherwise it'll run into
     * watchdog reset.
     *
     * @param exitInfoSubReason The sub-reason for the kill to be reported in
     *                  {@link ApplicationExitInfo#getSubReason}.
     */
    @Hide
    public abstract void killApplicationSync(String pkgName, int appId,
            @CanBeALL @UserIdInt int userId, String reason,
            @Reason int exitInfoReason, @SubReason int exitInfoSubReason);
