...
@FlaggedApi(Flags.FLAG_ENABLE_HOME_TIME_ZONE_API)
public final class TimeManager {
    ...
    /**
     * Constructs a new {@link TimeManager} instance.
     *
     * <p>Applications should not invoke this constructor directly. Instead, they should obtain an
     * instance through {@link Context#getSystemService(Class)} or {@link
     * Context#getSystemService(String)} using {@link Context#TIME_MANAGER_SERVICE}.
     *
     * @throws ServiceNotFoundException if the underlying time or time zone detector services are
     *     not available.
     */
    @Hide
     ...
     *
     * <p>The returned value may be {@code null} if the home time zone is not set or if the home
     * time zone is invalid. This API only supports Olson IDs (e.g. "Europe/London").
     */
    @RequiresPermission(android.Manifest.permission.READ_HOME_TIME_ZONE)
    @FlaggedApi(Flags.FLAG_ENABLE_HOME_TIME_ZONE_API)
    @Nullable
    public TimeZone getHomeTimeZone() {
        if (DEBUG) {
            Log.d(TAG, "getHomeTimeZone called");
        }
        try {
            String homeTimeZone = mITimeZoneDetectorService.getHomeTimeZone();
            if (homeTimeZone == null) {
                return null;
            }
            return TimeZone.getTimeZone(homeTimeZone);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
