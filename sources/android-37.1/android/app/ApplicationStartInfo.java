...
public final class ApplicationStartInfo implements Parcelable {
    ...
    public void writeToParcel(@NonNull Parcel dest, int flags) {
        ...
        if (Flags.appStartInfoTimestampNpeFix()) {
            // Verify that only non-null key and timestamp pairs are written to the parcel to avoid
            // a potential NPE.
            int count = 0;
            if (mStartupTimestampsNs != null) {
                for (int i = 0; i < mStartupTimestampsNs.size(); i++) {
                    if (mStartupTimestampsNs.keyAt(i) != null
                            && mStartupTimestampsNs.valueAt(i) != null) {
                        count++;
                    }
                }
            }
    ...
    public void dump(@NonNull PrintWriter pw, @Nullable String prefix, @Nullable String seqSuffix,
        ...
        sb.append(prefix)
                ...
                .append(" startComponent=").append(startComponentToString(mStartComponent));
        sb.append('\n');
        if (mStartIntent != null) {
