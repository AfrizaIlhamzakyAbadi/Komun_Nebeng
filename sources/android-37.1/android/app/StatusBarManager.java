...
import android.agenticon.AgentTaskOutcome;
...
import com.android.internal.statusbar.IAgentTaskCallback;
...
public class StatusBarManager {
     ...
     *
     * <p>Adding this action to the {@code intent-filter} of a component in the Manifest will set
     * that as the default icon and action to launch before a call to {@link #setAgentTask} with a
     * non {@code null} {@link AgentTaskState} is made.
     *
     * <p>Example use:
     * <pre>
     * &lt;activity
    ...
    public static final String ACTION_AGENT_TASK_MAIN = "android.app.action.AGENT_TASK_MAIN";
   ...
   /**
     ...
     *
     * <p>Requests will be performed on a best effort basis, with the last request sent taking
     * precedence in case of multiple simultaneous requests.
     *
     * <p>If the request is meant to accompany a {@link Notification}, the correct values should be
     * set to hide the Notification icon in the status bar (see
     * {@link Notification.Builder#setAgentInteractionFlags})
     *
     * <p>The default state icon and action should be set in the Manifest, using
     * {@link #ACTION_AGENT_TASK_MAIN}.
     *
     * @param request         the agent task information to show to the user
     * @param outcomeReceiver a callback for receiving the result of the request, or an error if the
     *                        call could not be processed.
     * @param executor        an executor for calling into {@code outcomeReceiver}
     * @see #ACTION_AGENT_TASK_MAIN
     * @see #canSetAgentTask()
     */
    @Hide
    @FlaggedApi(Flags.FLAG_STATUS_BAR_AGENT_ICON)
    @UserHandleAware
    public void setAgentTask(
            @NonNull AgentTaskUpdate request,
            @NonNull @CallbackExecutor Executor executor,
            @NonNull OutcomeReceiver<AgentTaskOutcome, Throwable> outcomeReceiver
    ) {
        ...
        try {
            final IStatusBarService svc = getService();
            if (svc != null) {
                svc.setAgentTask(mContext.getPackageName(), request, new IAgentTaskCallback.Stub() {
                    @Override
                    public void onResult(AgentTaskOutcome outcome) {
                        executor.execute(() -> outcomeReceiver.onResult(outcome));
                    }

                    @Override
                    public void onError(String message) {
                        executor.execute(() -> outcomeReceiver.onError(
                ...
                executor.execute(() -> outcomeReceiver.onError(
                        new RemoteException("Status bar service null")));
            }
        } catch (RemoteException ex) {
            throw ex.rethrowFromSystemServer();
        }
     ...
     *
     * <p>In order for a caller to be able to do this, they have to hold
     * {@link android.app.role.RoleManager#ROLE_ASSISTANT}, and the user setting has to be enabled.
     *
     * @return {@code true} if this app can currently send requests to modify the agents space, and
     *         the user has currently authorized this.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_STATUS_BAR_AGENT_ICON)
    @UserHandleAware
    public boolean canSetAgentTask() {
        try {
            final IStatusBarService svc = getService();
            if (svc != null) {
                return svc.canSetAgentTask(mContext.getPackageName(), mContext.getUserId());
            }
        } catch (RemoteException ex) {
            throw ex.rethrowFromSystemServer();
        }
    ...
    @Hide
    @FlaggedApi(Flags.FLAG_STATUS_BAR_AGENT_ICON)
    public boolean isAgentTaskFeatureSupported() {
        try {
            final IStatusBarService svc = getService();
            if (svc != null) {
                return svc.isAgentTaskFeatureSupported();
            }
        } catch (RemoteException ex) {
            throw ex.rethrowFromSystemServer();
        }
