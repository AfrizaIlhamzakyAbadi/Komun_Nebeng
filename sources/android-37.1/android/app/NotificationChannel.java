...
public final class NotificationChannel implements Parcelable {
    ...
    public void setVibrationPattern(long[] vibrationPattern) {
        ...
        try {
            ...
            // A pattern from which no valid effect mapping exists is itself invalid. Note
            // that the reverse is not true; some effects have no valid pattern.
            if (Flags.invalidChannelVibrationCleanup()) {
    ...
    private void populateFromXml(TypedXmlPullParser parser, boolean forRestore,
        ...
        long[] storedVibrationPattern = safeLongArray(parser, ATT_VIBRATION, null);
        setVibrationPattern(storedVibrationPattern);
        ...
        if (Flags.invalidChannelVibrationCleanup()) {
            // Pre-CB it was possible to create a channel with `enableVibration(true)` and
            // `setVibrationPattern(<invalid pattern, e.g. new long[] { 0 }>)`. This resulted in a
            // channel that did not vibrate, even though the vibrate switch was shown as ON in the
            // channel settings UI. We're now stricter. So for the previous case, convert these
            // channels to `enableVibration(false)`. This keeps their current behavior, but the UI
            // will reflect the truth (and the user now CAN enable vibration for them).
            boolean hadInvalidVibrationPattern = mVibrationPattern == null
                    && storedVibrationPattern != null && storedVibrationPattern.length > 0;
            if (hadInvalidVibrationPattern) {
    ...
    public static @Nullable String getChannelIdForBundleType(int type) {
        ...
        if (Flags.nmContextualDisplayLaunch()
                && type >= NotificationRule.USER_RULE_ID_START
                && type <= NotificationRule.USER_RULE_ID_END) {
