...
public final class ActivityThread extends ClientTransactionHandler
    ...
    */
    ...
    private class ApplicationThread extends IApplicationThread.Stub {
        ...
        @NeverCompile
        private void dumpMemInfoSharedBitmaps(PrintWriter pw, Debug.MemoryInfo memInfo) {
            pw.println(" ");
            pw.println(" Shared Bitmaps");
            ...
            if (SystemProperties.getBoolean("debug.hwui.bitmap_ashmem_long_name", false)) {
                // Shared bitmap memory maps have long names that uniquely identify the underlying
                // allocation. We can use this information to count unique instances.
                printRow(pw, TWO_COUNT_COLUMNS,
                        "Unique:", memInfo.uniqueBitmapCount, "", memInfo.uniqueBitmapSize);
            } else {
                pw.println(
                        "  (Enable debug.hwui.bitmap_ashmem_long_name sysprop to count unique "
                        + "bitmap allocations)");
            }
        }
        ...
        private void dumpMemInfo(PrintWriter pw, Debug.MemoryInfo memInfo, boolean checkin,
            ...
            dumpMemInfoSharedBitmaps(pw, memInfo);
        ...
        int getProcessState() {
            synchronized (ActivityThread.this.mAppThread) {
    ...
    public static void dumpBitmapsProto(ProtoOutputStream proto, int pid,
        ...
        // Force GC to clear weakly-held Bitmaps.
        // This ensures the dump accurately reflects only strongly reachable Bitmaps.
        forceGc();
        try {
    ...
    /**
     * Returns the last process state reported by ActivityManager.
     */
    @Hide
    public int getProcessState() {
        return mAppThread.getProcessState();
    }
    ...
    private ContextImpl createBaseContextForActivity(ActivityClientRecord r) {
        ContextImpl appContext = ContextImpl.createActivityContext(
                this, r.packageInfo, r.activityInfo, r.token, r.mDisplayId, r.overrideConfig);
    ...
    private ContextImpl createBaseContextForSandboxActivity(@NonNull ActivityClientRecord r) {
        ...
        final ContextImpl activityContext = ContextImpl.createActivityContext(
                this, sdkApk, r.activityInfo, r.token, r.mDisplayId, r.overrideConfig);
     ...
     * This must only be called from debug/dump commands.
     */
    ...
    private void setupGraphicsSupport(Context context) {
        ...
        if (!"android".equals(context.getPackageName())) {
            ...
            File cacheDir;
            if (Flags.improveMessageOnInvalidContextStorageAccess()) {
                cacheDir = context.getCacheDirNoCheck();
            } else {
                cacheDir = context.getCacheDir();
            }
            String tmpDir = cacheDir != null ? cacheDir.getAbsolutePath() : null;
            if (tmpDir != null) {
                // Provide a usable directory for temporary files
                System.setProperty("java.io.tmpdir", tmpDir);
                try {
                    android.system.Os.setenv("TMPDIR", tmpDir, true);
                    Slog.d(TAG, "Set OS env TMPDIR to " + tmpDir);
            ...
            final File codeCacheDir, deviceCacheDir;
            if (Flags.improveMessageOnInvalidContextStorageAccess()) {
                codeCacheDir = deviceContext.getCodeCacheDirNoCheck();
                deviceCacheDir = deviceContext.getCacheDirNoCheck();
            } else {
                codeCacheDir = deviceContext.getCodeCacheDir();
                deviceCacheDir = deviceContext.getCacheDir();
            }
    ...
    private void handleBindApplication(AppBindData data) {
        ...
        // Early initialization for Out of Process Rendering (OOPR).
        // Must happen before any RenderNodes or Bitmaps are created.
        if (com.android.graphics.libgui.flags.Flags.outOfProcessRendering()) {
            String ooprPackages = SystemProperties.get("viewroot.ipc_rendering_packages", "");
            if (!ooprPackages.isEmpty()) {
                for (String pkg : ooprPackages.split(",")) {
                    pkg = pkg.trim();
                    if (!pkg.isEmpty() && pkg.equals(data.appInfo.packageName)) {
                        android.graphics.HardwareRenderer.enableOopr();
                        break;
                    }
                }
            }
        ...
        try {
            ...
            if (b == null) {
                // Isolated processes cannot access ConnectivityService and do not have network
                // access. Proxy settings are not applicable.
                if (!Process.isIsolated()) {
                    Slog.wtf(TAG, "ConnectivityService is null in handleBindApplication!");
                }
    ...
    private InstrumentationInfo prepareInstrumentation(AppBindData data) {
        ...
        // Preemptively register instrumented apps with Perfetto.
        // This improves tracing support in testing scenarios.
        Trace.registerWithPerfetto();
