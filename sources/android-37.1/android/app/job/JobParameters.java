...
public class JobParameters implements Parcelable {
     ...
     *
     * Starting from a version of Android following
     * {@link android.os.Build.VERSION_CODES#CINNAMON_BUN},
     * the system will provide more specific stop reasons such as
     * {@link #STOP_REASON_CONSTRAINT_CONNECTIVITY_NETWORK_UNAVAILABLE}, or
     * {@link #STOP_REASON_CONSTRAINT_CONNECTIVITY_METERED_RESTRICTED}
     */
    ...
    /**
     * The requested connectivity constraint is no longer satisfied because there is no
     * active network on the device.
     */
    @FlaggedApi(Flags.FLAG_CONNECTIVITY_ENHANCED_PENDING_AND_STOP_REASONS)
    public static final int STOP_REASON_CONSTRAINT_CONNECTIVITY_NETWORK_UNAVAILABLE = 19;

    /**
     * The requested connectivity constraint is no longer satisfied because the app is
     * blocked by Data Saver or other background restrictions from using the available
     * metered network.
     * <p>
     * This stop reason specifically identifies when a job is halted due to
     * restricted data usage, ensuring jobs are not improperly executed under
    ...
    @IntDef(prefix = {"STOP_REASON_"}, value = {
            ...
            STOP_REASON_CONSTRAINT_CONNECTIVITY_NETWORK_UNAVAILABLE,
            STOP_REASON_CONSTRAINT_CONNECTIVITY_METERED_RESTRICTED,
    })
    ...
    public void enableCleaner() {
        if (Compatibility.isChangeEnabled(OVERRIDE_HANDLE_ABANDONED_JOBS)) {
            return;
        }
    ...
    public static class JobCleanupCallback implements Runnable {
        ...
        private final boolean mNonBlockingAbandonedJobs;
        ...
        public JobCleanupCallback(
            ...
            mNonBlockingAbandonedJobs = android.app.job.Flags.nonBlockingAbandonedJobs();
        }
        ...
        public void run() {
            ...
            try {
                if (mNonBlockingAbandonedJobs) {
                    mCallback.handleAbandonedJobAsync(mJobId);
                } else {
