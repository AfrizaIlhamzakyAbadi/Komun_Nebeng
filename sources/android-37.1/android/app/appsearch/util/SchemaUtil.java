...
public class SchemaUtil {
     ...
     */
    public static boolean checkPathRecursive(
        ...
        return checkPathRecursive(
                schemas, childSchema, targetSchemaType, propertyPath, segmentIndex + 1);
    }

    /**
     * Verifies that all specified property paths for a set of schemas point to a property of the
     * required schema type.
     *
     * <p>This method iterates through a map of schema names and their corresponding required
     * property paths. For each property path, it recursively checks if the terminal property type
     * matches {@code expectedSchemaType}.
     *
     * @param schemas A map of schema type names to the {@link AppSearchSchema} objects being
     *     verified. This map must contain all schemas referenced in {@code
     *     expectedSchemasPropertyPaths}.
     * @param expectedSchemasPropertyPaths A map where the key is the schema type, and the value is
     *     a Set of property paths within that schema that must adhere to the {@code
     *     expectedSchemaType}.
     * @param expectedSchemaType The required schema type that the properties pointed to by the
     *     paths must match.
     * @throws IllegalArgumentException If any property path does not exist or does not resolve to a
     *     property of the required {@code expectedSchemaType}.
     */
            ...
            for (String propertyPath : entry.getValue()) {
                if (!checkPathRecursive(
                    ...
                    throw new IllegalArgumentException(
                            "The property path of: "
                                    + propertyPath
                                    + " is not the required property type: "
                                    + expectedSchemaType);
                }
            }
        }
