...
public class PolicyEnforcementInfo {
    /**
     * Same as {@link com.android.server.pm.PackageManagerService.PLATFORM_PACKAGE_NAME}
     */
    ...
    /**
     * Returns true if the policy is only enforced by a supervision admin.
     * This encompasses restrictions natively enforced by the platform via the
     * supervision system entity as well as restrictions enforced by a supervision
     * role authority.
     */
    @Hide
    public boolean isOnlyEnforcedBySupervision() {
        return mAllAdmins.size() == 1 && isSupervisionAdmin(mAllAdmins.get(0));
    }

    /**
     * Returns true if the policy is only enforced by the supervision system entity.
     */
    @Hide
    public boolean isOnlyEnforcedBySupervisionSystemEntity() {
        return mAllAdmins.size() == 1 && isSupervisionSystemEntity(mAllAdmins.get(0));
    }
     ...
     * admin. If the policy is enforced by system authorities as base restriction, it won't be
     * shown as disabled by admin.
     */
    @Hide
    public boolean shouldShowEnforcingAdminDetails() {
        return isEnforced() && !isEnforcedAsBaseRestriction();
    }

    private boolean isEnforcedAsBaseRestriction() {
        // Base restriction covers where system sets the restriction using legacy
        // UserManager APIs. They are queried and added to PolicyEnforcementInfo by generic
        // platform package name by DevicePolicyManager. Other system entities such as supervision
        // or advanced protection that sets restrictions with DevicePolicyManager APIs are
        // represented with their proper system entity names.
        return mAllAdmins.stream().anyMatch(
                admin -> admin.getAuthority() instanceof SystemAuthority authority
                        && PLATFORM_PACKAGE_NAME.equals(authority.getSystemEntity()));
    }
