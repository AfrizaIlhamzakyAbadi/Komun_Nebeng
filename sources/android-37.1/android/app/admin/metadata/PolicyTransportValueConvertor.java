...
/**
 * Helper to convert a plain policy value to and from a policy transport. Use {@link
 * PolicyTransportValueConvertor#getInstance} to obtain the correct type-safe converter for a
 * policy.
 *
...
public abstract class PolicyTransportValueConvertor<T> {
    ...
    public static <T> PolicyTransportValueConvertor<T> getInstance(PolicyMetadata<T> metadata) {
        ...
        return (PolicyTransportValueConvertor<T>)
                switch (metadata) {
                    ...
                    default ->
                            throw new UnsupportedOperationException(
                                    "Unsupported policy conversion for " + metadata.getId());
                };
    }
    ...
    private static <T> PolicyTransportValueConvertor<List<T>> getListInstance(
        ...
        // instead.
        return (PolicyTransportValueConvertor)
                switch (metadata.getElementMetadata()) {
                    case StringPolicyMetadata m -> LIST_OF_STRING_CONVERTOR;
                    case PackagePolicyMetadata m -> LIST_OF_PACKAGE_CONVERTOR;
                    default ->
                            throw new UnsupportedOperationException(
                                    "Unsupported list policy conversion for " + metadata.getId());
                };
    }
