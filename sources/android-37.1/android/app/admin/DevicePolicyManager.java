...
public class DevicePolicyManager {
    ...
    /**
     * Constant for {@link #getMinimumRequiredWifiSecurityLevel()} and
     * {@link #setMinimumRequiredWifiSecurityLevel(int)}: personal network without WEP.
     * This level covers {@link #WIFI_SECURITY_PERSONAL} but not WEP.
     *
     * <p> When returned from {@link #getMinimumRequiredWifiSecurityLevel()}, the constant
     * represents the current minimum security level required.
     * When passed to {@link #setMinimumRequiredWifiSecurityLevel(int)}, it sets the
     ...
     * @see #WIFI_SECURITY_ENTERPRISE_192
     */
    @FlaggedApi(Flags.FLAG_DPM_SECURITY_LEVEL_WPA_ONLY)
     ...
     * {@link #ACTION_PROVISION_MULTIUSER_MANAGED_USER} when the device is not running a compatible
     * OS build.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_MULTI_USER_MANAGEMENT_DEVICE_PROVISIONING)
    ...
    @IntDef(prefix = { "STATUS_" }, value = {
            ...
            STATUS_INCOMPATIBLE_OS_BUILD, STATUS_OTHER_PROVISIONING_ERROR,
    ...
    /**
     * Service-specific error code for {@link #generateKeyPair}:
     * Indicates the call has failed with an internal error.
     */
    @Hide
     ...
     * <p>
     * This policy can be set by a device owner, a profile owner, or holders of the permission
     ...
     * @throws SecurityException if {@code admin} is not the device owner, a profile owner, or a
    ...
    /**
     * See {@link #ACCOUNT_FEATURE_DEVICE_OR_PROFILE_OWNER_ALLOWED}
     */
    @SystemApi
    ...
    public boolean removeKeyPair(@Nullable ComponentName admin, @NonNull String alias) {
        ...
        try {
            if (android.security.Flags.enableDeviceCertificates()) {
                return mService.removeKeyPairWithScope(
                        mContext.getPackageName(), alias, KeyChainManager.KEYPAIR_SCOPE_USER);
            }
    ...
    /**
     * This API can be called by the following to remove a certificate and private key pair
     * installed under a given alias, specifying whether it resides in user-specific or
     * device-wide keychain:
     * <ul>
     *    <li>Device owner</li>
     *    <li>Profile owner</li>
     *    <li>Delegated certificate installer</li>
     ...
     * </ul>
     *
     * <p>This method extends {@link #removeKeyPair(ComponentName, String)}, adding the
     * {@code scope} parameter to determine if the key pair should be removed from
     * user-specific or device-wide keychain. The calling application must hold the same
     * permission requirements as the base method.
     *
     * <p>Key differences in this overload:
     * <ul>
     *   <li>Using {@link KeyChainManager#KEYPAIR_SCOPE_DEVICE}: The key pair will be removed
     *       from the device-wide keychain. The caller MUST be running in an affiliated user
     *       to manage device keys. See {@link #isAffiliatedUser}. Credential management apps
     *       are not permitted to use this scope.
     *   <li>Using {@link KeyChainManager#KEYPAIR_SCOPE_USER}: The key pair is removed from
     *       the calling user's own keychain. This mirrors the behavior of the base
     *       {@code removeKeyPair} method.
     * </ul>
     *
     * @param alias The private key alias under which the certificate is installed.
     * @param scope The scope of the key pair: {@link KeyChainManager#KEYPAIR_SCOPE_USER} or
     *     {@link KeyChainManager#KEYPAIR_SCOPE_DEVICE}.
     * @return {@code true} if the private key alias no longer exists, {@code false} otherwise.
    ...
    public boolean removeKeyPair(
            @NonNull String alias, @KeyChainManager.KeyPairScope int scope) {
        throwIfParentInstance("removeKeyPair");
        try {
            return mService.removeKeyPairWithScope(mContext.getPackageName(), alias, scope);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public boolean hasKeyPair(@NonNull String alias) {
        ...
        try {
            if (android.security.Flags.enableDeviceCertificates()) {
                return mService.hasKeyPairWithScope(
                    mContext.getPackageName(), alias, KeyChainManager.KEYPAIR_SCOPE_USER);
            } else {
     ...
     * </ul>
     *
     * <p>This method extends {@link #hasKeyPair(ComponentName, String)}, adding the
     * {@code scope} parameter to determine if the key pair exists in the
     * user-specific or device-wide keychain. The calling application must hold the same
     * permission requirements as the base method.
     *
     * <p>Key differences in this overload:
     * <ul>
     *   <li>Using {@link KeyChainManager#KEYPAIR_SCOPE_DEVICE}: The key pair will be searched
     *       in the device-wide keychain. The caller MUST be running in an affiliated user
     *       to manage device keys. See {@link #isAffiliatedUser}. Credential management apps
     *       are not permitted to use this scope.
     *   <li>Using {@link KeyChainManager#KEYPAIR_SCOPE_USER}: The key pair is searched in
     *       the calling user's own keychain. This mirrors the behavior of the base
     *       {@code hasKeyPair} method.
     * </ul>
     *
     * @param alias The alias under which the key pair is installed.
     ...
     * @return {@code true} if a key pair with this alias exists, {@code false} otherwise.
     * @see #hasKeyPair(String)
     */
    @RequiresPermission(value = MANAGE_DEVICE_POLICY_CERTIFICATES, conditional = true)
    @FlaggedApi(android.security.Flags.FLAG_ENABLE_DEVICE_CERTIFICATES)
    public boolean hasKeyPair(@NonNull String alias, @KeyChainManager.KeyPairScope int scope) {
        throwIfParentInstance("hasKeyPair");
        try {
            return mService.hasKeyPairWithScope(mContext.getPackageName(), alias, scope);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     *       MUST be running in an affiliated user to use this scope. Credential management apps
     *       are not permitted to use this scope.
     ...
     *     KEYPAIR_SCOPE_DEVICE} is requested by a non-affiliated user or credential management app.
        ...
        try {
            ...
            switch (e.errorCode) {
                ...
                case KEY_GEN_FAILURE:
                    throw new KeyChainException("Internal error while generating key.", e);
                default:
                    throw new KeyChainException(
    ...
    */
     ...
     * <p> This method can be called on the {@link DevicePolicyManager} instance,
     * returned by {@link #getParentProfileInstance(ComponentName)}, where the caller must be
     * a profile owner of an organization-owned device. If called on the parent instance, the
     * policy will be applied to the personal profile as well as any associated user profiles.
     *
     * @param policy One of the {@code NearbyStreamingPolicy} constants.
     * @throws SecurityException if caller is not a device or profile owner.
     */
    public void setNearbyAppStreamingPolicy(@NearbyStreamingPolicy int policy) {
        // TODO(b/440352486): Add documentation with SDK version from which this policy can be
        //  applied to parent instances
        if (!android.companion.virtualdevice.flags.Flags
                .nearbyAppStreamingPolicySupportParentProfile()) {
        ...
        try {
            mService.setNearbyAppStreamingPolicy(policy, mParentInstance);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public @NearbyStreamingPolicy int getNearbyAppStreamingPolicy(int userId) {
        ...
        try {
            return mService.getNearbyAppStreamingPolicy(userId, mParentInstance);
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
        }
    }
     ...
     *         {@code MANAGE_MULTIUSER_DEVICE_PROVISIONING_STATE} or
     *         {@code MANAGE_PROFILE_AND_DEVICE_OWNERS} permission.
     */
    @SystemApi
    @FlaggedApi(Flags.FLAG_MULTI_USER_MANAGEMENT_DEVICE_PROVISIONING)
    @RequiresPermission(anyOf = {
    ...
    @SystemApi
    @FlaggedApi(Flags.FLAG_MULTI_USER_MANAGEMENT_DEVICE_PROVISIONING)
    @RequiresPermission(value = MANAGE_MULTIUSER_DEVICE_PROVISIONING_STATE)
     ...
     * the other.
     * <p>
     * <b>Warning:</b> Do not use this method in Android Automotive environments
     * featuring {@link android.content.pm.PackageManager#FEATURE_CAR_SPLITSCREEN_MULTITASKING}.
     * This automotive split-screen multi-tasking feature does not support Lock Task Mode.
     * Consequently, if this method is called in such an environment, it will not work
     * as expected, and the app will not remain locked as the single, exclusive application.
     *
     ...
     * device to disable the status bar.
     * <p>
...
//    @RequiresPermission(value = MANAGE_DEVICE_POLICY_WIFI, conditional = true)
     ...
     * <li>{@link #wipeData}</li>
     * <li>{@link #setNearbyAppStreamingPolicy}</li>
     * <li>{@link #getNearbyAppStreamingPolicy}</li>
     * </ul>
     *
     * @param admin Which {@link DeviceAdminReceiver} this request is associated with or
     ...
     * May be called by the device owner. Starting from Android 18, it can also be called by the
     ...
     *
     * @throws SecurityException if the caller is neither the device owner nor the profile owner of
     * an organization-owned device.
     */
     ...
     *
     * <p>May only be called by the device owner.
     *
     ...
     *         Returns {@code PRIVATE_DNS_SET_ERROR_FAILURE_SETTING} if it could not be set.
     *         Returns {@code PRIVATE_DNS_SET_ERROR_HOST_NOT_SERVING} if the specified host does not
     *         implement RFC7858.
     *
     ...
     *
     * <p>This may be called by the device owner. Starting from Android 18, it can also be called
     ...
     * @throws SecurityException if the caller is neither the device owner nor a profile owner
     ...
     *
     * @throws IllegalArgumentException when package with name returned by {@link
     *     MultiuserManagedDeviceProvisioningParams#getDeviceControllerPackageName()} is not found.
     * @throws ProvisioningException if an error occurred during provisioning.
     */
    @SystemApi
     ...
     *
     * <p> The security levels are explicitly ordered from least secure to most secure as follows:
     * <ol>
     *   <li>{@link #WIFI_SECURITY_OPEN}</li>
     *   <li>{@link #WIFI_SECURITY_PERSONAL}</li>
     *   <li>{@link #WIFI_SECURITY_PERSONAL_WPA}</li>
     *   <li>{@link #WIFI_SECURITY_ENTERPRISE_EAP}</li>
     *   <li>{@link #WIFI_SECURITY_ENTERPRISE_192}</li>
     * </ol>
     *
    ...
    /**
     * Returns the resolved value of the given per-user policy. The resolved value is the value that
     * is currently being enforced on the device for the context user after conflict resolution has
     * been performed between all the policies set by all admins at both the device and user scope.
     *
     * <p>Use {@link #getPolicy} to get the value set by the caller, which may differ.
     *
     ...
     * @throws SecurityException If the caller does not have sufficient permissions to get the
     *     specified policy. Check the documentation of individual identifiers for more details. The
     *     QUERY_ADMIN_POLICY permission can in most cases be used to replace the per-policy
     *     permission, but the cros-user permission is still checked when querying a different user.
     */
    @FlaggedApi(FLAG_POLICY_STREAMLINING)
    @UserHandleAware
    @Nullable
    ...
    private static final Object sPolicyValueCallbackHandlerLock = new Object();
    private static PolicyValueCallbackHandler sPolicyValueCallbackHandler;

    /**
     * Resets the policy value callback handler for testing.
     */
    @Hide
    @VisibleForTesting
    public static void resetPolicyValueCallbackHandlerForTesting() {
        synchronized (sPolicyValueCallbackHandlerLock) {
            sPolicyValueCallbackHandler = null;
        }
    }

    /**
     * Subscribes to changes to the resolved value of the given policy.
     *
     * <p> The callback will be invoked when the resolved policy value changes. It will not
     * be invoked with the initial value upon registration. Callers should use
     * {@link #getResolvedDeviceWidePolicy} to retrieve the current value.
     *
     * <p> Can only applied on a {@link #RESOURCE_DEVICE_WIDE} policy. Since device-wide policies
     * do not change per-user, the user is ignored.
     *
     * <p> If the same callback instance is registered multiple times (e.g., for different
     * policies), the executor provided in each registration will be used for that policy's
     * updates. Consequently, the callback may be invoked by different executors concurrently.
     * Note that when registering a callback for the same policy identifier, only the executor
     * from the first call is taken into account and future registrations are ignored.
     *
     * @param <T> The type of the policy.
    ...
    public <T> void addResolvedDeviceWidePolicyCallback(
        ...
        throwIfParentInstance("addResolvedDeviceWidePolicyCallback");
        if (mService != null) {
            synchronized (sPolicyValueCallbackHandlerLock) {
                if (sPolicyValueCallbackHandler == null) {
                    sPolicyValueCallbackHandler =
                            new PolicyValueCallbackHandler(mContext.getPackageName(), mService);
                }
            }
            sPolicyValueCallbackHandler.addResolvedDeviceWidePolicyCallback(id, executor, callback);
        }
    }

    /**
     * Remove a callback for policy changes.
     *
    ...
    public <T> void removeResolvedDeviceWidePolicyCallback(
        ...
        throwIfParentInstance("removeResolvedDeviceWidePolicyCallback");
        if (mService != null) {
            synchronized (sPolicyValueCallbackHandlerLock) {
                if (sPolicyValueCallbackHandler != null) {
                    sPolicyValueCallbackHandler.removeResolvedDeviceWidePolicyCallback(
                            id, callback);
                }
            }
        }
