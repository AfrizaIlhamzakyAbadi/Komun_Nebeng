...
"/vendor/etc/aconfig_flags.pb",
        ...
        for (String path : parsedFlagsProtoPaths()) {
            try (FileInputStream inputStream = new FileInputStream(path)) {
                ...
                for (parsed_flag flag : parsedFlags.parsedFlag) {
                    if (isContainerMatch(flag.container, path)) {
                        result.add(flag);
                    }
                }
            }
    ...
    private static boolean isContainerMatch(String container, String path) {
        ...
        if ("apex".equals(partition)) {
            if (parts.length < 3) {
                return false;
            }
            String apexName = parts[2];
            // Normalize apex name: com.google.android.foo -> com.android.foo
            String normalizedApexName = apexName.replace("com.google.", "com.");
            return container.equals(normalizedApexName);
        } else {
            return container.equals(partition);
        }
    }
