...
import static com.android.internal.tracing.winscope.protos.Softinputwindow.SoftInputWindowProto.WINDOW_STATE;
    ...
    public void show() {
        switch (mWindowState) {
            ...
            case WindowState.SHOWN_AT_LEAST_ONCE:
                ...
                try {
                    super.show();
                    if (Flags.requestApplyInsetsOnShow()) {
                        // The input view or candidates view might be created after WMS
                        // dispatches the initial insets, causing them to miss the dispatch.
                        // Explicitly requesting insets here ensures that the newly created view
                        // hierarchy receives the latest cached insets when the window is shown.
