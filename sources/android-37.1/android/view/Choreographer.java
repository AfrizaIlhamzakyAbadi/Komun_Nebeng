...
public final class Choreographer {
    ...
    // Enable/disable buffer stuffing recovery.
    private static final boolean USE_BUFFER_STUFFING_RECOVERY = SystemProperties.getBoolean(
            "debug.choreographer.use_buffer_stuffing_recovery", true);
    ...
    private final AnimationUtils.AnimationTime mAnimationTime = AnimationUtils.getAnimationTime();
    ...
    public void onWaitForBufferRelease(long durationNanos) {
        if (!USE_BUFFER_STUFFING_RECOVERY) {
            return;
        }
    ...
    public long getFrameTimeNanos() {
        synchronized (mLock) {
            ...
            } else if (noMoreResync()) {
                return mAnimationTime.getCurrentNanos();
            } else {
    ...
    private void scheduleFrameLocked(long now) {
        if (!mFrameScheduled) {
            ...
            if (USE_VSYNC) {
                ...
                long lastFrame =
                        (noMoreResync() ? mAnimationTime.getLastVsyncNanos() : mLastFrameTimeNanos);
                final long nextFrameTime = Math.max(
                        lastFrame / TimeUtils.NANOS_PER_MS + sFrameDelay, now);
                if (DEBUG_FRAMES) {
    ...
    private boolean hasPendingInput() {
        synchronized (mLock) {
            return mCallbackQueues[CALLBACK_INPUT].hasDueCallbacksLocked(
    ...
    BufferStuffingState.RecoveryAction updateBufferStuffingState(long frameTimeNanos,
        ...
        if (bufferStuffingMultiRecovery()) {
            ...
            if (mBufferStuffingState.isStuffed.getAndSet(false)) {
                ...
                if (mBufferStuffingState.maxDelayReached()) {
                    Trace.instant(Trace.TRACE_TAG_VIEW,
                            "buffer stuffed - max recovery delay reached");
                } else if (hasPendingInput()) {
                    Trace.instant(Trace.TRACE_TAG_VIEW,
                            "buffer stuffed - skip recovery delay, pending input");
                } else {
                    Trace.instant(Trace.TRACE_TAG_VIEW, "buffer stuffed - delay a frame");
    ...
    void doFrame(long frameTimeNanos, int frame,
            DisplayEventReceiver.VsyncEventData vsyncEventData) {
        if (noMoreResync()) {
            doFrameNoAnimationResync(frameTimeNanos, vsyncEventData);
            return;
        }
        ...
        try {
            ...
            synchronized (mLock) {
                ...
                mFrameInfo.setVsync(intendedFrameTimeNanos, frameTimeNanos,
                        ...
                        vsyncEventData.frameInterval, frameTimeNanos, -1);
            ...
            AnimationUtils.lockAnimationClock(frameTimeNanos, frameIntervalNanos);
    ...
    void doFrameNoAnimationResync(
                long frameTimeNanos, DisplayEventReceiver.VsyncEventData vsyncEventData) {
        // TODO(b/470321586): once noMoreResync lands, FrameData.update should be changed to no
        // longer update the current time, but simply copy over the timeline data.
        ...
        try {
            ...
            if (updateBufferStuffingState(frameTimeNanos, vsyncEventData)
                    == BufferStuffingState.RecoveryAction.DELAY_FRAME) {
                // Intentional frame delay to help reduce queued buffer count.
                mBufferStuffingState.numberWaitsForNextVsync++;
                mBufferStuffingState.accumulatedDelayNanos += frameIntervalNanos;
                scheduleVsyncLocked();
            ...
            synchronized (mLock) {
                if (!mFrameScheduled) {
                    Trace.instant(Trace.TRACE_TAG_VIEW, "Frame not scheduled");
                    return; // no work to do
                }

                // TODO(b/470321586): once noMoreResync lands, this can be removed, and
                // updateBufferStuffingState can be simplified.
                mLastNoOffsetFrameTimeNanos = frameTimeNanos;

                if (frameTimeNanos < mAnimationTime.getLastVsyncNanos()) {
                    Trace.instant(Trace.TRACE_TAG_VIEW, "Frame time goes backward");
                    if (mBufferStuffingState.isRecovering) {
                        mBufferStuffingState.numberWaitsForNextVsync++;
                    }
                    scheduleVsyncLocked();
                ...
                final long jitterNanos = startNanos - frameTimeNanos;
                if (jitterNanos >= frameIntervalNanos) {
                    long renderTime = timeline.mDeadlineNanos - frameTimeNanos;
                    if (frameIntervalNanos == 0) {
                        frameTimeNanos = startNanos;
                        Trace.instant(Trace.TRACE_TAG_VIEW, "Vsync data empty due to timeout");
                    } else {
                        frameTimeNanos = startNanos - jitterNanos % frameIntervalNanos;
                    }

                    long minDeadline = frameTimeNanos + renderTime * 4 / 5;
                    timeline = mFrameData.resync(mDisplayEventReceiver, minDeadline);
                    resynced = true;
                }

                final long resyncDelta = mAnimationTime.tick(frameTimeNanos, frameIntervalNanos);

                mFrameInfo.setVsync(frameTimeNanos - resyncDelta, frameTimeNanos,
                        timeline.mVsyncId, timeline.mDeadlineNanos, startNanos,
                        vsyncEventData.frameInterval, mAnimationTime.elapsedSinceAnimationStart(),
                        mAnimationTime.getAnimationCounter());
                mFrameScheduled = false;
                mLastFrameIntervalNanos = frameIntervalNanos;
                mLastVsyncEventData.copyFrom(vsyncEventData);

                mFrameData.mFrameTimeNanos = mAnimationTime.getCurrentNanos();
                mFrameData.mFrameTimeNanosForInput = frameTimeNanos;
            }
            ...
            mInDoFrameCallback = false;
            mAnimationTime.onCallbacksCompleted();
    ...
    private void scheduleVsyncLocked() {
        try {
            ...
            mAnimationTime.onVsyncScheduled();
        } finally {
            Trace.traceEnd(Trace.TRACE_TAG_VIEW);
        }
    }
    ...
    // Used in robolectric tests.
    private static boolean isNoMoreResyncFlagEnabled() {
        return noMoreResync();
    }
    ...
    public static class FrameData {
        ...
        public FrameData(long frameTimeNanos) {
            ...
            mFrameTimeNanosForInput = frameTimeNanos;
            setInCallback(true);
        }
        ...
        @Hide
        public long getFrameTimeNanosForInput() {
            checkInCallback();
            return mFrameTimeNanosForInput;
        }
        ...
        /**
         * Update the frame timeline data skipping frames, when we are late.
         */
        FrameTimeline resync(DisplayEventReceiver displayEventReceiver, long minDeadline) {
            int newPreferredIndex = 0;
            // Look for a non-past deadline timestamp in the existing frame data. Otherwise, binder
            // query for new frame data. Note that binder is relatively slow, O(ms), so it is
            // only called when the existing frame data does not hold a valid frame.
            while (newPreferredIndex < mFrameTimelines.length - 1
                    && mFrameTimelines[newPreferredIndex].mDeadlineNanos < minDeadline) {
                newPreferredIndex++;
            }

            if (USE_VSYNC && mFrameTimelines[newPreferredIndex].mDeadlineNanos < minDeadline) {
                DisplayEventReceiver.VsyncEventData latestVsyncEventData =
                        displayEventReceiver.getLatestVsyncEventData();
                if (latestVsyncEventData == null) {
                    Log.w(TAG, "Could not get latest VsyncEventData. Did SurfaceFlinger crash?");
                } else {
                    update(0, latestVsyncEventData);
                }
            } else {
                mPreferredFrameTimelineIndex = newPreferredIndex;
            }
            return mFrameTimelines[mPreferredFrameTimelineIndex];
        }
    }
