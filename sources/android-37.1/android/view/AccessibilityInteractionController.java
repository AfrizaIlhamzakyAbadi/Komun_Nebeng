...
public final class AccessibilityInteractionController {
    ...
    private void scheduleMessage(Message message, int interrogatingPid, long interrogatingTid,
            boolean considerRequestPreparers) {
        // If more time is needed to handle a prior request or prepare for this one,
        // don't schedule the message.
        if (considerRequestPreparers
    ...
    private void scheduleAllMessagesWaitingForRequestPreparerLocked() {
        ...
        for (int i = 0; i < numMessages; i++) {
            ...
            scheduleMessage(request.mMessage, request.mInterrogatingPid,
                    ...
                    (i != 0) /* First request can ignore preparers, as the app should be ready. */);
        }
    ...
    private void getWindowSurfaceInfoUiThread(IWindowSurfaceInfoCallback callback) {
        try {
            ...
            if (sc.isValid()) {
                ...
                if (Flags.fixTakeScreenshotOfWindowBounds()) {
                    // For floating windows, the capture bounds need to be offset by the surface
                    // insets. For non-floating windows, the surface insets are 0, so this
                    // behaves correctly.
                    Rect insets = mViewRootImpl.mWindowAttributes.surfaceInsets;
                    Rect crop = new Rect(
                            insets.left, insets.top,
                            insets.left + mViewRootImpl.mWidth,
                            insets.top + mViewRootImpl.mHeight);
                    callback.provideWindowSurfaceInfoWithBounds(
                            mViewRootImpl.getWindowFlags(),
                            Process.myUid(),
                            copiedSc,
                            crop);
                } else {
                    callback.provideWindowSurfaceInfo(
                            mViewRootImpl.getWindowFlags(),
                            Process.myUid(),
                            copiedSc);
                }
