...
public final class PixelCopy {
    ...
    public static void request(@NonNull SurfaceView source, @NonNull Bitmap dest,
        ...
        final SurfaceView.SurfaceForPixelCopy surfaceForPixelCopy = source.getSurfaceForPixelCopy();
        request(surfaceForPixelCopy.getSurface(), dest,
                wrapListener(listener, surfaceForPixelCopy), listenerThread);
    }
    ...
    public static void request(@NonNull SurfaceView source, @Nullable Rect srcRect,
        ...
        final SurfaceView.SurfaceForPixelCopy surfaceForPixelCopy = source.getSurfaceForPixelCopy();
        request(surfaceForPixelCopy.getSurface(), srcRect, dest,
                wrapListener(listener, surfaceForPixelCopy), listenerThread);
    }

    private static OnPixelCopyFinishedListener wrapListener(
            @NonNull OnPixelCopyFinishedListener listener,
            @NonNull SurfaceView.SurfaceForPixelCopy surfaceForPixelCopy) {
        return new OnPixelCopyFinishedListener() {
            @Override
            public void onPixelCopyFinished(int copyResult) {
                try {
                    listener.onPixelCopyFinished(copyResult);
                } finally {
                    if (surfaceForPixelCopy.isTemporary()) {
                        surfaceForPixelCopy.getSurface().release();
                    }
                }
            }
        };
    ...
    public static final class Request {
        ...
        private final boolean mShouldReleaseSource;
        private final Rect mSourceInsets;
        private Rect mSrcRect;
        private Bitmap mDest;

        private Request(Surface source, Rect sourceInsets) {
            this(source, sourceInsets, false);
        }

        private Request(Surface source, Rect sourceInsets, boolean shouldReleaseSource) {
            this.mSource = source;
            this.mSourceInsets = sourceInsets;
            this.mShouldReleaseSource = shouldReleaseSource;
        }
        ...
        public static final class Builder {
            ...
            public static @NonNull Builder ofSurface(@NonNull SurfaceView source) {
                final SurfaceView.SurfaceForPixelCopy surfaceForPixelCopy =
                        source.getSurfaceForPixelCopy();
                final Surface surface = surfaceForPixelCopy.getSurface();
                if (surface == null || !surface.isValid()) {
                    if (surface != null && surfaceForPixelCopy.isTemporary()) {
                        surface.release();
                    }
                    throw new IllegalArgumentException("Source must not be null & must be valid");
                }
                final Request request =
                        new Request(surface, null, surfaceForPixelCopy.isTemporary());
        ...
        public void request(@NonNull Executor callbackExecutor,
            ...
            final Consumer<Result> wrappedListener = (result) -> {
                try {
            ...
            if (!mSource.isValid()) {
                callbackExecutor.execute(() -> wrappedListener.accept(
        ...
        private void maybeReleaseSource() {
            if (mShouldReleaseSource && mSource != null) {
                mSource.release();
            }
        }
    }
