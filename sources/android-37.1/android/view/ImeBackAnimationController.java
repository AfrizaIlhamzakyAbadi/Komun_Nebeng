...
public class ImeBackAnimationController implements OnBackAnimationCallback {
    ...
    public void onBackStarted(@NonNull BackEvent backEvent) {
        if (!isBackAnimationAllowed()) {
            ...
            if (com.android.window.flags.Flags.syncedInsetsAnimation()
                ...
                Log.d(TAG, "onBackStarted -> not playing predictive back animation due to no "
                        + "animation callback registered");
            } else {
    ...
    private void startPostCommitAnim(boolean triggerBack) {
        ...
        long duration;
        boolean isAdjustResize =
                (mViewRoot.mWindowAttributes.softInputMode & SOFT_INPUT_MASK_ADJUST)
                        == SOFT_INPUT_ADJUST_RESIZE;
        final boolean isSyncedAnimation =
                com.android.window.flags.Flags.predictiveBackSyncedImeAnimation()
                        && mViewRoot.usesSyncedInsetsAnimationByDefault()
                        && isAdjustResize;
        if (triggerBack && (mViewRoot.mView.hasWindowInsetsAnimationCallback() || isSyncedAnimation)
    ...
    private boolean isBackAnimationAllowed() {
        ...
        // 3. edge-to-edge is not enabled AND
        // 4. the synchronized insets animation flags are not enabled.
