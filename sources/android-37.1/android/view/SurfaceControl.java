...
import static com.android.internal.tracing.winscope.protos.Surfacecontrol.SurfaceControlProto.HASH_CODE;
import static com.android.internal.tracing.winscope.protos.Surfacecontrol.SurfaceControlProto.LAYER_ID;
import static com.android.internal.tracing.winscope.protos.Surfacecontrol.SurfaceControlProto.NAME;
...
public final class SurfaceControl implements Parcelable {
    ...
    private static final NativeProperties RELEASED_NATIVE_PROPERTIES =
            new NativeProperties(0, "<released>", INVALID_LAYER_ID);
    ...
    private static native void nativeSetBackgroundBlurRadius(long transactionObj, long nativeObject,
            int blurRadius);
    private static native void nativeSetBlurThrottlingTargetFps(long transactionObj,
            long nativeObject, int blurThrottlingTargetFps);
    ...
    private static native void nativeSetDisplayExclusionMask(long transactionObj,
            IBinder displayToken, int mask);
    ...
    private static native int nativeSetDesiredDisplayModeSpecs(
    ...
    public static class JankData {
        ...
        private final @DurationNanosLong long mActualPresentDeltaNs;
        ...
        public JankData(long frameVsyncId, @JankType int jankTypeLegacy,
                ...
                long presentDelayNs, long actualPresentDeltaNs, double jankScore) {
            ...
            mActualPresentDeltaNs = actualPresentDeltaNs;
        ...
        /**
         * Returns the time between actual present time of this frame and the previous frame.
         *
         * @return actual present delta in ns
         */
        @Hide
        public @DurationNanosLong long getActualPresentDeltaNanos() {
            return mActualPresentDeltaNs;
        }
    ...
    /**
     * Surface flag: The surface would be opaque if not for client-drawn rounded corners.
     */
    @Hide
    public static final int SURFACE_WOULD_BE_OPAQUE_IF_NOT_FOR_CLIENT_ROUNDED_CORNERS = 0x8000;
     ...
     * com.android.internal.tracing.winscope.protos.Surfacecontrol.SurfaceControlProto}.
     *
    ...
    /**
     * Result of setting desired display mode specs.
     */
    @Hide
    public enum ModeSpecsUpdateResult {
        /**
         * Indicates that display mode changes will occur. The client should wait for
         * confirmation that the change is complete before calling this method again.
         * <p>
         * The client should expect an invocation of the listener registered via
         * {@link #addDisplayEventListener} when this value is returned.
         * Clients should not expect this callback in other cases.
         */
        SUCCESS_MODES_CHANGED,

        /**
         * Indicates that no display mode changes will occur. The client may call this
         * method again immediately.
         */
        SUCCESS_MODES_UNCHANGED,

        /**
         * Indicates that an error occurred when updating display mode specs.
         */
        FAILURE;

        /**
         * Converts an integer to a ModeSpecsUpdateResult.
         */
        @Hide
        @NonNull
        public static ModeSpecsUpdateResult fromInt(int value) {
            return switch (value) {
                case 0 -> SUCCESS_MODES_CHANGED;
                case 1 -> SUCCESS_MODES_UNCHANGED;
                case 2 -> FAILURE;
    ...
    /**
     * Listener for display-related events.
     */
    @Hide
    public interface OnDisplayEventListener {
        /**
         * Called when a desired display mode specs update request has been processed.
         *
         * @param applyToken The display sync token
         * @param result the ModeChangeResult for the corresponding modeset
         */
        void onDisplayModeChange(@NonNull IBinder applyToken, ModeChangeResult result);
    }

    private static class DisplayEventListenerWrapper {

        private final OnDisplayEventListener mListener;

        DisplayEventListenerWrapper(OnDisplayEventListener listener) {
            mListener = listener;
        }

        // Called by JNI
        private void dispatchDisplayModeChange(IBinder applyToken, int result) {
            mListener.onDisplayModeChange(applyToken, ModeChangeResult.fromInt(result));
        }
    }

    /**
     * Result of a display mode update, reported via {@link OnDisplayEventListener}.
     *
     * This indicates the final status of a mode specs update operation, including updating
     * hardware composer (HWC) modes. Only after receiving this callback should clients
     * send another {@link #setDesiredDisplayModeSpecs} call.
     */
    @Hide
    public enum ModeChangeResult {
        /**
         * All mode sets have been successfully updated.
         */
        SUCCESS,

        /**
         * The mode spec update failed and displays remain in their original state.
         */
        REJECTED,

        /**
         * A fatal issue occurred when updating display modes
         */
        FAILURE;

        /**
         * Converts an integer to a ModeChangeResult.
         */
        @Hide
        @NonNull
        public static ModeChangeResult fromInt(int value) {
            return switch (value) {
                case 0 -> SUCCESS;
                case 1 -> REJECTED;
                case 2 -> FAILURE;
    ...
    private static final Map<OnDisplayEventListener, Long> sDisplayEventListeners = new HashMap<>();

    /**
     * Adds a listener for display events.
     */
    @Hide
    public static void addDisplayEventListener(
            @NonNull OnDisplayEventListener listener) {
        synchronized (sDisplayEventListeners) {
            if (sDisplayEventListeners.containsKey(listener)) {
                return;
            }
            final DisplayEventListenerWrapper wrapper = new DisplayEventListenerWrapper(listener);
            long nativeListener = nativeCreateDisplayEventListenerWrapper(wrapper);
            if (nativeListener != 0) {
                nativeAddDisplayEventListener(nativeListener);
                sDisplayEventListeners.put(listener, nativeListener);
            }
        }
    }

    /**
     * Removes a listener for display events.
     */
    @Hide
    public static void removeDisplayEventListener(
            @NonNull OnDisplayEventListener listener) {
        synchronized (sDisplayEventListeners) {
            Long nativeListener = sDisplayEventListeners.remove(listener);
            if (nativeListener != null) {
                nativeRemoveDisplayEventListener(nativeListener);
                nativeDestroyDisplayEventListenerWrapper(nativeListener);
            }
        }
    }

    private static native long nativeCreateDisplayEventListenerWrapper(
            DisplayEventListenerWrapper listener);
    private static native void nativeDestroyDisplayEventListenerWrapper(long ptr);
    private static native void nativeAddDisplayEventListener(long ptr);
    private static native void nativeRemoveDisplayEventListener(long ptr);
     ...
     */
    @Hide
    public static ModeSpecsUpdateResult setDesiredDisplayModeSpecs(
            @NonNull IBinder applyToken, DesiredDisplayModeSpecs[] desiredDisplayModeSpecs) {
        ...
        final int result = nativeSetDesiredDisplayModeSpecs(applyToken, desiredDisplayModeSpecs);
        return ModeSpecsUpdateResult.fromInt(result);
    }
    ...
    /**
     * Returns whether the device supports atomic multi-display mode sets.
     * If this returns false, we could send only single display mode specs at a time to
     * {@link #setDesiredDisplayModeSpecs}.
     */
    @Hide
    public static boolean getMultiDisplayModeSetSupport() {
        return nativeGetMultiDisplayModeSetSupport();
    }
    ...
    public static class Transaction implements Closeable, Parcelable {
        ...
        public @NonNull Transaction setCrop(@NonNull SurfaceControl sc, @Nullable Rect crop) {
            ...
            if (crop != null) {
                Preconditions.checkArgument(crop.isValid(), "Crop %s isn't valid", crop);
                nativeSetWindowCrop(mNativeObject, sc.mNativeObject,
                        crop.left, crop.top, crop.right, crop.bottom);
            } else {
                nativeSetWindowCrop(mNativeObject, sc.mNativeObject, 0, 0, 0, 0);
        ...
        /**
         * Sets the target frame rate for blur throttling of the {@link SurfaceControl}.
         * If the target frame rate provided is between 1 and 29, it will be treated as 30.
         *
         * @param sc SurfaceControl.
         * @param fps Target fps for blur throttling, pass 0 or a negative number to disable.
         * @return itself.
         */
        @Hide
        public Transaction setBlurThrottlingTargetFps(SurfaceControl sc, int fps) {
            checkPreconditions(sc);
            if (SurfaceControlRegistry.sCallStackDebuggingEnabled) {
                SurfaceControlRegistry.getProcessInstance().checkCallStackDebugging(
                        "setBlurThrottlingTargetFps", this, sc, "fps=" + fps);
            }
            fps = (fps > 0 && fps < 30) ? 30 : fps;
            nativeSetBlurThrottlingTargetFps(mNativeObject, sc.mNativeObject, fps);
            return this;
        }
        ...
        @Hide
        @NonNull
        public Transaction setWouldBeOpaqueIfNotForClientRoundedCorners(
                @NonNull SurfaceControl sc, boolean wouldBeOpaque) {
            checkPreconditions(sc);
            if (wouldBeOpaque) {
                nativeSetFlags(mNativeObject, sc.mNativeObject,
                        SURFACE_WOULD_BE_OPAQUE_IF_NOT_FOR_CLIENT_ROUNDED_CORNERS,
                        SURFACE_WOULD_BE_OPAQUE_IF_NOT_FOR_CLIENT_ROUNDED_CORNERS);
            } else {
                nativeSetFlags(mNativeObject, sc.mNativeObject, 0,
                        SURFACE_WOULD_BE_OPAQUE_IF_NOT_FOR_CLIENT_ROUNDED_CORNERS);
            }
            return this;
        }
        ...
        /**
         * Sets the exclusion mask for a display. Layers with matching composition flags
         * will be excluded from this display's output.
         *
         * @param displayToken The display to apply the filter to.
         * @param mask The bitwise mask of layer flags to exclude.
         */
        @Hide
        public Transaction setDisplayExclusionMask(IBinder displayToken, int mask) {
            if (displayToken == null) {
                throw new IllegalArgumentException("displayToken must not be null");
            }
            nativeSetDisplayExclusionMask(mNativeObject, displayToken, mask);
            return this;
        }
        ...
        /**
         * This needs to be public for Transaction to be used as a parameter in an AIDL interface
         */
        @Hide
        public void readFromParcel(Parcel in) {
            mNativeObject = 0;
