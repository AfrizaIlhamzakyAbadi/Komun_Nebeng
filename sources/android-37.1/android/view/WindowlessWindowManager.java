...
import android.window.sharedsurface.SharedAnimationFilter;
...
public class WindowlessWindowManager implements IWindowSession {
    ...
    private class State {
        ...
        WindowContainerToken mAssociatedWindowContainer;
    ...
    final SparseArray<ResizeCompleteCallback> mResizeCompletionForWindow =
            new SparseArray<ResizeCompleteCallback>();
    ...
    /**
     * Sets a {@link ResizeCompleteCallback} which will be called when the frame (identified by
     * viewRoot.mSeqId) of the given {@link ViewRootImpl} is drawn.
     */
    void setCompletionCallback(ViewRootImpl viewRoot, ResizeCompleteCallback callback) {
        synchronized (this) {
            final int key = getKeyOfResizeCompletionForWindow(
                    viewRoot.mWindow, viewRoot.mSeqId);
            if (mResizeCompletionForWindow.get(key) != null) {
                Log.w(TAG, "Unsupported overlapping resizes");
            }
            mResizeCompletionForWindow.put(key, callback);
        }
    }

    private int getKeyOfResizeCompletionForWindow(@NonNull IWindow window, int seqId) {
        return com.android.window.flags.Flags.perfectEdge()
                ? Objects.hashCode(window.asBinder()) ^ seqId
                : Objects.hashCode(window.asBinder());
    }

    /**
     * Sets the associated window container for the given windowless window. This should only be
     * used for windowless windows that are not hosted in other windows.
     */
    @Hide
    public void setAssociatedWindowContainer(IBinder window,
            @Nullable WindowContainerToken attachedToToken) {
        State state;
        synchronized (this) {
            ...
            state.mAssociatedWindowContainer = attachedToToken;
        ...
        synchronized (this) {
            ...
            if (state.mInputChannelToken != null) {
                try {
                    ...
                    params.attachedToToken = state.mAssociatedWindowContainer;
    ...
    public int relayout(IWindow window, WindowManager.LayoutParams inAttrs,
        ...
        final SurfaceControl outSurfaceControl = surface;
        ...
        if (outRelayoutResult != null) {
            if (com.android.window.flags.Flags.perfectEdge()) {
                outRelayoutResult.syncSeqId = lastSyncSeqId;
            }
    ...
    @Override
    public boolean prepareFrame(IWindow window, int seqId) {
        return true;
    }
    ...
    public void finishDrawing(android.view.IWindow window,
        ...
        synchronized (this) {
            final int key = getKeyOfResizeCompletionForWindow(window, seqId);
            final ResizeCompleteCallback c = mResizeCompletionForWindow.get(key);
            if (c == null) {
            ...
            mResizeCompletionForWindow.remove(key);
        }
    }
    ...
    public android.os.IBinder performDrag(android.view.IWindow window, int flags,
            ...
            int touchPointerId, int metaState, float touchX, float touchY,
            float thumbCenterX, float thumbCenterY, android.content.ClipData data) {
        try {
            return mRealWm.performDrag(window, flags, surface, touchSource, touchDeviceId,
                    touchPointerId, metaState, touchX, touchY, thumbCenterX, thumbCenterY, data);
        } catch (RemoteException e) {
    ...
    public void reportDropResult(android.view.IWindow window, boolean consumed) {
        try {
            mRealWm.reportDropResult(window, consumed);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void cancelDragAndDrop(android.os.IBinder dragToken, boolean skipAnimation) {
        try {
            mRealWm.cancelDragAndDrop(dragToken, skipAnimation);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void dragRecipientEntered(android.view.IWindow window) {
        try {
            mRealWm.dragRecipientEntered(window);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void dragRecipientExited(android.view.IWindow window) {
        try {
            mRealWm.dragRecipientExited(window);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public boolean dropForAccessibility(IWindow window, int x, int y) {
        try {
            return mRealWm.dropForAccessibility(window, x, y);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    @Override
    public void addSharedAnimationProvider(IWindow window, ISharedAnimationProvider provider,
            SharedAnimationParams animationParams,
            List<SharedAnimationFilter> filters) { }

    @Override
    public void removeSharedAnimationProvider(ISharedAnimationProvider provider) { }

    @Override
    public void updateSharedAnimationFilters(ISharedAnimationProvider provider,
            List<SharedAnimationFilter> filters) { }

    @Override
    public void updateSharedAnimationParams(ISharedAnimationProvider provider,
            SharedAnimationParams params) { }
