...
public class ScaleGestureDetector {
    ...
    private float mClassifiedPinchScaleFactor = 1.0f;
    ...
    private boolean handleClassifiedPinch(MotionEvent event) {
        ...
        var spanX = Math.abs(event.getX(1) - event.getX(0));
        var spanY = Math.abs(event.getY(1) - event.getY(0));
        var span = (float) Math.hypot(spanX, spanY);
        for (int i = 0; i < event.getHistorySize(); i++) {
            float historicalScaleFactor = event.getHistoricalAxisValue(
                    MotionEvent.AXIS_GESTURE_PINCH_SCALE_FACTOR, 0, i);
            mClassifiedPinchScaleFactor *= historicalScaleFactor;
        }
        float currentScaleFactor = event.getAxisValue(
                MotionEvent.AXIS_GESTURE_PINCH_SCALE_FACTOR, 0);
        mClassifiedPinchScaleFactor *= currentScaleFactor;
    ...
    private void callOnScale(
        ...
        if (updatePrev) {
            ...
            mPrevSpan = mCurrSpan;
            mPrevTime = mCurrTime;
            mClassifiedPinchScaleFactor = 1.0f;
        }
    }
