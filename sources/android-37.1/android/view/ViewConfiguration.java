...
public class ViewConfiguration {
    ...
    /**
     * The deceleration factor in a {@link KinematicScrollerSystem} following laminar drag forces
     */
    private static final float FLING_DECELERATION_FACTOR = 1.8f;

    /**
     * The final approach deceleration of a {@link KinematicScrollerSystem} following laminar drag
     * forces. Given in Dp/s^2
     */
    private static final float FLING_FINAL_APPROACH_DECELERATION_DP = 50f;

    /**
     * The velocity threshold of a {@link KinematicScrollerSystem} following laminar drag forces.
     * The current velocity of the scroller is compared to this threshold to choose on computations.
     * Given in Dp/s.
     */
    private static final float FLING_DECELERATION_VELOCITY_THRESHOLD_DP = 600f;

    /**
     * The minimum velocity of a fling such that a {@link KinematicScrollerSystem} accumulates the
     * fling velocity. Given in Dp/s.
     */
    private static final float FLING_VELOCITY_ACCUMULATION_THRESHOLD_DP = 7500f;

    /**
     * The maximum velocity of a fling such that would be accumulated over the current velocity of a
     * a {@link KinematicScrollerSystem}. Given in Dp/s.
     */
    private static final float FLING_MAX_VELOCITY_ACCUMULATED_DP = 5000f;

    /**
     * The time interval that a {@link KinematicScrollerSystem} will take to fully accumulate a
     * velocity. Given in milliseconds.
     */
    private static final long FLING_ACCUMULATION_TIME_MS = 50L;

    /**
     * The timeout interval since the scroller was finished/aborted within which consecutive flings
     * are allowed to accumulate velocity. Given in milliseconds.
     */
    ...
    private final boolean mRotaryNavigationEnabled;
    private final int mRotaryNavigationThresholdPixels;
    ...
    private final float mFlingLaminarDeceleration;
    private final float mScaledFlingFinalApproachDeceleration;
    private final float mScaledFlingDecelerationThreshold;
    private final float mScaledFlingAccumulationThreshold;
    private final float mScaledFlingMaximumAccumulatedVelocity;
    private final long mFlingAccumulationTimeMillis;
    private final long mFlingAccumulationTimeoutMillis;
    ...
    public ViewConfiguration() {
        ...
        mRotaryNavigationEnabled = false;
        mRotaryNavigationThresholdPixels = -1;
        ...
        mFlingLaminarDeceleration = FLING_DECELERATION_FACTOR;
        mScaledFlingFinalApproachDeceleration = FLING_FINAL_APPROACH_DECELERATION_DP;
        mScaledFlingDecelerationThreshold = FLING_DECELERATION_VELOCITY_THRESHOLD_DP;
        mScaledFlingAccumulationThreshold = FLING_VELOCITY_ACCUMULATION_THRESHOLD_DP;
        mScaledFlingMaximumAccumulatedVelocity = FLING_MAX_VELOCITY_ACCUMULATED_DP;
        mFlingAccumulationTimeMillis = FLING_ACCUMULATION_TIME_MS;
        mFlingAccumulationTimeoutMillis = FLING_ACCUMULATION_TIMEOUT_MS;
    }
    ...
    private ViewConfiguration(@NonNull @UiContext Context context) {
        ...
        mRotaryNavigationEnabled = res.getBoolean(R.bool.config_rotaryNavigationEnabled);
        mRotaryNavigationThresholdPixels = res.getDimensionPixelSize(
                com.android.internal.R.dimen.config_rotaryNavigationThreshold);
        ...
        mFlingLaminarDeceleration = FLING_DECELERATION_FACTOR;
        mScaledFlingFinalApproachDeceleration =
                sizeAndDensity * FLING_FINAL_APPROACH_DECELERATION_DP;
        mScaledFlingDecelerationThreshold =
                sizeAndDensity * FLING_DECELERATION_VELOCITY_THRESHOLD_DP;
        mScaledFlingAccumulationThreshold =
                sizeAndDensity * FLING_VELOCITY_ACCUMULATION_THRESHOLD_DP;
        mScaledFlingMaximumAccumulatedVelocity = sizeAndDensity * FLING_MAX_VELOCITY_ACCUMULATED_DP;
        mFlingAccumulationTimeMillis = FLING_ACCUMULATION_TIME_MS;
        mFlingAccumulationTimeoutMillis = FLING_ACCUMULATION_TIMEOUT_MS;
    ...
    /**
     * @return the deceleration factor in a {@link KinematicScrollerSystem} following laminar drag
     *     forces.
     */
    @FlaggedApi(Flags.FLAG_PHYSICAL_SCROLLING)
    public float getFlingLaminarDeceleration() {
        return mFlingLaminarDeceleration;
    }

    /**
     * @return the final approach deceleration of a {@link KinematicScrollerSystem} following
     *     laminar drag forces. Measured in pixels per second^2.
     */
    @FlaggedApi(Flags.FLAG_PHYSICAL_SCROLLING)
    public float getScaledFlingFinalApproachDeceleration() {
        return mScaledFlingFinalApproachDeceleration;
    }

    /**
     * @return the velocity threshold of a {@link KinematicScrollerSystem} following laminar drag
     *     forces. Measured in pixels per second.
     */
    @FlaggedApi(Flags.FLAG_PHYSICAL_SCROLLING)
    public float getScaledFlingDecelerationThreshold() {
        return mScaledFlingDecelerationThreshold;
    }

    /**
     * @return the minimum velocity of a fling such that a {@link KinematicScrollerSystem}
     *     accumulates the fling velocity. Measured in pixels per second.
     */
    @FlaggedApi(Flags.FLAG_PHYSICAL_SCROLLING)
    public float getScaledFlingAccumulationThreshold() {
        return mScaledFlingAccumulationThreshold;
    }

    /**
     * @return the maximum velocity of a fling such that would be accumulated over the current
     *     velocity of a {@link KinematicScrollerSystem}. Measured in pixels per second.
     */
    @FlaggedApi(Flags.FLAG_PHYSICAL_SCROLLING)
    public float getScaledFlingMaximumAccumulatedVelocity() {
        return mScaledFlingMaximumAccumulatedVelocity;
    }

    /**
     * @return the time interval that a {@link KinematicScrollerSystem} will take to fully
     *     accumulate a velocity. Measured in milliseconds.
     */
    @FlaggedApi(Flags.FLAG_PHYSICAL_SCROLLING)
    public long getFlingAccumulationTimeMillis() {
        return mFlingAccumulationTimeMillis;
    }

    /**
     * @return The timeout interval since the scroller was finished/aborted within which consecutive
     *     flings are allowed to accumulate velocity. For the user to add velocity to an ongoing
     *     fling, they must be touching the screen for no longer than this number of milliseconds
     *     before releasing the follow-up fling.
     */
    @FlaggedApi(Flags.FLAG_PHYSICAL_SCROLLING)
    public long getFlingAccumulationTimeoutMillis() {
        return mFlingAccumulationTimeoutMillis;
    }
    ...
    /**
     * Checks whether focus navigation is enabled for a motion generated by a specific
     * input device configuration and motion axis.
     *
     * <p>When enabled, the input should be used for focus navigation and highlighting in
     * addition to or instead of scrolling. This allows for a touch-free navigation experience.
     *
     * @param inputDeviceId the ID of the {@link InputDevice} that generated the motion.
     * @param axis the axis of the motion.
     * @param source the input source of the motion.
     * @return {@code true} if focus navigation is enabled for the given configuration.
     */
    @FlaggedApi(android.view.flags.Flags.FLAG_HW_ROTARY_NAVIGATION)
    public boolean isFocusNavigationEnabled(int inputDeviceId, int axis, int source) {
        if (!isInputDeviceInfoValid(inputDeviceId, axis, source)) {
            return false;
        }

        if (source == InputDevice.SOURCE_ROTARY_ENCODER && axis == MotionEvent.AXIS_SCROLL) {
            return mRotaryNavigationEnabled;
        }
    ...
    /**
     * Provides the focus traversal threshold (in pixels) for a motion generated by a specific
     * input device configuration and motion axis.
     *
     * <p>Focus traversal threshold here refers to the minimum movement distance required
     * to trigger a focus traversal event.
     *
     * @param inputDeviceId the ID of the {@link InputDevice} that generated the motion.
     * @param axis the axis of the motion.
     * @param source the input source of the motion.
     * @return the focus traversal threshold in pixels. If focus traversal is not supported for the
     *      given configuration, this method returns -1.
     */
    @FlaggedApi(android.view.flags.Flags.FLAG_HW_ROTARY_NAVIGATION)
    public int getFocusTraversalThreshold(int inputDeviceId, int axis, int source) {
        if (!isInputDeviceInfoValid(inputDeviceId, axis, source)) {
            return -1;
        }

        if (source == InputDevice.SOURCE_ROTARY_ENCODER && axis == MotionEvent.AXIS_SCROLL) {
            return mRotaryNavigationEnabled ? mRotaryNavigationThresholdPixels : -1;
        }
