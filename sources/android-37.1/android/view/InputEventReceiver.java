...
public abstract class InputEventReceiver {
    ...
    private static native void nativeDispose(long receiverPtr, boolean finalized);
    ...
    private void dispose(boolean finalized) {
        ...
        if (mReceiverPtr != 0) {
            nativeDispose(mReceiverPtr, finalized);
            mReceiverPtr = 0;
        }
    ...
    private static boolean dispatchFocusEvent(
            WeakReference<InputEventReceiver> weakReceiver, int seq, boolean hasFocus) {
        final InputEventReceiver receiver = weakReceiver.get();
        if (receiver == null || receiver.mReceiverPtr == 0) {
            return false;
        }
        receiver.onFocusEvent(hasFocus);
        nativeFinishInputEvent(receiver.mReceiverPtr, seq, true);
        return true;
    }

    // Called from native code.
    private static boolean dispatchPointerCaptureEvent(
            WeakReference<InputEventReceiver> weakReceiver,
            int seq, boolean pointerCaptureEnabled) {
        final InputEventReceiver receiver = weakReceiver.get();
        if (receiver == null || receiver.mReceiverPtr == 0) {
            return false;
        }
        receiver.onPointerCaptureEvent(pointerCaptureEnabled);
        nativeFinishInputEvent(receiver.mReceiverPtr, seq, true);
        return true;
    }

    // Called from native code.
    private static boolean dispatchDragEvent(
            WeakReference<InputEventReceiver> weakReceiver,
            int seq, boolean isExiting, float x, float y, int displayId,
            int inputSource, int metaState) {
        final InputEventReceiver receiver = weakReceiver.get();
        if (receiver == null || receiver.mReceiverPtr == 0) {
            return false;
        }
        receiver.onDragEvent(isExiting, x, y, displayId, inputSource, metaState);
        nativeFinishInputEvent(receiver.mReceiverPtr, seq, true);
        return true;
    }

    // Called from native code.
    private static boolean dispatchTouchModeChanged(
            WeakReference<InputEventReceiver> weakReceiver, int seq, boolean inTouchMode) {
        final InputEventReceiver receiver = weakReceiver.get();
        if (receiver == null || receiver.mReceiverPtr == 0) {
            return false;
        }
        receiver.onTouchModeChanged(inTouchMode);
        nativeFinishInputEvent(receiver.mReceiverPtr, seq, true);
        return true;
    }

    // Called from native code.
    private static boolean dispatchBatchedInputEventPending(
            WeakReference<InputEventReceiver> weakReceiver, int source) {
        final InputEventReceiver receiver = weakReceiver.get();
        if (receiver == null || receiver.mReceiverPtr == 0) {
            return false;
        }
        receiver.onBatchedInputEventPending(source);
        return true;
    }

    // Called from native code.
    @SuppressWarnings("unused")
    private static boolean dispatchInputEvent(
            WeakReference<InputEventReceiver> weakReceiver, int seq, InputEvent event) {
        final InputEventReceiver receiver = weakReceiver.get();
        if (receiver == null || receiver.mReceiverPtr == 0) {
            return false;
        }
        receiver.mSeqMap.put(event.getSequenceNumber(), seq);
