...
public class ViewPropertyAnimator {
    ...
    // Indexed by bit position: TRANSLATION_X=0, TRANSLATION_Y=1, ..., ALPHA=11
    ...
    private void startAnimation() {
        ...
        if (Trace.isEnabled()) {
            animator.setTraceName(buildTraceName(propertyMask));
        }
    ...
    /**
     * Builds a descriptive trace name from the view and animated property bitmask.
     * Called once per {@link #startAnimation()} when tracing is enabled.
     */
    private String buildTraceName(int propertyMask) {
        String viewId = "unknown";
        try {
            if (mView.getId() != View.NO_ID) {
                viewId = mView.getResources().getResourceEntryName(mView.getId());
            }
        } catch (Resources.NotFoundException e) {
            // ignore
        }

        return "viewPropertyAnimator:" + mView.getClass().getSimpleName()
                + "(id/" + viewId + "):" + propertyMaskToString(propertyMask);
    }

    static String propertyMaskToString(int mask) {
        StringBuilder sb = new StringBuilder();
        while (mask != 0) {
            int bit = Integer.numberOfTrailingZeros(mask);
            if (bit < PROPERTY_NAMES.length) {
                if (sb.length() > 0) sb.append(',');
                sb.append(PROPERTY_NAMES[bit]);
            }
