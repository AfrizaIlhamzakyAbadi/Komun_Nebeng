...
import static com.android.internal.tracing.winscope.protos.Inputmethodeditor.InputMethodClientsTraceProto.ClientSideProto.DISPLAY_ID;
import static com.android.internal.tracing.winscope.protos.Inputmethodeditor.InputMethodClientsTraceProto.ClientSideProto.EDITOR_INFO;
import static com.android.internal.tracing.winscope.protos.Inputmethodeditor.InputMethodClientsTraceProto.ClientSideProto.INPUT_CONNECTION;
import static com.android.internal.tracing.winscope.protos.Inputmethodeditor.InputMethodClientsTraceProto.ClientSideProto.INPUT_CONNECTION_CALL;
import static com.android.internal.tracing.winscope.protos.Inputmethodeditor.InputMethodClientsTraceProto.ClientSideProto.INPUT_METHOD_MANAGER;
import static com.android.internal.tracing.winscope.protos.Inputmethodeditor.InputMethodClientsTraceProto.ClientSideProto.VIEW_ROOT_IMPL;
import static com.android.internal.tracing.winscope.protos.Inputmethodmanager.InputMethodManagerProto.ACTIVE;
import static com.android.internal.tracing.winscope.protos.Inputmethodmanager.InputMethodManagerProto.CUR_ID;
import static com.android.internal.tracing.winscope.protos.Inputmethodmanager.InputMethodManagerProto.FULLSCREEN_MODE;
import static com.android.internal.tracing.winscope.protos.Inputmethodmanager.InputMethodManagerProto.NEXT_SERVED_VIEW;
import static com.android.internal.tracing.winscope.protos.Inputmethodmanager.InputMethodManagerProto.SERVED_CONNECTING;
import static com.android.internal.tracing.winscope.protos.Inputmethodmanager.InputMethodManagerProto.SERVED_VIEW;
...
public final class InputMethodManager {
    ...
    @IntDef({
        ...
        IM_PICKER_ENTRY_POINT_QUICK_SETTINGS,
    })
    ...
    /**
     * The entry point value used when the user opens the IME Switcher Menu by tapping the
     * Keyboard tile in Quick Settings.
     */
    @Hide
     ...
     * @param displayId The ID of the display whose assigned user's input method chooser dialog
     *     should be shown. That dialog will be shown on the display with this ID.
     */
    @Hide
     ...
     *
     * <p>There's no guarantee that this toggle is an atomic operation. There's potential risk of a
     * race condition when concurrency is involved, or when this is invoked in quick succession.
     *
     * @param showAuxiliarySubtypes Set true to show auxiliary input methods.
     * @param entryPoint The entry point where the chooser dialog was requested from.
     * @param displayId The ID of the display whose assigned user's input method chooser dialog
     *     should be toggled. If that dialog is currently hidden, it will be shown on the display
     *     with this ID. If that dialog is currently shown, regardless of whether it is shown on the
     *     display with this ID, it will be hidden.
     */
    @Hide
    // This non-public API was added to support the workaround for visibility control of
    // InputMethodPicker when opened via "IME" StatusBar QuickActions LaunchChip in SystemUI.
    // TODO: b/518638459 - Migrate "IME" to PopupChip embedding InputMethodPicker (once the latter
    // is embeddable) then dismantle this API, unless it's since also been used for other purposes.
    @RequiresPermission(Manifest.permission.WRITE_SECURE_SETTINGS)
     ...
     *
     * @param displayId The ID of the display whose assigned user's input method chooser dialog
     *     should be hidden. That dialog will be hidden, regardless of whether it's currently shown
     *     on the display with this ID.
     */
    @Hide
    // This non-public API was added to support the workaround for visibility control of
    // InputMethodPicker when opened via "IME" StatusBar QuickActions LaunchChip in SystemUI.
    ...
    public void waitUntilNoPendingRequests(long timeoutMs) {
        IInputMethodManagerGlobalInvoker.getImeTrackerService()
                .waitUntilNoPendingRequests(timeoutMs);
    }
    ...
    public void finishTrackingPendingRequests() {
        IInputMethodManagerGlobalInvoker.getImeTrackerService().finishTrackingPendingRequests();
    }
