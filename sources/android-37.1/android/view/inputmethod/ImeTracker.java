...
import com.android.internal.inputmethod.ImeTrackerServiceWrapper;
...
public interface ImeTracker {
    ...
    @IntDef(prefix = { "PHASE_" }, value = {
            ...
            PHASE_SERVER_REQUESTED_VISIBLE_TYPES_NOT_CHANGED,
            ...
            PHASE_SERVER_NO_IME_INSETS_SOURCE_PROVIDER,
    })
    @Retention(RetentionPolicy.SOURCE)
    @interface Phase {}
    ...
    int PHASE_SERVER_REQUESTED_VISIBLE_TYPES_NOT_CHANGED =
            ImeProtoEnums.PHASE_SERVER_REQUESTED_VISIBLE_TYPES_NOT_CHANGED;
    ...
    /** There is no ImeInsetsSourceProvider to dispatch the request to show the IME to. */
    int PHASE_SERVER_NO_IME_INSETS_SOURCE_PROVIDER =
            ImeProtoEnums.PHASE_SERVER_NO_IME_INSETS_SOURCE_PROVIDER;
    ...
    /**
     * Get the singleton show/hide request tracker instance.
     *
     * @return the singleton show/hide request tracker instance
     */
    @NonNull
    static ImeShowTracker forLogging() {
        return SHOW_TRACKER;
    }
    ...
    /** The singleton IME tracker instance for show/hide requests. */
    @NonNull ImeShowTracker SHOW_TRACKER = new ImeShowTracker();

    /** The singleton IME tracker instance for instrumenting jank metrics. */
    @NonNull ImeJankTracker JANK_TRACKER = new ImeJankTracker();

    /** The singleton IME tracker instance for instrumenting latency metrics. */
    @NonNull ImeLatencyTracker LATENCY_TRACKER = new ImeLatencyTracker();
    ...
    final class Token implements Parcelable {
        ...
        private static String createTag(@NonNull String component) {
            return String.format("%s:%08x", component, ThreadLocalRandom.current().nextInt());
        }
    ...
    /**
     * A tracker instance which is in charge of logging show/hide request updates and propagating
     * them to {@link com.android.server.inputmethod.ImeTrackerService}. This class disallows
     * instantiating from outside, use {@link #forLogging()} to get the singleton.
     */
    final class ImeShowTracker implements ImeTracker, AutoCloseable {
        /** Use {@link #getService} to read this value. */
        @Nullable private ImeTrackerServiceWrapper mService;

        /** Whether relevant system property values have been fetched for the first time. */
        @NonNull private AtomicBoolean mSystemPropertiesInitialized = new AtomicBoolean(false);

        /**
         * Whether {@link #onProgress} calls should be logged. Use {@link #shouldLogProgress} to
         * read this value.
         */
        @NonNull private AtomicBoolean mLogProgress = new AtomicBoolean();

        /**
         * Whether the stack trace at the request call site should be logged. Use {@link
         * #shouldLogStackTrace} to read this value.
         */
        @NonNull private AtomicBoolean mLogStackTrace = new AtomicBoolean();

        /** A {@link Runnable} which is registered as a callback for system property changes. */
        @NonNull
        private final Runnable mUpdateSystemPropertiesRunnable = this::updateSystemProperties;

        @VisibleForTesting
        public ImeShowTracker() {
            // Update system properties when they change.
            SystemProperties.addChangeCallback(mUpdateSystemPropertiesRunnable);
        ...
        @Override
        public void close() {
            SystemProperties.removeChangeCallback(mUpdateSystemPropertiesRunnable);
        }
        ...
        public Token onStart(
            ...
            onStart(token, uid, type, origin, reason, fromUser, userId, displayId);
            return token;
        }
        ...
        public void onStart(
            ...
            log(
                    "%s: %s at %s reason %s fromUser %b%s displayId %d%s",
                    token.mTag,
        ...
        public void onProgress(@Nullable Token token, @Phase int phase) {
            ...
            var service = getService();
            if (service != null) {
                service.onProgress(token, phase);
            }
        ...
        public void onCancelled(@Nullable Token token, @Phase int phase) {
            ...
            var service = getService();
            if (service != null) {
                service.onCancelled(token, phase);
            }
        ...
        @VisibleForTesting
        @Nullable
        public ImeTrackerServiceWrapper getService() {
            if (mService == null) {
                // Note that this is not guaranteed to return a non-null value.
                mService = IInputMethodManagerGlobalInvoker.getImeTrackerService();
            }
        ...
        @VisibleForTesting
        public void log(@NonNull String messageString, @NonNull Object... args) {
            if (android.tracing.Flags.imetrackerProtolog()) {
        ...
        private boolean shouldLogProgress() {
            initializeSystemPropertiesIfNeeded();
            return mLogProgress.get();
        }

        /** Whether the stack trace at the request call site should be logged. */
        private boolean shouldLogStackTrace() {
            initializeSystemPropertiesIfNeeded();
            return mLogStackTrace.get();
        }

        /**
         * Initializes the system properties for the first time if they have not already been
         * initialized.
         */
        private void initializeSystemPropertiesIfNeeded() {
            if (!mSystemPropertiesInitialized.get()) {
        ...
        public void updateSystemProperties() {
            mLogProgress.set(SystemProperties.getBoolean("persist.debug.imetracker", false));
            mLogStackTrace.set(
                    SystemProperties.getBoolean("persist.debug.imerequest.logstacktrace", false));
            mSystemPropertiesInitialized.set(true);
        }
    }
