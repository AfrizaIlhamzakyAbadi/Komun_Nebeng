...
public final class InlineSuggestionsRequest implements Parcelable {
    ...
    private int extrasHashCode() {
        // Bundle does not implement hashCode by default, meaning different Bundle objects with the
        // same keys and values will have different hashCode values. This means that parceling then
        // unparceling this object will result in different hashCode values if we use the default
        // hashCode implementation that is generated. Two objects that are equal must return the
        // same hashCode, so we return something stable that will have the same value for two
        // bundles that are equal. Note that it's okay for two different objects to return the same
        // hash so the hash method does not need to ensure uniqueness.
        return mExtras.size() + mExtras.keySet().hashCode();
    }
