...
public class ManifestConfigSource implements ConfigSource {
    ...
    public NetworkSecurityConfig getBaseConfig() {
        return getConfigSource().getBaseConfig();
    }
        ...
        synchronized (mLock) {
            ...
            if (configResource != 0) {
                ...
                if (mApplicationInfo.uid == android.os.Process.myUid()) {
                    isDeprecationEnabled =
                            CompatChanges.isChangeEnabled(DEPRECATE_USES_CLEARTEXT_TRAFFIC);
                } else {
                    try {
                        isDeprecationEnabled = CompatChanges.isChangeEnabled(
                                DEPRECATE_USES_CLEARTEXT_TRAFFIC, mApplicationInfo.uid);
                    } catch (RuntimeException e) {
                        isDeprecationEnabled = mApplicationInfo.targetSdkVersion
    ...
    private static final class DefaultConfigSource implements ConfigSource {
        ...
        DefaultConfigSource(boolean usesCleartextTraffic, ApplicationInfo info) {
            mBaseConfig = NetworkSecurityConfig.createBaseConfigBuilder(info)
