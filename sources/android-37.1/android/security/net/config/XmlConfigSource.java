...
public class XmlConfigSource implements ConfigSource {
    ...
    private Pin parsePin(XmlResourceParser parser)
        ...
        if (!Pin.isSupportedDigestAlgorithm(digestAlgorithm)) {
            throw new ParserException(parser,
                                      "Unsupported pin digest algorithm: " + digestAlgorithm);
        }
        ...
        if (decodedDigest.length != expectedLength) {
            throw new ParserException(parser,
                                      "digest length " + decodedDigest.length
    ...
    private void parseNetworkSecurityConfig(XmlResourceParser parser)
        ...
        NetworkSecurityConfig.Builder platformDefaultBuilder =
                NetworkSecurityConfig.createBaseConfigBuilder(mApplicationInfo);
        ...
        if (networkSecurityConfigLocalhost() && !isLocalhostDefined) {
            NetworkSecurityConfig.Builder localhostBuilder =
                    NetworkSecurityConfig.createLocalhostConfigBuilder();
        ...
        mBaseConfig = baseConfigBuilder.build();
        mDomainMap = configs;
    }
