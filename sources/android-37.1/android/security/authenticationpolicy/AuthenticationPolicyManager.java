...
import static android.Manifest.permission.MANAGE_CROSS_DEVICE_AUTHENTICATED_MESSAGES;
 ...
 *
 * <h3>Secure Lock</h3>
 * Secure lockdown includes a set of device locking capabilities to protect users in "at risk"
 * environments.
 * <p>
 * To use secure lock, call {@link #enableSecureLockDevice} to enable it on the device.
 ...
 *
 * <h3>Cross Device Authentication Framework</h3>
 * AuthenticationPolicyManager exposes a set of methods that can be used by privileged components
 * to gate access to select functionality.
 * <p>
 * This will require the caller to have the
 * {@link android.Manifest.permission#MANAGE_CROSS_DEVICE_AUTHENTICATED_MESSAGES} permission.
 *
 * <p>
 * Cross device access requests can be initiated on one device by calling
 * {@link #createCrossDeviceAuthenticationRequest(DeviceId, String, Executor OutcomeReceiver)}.
 * The returned token should be transmitted to another device, via any means, and the receiving
 * service should then use that token to call
 * {@link #parseCrossDeviceAuthenticationRequest(byte[], String, Executor, OutcomeReceiver)}
 * before executing the request. This ensures that a common set of security checks are
 * verified before executing the request and it decouples many form factor specific checks from
 * the service being guarded.
 */
@SystemApi
...
public final class AuthenticationPolicyManager {
    ...
    /**
     * Creates a new cross device authentication request associated with the given
     * {@link android.companion.CompanionDeviceManager}'s remote device ID for the current
     * user on this device.
     * <p>
     * This method is intended to be called by system components that act as proxies for
     * applications initiating cross-device functionality. The {@code requestingPackageName} for
     * the originating app must be explicitly provided to validate that the app is present on
     * both the local and remote devices and that the returned request object is not used for
     * other apps.
     * <p>
     * Returns an opaque token to be passed to the remote device and used as a parameter in
     * {@link #parseCrossDeviceAuthenticationRequest(byte[], String, Executor, OutcomeReceiver)}
     * to unpack the data and run authorization checks.
     *
     * @param deviceId the ID of the remote device where this request will be processed
     * @param requestingPackageName the package name of the app requesting authentication
     * @param callbackExecutor the executor on which to invoke the callback
     * @param callback the receiver for the encrypted request on success, or an exception on failure
     */
    @RequiresPermission(MANAGE_CROSS_DEVICE_AUTHENTICATED_MESSAGES)
    @FlaggedApi(FLAG_AGENT_AUTH_XDEVICE_TOKEN)
    @SystemApi
    public void createCrossDeviceAuthenticationRequest(@NonNull DeviceId deviceId,
            @NonNull String requestingPackageName,
            @NonNull @CallbackExecutor Executor callbackExecutor,
            @NonNull OutcomeReceiver<byte[], CrossDeviceAuthenticationException> callback) {
        Objects.requireNonNull(deviceId, "deviceId cannot be null");
        Objects.requireNonNull(requestingPackageName, "requestingPackageName cannot be null");
        Objects.requireNonNull(callbackExecutor, "callbackExecutor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");

        try {
            mAuthenticationPolicyService.createCrossDeviceAuthenticationRequest(
                    deviceId, requestingPackageName,
                    new ICreateCrossDeviceAuthenticationCallback.Stub() {
                        @Override
                        ...
                        public void onError(int errorCode, String errorMessage) {
                            final long identity = Binder.clearCallingIdentity();
                            try {
                                callbackExecutor.execute(() -> callback.onError(
                                        new CrossDeviceAuthenticationException(
    ...
    /**
     * Creates a new cross device authentication using a mock source device instead of the
     * device under test. This can be used in test along with
     * {@link #parseCrossDeviceAuthenticationRequest} to simulate a cross device authentication
     * scenarios originating from different device types.
     *
     * @param testSourceDevice the test source device information to use instead of the device
     *                         under test
     * @param deviceId the ID of the device where this request will be processed
     * @param callbackExecutor the executor on which to invoke the callback
     * @param callback the receiver for the encrypted request on success, or an exception on failure
     */
    @TestApi
    @RequiresPermission(allOf = {
            android.Manifest.permission.MANAGE_CROSS_DEVICE_AUTHENTICATED_MESSAGES,
            android.Manifest.permission.TEST_BIOMETRIC
    })
    @FlaggedApi(FLAG_AGENT_AUTH_XDEVICE_TOKEN)
    public void createTestCrossDeviceAuthenticationRequest(
            @NonNull CrossDeviceAuthenticationSourceDeviceInfo testSourceDevice,
            @NonNull DeviceId deviceId,
            @NonNull @CallbackExecutor Executor callbackExecutor,
            @NonNull OutcomeReceiver<byte[], CrossDeviceAuthenticationException> callback) {
        Objects.requireNonNull(testSourceDevice, "testSourceDevice cannot be null");
        Objects.requireNonNull(deviceId, "deviceId cannot be null");
        Objects.requireNonNull(callbackExecutor, "callbackExecutor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");

        try {
            mAuthenticationPolicyService.createTestCrossDeviceAuthenticationRequest(
                    testSourceDevice, deviceId,
                    new ICreateCrossDeviceAuthenticationCallback.Stub() {
                        @Override
                        public void onSuccess(byte[] result) {
                            final long identity = Binder.clearCallingIdentity();
    ...
    /**
     * Unpacks a cross device authentication request that was created on a remote device.
     * <p>
     * This method performs two key actions:
     * <ol>
     *     <li><b>Basic policy enforcement:</b> Decrypts the request and performs common
     *     authentication checks, such as:
     *     <ul>
     *         <li>Verifying the remote device is known by
     *         {@link android.companion.CompanionDeviceManager}.</li>
     *         <li>Checking the lock states of both the local and remote devices.</li>
     *         <li>Comparing timestamps to prevent replay attacks.</li>
     *     </ul>
     *     </li>
     *     <li><b>Cross device state aggregation:</b> Exposes aggregate device state information
     *     to the calling system service that is providing cross device functionality to enable
     *     it to implement finer-grained policies beyond those common checks.</li>
     * </ol>
     *
     * @param encryptedRequest the encrypted request obtained from another device
     * @param originatingPackage the package name of the app associated with the request
     * @param callbackExecutor the executor on which to invoke the callback
     * @param callback the receiver for the decrypted device state on success, or an exception on
     *                failure
     */
    @RequiresPermission(MANAGE_CROSS_DEVICE_AUTHENTICATED_MESSAGES)
    @FlaggedApi(FLAG_AGENT_AUTH_XDEVICE_TOKEN)
    @SystemApi
    public void parseCrossDeviceAuthenticationRequest(
            @NonNull byte[] encryptedRequest,
            @NonNull String originatingPackage,
            @NonNull Executor callbackExecutor,
            @NonNull OutcomeReceiver<CrossDeviceAuthenticatedMessage,
                    CrossDeviceAuthenticationException> callback) {
        Objects.requireNonNull(encryptedRequest, "encryptedRequest cannot be null");
        Objects.requireNonNull(originatingPackage, "originatingPackage cannot be null");
        Objects.requireNonNull(callbackExecutor, "callbackExecutor cannot be null");
        Objects.requireNonNull(callback, "callback cannot be null");

        try {
            mAuthenticationPolicyService.parseCrossDeviceAuthenticationRequest(encryptedRequest,
                    originatingPackage,
                    new IParseCrossDeviceAuthenticationCallback.Stub() {
                        @Override
                        public void onSuccess(CrossDeviceAuthenticatedMessage result) {
                            final long identity = Binder.clearCallingIdentity();
                            try {
                                callbackExecutor.execute(() -> callback.onResult(result));
                            } finally {
