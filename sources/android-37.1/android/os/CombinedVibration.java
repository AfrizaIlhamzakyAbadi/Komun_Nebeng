...
public abstract class CombinedVibration implements Parcelable {
    ...
    /**
     * Returns whether this combined vibration is timeline anchored.
     *
     * <p>A combined vibration is timeline anchored if all its effects are timeline anchored.
     * Parallel combinations require that all effects be either all timeline anchored or all not
     * timeline anchored.
     */
    @Hide
    ...
    public static final class ParallelCombination {
         ...
         *
         * <p>If {@link Flags#compositionApi()} is enabled, all effects added to this combination
         * must be consistent regarding timeline anchoring: they must either all be timeline
         * anchored (see {@link VibrationEffect#isTimelineAnchored()}) or none of them should be.
         *
         * <p>The {@link ParallelCombination} object is still valid after this
         ...
         * @throws IllegalStateException if the combination is empty or if it contains a mix of
         *                               timeline anchored and non-timeline anchored effects.
         *
         */
        @NonNull
        public CombinedVibration combine() {
            ...
            if (Flags.compositionApi()) {
                boolean isAllEffectsTimelineAnchored = true;
                int timelineAnchoredCount = 0;
                for (int i = 0; i < mEffects.size(); i++) {
                    if (mEffects.valueAt(i).isTimelineAnchored()) {
                        timelineAnchoredCount++;
                    } else {
                        isAllEffectsTimelineAnchored = false;
                    }
                }
                if (!isAllEffectsTimelineAnchored && timelineAnchoredCount > 0) {
                    throw new IllegalStateException(
                            "Composed effects with timeline anchoring cannot be combined in"
                                + " parallel with non-timeline anchored effects.");
                }
            }
    ...
    public static final class Mono extends CombinedVibration {
        ...
        public boolean isTimelineAnchored() {
            return mEffect.isTimelineAnchored();
        }
    ...
    public static final class Stereo extends CombinedVibration {
        ...
        @Override
        public boolean isTimelineAnchored() {
            return mEffects.size() > 0 && mEffects.valueAt(0).isTimelineAnchored();
        }
