...
public final class LooperDoctor {
    ...
    private static final LooperDoctor sLooperDoctor = new LooperDoctor.Builder().build();

    /**
     * Convenience function to install LooperDoctor on the Looper of a HandlerThread.
     *
     * @param thread The HandlerThread whose Looper we should install
     *                a LooperDoctor onto.
     */
    public static void install(@NonNull HandlerThread thread) {
        if (Flags.messageQueueMonitoringEnabled()) {
            thread.getLooper().setLooperDoctor(sLooperDoctor);
        }
    }
    ...
    private static boolean isMqTracingEnabled() {
