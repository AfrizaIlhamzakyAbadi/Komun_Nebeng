...
class PowerComponents {
    private static final String TAG = "PowerComponents";
    ...
    static final class Builder {
        ...
        /**
         * Sets the consumed power for the specified key.
        ...
        public Builder setConsumedPower(BatteryConsumer.Key key, double componentPower) {
            if (componentPower < 0) {
                Slog.wtfStack(TAG, "Negative componentPower: " + componentPower + " skipped");
                return this;
            }
        ...
        /**
         * Adds consumed power for the specified key.
         * Negative values are ignored.
         */
        @NonNull
        ...
        /**
         * Sets the usage duration for the specified key.
         * Negative values are ignored.
        ...
        public Builder setUsageDurationMillis(BatteryConsumer.Key key,
            ...
            if (componentUsageDurationMillis < 0) {
                Slog.wtfStack(TAG, "Negative componentUsageDurationMillis: "
                        + componentUsageDurationMillis + " skipped");
                return this;
            }
        ...
        /**
         * Adds usage duration for the specified key.
         * Negative values are ignored.
         */
        @NonNull
        ...
        private void addPowerAndDuration(BatteryConsumer.BatteryConsumerData otherData) {
            ...
            for (BatteryConsumer.Key key : mData.layout.keys) {
                ...
                if (mData.hasValue(key.mPowerColumnIndex)
                    ...
                    double otherPower = otherData.getDouble(otherKey.mPowerColumnIndex);
                    if (otherPower < 0) {
                        Slog.wtfStack(TAG, "Negative power in other data: " + otherPower
                                + ", treating as 0");
                        otherPower = 0;
                    }
                    mData.putDouble(key.mPowerColumnIndex,
                            mData.getDouble(key.mPowerColumnIndex) + otherPower);
                }
                if (mData.hasValue(key.mDurationColumnIndex)
                        || otherData.hasValue(otherKey.mDurationColumnIndex)) {
                    long otherDuration = otherData.getLong(otherKey.mDurationColumnIndex);
                    if (otherDuration < 0) {
                        Slog.wtfStack(TAG, "Negative duration in other data: " + otherDuration
                                + ", treating as 0");
                        otherDuration = 0;
                    }
                    mData.putLong(key.mDurationColumnIndex,
                            mData.getLong(key.mDurationColumnIndex) + otherDuration);
                }
            }
        }
