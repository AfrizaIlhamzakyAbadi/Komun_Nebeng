...
public class Process {
    ...
    public static ProcessStartResult startWebView(@NonNull final String processClass,
                                                  ...
                                                  long startSeq,
                                                  @Nullable String[] zygoteArgs) {
        IZygoteProcess webviewZygoteProcess = WebViewZygote.getProcess();
        if (webviewZygoteProcess == null) {
            Log.wtf(LOG_TAG, "WebView zygote is not initialized, falling back to regular zygote");
            return start(
                    processClass,
                    niceName,
                    uid,
    ...
    public static final void setThreadPriority(int tid,
        ...
        if (Process.myTid() == tid) {
            ...
            // We use a direct version check here, since this is only detectable by code that
            // already violated the @IntRange spec, and we just made violations obvious.
            ...
            setThreadPriorityNative(tid, priority);  // Clamps priority if needed.
        }
    }
    ...
    /**
     * Set the process group for the given PID and UID.
     */
    @Hide
    public static final native void setProcessGroup(int uid, int pid, int group)
            throws IllegalArgumentException, SecurityException;
    ...
    /**
     * Create a new process group in the cgroup uid/pid hierarchy, specifying the actual UID.
     *
     * @return <0 in case of error
     */
    @Hide
    public static final native int createProcessGroup(int uid, int actualUid, int pid);
    ...
    public static final void setThreadPriority(
        ...
        if (!succ) {
            if (priority < -20 || priority > THREAD_PRIORITY_LOWEST) {
                // TODO: Consider adding a test for the following old SDK workaround. We
                // previously tested for the clamping behavior, though it always violated the
                // @IntRange spec.
                if (VMRuntime.getRuntime().getTargetSdkVersion()
                        < Build.VERSION_CODES.CINNAMON_BUN) {
                    // Some legacy apps pass out of range values, expecting Linux kernel behavior.
                    // Apparently @IntRange was never enforced.
                    if (priority < -20) {
                        priority = -20;
                    }
                    ...
                    succ = VMRuntime.getRuntime().setThreadNiceness(Thread.currentThread(),
                                                                    priority);
                } else {
                    throw new IllegalArgumentException("Priority/niceness "
                            + priority + " is invalid");
                }
            }
            if (!succ) {
