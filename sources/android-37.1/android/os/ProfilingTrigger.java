...
import static android.os.ProfilingManager.TRIGGER_ALL;
...
public final class ProfilingTrigger {
    ...
    // LINT.ThenChange(:trigger_intdef)

    // LINT.IfChange(trigger_intdef)
    ...
    // LINT.ThenChange(:use_background_system_trace)
    ...
    public static boolean isValidRequestTriggerType(int triggerType) {
        return triggerType == TRIGGER_TYPE_APP_FULLY_DRAWN
                ...
                || triggerType == TRIGGER_TYPE_APP_REQUEST_RUNNING_TRACE
                || triggerType == TRIGGER_TYPE_KILL_FORCE_STOP
                || triggerType == TRIGGER_TYPE_KILL_RECENTS
                || triggerType == TRIGGER_TYPE_KILL_TASK_MANAGER
                || triggerType == TRIGGER_TYPE_OOM
                ...
                || triggerType == TRIGGER_TYPE_COLD_START
                || (android.os.profiling.anomaly.flags.Flags.anomalyDetectorCoreC()
    ...
    /**
     * Check whether the provided trigger type uses background system trace.
     */
    @Hide
    public static boolean usesBackgroundSystemTrace(int triggerType) {
        // LINT.IfChange(use_background_system_trace)
        // Explicitly exclude triggers that don't use background system trace.
        if (triggerType == TRIGGER_TYPE_NONE
                || triggerType == TRIGGER_TYPE_OOM
                || triggerType == TRIGGER_TYPE_COLD_START) {
            return false;
        }
        ...
        return triggerType == TRIGGER_TYPE_APP_FULLY_DRAWN
                ...
                || triggerType == TRIGGER_TYPE_KILL_TASK_MANAGER
                || triggerType == TRIGGER_TYPE_KILL_EXCESSIVE_CPU_USAGE
                // Anomaly and AppCompat triggers are included because they may request to clone the
                // background trace via PROFILING_TYPE_SYSTEM_TRACE_ONGOING.
                || triggerType == TRIGGER_TYPE_ANOMALY
                || triggerType == TRIGGER_TYPE_APP_COMPAT
                // TRIGGER_ALL includes all triggers, some of which require background trace.
                || triggerType == TRIGGER_ALL;
