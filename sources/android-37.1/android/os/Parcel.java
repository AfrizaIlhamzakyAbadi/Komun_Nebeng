...
public final class Parcel {
    ...
    private <T> T readValue(@Nullable ClassLoader loader, @Nullable Class<T> clazz,
        ...
        if (isLengthPrefixed(type)) {
            ...
            if (actual != length) {
                boolean failOnMismatch = CompatChanges.isChangeEnabled(PARCEL_HARDENING);
    ...
    private static void makeAccessible(AccessibleObject member) {
        // Calling member.setAccessible(true) *should* be fine on a real device too,
        // but in order to avoid possibly impacting the device side behavior, we only do it
        // on Ravenwood for now.
    }

    private static void makeAccessible$ravenwood(AccessibleObject member) {
     ...
     * Reads an array set.
     *
     * @param loader The class loader to use, or null to use the default class loader.
     */
    @Hide
    @UnsupportedAppUsage
