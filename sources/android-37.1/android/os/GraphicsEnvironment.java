...
public class GraphicsEnvironment {
    ...
    // b/512126823. Early initialize EGL driver to make sure any driver swap is completed before
    // app issues egl calls.
    // mForceEGLEarlyLoad is set to true when current process requests to load non DEFAULT driver:
    // ANGLE, NATIVE, or updatable driver apk. In these cases, there is a chance we need to unload
    // system default driver and then load requested driver. To prevent the race condition when app
    // still uses functions from the system default driver while the unload is happening, we force
    // the driver unload early before app issues egl calls.
    private boolean mForceEGLEarlyLoad = false;
    ...
    public void setup(Context context, Bundle coreSettings) {
        ...
        if (android.os.Flags.earlyInitEglDriver() && mForceEGLEarlyLoad) {
            Log.v(TAG, "Early Initialize EGL");
            nativeEarlyInitEgl();
        }
    }
    ...
    private String getDebugLayerAppPaths(IPackageManager pm, String packageName) {
        ...
        if (appInfo == null) {
            Log.w(TAG, "Debug layer app '" + packageName
                    + "' not discovered. If installed, ensure it was installed via 'adb install "
                    + "--force-queryable' or that the target app has package visibility "
                    + "restrictions lifted. See https://developer.android.com/training/package-visibility");
            return "";
        }
    ...
    private String queryAngleChoice(
        ...
        Resources contextResource = context.getResources();
        if (contextResource == null) {
            Log.e(
                    TAG,
                    "Failed to get resources from application context, config_angleAllowList,"
                        + " config_angleDenyList, config_angleGlobalDenyList,"
                        + " config_angleForGamesEnabled will be ignored, returning DEFAULT driver"
                        + " choice as a fallback");
            return ANGLE_GL_DRIVER_CHOICE_DEFAULT;
        }
        final String[] angleAllowListPackages =
                contextResource.getStringArray(R.array.config_angleAllowList);
        ...
        final String[] deviceDenylist =
                contextResource.getStringArray(R.array.config_angleDenyList);
        if (DEBUG) {
        ...
        final String[] globalDenylist =
                contextResource.getStringArray(R.array.config_angleGlobalDenyList);
        if (DEBUG) {
        ...
        if (applicationInfoWithMetaData.category == ApplicationInfo.CATEGORY_GAME) {
            ...
            try {
                if (contextResource.getBoolean(R.bool.config_angleForGamesEnabled)) {
                    if (DEBUG) {
    ...
    private boolean setupAngle(
        ...
        if (android.os.Flags.useQueryAngleChoice()) {
            ...
            if (settingsChoice.equals(ANGLE_GL_DRIVER_CHOICE_ANGLE)) {
                ...
                setupANGLE = true;
                if (android.os.Flags.earlyInitEglDriver()) {
                    mForceEGLEarlyLoad = true;
                }
                ...
                if (persistChoice != null) {
                    ...
                    if (!android.os.Flags.doNotSetNativeDriverOptionForDefaultAngleQueryResult()) {
                        if (!setupANGLE) {
                            // We are using readonly choice and it is not ANGLE, inform the loader
                            nativeSetAngleInfo("", true, packageName, null);
                        }
    ...
    private boolean setupAngleFromApk(Context context, Bundle bundle, PackageManager packageManager,
        ...
        if (android.os.Flags.removeSetupAngleFromSettingsApk()) {
            if (angleInfo == null) {
                Log.v(
                        TAG,
                        "No ANGLE debug package found. Loading ANGLE libraries from ANGLE Settings"
                            + " apk is deprecated in Android 17. ANGLE in system will be loaded");
                return false;
            }
        } else {
                ...
                try {
                    ...
                    angleInfo =
                            packageManager.getApplicationInfo(
                                    anglePkgName, PackageManager.MATCH_SYSTEM_ONLY);
                } catch (PackageManager.NameNotFoundException e) {
