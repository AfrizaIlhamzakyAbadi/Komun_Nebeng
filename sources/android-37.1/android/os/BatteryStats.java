...
//    public static final long POWER_DATA_UNAVAILABLE = -1L;
    ...
    private void dumpHistory(PrintWriter pw, int flags, long monotonicClockStartTime,
            boolean checkin) {
        ...
        try (BatteryStatsHistoryIterator iterator =
                iterateBatteryStatsHistory(monotonicClockStartTime, MonotonicClock.UNDEFINED)) {
            HistoryItem rec;
            while ((rec = iterator.next()) != null) {
                try {
                    ...
                    if (rec.time >= monotonicClockStartTime) {
                        if (monotonicClockStartTime > 0 && !printed) {
     ...
     */
    @SuppressWarnings("unused")
    public void dump(Context context, PrintWriter pw, int flags, int reqUid,
            long monotonicClockStartTime, BatteryStatsDumpHelper dumpHelper) {
        synchronized (this) {
        ...
        if ((flags&DUMP_HISTORY_ONLY) != 0 || !filtering) {
            dumpHistory(pw, flags, monotonicClockStartTime, false);
            pw.println();
        }
    ...
    public void dumpCheckin(Context context, PrintWriter pw, List<ApplicationInfo> apps, int flags,
        ...
        if ((flags & (DUMP_INCLUDE_HISTORY | DUMP_HISTORY_ONLY)) != 0) {
            dumpHistory(pw, flags, monotonicClockStartTime, true);
        }
    ...
    public void dumpProtoLocked(Context context, FileDescriptor fd, List<ApplicationInfo> apps,
            int flags, long monotonicClockStartTime, BatteryStatsDumpHelper dumpHelper) {
        final ProtoOutputStream proto = new ProtoOutputStream(fd);
        prepareForDumpLocked();

        if ((flags & (DUMP_INCLUDE_HISTORY | DUMP_HISTORY_ONLY)) != 0) {
            dumpProtoHistoryLocked(proto, flags, monotonicClockStartTime);
            proto.flush();
            return;
        }
    ...
    private void dumpProtoHistoryLocked(ProtoOutputStream proto, int flags,
            long monotonicClockStartTime) {
