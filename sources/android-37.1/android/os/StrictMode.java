...
public final class StrictMode {
    ...
    @Hide
    @IntDef(flag = true, prefix = { "DETECT_THREAD_", "PENALTY_", "BINDER_HINT_" }, value = {
            ...
            BINDER_HINT_BLOCKING_MAIN_THREAD,
    })
    ...
    /**
     * Non-public Binder hints which indicates the call is blocking the main thread.
     *
     * <p>This must be kept in sync with the constant in libs/binder/include/binder/IPCThreadState.h
     */
    @Hide
    public static final int BINDER_HINT_BLOCKING_MAIN_THREAD = 1 << 22;

    /**
     * Mask for the process state hint. This is a 3-bit enum, unlike the other single-bit flags.
     *
     * Bits 19-21 are reserved to encode a 3-bit integer representing the process state
     * of the caller, which is set on the thread policy mask so that Binder transactions can
     * automatically propagate the caller's process state to the receiver thread.
     */
    @Hide
    public static final int BINDER_HINT_PROC_STATE_MASK = 0x00380000; // Bits 19-21

    @Hide
    public static final int BINDER_HINT_ALL_MASK = 0x00780000; // Bits 19-22

    @Hide
    private static final int PROC_STATE_SHIFT = 19;

    @Hide
    public static final int BINDER_HINT_PROC_STATE_UNKNOWN = 0 << PROC_STATE_SHIFT;

    @Hide
    // 001 (0x00080000)
    public static final int BINDER_HINT_PROC_STATE_PERSISTENT = 1 << PROC_STATE_SHIFT;

    @Hide
    // 010 (0x00100000)
    public static final int BINDER_HINT_PROC_STATE_FOREGROUND = 2 << PROC_STATE_SHIFT;

    @Hide
    // 011 (0x00180000)
    public static final int BINDER_HINT_PROC_STATE_BACKGROUND = 3 << PROC_STATE_SHIFT;

    @Hide
    // 100 (0x00200000)
    public static final int BINDER_HINT_PROC_STATE_CACHED = 4 << PROC_STATE_SHIFT;

    @Hide
    public static final int PENALTY_ALL = 0xff870000;
    ...
    private static void registerBackgroundActivityLaunchCallback() {
        if (sBackgroundActivityLaunchCallback == null) {
            ...
            // The remote call should not be part of the synchronized block
            try {
                ...
                sBackgroundActivityLaunchCallback = null;
                // ignore
            } catch (RemoteException e) {
                sBackgroundActivityLaunchCallback = null;
                Log.e(TAG, "RemoteException handling StrictMode violation", e);
