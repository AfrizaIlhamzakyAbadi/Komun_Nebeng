...
import static com.android.internal.FrameworksBaseTrackEventOuterClass.AndroidBitmap.SIZE;
import static com.android.internal.FrameworksBaseTrackEventOuterClass.AndroidBitmap.WIDTH;
import static com.android.internal.FrameworksBaseTrackEventOuterClass.AndroidBitmap.HEIGHT;
import static com.android.internal.FrameworksBaseTrackEventOuterClass.AndroidBitmap.DENSITY;
import static com.android.internal.FrameworksBaseTrackEventOuterClass.AndroidBitmap.CONFIG;
import static com.android.internal.FrameworksBaseTrackEventOuterClass.AndroidBitmap.MUTABLE_PIXELS;
import static com.android.internal.FrameworksBaseTrackEventOuterClass.AndroidBitmap.ID;
import static com.android.internal.FrameworksBaseTrackEventOuterClass.FrameworksBaseTrackEvent.BITMAP;
...
public final class Bitmap implements Parcelable {
    ...
    // Parcel as a blob in place instead of using ashmem if the size is within this limit
    // See the same definition in libs/hwui/jni/Bitmap.cpp
    // LINT.IfChange(blob_inplace_limit)
    private static final long BLOB_INPLACE_LIMIT = 12 * 1024;
    // LINT.ThenChange(libs/hwui/jni/Bitmap.cpp:blob_inplace_limit)
    ...
    private static final long UNDEFINED_SOURCE_ID = -1;
    ...
    private long mSourceId = UNDEFINED_SOURCE_ID;
    ...
    public long getSharedId() {
        return mSourceId != UNDEFINED_SOURCE_ID ? mSourceId : mId;
    }

    private static volatile Cleaner sSharedCleaner = null;
    private static final Object sCleanerLock = new Object();

    private static Cleaner getSharedCleaner() {
        Cleaner result = sSharedCleaner;
        if (result == null) {
            synchronized (sCleanerLock) {
                result = sSharedCleaner;
                if (result == null) {
                    sSharedCleaner = result = Cleaner.create();
                }
            }
        }
        return result;
    ...
    private static final ReferenceQueue<SharedAllocationHolder> sSharedAllocationsQueue =
            new ReferenceQueue<>();

    private static class SharedAllocationReference extends WeakReference<SharedAllocationHolder> {
        final long mSharedId;

        SharedAllocationReference(SharedAllocationHolder holder,
                                  ReferenceQueue<SharedAllocationHolder> queue) {
            super(holder, queue);
            mSharedId = holder.mSharedId;
        }
    }

    private static void processReferenceQueue() {
        Reference<? extends SharedAllocationHolder> ref;
        while ((ref = sSharedAllocationsQueue.poll()) != null) {
            sSharedAllocations.remove(((SharedAllocationReference) ref).mSharedId, ref);
        }
    }

    /**
     * Helper class to manage deduplicated native memory accounting for shared bitmaps.
     * When multiple Bitmap instances in the same process point to the same shared ashmem
     * or hardware allocation, we only want to register the native memory size once with
     * the JVM's NativeAllocationRegistry to avoid triggering excessive GCs.
     *
     * SharedAllocationHolder uses a process-global reference count keyed by a shared identity.
     * It ensures the allocation is registered when the first Bitmap instance is created
     * and unregistered only when the last instance is recycled or garbage-collected.
     */
    static class SharedAllocationHolder {
        private final Runnable mCleaner;
        private final AtomicInteger mRefCount = new AtomicInteger(1);
        private final long mSharedId;

        public SharedAllocationHolder(long sharedId, long nativePtr, int size, boolean malloc) {
            mSharedId = sharedId;
            NativeAllocationRegistry registry = createSizeTrackingRegistry(size, malloc);
            mCleaner = registry.registerNativeAllocation(this, nativePtr);
        }

        /**
         * Increments the reference count.
         * @return true if incremented successfully, false if the holder is already dead.
         */
        public boolean increment() {
            int current;
            do {
                current = mRefCount.get();
                if (current == 0) return false;
            } while (!mRefCount.compareAndSet(current, current + 1));
            return true;
        }
        ...
        public void decrement() {
            if (mRefCount.decrementAndGet() == 0) {
                mCleaner.run();
                sSharedAllocations.computeIfPresent(mSharedId, (id, ref) -> {
                    if (ref.get() == this) {
                        return null;
                    }
                    return ref;
    ...
    /**
     * Map of shared allocation IDs to their respective holders. We use a HashMap with
     * WeakReference values so that the SharedAllocationHolder can be garbage-collected
     * when no Bitmap instances in this process are holding a strong reference to it.
     */
    private static final ConcurrentHashMap<Long, SharedAllocationReference>
            sSharedAllocations = new ConcurrentHashMap<>();

    /**
     * Two NativeAllocationRegistry instances are used to register the native
     * allocation for each bitmap:
     *
     *   1. SizeTracking: One with a no-op free function for updating the native
     *      allocation size of the pixel data only, without actually releasing the
     *      native object that's associated with the bitmap instance.
     *
     *   2. ObjectTracking: sObjectTrackingRegistry is the other static one with a
     *      valid freeFunction and a default (estimated) size to actually release
     ...
     *  released.
     */
    private static NativeAllocationRegistry sObjectTrackingRegistry = null;

    private static NativeAllocationRegistry createSizeTrackingRegistry(int size, boolean malloc) {
        long noop = nativeGetNativeNoop();
        if (com.android.libcore.readonly.Flags.nativeMetrics()) {
            return malloc ? NativeAllocationRegistry.createMalloced(Bitmap.class, noop, size)
                          : NativeAllocationRegistry.createNonmalloced(Bitmap.class, noop, size);
        }
        ClassLoader loader = Bitmap.class.getClassLoader();
        return malloc ? NativeAllocationRegistry.createMalloced(loader, noop, size)
                      : NativeAllocationRegistry.createNonmalloced(loader, noop, size);
    }

    private static NativeAllocationRegistry createObjectTrackingRegistry() {
        long free = nativeGetNativeFinalizer();
        if (com.android.libcore.readonly.Flags.nativeMetrics()) {
            return NativeAllocationRegistry.createMalloced(Bitmap.class, free);
        }
        return NativeAllocationRegistry.createMalloced(Bitmap.class.getClassLoader(), free);
    }

    private Runnable mRecycler;
    private SharedAllocationHolder mSharedHolder;

    private void registerNativeAllocation(boolean malloc) {
        final int size = getAllocationByteCount();

        boolean isShared = Flags.nativeAllocationRegistryDedupeAshmemBitmaps() &&
                           (isShared() ||
                            (nativeIsImmutable(mNativePtr) && getConfig() == Config.HARDWARE));

        if (isShared) {
            processReferenceQueue();
            long sharedId = getSharedId();
            mSharedHolder = sSharedAllocations.compute(sharedId, (id, ref) -> {
                SharedAllocationHolder existing = (ref != null) ? ref.get() : null;
                if (existing != null && existing.increment()) {
                    return ref;
                }
                SharedAllocationHolder newHolder =
                        new SharedAllocationHolder(id, mNativePtr, size, malloc);
                return new SharedAllocationReference(newHolder, sSharedAllocationsQueue);
            }).get();

            final SharedAllocationHolder holderToClear = mSharedHolder;
            final Cleaner.Cleanable cleanable = getSharedCleaner().register(this,
                    () -> holderToClear.decrement());
            mRecycler = () -> cleanable.clean();
        } else {
            mRecycler = createSizeTrackingRegistry(size, malloc)
                    .registerNativeAllocation(this, mNativePtr);
        }

        if (sObjectTrackingRegistry == null) {
            sObjectTrackingRegistry = createObjectTrackingRegistry();
        }
        sObjectTrackingRegistry.registerNativeAllocation(this, mNativePtr);
    }
    ...
    Bitmap(long nativeBitmap, int width, int height, int density,
        ...
        this(0, UNDEFINED_SOURCE_ID, nativeBitmap, width, height, density, requestPremultiplied,
                ninePatchChunk, ninePatchInsets, true);
    }

    // called from JNI and Bitmap_Delegate.
    Bitmap(long id, long sourceId, long nativeBitmap, int width, int height, int density,
            boolean requestPremultiplied, byte[] ninePatchChunk,
    ...
    /**
     * Returns true if the bitmap is backed by ashmem
     */
    @Hide
    @android.ravenwood.annotation.RavenwoodReplace(reason = "ashmem not supported")
    public boolean isShared() {
        return nativeIsBackedByAshmem(mNativePtr);
    }

    private boolean isShared$ravenwood() {
        return false;
    }
    ...
    public static Bitmap createScaledAshmemBitmap(
        ...
        boolean isHardware = src.getConfig() == Config.HARDWARE;
        if (isHardware) {
        ...
        if (PerfettoCategories.BITMAP_CATEGORY.isEnabled()) {
            PerfettoTrackEventBuilder builder =
                    com.android.internal.dev.perfetto.sdk.PerfettoTrace
                            .begin(PerfettoCategories.BITMAP_CATEGORY, "Bitmap_createBitmap")
                            .setFlow(flowId);
    ...
    */
    ...
    public static final @NonNull Parcelable.Creator<Bitmap> CREATOR
            = new Parcelable.Creator<Bitmap>() {
                ...
                public Bitmap createFromParcel(Parcel p) {
                    long sourceId = p.readLong();
                    Bitmap bm = nativeCreateFromParcel(p, sourceId);
                    if (bm == null) {
    ...
    public void writeToParcel(@NonNull Parcel p, int flags) {
        ...
        Bitmap b = (Flags.cacheAshmemBitmaps() && !isShared() &&
                    getAllocationByteCount() > BLOB_INPLACE_LIMIT) ?
                BitmapCache.getShared(this) : this;

        p.writeLong(b.mId);
        if (!nativeWriteToParcel(b.mNativePtr, b.mDensity, p)) {
            throw new RuntimeException("native writeToParcel failed");
        }
        if (b.hasGainmap()) {
            p.writeBoolean(true);
            p.writeTypedObject(b.mGainmap, flags);
        } else {
    ...
    private static native Bitmap nativeCreateFromParcel(Parcel p, long sourceId);
    private static native void nativeSkipFromParcel(Parcel p);
