...
public class RippleDrawable extends LayerDrawable {
    ...
    private void updateMaskShaderIfNeeded() {
        ...
        if (Flags.rippleDrawableHwAccelMask()) {
            Picture picture = new Picture();
            Canvas pictureCanvas = picture.beginRecording(bounds.width(), bounds.height());
            final int saveCount = pictureCanvas.save();
            pictureCanvas.translate(-bounds.left, -bounds.top);
            if (maskType == MASK_EXPLICIT) {
                drawMask(pictureCanvas);
            } else if (maskType == MASK_CONTENT) {
                drawContent(pictureCanvas);
            }
            pictureCanvas.restoreToCount(saveCount);
            ...
            mMaskBuffer = Bitmap.createBitmap(picture);
            ...
            if (mMaskBuffer == null
                    ...
                    || mMaskBuffer.getConfig() == Bitmap.Config.HARDWARE) {
                if (mMaskBuffer != null) {
                    mMaskBuffer.recycle();
                }
