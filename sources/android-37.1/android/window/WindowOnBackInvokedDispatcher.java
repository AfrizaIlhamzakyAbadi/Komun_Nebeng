...
public class WindowOnBackInvokedDispatcher implements OnBackInvokedDispatcher {
    ...
    /**
     * Runnable that sends the top callback to the window manager.
     */
    private final Runnable mScheduledOnBackInvokedCallbackUpdater = () -> {
        OnBackInvokedCallbackInfo callbackInfo;
        IWindowSession windowSession;
        IWindow window;
        OnBackInvokedCallback callback;
        synchronized (mLock) {
            callback = getTopCallback();
            callbackInfo = createCallbackInfo(callback);
            windowSession = mWindowSession;
            window = mWindow;
        }
        if (windowSession == null || window == null) {
            return;
        }
        try {
            Log.d(TAG, "setOnBackInvokedCallbackInfo: " + callback);
            windowSession.setOnBackInvokedCallbackInfo(window, callbackInfo);
    ...
    public void onMotionEvent(MotionEvent ev) {
        ...
        // Drop local motion events if the Shell will pilfer pointers. This prevents residual touch
        // inputs on the UI thread from resetting the starting coordinates and breaking animations.
        if (!mTouchTracker.isGenerateProgressAllowed()) {
            return;
        }
    ...
    public void onBackStarted(OnBackInvokedCallback callback, BackEvent backEvent,
        ...
        if (Flags.offloadSetTopOnBackInvoked()) {
            synchronized (mLock) {
                if (!mAllCallbacks.containsKey(callback)) {
                    return;
                }
    ...
    private void setTopOnBackInvokedCallback(@Nullable OnBackInvokedCallback callback) {
        ...
        if (Flags.offloadSetTopOnBackInvoked()) {
            BackgroundThread.getHandler().removeCallbacks(mScheduledOnBackInvokedCallbackUpdater);
            BackgroundThread.getHandler().post(mScheduledOnBackInvokedCallbackUpdater);
        } else {
            try {
                mWindowSession.setOnBackInvokedCallbackInfo(mWindow, createCallbackInfo(callback));
            } catch (RemoteException e) {
                Log.e(TAG, "Failed to set OnBackInvokedCallback to WM. Error: " + e);
            }
        }
    ...
    @GuardedBy("mLock")
    private OnBackInvokedCallbackInfo createCallbackInfo(@Nullable OnBackInvokedCallback callback) {
        if (callback == null) {
            return null;
        }
    ...
    private class OnBackInvokedCallbackWrapper extends IOnBackInvokedCallback.Stub {
        ...
        @Override
        public void onBackStarted(BackMotionEvent backEvent, boolean appGenerateProgressAllowed) {
            mHandler.post(() -> {
                ...
                mTouchTracker.setGenerateProgressAllowed(appGenerateProgressAllowed);
