...
public class TypedValue {
    ...
    /**
     * Validate that the mantissa is within the valid range for a complex data integer.
     *
     * @param mantissa an integer representing the mantissa.
     * @return true if the mantissa is within the valid range, false otherwise.
     */
    @Hide
    private static boolean isValidMantissa(@IntRange(from = -0x800000, to = 0x7FFFFF) int mantissa) {
        return mantissa >= -0x800000 && mantissa < 0x800000;
    }
    ...
    public static int floatToComplex(@FloatRange(from = -0x800000, to = 0x7FFFFF) float value) {
        ...
        try {
            ...
            if (absValue < 1f) {
                int mantissa = Math.round(value * (1 << 23));
                if (isValidMantissa(mantissa)) {
                    return createComplex(mantissa, TypedValue.COMPLEX_RADIX_0p23);
                }
            }
            // If the magnitude is less than 2^8, use 8 magnitude digits
            if (absValue < (float) (1 << 8)) {
                int mantissa = Math.round(value * (1 << 15));
                if (isValidMantissa(mantissa)) {
                    return createComplex(mantissa, TypedValue.COMPLEX_RADIX_8p15);
                }
            }
            // If the magnitude is less than 2^16, use 16 magnitude digits
            if (absValue < (float) (1 << 16)) {
                int mantissa = Math.round(value * (1 << 7));
                if (isValidMantissa(mantissa)) {
                    return createComplex(mantissa, TypedValue.COMPLEX_RADIX_16p7);
                }
            }
