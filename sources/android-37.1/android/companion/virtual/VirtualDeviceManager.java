 ...
 * </ul>
 *
 * <p>Devices that support the ComputerControl feature:
 *
 * <ul>
 *   <li>MUST only allow applications to use the Computer Controls API if it is listed in the latest
 *       version of the agent allowlist.
 *   <li>MUST NOT allow a Computer Controls Agent to automate any target applications that:
 *       <ul>
 *         <li>Are not included in the latest version of the allowlist OR
 *         <li>Are included in the latest version of the Computer Controls denylist OR
 *         <li>Do not have explicit opt-in user consent for automation by the requesting Computer
 *             Controls Agent
 *       </ul>
 *   <li>MAY remove applications from the Computer Controls allowlist, or MAY add applications to
 *       the Computer Controls denylist, provided it results in a more restrictive set of
 *       automatable applications
 * </ul>
 *
 * <p>ComputerControl may only be used by agent applications holding the {@link
 * android.Manifest.permission#ACCESS_COMPUTER_CONTROL} permission and meeting the relevant policy
 * requirements. Only the current ASSISTANT role holder can use ComputerControl.
 *
...
public final class VirtualDeviceManager {
    ...
    @RequiresPermission(anyOf = {
            ...
            android.Manifest.permission.ACCESS_COMPUTER_CONTROL
    })
    public boolean isPackageTargetableForComputerControlAutomation(@NonNull String packageName,
        ...
        try {
            return mService.isPackageTargetableForComputerControlAutomation(
                    packageName, userId, mContext.getAttributionSource());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public static class VirtualDevice implements AutoCloseable {
         ...
         * (e.g., if the device does not support
         * {@link android.content.pm.PackageManager#FEATURE_CAMERA_ANY}).
    ...
    public interface ActivityListener {
         ...
         * @param user The user associated with the activity whose launch is requested.
         */
        @FlaggedApi(Flags.FLAG_ACTIVITY_LAUNCH_REQUESTED_API)
    ...
    private static class LocalComputerControlConsentManager implements
        ...
        @Override
        @RequiresPermission(android.Manifest.permission.MANAGE_COMPUTER_CONTROL_CONSENT)
        public void setUserConsent(int agentUid, @NonNull String agentPackageName,
                @NonNull String targetPackageName,
                @ComputerControlConsentManager.ConsentState int consentState) {
            try {
                mConsentManager.setUserConsent(agentUid, agentPackageName, targetPackageName,
                        consentState);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        ...
        @Override
        @ComputerControlConsentManager.ConsentState
        @RequiresPermission(anyOf = {
                android.Manifest.permission.MANAGE_COMPUTER_CONTROL_CONSENT,
                android.Manifest.permission.ACCESS_COMPUTER_CONTROL
        })
        public int getUserConsent(int agentUid, @NonNull String agentPackageName,
                @NonNull String targetPackageName) {
            try {
                return mConsentManager.getUserConsent(agentUid, agentPackageName,
                        targetPackageName);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        ...
        @Override
        @RequiresPermission(android.Manifest.permission.MANAGE_COMPUTER_CONTROL_CONSENT)
        public void clearAllUserConsents(int agentUid,
                @NonNull String agentPackageName) {
            try {
                mConsentManager.clearAllUserConsents(agentUid, agentPackageName);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
        ...
        @NonNull
        @RequiresPermission(android.Manifest.permission.MANAGE_COMPUTER_CONTROL_CONSENT)
        public Map<String, Integer> getUserConsents(int agentUid,
                @NonNull String agentPackageName) {
            try {
                return (Map<String, Integer>) mConsentManager.getUserConsents(agentUid,
                        agentPackageName);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
