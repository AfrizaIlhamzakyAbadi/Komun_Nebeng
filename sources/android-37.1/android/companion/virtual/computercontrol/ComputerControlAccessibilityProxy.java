...
final class ComputerControlAccessibilityProxy extends AccessibilityDisplayProxy
    ...
    /**
     * Declares that the session is unstable for the duration of the returned
     * {@link UnstablePeriod}.
     *
     * <p>When the returned object is closed, the session may become stable again after the
     * idle timeout expires.
     */
    @NonNull
    UnstablePeriod newUnstablePeriod(@ComputerControlSession.UnstableReason int reason) {
        synchronized (this) {
            if (mStabilitySignalTracker != null) {
                return mStabilitySignalTracker.newUnstablePeriod(reason);
            }
            return () -> {};
        }
    }

    /** Represents a period during which the session is considered unstable. */
    interface UnstablePeriod extends AutoCloseable {
        @Override
        void close();
    }
        ...
        synchronized (this) {
            ...
            final var callbackRecord = new ComputerControlSession.StabilityListener() {
                ...
                public void onSessionStable() {
                    executeSafely(listener::onSessionStable);
                }

                @Override
                public void onSessionUnstable(@ComputerControlSession.UnstableReason int reason) {
                    executeSafely(() -> listener.onSessionUnstable(reason));
                }
                ...
                private void executeSafely(Runnable runnable) {
                    synchronized (ComputerControlAccessibilityProxy.this) {
                        ...
                        if (mStabilitySignalTracker != null
                                && mStabilitySignalTracker.mStabilityListener == this) {
    ...
    private static final class StabilitySignalTracker implements AutoCloseable,
        ...
        private void checkStability() {
            if (mUnstableReason == null || mEventIdleTracker.hasPendingCallback()
                    || !mIsFirstFrameReceived || mActiveUnstablePeriods > 0) {
                return;
            }
