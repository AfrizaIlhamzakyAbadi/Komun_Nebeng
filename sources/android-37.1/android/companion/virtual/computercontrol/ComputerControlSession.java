...
import android.companion.virtual.computercontrol.IComputerControlSession.IInteractionResultReceiver;
...
public final class ComputerControlSession implements AutoCloseable {
    ...
    @Hide
    public static final String EXTRA_IS_BLOCKED =
            "android.companion.virtual.computercontrol.extra.IS_BLOCKED";

    @Hide
    public static final String EXTRA_PREVIEW_INTENT =
            "android.companion.virtual.computercontrol.extra.PREVIEW_INTENT";
    ...
    /**
     * Error code indicating that the caller does not have permission to perform the operation.
     *
     * <p>This can occur if the user did not approve the creation of a new session. Starting from
     * version 5, this is also possible if the user did not approve the addition of new packages,
     * or if the caller does not have a visible non-toast window on any display or does not have a
     * valid notification channel.</p>
     */
    ...
    /**
     * Reason indicating that the session was suspended due to a {@link #notifySuspend()} request
     * from the caller.
     *
     * <p>This is a special blocked state indicating that the caller has suspended the session.
     * While suspended, the system may change to UI so that it no longer appears as though the
     * caller is actively automating the target applications. It is intended to be used when the
     * automation task for this session is complete and the caller is waiting for additional
     ...
     * caller, such as {@link #tap}.
     */
    public static final int UNSTABLE_REASON_CALLER_INTERACTION = 1;

    /**
     * Reason indicating that there was a change in the session's insets.
     *
     * <p>Insets changes may cause the app's content to be resized and its layout to change in
     * either subtle or significant ways, depending on both the inset changes and also on how
     * the app handles insets.
     *
     * <p>The session may encounter insets changes when the session is being displayed through
     * the {@link com.android.extensions.computercontrol.view.MirrorView}. The MirrorView will
     * adjust the session's insets to optimize the app's layout for the viewing by the user.
     */
    ...
    private final IComputerControlLifecycleCallback mRemoteLifecycleCallback =
            new IComputerControlLifecycleCallback.Stub() {
                ...
                public void onClosed(@SessionCloseReason int closeReason) {
                    // We don't unregister the accessibility proxy here because we rely on the
                    // display removal to automatically unregister it.
                    releaseResources();
                    synchronized (mLifecycle) {
    ...
    public ComputerControlSession(int displayId,
        ...
        this(displayId, session, a11yManager, onClosedRunnable, DisplayManagerGlobal.getInstance());
    }
    ...
    public ComputerControlSession(int displayId,
        ...
        mHandlerThread = new HandlerThread("ComputerControl-ComputerControlSession");
     ...
     * @deprecated Use {@link #startActivity(Intent, Bundle)} instead.
     */
    @Deprecated
    public void launchApplication(@NonNull String packageName) {
        launchApplication(packageName, /* className= */ null);
    ...
    public void launchApplication(@NonNull ComponentName component) {
        launchApplication(component.getPackageName(), component.getClassName());
        mAccessibilityProxy.resetStabilityState(UNSTABLE_REASON_CALLER_INTERACTION);
    }

    /**
     * Starts an activity using the provided intent in the computer control
     * session.
     *
     * @param intent the intent to launch. Must have a package or component set.
     * @param options additional options for launching the activity, typically constructed
     *                using {@link android.app.ActivityOptions} (e.g., via
     *                {@link android.app.ActivityOptions#toBundle()}). Note that the launch
     *                display will be overridden by the session.
     * @throws IllegalArgumentException if the intent does not have a package or component set.
     */
    public void startActivity(@NonNull Intent intent, @Nullable Bundle options) {
        try {
            mSession.startActivity(intent, options);
        } catch (RemoteException e) {
     ...
     * {@link #startActivity(Intent, Bundle)}</p>
     */
    ...
    /**
     * Exception used to indicate how an interaction request failed.
     */
    public static class InteractionException extends Exception {

        /** An unknown error occurred when processing this interaction request. */
        public static final int ERROR_UNKNOWN = 0;

        /** The interaction was canceled, as requested by the caller. */
        public static final int ERROR_CANCELED = 1;

        /** The interaction failed. */
        public static final int ERROR_FAILED = 2;

        /** The interaction was not permitted. */
        ...
        @Hide
        @IntDef({ERROR_UNKNOWN, ERROR_CANCELED, ERROR_FAILED, ERROR_NOT_PERMITTED})
        @Retention(RetentionPolicy.SOURCE)
        public @interface ErrorCode {
        }

        @Hide
        public InteractionException(@ErrorCode int errorCode) {
    ...
    public void requestScreenshot(@NonNull @CallbackExecutor Executor executor,
        ...
        try {
            ...
            mSession.requestScreenshot(new IInteractionResultReceiver.Stub() {
                @Override
                public void onInteractionResult(int result) {
                    if (result == IInteractionResultReceiver.Result.SUCCESS) {
                        synchronized (mImageReaderLock) {
                            callback.isRequestSuccess.set(true);
                            fireScreenshotCallbackIfReady(callback);
                        }
                    } else {
                        onScreenshotError(callback, translateResultToScreenshotError(result),
                                "Server error: " + result);
                    }
                }
            });
    ...
    private static int translateResultToScreenshotError(int result) {
        return switch (result) {
            case IInteractionResultReceiver.Result.NOT_PERMITTED ->
                    ScreenshotException.ERROR_PROHIBITED;
            case IInteractionResultReceiver.Result.CANCELLED -> ScreenshotException.ERROR_CANCELED;
            default -> ScreenshotException.ERROR_INTERNAL;
        };
    }

    /**
     * Requests a snapshot of the current UI structure of the screen asynchronously.
     *
     * @param executor The executor on which the callback will be invoked.
     * @param receiver The outcome receiver callback to be invoked when the snapshot is available
     *                 or encounters any issues.
     * @param cancellationSignal Optional cancellation signal to cancel the request.
     */
    public void requestUiSnapshot(@NonNull @CallbackExecutor Executor executor,
            @NonNull OutcomeReceiver<ComputerControlUiSnapshot, ScreenshotException> receiver,
            @Nullable CancellationSignal cancellationSignal) {
        Objects.requireNonNull(executor, "Executor must not be null");
        Objects.requireNonNull(receiver, "OutcomeReceiver must not be null");
        ...
        final List<AccessibilityWindowInfo> windows = getAccessibilityWindows();
        executor.execute(() -> {
            if (cancellationSignal != null && cancellationSignal.isCanceled()) {
                receiver.onError(new ScreenshotException(ScreenshotException.ERROR_CANCELED));
            } else {
                receiver.onResult(ComputerControlUiSnapshot.fromAccessibilityWindowInfos(windows));
            }
        });
    }
     ...
     * @param cancellationSignal a {@link CancellationSignal} to cancel the ongoing interaction.
     * @param executor           the {@link Executor} to run the receiver on.
     * @param receiver           the {@link OutcomeReceiver} to receive the result of the
     *                           interaction.
     */
    public void tap(@IntRange(from = 0) int x, @IntRange(from = 0) int y,
            @Nullable CancellationSignal cancellationSignal,
            @Nullable @CallbackExecutor Executor executor,
            @Nullable OutcomeReceiver<Void, InteractionException> receiver) {
        validateTouchCoordinates(x, y);
        ICancellationSignal syncSignal;
        try {
            syncSignal = mSession.tap(x, y, createInteractionResultReceiver(executor, receiver));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
        if (cancellationSignal != null) {
            cancellationSignal.setRemote(syncSignal);
        }
    }
     ...
     * display.</p>
     *
     * @param fromX              x-coordinate of the start position
     * @param fromY              y-coordinate of the start position
     * @param toX                x-coordinate of the end position
     * @param toY                y-coordinate of the end position
     * @param cancellationSignal a {@link CancellationSignal} to cancel the ongoing interaction.
     * @param executor           the {@link Executor} to run the receiver on.
     * @param receiver           the {@link OutcomeReceiver} to receive the result of the
     *                           interaction.
     */
    public void swipe(@IntRange(from = 0) int fromX, @IntRange(from = 0) int fromY,
        ...
        ICancellationSignal syncSignal;
        try {
            syncSignal = mSession.swipe(fromX, fromY, toX, toY,
                    createInteractionResultReceiver(executor, receiver));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
        if (cancellationSignal != null) {
    ...
    public void longPress(@IntRange(from = 0) int x, @IntRange(from = 0) int y,
        ...
        ICancellationSignal syncSignal;
        try {
            syncSignal = mSession.longPress(x, y,
                    createInteractionResultReceiver(executor, receiver));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    ...
    public void insertText(@NonNull String text, boolean replaceExisting, boolean commit,
        ...
        ICancellationSignal syncSignal;
        try {
            syncSignal = mSession.insertText(text, replaceExisting, commit,
                    createInteractionResultReceiver(executor, receiver));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    ...
    public void performAction(@Action int actionCode,
        ...
        ICancellationSignal syncSignal;
        try {
            syncSignal = mSession.performAction(actionCode,
                    createInteractionResultReceiver(executor, receiver));
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
     ...
     *
     * @deprecated Use {@link #requestUiSnapshot(Executor, OutcomeReceiver, CancellationSignal)}
     *             instead.
     */
    @NonNull
    @Deprecated
    ...
    /**
     * Notifies the system that the caller has completed the task successfully and is waiting for
     * additional commands from the user, so the caller is suspending the session. If the
     * request is successful, the session will enter the blocked lifecycle state
     * ({@link LifecycleCallback#onBlocked(int, String)}), with
     * {@link #BLOCK_REASON_SUSPENDED}.
     *
     * <p>When the session is blocked due to this suspend request, the system will treat the target
     * applications for this session as no longer being automated. For example, the user will be
     * allowed to launch the target applications from the launcher without any warnings
     * about the app being actively used by the caller.
     */
    public void notifySuspend() {
        try {
            mSession.notifySuspend();
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Attempts to resume the session when it is blocked or suspended.
     *
     * <p>If the user is interacting with the session through the interactive mirror view to
     * resolve the blockage for session, this should be called when the user explicitly chooses
     * to end their control of the session.
     *
     * <p>This should also be called to regain control of the session when needed if the session
     * was previously suspended using {@link #notifySuspend()}.
     *
     * @return {@code true} if the session successfully resumed or changed its block reason;
     *         {@code false} if the session was not blocked or if the blockage could not be
     *         resolved and the session remained in the exact same blocked state.
     */
    public boolean requestResume() {
        try {
            return mSession.requestResume();
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Adds the package names of the applications that may be automated during this session.
     *
     * <p>Adding new packages to an ongoing session always requires user consent, irrespective of
     * previous consent from the user for the same package.</p>
     *
     * <p>Any package names specified in the list that are already allowed in the session will be
     * ignored. If all package names specified in the list are already allowed, the callback will
     * be notified with success immediately.</p>
     *
     * <p>The caller app can use this before calling {@link #startActivity(Intent, Bundle)} API to
     * launch a new application mid-way through the session or to unblock itself from unexpected
     * app launches on the CC display. We send the package name of the blocking package in
     * {@link LifecycleCallback#onBlocked(int, String)}, which the caller can add to target package
     * list and subsequently call {@link #requestResume()} to resume automation of the newly opened
     * application.</p>
     *
     * <p>If the user approves the request, all the specified package names will be added to the
     * session. If the user denies the request for any of the packages, none of the package names
     * will be added, and the session remains in its previous state.</p>
     *
     * @param packageNames list of package names to be added as target packages.
     * @param executor executor to run the callback on.
     * @param callback callback to be notified about the result of the operation.
     * @throws Exception if any of the provided package names are invalid or if adding them would
     *         exceed the maximum number of target packages.
     */
    @Hide
    public void addPackagesForAutomation(@NonNull List<String> packageNames,
            @NonNull @CallbackExecutor Executor executor,
            @NonNull TargetPackagesAdditionCallback callback) {
        Objects.requireNonNull(packageNames);
        Objects.requireNonNull(executor);
        Objects.requireNonNull(callback);

        try {
            mSession.addPackagesForAutomation(packageNames,
                    new ITargetPackagesAdditionCallback.Stub() {
                        @Override
                        public void onTargetPackagesAdditionPending(
                                @NonNull PendingIntent pendingIntent) {
                            Binder.withCleanCallingIdentity(() ->
                                    executor.execute(() ->
                                            callback.onTargetPackagesAdditionPending(
                        ...
                        public void onTargetPackagesAdditionSuccess() {
                            Binder.withCleanCallingIdentity(() ->
                                    executor.execute(callback::onTargetPackagesAdditionSuccess));
                        }

                        @Override
                        public void onTargetPackagesAdditionFailure(
                                @TargetPackagesAdditionCallback.TargetPackagesAdditionError
                                        int errorCode) {
                            Binder.withCleanCallingIdentity(() ->
                                    executor.execute(() ->
                                            callback.onTargetPackagesAdditionFailure(errorCode)));
                        }
                    });
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
    ...
    private void launchApplication(@NonNull String packageName,
            @Nullable String className) {
        try {
            mSession.launchApplication(packageName, className);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    private void releaseResources() {
        ...
        mAccessibilityProxy.close();
    }

    @Hide
    public void onMirrorInsetsChanged() {
        mAccessibilityProxy.resetStabilityState(UNSTABLE_REASON_INSETS_CHANGE);
    }
    ...
    private void onScreenshotError(ScreenshotCallbackRecord callback,
        ...
        synchronized (mImageReaderLock) {
            ...
            // Always discard any image that arrived if the server reports a failure
    ...
    @NonNull
    private IInteractionResultReceiver createInteractionResultReceiver(@Nullable Executor executor,
            @Nullable OutcomeReceiver<Void, InteractionException> receiver) {
        if (receiver != null && executor == null) {
            throw new IllegalArgumentException(
                    "executor must not be null when receiver is provided");
        }
        var unstablePeriod = mAccessibilityProxy.newUnstablePeriod(
                UNSTABLE_REASON_CALLER_INTERACTION);

        return new IInteractionResultReceiver.Stub() {
            @Override
            public void onInteractionResult(int result) {
                unstablePeriod.close();
                ...
                executor.execute(() ->
                        Binder.withCleanCallingIdentity(() -> {
                            switch (result) {
                                case IInteractionResultReceiver.Result.SUCCESS -> receiver.onResult(
                                        null);
                                case IInteractionResultReceiver.Result.CANCELLED ->
                                        receiver.onError(
                                                new InteractionException(
                                                        InteractionException.ERROR_CANCELED));
                                case IInteractionResultReceiver.Result.NOT_PERMITTED ->
                                        receiver.onError(
                                                new InteractionException(
                                                        InteractionException.ERROR_NOT_PERMITTED));
                                default -> receiver.onError(
                                        new InteractionException(
                                                InteractionException.ERROR_FAILED));
                            }
                        }));
            }
        };
    ...
    public interface LifecycleCallback {
         ...
         *
         * <ul>
         *   <li>Interactions with the session (e.g. {@link #tap}) are allowed.
    ...
    /**
     * Callback for target packages addition operation (e.g. {@link
     * #addPackagesForAutomation(List, Executor, TargetPackagesAdditionCallback)})
     */
    public interface TargetPackagesAdditionCallback {
        ...
        @IntDef(prefix = "ERROR_", value = {
                ...
                ERROR_PERMISSION_DENIED})
        @Target({ElementType.TYPE_PARAMETER, ElementType.TYPE_USE})
        @interface TargetPackagesAdditionError {
        }

        /**
         * Unknown target packages addition error.
         */
        int ERROR_UNKNOWN = ComputerControlSession.ERROR_UNKNOWN;

        /**
         * Error code indicating that the caller does not have permission to perform the operation.
         *
         * <p>This can occur if the user did not approve the addition of new packages, or if the
         * caller does not have a visible non-toast window on any display.</p>
         */
        int ERROR_PERMISSION_DENIED = ComputerControlSession.ERROR_PERMISSION_DENIED;

        /**
         * Called when the target packages addition request needs to be approved by the user.
         *
         * @param intentSender an {@link IntentSender} which applications should use to launch
         *   the UI for the user to allow the addition.
         */
        void onTargetPackagesAdditionPending(@NonNull IntentSender intentSender);

        /** Called when the target packages have been successfully added. */
        void onTargetPackagesAdditionSuccess();

        /**
         * Called when the target packages addition failed.
         *
         * @param errorCode the reason for failure.
         */
        void onTargetPackagesAdditionFailure(@TargetPackagesAdditionError int errorCode);
    }
                    ...
                    new InjectedA11yManager() {
                        ...
                        public void registerDisplayProxy(AccessibilityDisplayProxy proxy) {
                            try {
                                ...
                                Log.e(TAG, "Failed to register accessibility display proxy", e);
                            }
                        }
                        ...
                        public void unregisterDisplayProxy(AccessibilityDisplayProxy proxy) {
                            try {
                                ...
                                Log.e(TAG, "Failed to unregister accessibility display proxy", e);
                            }
                        }
