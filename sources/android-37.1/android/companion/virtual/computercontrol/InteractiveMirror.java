...
public final class InteractiveMirror implements AutoCloseable {
    ...
    /**
     * Sets the display ID of the host window where the mirror is shown.
     */
    @Hide
    @RequiresNoPermission
    public void setHostDisplayId(int displayId) {
        try {
            mMirror.setHostDisplayId(displayId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     */
    @RequiresNoPermission
    public void updateScale(float sessionToMirrorScale) {
        if (sessionToMirrorScale <= 0) {
            return;
        }

        try {
            mMirror.updateScale(sessionToMirrorScale);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
