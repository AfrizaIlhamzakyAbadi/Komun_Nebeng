...
public abstract class ContentSafetyService extends Service {
        ...
        if (SERVICE_INTERFACE.equals((intent.getAction()))) {
            return new IContentSafetyService.Stub() {
                ...
                public List<String> getSupportedMimeTypes() {
                    List<String> types = onGetSupportedMimeTypes();
                    return types != null ? Collections.unmodifiableList(types)
                ...
                @Override
                @RequiresPermission(android.Manifest.permission.CHECK_CONTENT_SAFETY)
                public void cancelClassification(List<LocusId> locusIds) {
                    mHandler.executeOrSendMessage(
                            obtainMessage(
                                    ContentSafetyService::onCancelClassification,
                                    ContentSafetyService.this,
                                    locusIds));
    ...
    /**
     * Provides a list of the supported MIME file types that can be provided to be checked
     */
    @FlaggedApi(FLAG_CONTENT_SAFETY_API_V2)
    @NonNull
    public List<String> onGetSupportedMimeTypes() {
        return Collections.emptyList();
    }

    /**
     * Called to cancel content classification for the provided content {@link LocusId}s.
     */
    @FlaggedApi(FLAG_CONTENT_SAFETY_API_V2)
    public void onCancelClassification(@NonNull List<LocusId> locusIds) {};
