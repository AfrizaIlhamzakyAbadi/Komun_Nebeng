...
import android.app.contentsafety.CheckContentCallback;
 ...
 * {@link ContentSafetySandboxedService#onRequestClassification}.
 *
 ...
 *  and then {@link ContentSafetySandboxedService#onRequestClassification}.
 *
 * <pre>
 *     {@literal
...
public abstract class ContentSafetySandboxedService extends Service {
        ...
        if (SERVICE_INTERFACE.equals(intent.getAction())) {
            return new IContentSafetySandboxedService.Stub() {
                ...
                public void requestClassification(
                        List<ClassifiableContent> input,
                    ...
                    mHandler.executeOrSendMessage(
                            obtainMessage(
                                    ContentSafetySandboxedService::onRequestClassification,
                                    ContentSafetySandboxedService.this,
                                    input,
                                    new CheckContentCallback() {
                                        @Override
                                        public void onClassification(
                                                List<ContentClassificationResult> result) {
                                            try {
                                                callback.onClassification(result);
                                            } catch (RemoteException e) {
                                                Slog.e(TAG, "Failed to send result back to client",
                                                        e);
                                            }
                                        }
                                        ...
                                        public void onClassificationComplete() {
                                            try {
                                                callback.onClassificationComplete();
                                            } catch (RemoteException e) {
                                                Slog.e(TAG, "Failed to send result back to client",
                                                        e);
                                            }
    ...
    /**
     * @deprecated Use {@link #onRequestClassification(List, CheckContentCallback)} instead
     *
    ...
    /**
     * Invoked by {@link ContentSafetyManager#requestClassification}.
     *
     * @param input list of classifiable contents to be checked.
     * @param callback to populate the result of the checkContent call.
     */
    @FlaggedApi(FLAG_CONTENT_SAFETY_API_V2)
    @RequiresPermission(android.Manifest.permission.CHECK_CONTENT_SAFETY)
    public void onRequestClassification(
            @NonNull List<ClassifiableContent> input,
