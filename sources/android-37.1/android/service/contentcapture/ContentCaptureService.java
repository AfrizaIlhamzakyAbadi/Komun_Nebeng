...
public abstract class ContentCaptureService extends Service {
    ...
    /**
     * Explicitly sets the Content Capture version status for the given packages and activities.
     *
     * <p>This method allows the service to granularly control which Content Capture Version
     * is active for targeted applications or specific components. This is particularly useful
     * during transitions between API iterations.
     *
     * <p>The configuration provided in this method is stateless and non-accumulating. Each call
     * completely replaces the previous configuration.</p>
     *
     * <p>A version change is broadcast to allowlisted applications immediately and is made
     * available to any allowlisted application that starts after the change. However, already
     * created {@link ContentCaptureSession} objects are not updated to the new version
     * configuration. Therefore, a connected Activity will continue to report to the Content Capture
     * version that was active during its creation. Only after a restart will the activities pick
     * up the new version configuration.</p>
     *
     * <p>To evaluate which Content Capture version is utilized for a particular activity, the
     * system performs the following resolution sequence:
     * <ol>
     *   <li>First, the system examines the {@code activities} map. If the exact component matches,
     *       the corresponding version configuration is applied.</li>
     *   <li>If the activity is not specified, the system inspects the {@code packages} map.
     *       If the activity's package is found within this map, the provided version
     *       configuration is applied.</li>
     *   <li>Finally, the system defaults to enabling {@link ContentCaptureSession#VERSION_1}.</li>
     * </ol>
     *
     * <p><b>Note:</b> Content Capture versions are mutually exclusive for a given activity.
     * Only one version can be active at a time.</p>
     *
     * <p><b>Note:</b> The version configuration does not override the allowlist. A package or
     * component that is not allowlisted will not be considered for version control. Make sure that
     * all given packages and components are also allowlisted.</p>
     *
     * @param packages A map of package names to their corresponding Version, or {@code null} to
     *                 reset version control for all packages.
     * @param activities A map of ComponentNames to their corresponding Version, or {@code null} to
     *                   reset version control for all activities.
     *
     * @throws IllegalStateException if called before the service is connected, as signaled by
     *         {@link #onConnected()}.
     */
    @FlaggedApi(Flags.FLAG_CCAPI_NEXT_ENABLED)
    public final void setContentCaptureVersions(@Nullable Map<String, Integer> packages,
            @Nullable Map<ComponentName, Integer> activities) {
        final IContentCaptureServiceCallback callback = mContentCaptureServiceCallback;
        if (callback == null) {
            throw new IllegalStateException("Service not connected; "
                    + "this method can only be called after onConnected()");
        }

        try {
            callback.setContentCaptureVersions(packages, activities);
        } catch (RemoteException e) {
            e.rethrowFromSystemServer();
        }
    }
