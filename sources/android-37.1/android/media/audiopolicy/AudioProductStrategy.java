...
import static android.media.audiopolicy.AudioAttributesMatchHelper.MATCH_MAX_SCORE_BIT_DEPTH;
...
public final class AudioProductStrategy implements Parcelable {
    ...
    private static final int MATCH_ON_ZONE_ID_SCORE = 1 << MATCH_MAX_SCORE_BIT_DEPTH;
    private static final int MATCH_EQUALS = MATCH_ON_ZONE_ID_SCORE
            | AudioAttributesMatchHelper.MATCH_ATTRIBUTES_EQUALS;
    ...
    private static boolean isDefaultMatchScore(int matchScore) {
        if (multiZoneAudio()) {
            return (matchScore == AudioAttributesMatchHelper.MATCH_ON_DEFAULT_SCORE) || (matchScore
                    == MATCH_ON_ZONE_ID_SCORE);
        }
        return matchScore == AudioAttributesMatchHelper.MATCH_ON_DEFAULT_SCORE;
    }
    ...
    private static boolean isMatchScoreEquals(int matchScore) {
        ...
        return matchScore == AudioAttributesMatchHelper.MATCH_ATTRIBUTES_EQUALS;
    }
    ...
    private static AudioAttributesGroup getAudioAttributesGroupForAttributes(
        ...
        int matchScore = AudioAttributesMatchHelper.NO_MATCH;
    ...
    public int getLegacyStreamTypeForAudioAttributes(@NonNull AudioAttributes attributes) {
        ScoredAudioAttributesGroup scoredAag;
        // If there is no match (score = AudioAttributesMatchHelper.NO_MATCH) audio attributes
        // group (aag) is guaranteed to be null
        if (multiZoneAudio()) {
    ...
    public boolean supportsAudioAttributes(@NonNull AudioAttributes aa) {
        ...
        return getScoredAttributeGroupForAttribute(aa).getScore()
                > AudioAttributesMatchHelper.MATCH_ON_DEFAULT_SCORE;
    }
    ...
    public boolean supportsAudioAttributes(@NonNull AudioAttributes aa, int zoneId) {
        ...
        return score > AudioAttributesMatchHelper.MATCH_ON_DEFAULT_SCORE
     ...
     * @param zoneId to be considered
     * @return {@code AudioAttributesMatchHelper.NO_MATCH} if not supporting the given
    ...
    private ScoredAudioAttributesGroup getScoredAttributeGroupForAttribute(
        ...
        int bestScore = AudioAttributesMatchHelper.NO_MATCH;
     ...
     *         {@code AudioAttributesMatchHelper.NO_MATCH} and the {@link AudioAttributesGroup}
     *         is {@code null}.
     */
    private ScoredAudioAttributesGroup getScoredAttributeGroupForAttribute(
    ...
    public int getVolumeGroupIdForAudioAttributes(@NonNull AudioAttributes attributes) {
        ...
        AudioAttributesGroup aag = scoredAag.getAudioAttributesGroup();
        int score = scoredAag.getScore();
        return (aag != null && score != AudioAttributesMatchHelper.MATCH_ON_DEFAULT_SCORE)
    ...
    private static int attributesAndZonesMatchesScore(@NonNull AudioAttributes refAttr,
        ...
        if (zoneId != refZoneId && refZoneId != DEFAULT_ZONE_ID) {
            ...
            return AudioAttributesMatchHelper.NO_MATCH;
        }
        int score = AudioAttributesMatchHelper.MATCH_ON_DEFAULT_SCORE;
        if (refZoneId == zoneId) {
            score |= MATCH_ON_ZONE_ID_SCORE;
        }
        final int attributeMatchScore =
                AudioAttributesMatchHelper.attributesMatchesScore(refAttr, attr);
        if (attributeMatchScore == AudioAttributesMatchHelper.NO_MATCH) {
            return AudioAttributesMatchHelper.NO_MATCH;
        }
    ...
    private static int attributesMatchesScore(@NonNull AudioAttributes refAttr,
        ...
        return AudioAttributesMatchHelper.attributesMatchesScore(refAttr, attr);
    }
     ...
     *         {@link AudioAttributes},
        ...
        public int getAttributesMatchingScore(@NonNull AudioAttributes attributes) {
            int strategyScore = AudioAttributesMatchHelper.NO_MATCH;
            for (AudioAttributes refAa : mAudioAttributes) {
                int attributesGroupScore =
                        AudioAttributesMatchHelper.attributesMatchesScore(refAa, attributes);
    ...
    public static void dump(@NonNull PrintWriter pw) {
        ...
        if (strategies.isEmpty()) {
            pw.printf("%sEMPTY! (Empty product strategies)\n", INDENT);
        } else {
            strategies.forEach(aps -> {
                String apsStr = aps.toString(INDENT + INDENT);
                if (aps.isInternalStrategy()) {
                    String[] lines = apsStr.split("\n");
                    if (lines.length > 1) {
                        lines[1] = lines[1] + " (Internal)";
                        apsStr = String.join("\n", lines);
                    }
                }
                pw.printf("%s%s\n", INDENT, apsStr);
        ...
        List<AudioVolumeGroup> groups = AudioVolumeGroup.getAudioVolumeGroups();
        if (groups.isEmpty()) {
            pw.printf("%sEMPTY! (Empty volume groups)\n", INDENT);
        } else {
            groups.forEach(avg -> {
