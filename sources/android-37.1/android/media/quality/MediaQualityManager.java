...
public final class MediaQualityManager {
    ...
    /**
     * Gets the static Dolby Audio Processing capabilities of the device.
     *
     * <p>This includes information like the supported sound modes and whether features
     * like Atmos, Volume Leveler, and Surround Virtualizer are supported by the hardware.
     * This information is static and is not expected to change at runtime.
     *
     * @return A {@link DolbyAudioProcessingCapabilities} object describing the hardware's
     * Dolby Audio Processing capabilities, or {@code null} if the information is not available.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    @Nullable
    public DolbyAudioProcessingCapabilities getDolbyAudioProcessingCapabilities() {
        try {
            return mService.getDolbyAudioProcessingCapabilities(mContext.getUserId());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     *
     * <p>This returns the current sound mode and the enabled state of features like
     * Atmos, Volume Leveler, and Surround Virtualizer.
     *
     * @return A {@link DolbyAudioProcessingSettings} object containing the current settings for
     * Dolby Audio Processing, or {@code null} if the settings are not available.
     */
    @Hide
    @RequiresPermission(android.Manifest.permission.MANAGE_GLOBAL_SOUND_QUALITY_SERVICE)
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    @Nullable
    public DolbyAudioProcessingSettings getDolbyAudioProcessingSettings() {
        try {
            return mService.getDolbyAudioProcessingSettings(mContext.getUserId());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Sets the current Dolby Audio Processing settings.
     *
     * <p>This updates the sound mode and the enabled state of features like
     * Atmos, Volume Leveler, and Surround Virtualizer for the device.
     *
     * @param settings A {@link DolbyAudioProcessingSettings} object containing the desired
     * configurations, or {@code null} to use default settings.
     */
    @Hide
    @RequiresPermission(android.Manifest.permission.MANAGE_GLOBAL_SOUND_QUALITY_SERVICE)
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    public void setDolbyAudioProcessingSettings(@Nullable DolbyAudioProcessingSettings settings) {
        try {
            mService.setDolbyAudioProcessingSettings(settings, mContext.getUserId());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Gets the static DTS Virtual:X capabilities of the device.
     *
     * <p>This includes information about supported features like TbHdx, Limiter,
     * TruSurroundX, and others. This information is static and is not expected
     * to change at runtime.
     *
     * @return A {@link DtsVirtualXCapabilities} object describing the hardware's
     * DTS Virtual:X capabilities, or {@code null} if the information is not available.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    @Nullable
    public DtsVirtualXCapabilities getDtsVirtualXCapabilities() {
        try {
            return mService.getDtsVirtualXCapabilities(mContext.getUserId());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     *
     * <p>This returns the current enabled state of features like TbHdx, Limiter,
     * TruSurroundX, etc.
     *
     * @return A {@link DtsVirtualXSettings} object containing the current settings for
     * DTS Virtual:X, or {@code null} if the settings are not available.
     */
    @Hide
    @RequiresPermission(android.Manifest.permission.MANAGE_GLOBAL_SOUND_QUALITY_SERVICE)
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    @Nullable
    public DtsVirtualXSettings getDtsVirtualXSettings() {
        try {
            return mService.getDtsVirtualXSettings(mContext.getUserId());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Sets the current DTS Virtual:X settings.
     *
     * <p>This updates the enabled state of DTS Virtual:X features for the device.
     *
     * @param settings A {@link DtsVirtualXSettings} object containing the desired
     * configurations, or {@code null} to use default settings.
     */
    @Hide
    @RequiresPermission(android.Manifest.permission.MANAGE_GLOBAL_SOUND_QUALITY_SERVICE)
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    public void setDtsVirtualXSettings(@Nullable DtsVirtualXSettings settings) {
        try {
            mService.setDtsVirtualXSettings(settings, mContext.getUserId());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Updates a specific stream status variant for an existing picture profile.
     *
     * @param profileId the id of the profile to be updated.
     * @param streamStatus the stream status (e.g. {@link PictureProfile#STATUS_HDR10}).
     * @param params the new parameters for the variant.
     */
    @Hide
    public void updatePictureProfileVariant(
            @NonNull String profileId,
            @NonNull @MediaQualityContract.StreamStatusValue String streamStatus,
            @NonNull PersistableBundle params) {
        try {
            mService.updatePictureProfileVariant(
                    profileId, streamStatus, params, mUserHandle.getIdentifier());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Controls the hardware-level mute state of the entire display panel.
     *
     * When panel mute is enabled, the display output is completely driven to black
     * at the hardware/SoC level, including OSD and System UI.
     *
     * @param enabled True to force the panel to black, false to restore output.
     * @return true if the operation was successful, false otherwise.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    @RequiresPermission(android.Manifest.permission.MANAGE_GLOBAL_PICTURE_QUALITY_SERVICE)
    public boolean setPanelMute(boolean enabled) {
        try {
            return mService.setPanelMute(enabled, mUserHandle.getIdentifier());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Retrieves the current mute state of the display panel.
     *
     * @return True if the entire panel is currently blanked.
     */
    @Hide
    @FlaggedApi(Flags.FLAG_MEDIA_QUALITY_FW_D)
    @RequiresPermission(android.Manifest.permission.MANAGE_GLOBAL_PICTURE_QUALITY_SERVICE)
    public boolean isPanelMuted() {
        try {
            return mService.isPanelMuted(mUserHandle.getIdentifier());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
