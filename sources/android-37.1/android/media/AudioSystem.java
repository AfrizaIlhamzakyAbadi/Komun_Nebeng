...
public class AudioSystem
{
    ...
    public static long audioFormatToBluetoothA2dpSourceCodec(
        ...
        return switch (audioFormat) {
            ...
            case AUDIO_FORMAT_LDAC -> BluetoothCodecType.CODEC_ID_SONY_LDAC;
    ...
    /**
     * Command successful or Media server restarted. see ErrorCallback
     */
    @Hide
    ...
    @Hide
    public static final Set<Integer> DEVICE_IN_ALL_BLE_CENTRAL_BROADCAST_SET =
            Set.of(DEVICE_IN_BLE_CENTRAL_BROADCAST);
    ...
    @Hide
    @android.ravenwood.annotation.RavenwoodKeep
    public static boolean isBluetoothLeCentralBroadcastDevice(int deviceType) {
        return DEVICE_IN_ALL_BLE_CENTRAL_BROADCAST_SET.contains(deviceType);
    }
    ...
    // TODO(b/517633395): Add ASHA force configuration to stable HAL definitions
    // (AudioPolicyForceUse.aidl and audio_policy_engine_configuration.xsd)
    @Hide public static final int FORCE_BT_ASHA = 17;

    @Hide public static final int NUM_FORCE_CONFIG = 18;
    ...
    public static String forceUseConfigToString(int config) {
        switch (config) {
            ...
            case FORCE_BT_ASHA: return "FORCE_BT_ASHA";
   ...
   @Hide
    ...
    /**
     * enables or disables the master mono mode.
     */
    @Hide
    public static native int setMasterMono(boolean mono);

    /**
     * enables or disables the RTT mode.
     */
    @Hide
    public static native int setRttEnabled(boolean enabled);

    /**
     * returns master balance value in range -1.f -> 1.f, where 0.f is dead center.
     */
    @TestApi
    ...
    /**
     * Changes the audio balance of the device.
     */
    @TestApi
    @RequiresPermission(android.Manifest.permission.MODIFY_AUDIO_SETTINGS)
    ...
    @Hide
    public static native int listAudioPorts(ArrayList<AudioPort> ports, int[] generation, int type);
    ...
    public static int getPlatformType(Context context) {
        ...
        } else if (context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_PC)) {
            return PLATFORM_PC;
        } else {
