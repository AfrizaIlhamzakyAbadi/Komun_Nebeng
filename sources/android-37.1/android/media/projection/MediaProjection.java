...
import static com.android.media.projection.flags.Flags.FLAG_RECORDING_OVERLAY_STOP_AT_LAYER;
...
public final class MediaProjection {
  ...
  public VirtualDisplay createVirtualDisplay(
    ...
    /**
     * Sets the provided view's window token as the stop at layer for the recording overlay.
     *
     * <p>When set, MediaProjection will enable recording overlay mode. When in this mode, instead
     * of recording all windows on the virtual display, only the windows below the provided view's
     * window will be recorded. This will stop the recording of the view's window and any windows
     * above the provided view's window, such as system overlays.
     *
     * <p>The provided view must remain attached to the window for the duration of the
     * recording session. If the view is removed, the recording session will leave recording
     * overlay mode and fall back to recording the entire display hierarchy (all windows). If this
     * method is called before the view is attached to the window, calling this method will have
     * no effect on the MediaProjection session.
     *
     * <p>The view's window must be of type
     * {@link WindowManager.LayoutParams#TYPE_APPLICATION_OVERLAY} and owned by the same uid as the
     * MediaProjection session.
     *
     * <p>This method must be called before {@link #createVirtualDisplay(String, int, int, int, int,
     * Surface, VirtualDisplay.Callback, Handler)}. If called after a virtual display is created,
     * it will have no effect on the current recording session.
     *
     * @param view The view to set the recording overlay stop at layer to.
     */
    @Hide
    @SystemApi
    @RequiresPermission(android.Manifest.permission.REMOTE_DISPLAY_PROVIDER)
    @FlaggedApi(FLAG_RECORDING_OVERLAY_STOP_AT_LAYER)
    public void setRecordingOverlayStopAtLayer(@NonNull View view) {
        Objects.requireNonNull(view, "view must not be null");
        try {
            mImpl.setRecordingOverlayStopAtLayer(view.getWindowToken());
        } catch (RemoteException e) {
            Log.e(TAG, "Unable to set recording overlay stop at layer", e);
        }
    }
