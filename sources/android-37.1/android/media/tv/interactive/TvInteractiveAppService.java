...
public abstract class TvInteractiveAppService extends Service {
    ...
    public final IBinder onBind(@NonNull Intent intent) {
        ITvInteractiveAppService.Stub tvIAppServiceBinder = new ITvInteractiveAppService.Stub() {
            ...
            public void createSession(InputChannel channel, ITvInteractiveAppSessionCallback cb,
                    String iAppServiceId, int type, int subtype) {
                if (cb == null) {
                    return;
                }
            ...
            @Override
            public void searchOperatorApp(OperatorAppServiceInfo opAppInfo, int method,
                    Bundle extra) {
                onSearchOperatorApp(opAppInfo, method, extra);
            }

            @Override
            public void cancelOngoingOperatorAppPackageOperation(OperatorAppServiceInfo opAppInfo) {
                onCancelOngoingOperatorAppPackageOperation(opAppInfo);
            }

            @Override
            public void updateOperatorApp(OperatorAppServiceInfo opAppInfo, int method,
                    Bundle extra) {
                onUpdateOperatorApp(opAppInfo, method, extra);
            }

            @Override
            public void installOperatorApp(OperatorAppServiceInfo opAppInfo) {
                onInstallOperatorApp(opAppInfo);
            }

            @Override
            public void uninstallOperatorApp(OperatorAppServiceInfo opAppInfo, int method,
                    String launchLocation) {
                onUninstallOperatorApp(opAppInfo, method, launchLocation);
            }

            @Override
            public void requestOperatorAppPackageStatus(OperatorAppServiceInfo opAppInfo) {
                onRequestOperatorAppPackageStatus(opAppInfo);
            }
        };
    ...
    /**
     * Notifies the system that the Operator App package status has changed.
     *
     ...
     */
    @Hide
    public final void notifyOperatorAppPackageStatus(@NonNull OperatorAppServiceInfo opAppInfo,
            @OperatorAppServiceInfo.OperatorAppPackageStatus int status, @Nullable Bundle extra) {
        SomeArgs args = SomeArgs.obtain();
        args.arg1 = opAppInfo;
        args.arg2 = status;
        args.arg3 = extra;
        mServiceHandler
                .obtainMessage(ServiceHandler.DO_NOTIFY_OPERATOR_APP_PACKAGE_STATUS, args)
                .sendToTarget();
    }

    /**
     * Called when Operator App search is requested.
     *
     * @param opAppInfo The information of the Operator App service.
     ...
     */
    @Hide
    public void onSearchOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo,
            @OperatorAppServiceInfo.OperatorAppSearchMethod int method,
            @Nullable Bundle extra) {
    }

    /**
     * Called when an ongoing Operator App package operation cancellation is
     * requested.
     ...
     */
    @Hide
    public void onCancelOngoingOperatorAppPackageOperation(
            @NonNull OperatorAppServiceInfo opAppInfo) {
    }

    /**
     * Called when Operator App update is requested.
     *
     * @param opAppInfo The information of the Operator App service.
     * @param method    The search method.
     * @param extra     The extra information.
     */
    @Hide
    public void onUpdateOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo,
            @OperatorAppServiceInfo.OperatorAppSearchMethod int method,
            @Nullable Bundle extra) {
    }

    /**
     * Called when Operator App installation is requested.
     *
     * @param opAppInfo The Operator App information to install.
     */
    @Hide
    public void onInstallOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo) {
    }

    /**
     * Called when Operator App uninstallation is requested.
     *
     * @param opAppInfo      The information of the Operator App service.
     ...
     */
    @Hide
    public void onUninstallOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo,
            @OperatorAppServiceInfo.OperatorAppUninstallMethod int method,
            @NonNull String launchLocation) {
    }

    /**
     * Called when Operator App package status is requested.
     *
     * @param opAppInfo The information of the Operator App service.
     */
    @Hide
    public void onRequestOperatorAppPackageStatus(@NonNull OperatorAppServiceInfo opAppInfo) {
    }
    ...
    /**
     * Returns a concrete implementation of {@link Session} with a designated subtype.
     *
     * <p>May return {@code null} if this TV Interactive App service fails to create a session for
     * some reason.
     *
     * @param iAppServiceId The ID of the TV Interactive App associated with the session.
     * @param type The type of the TV Interactive App associated with the session.
     * @param subtype The subtype of the TV Interactive App associated with the session.
     */
    @Hide
    @Nullable
    public Session onCreateSession(
            @NonNull String iAppServiceId,
            @TvInteractiveAppServiceInfo.InteractiveAppType int type,
            @TvInteractiveAppServiceInfo.InteractiveAppSubtype int subtype) {
        return onCreateSession(iAppServiceId, type);
    }
    ...
    public abstract static class Session implements KeyEvent.Callback {
        ...
        /**
         * Returns the hash code of the window token for the media view.
         * This can be used to check if 2 sessions' {@code TvInteractiveAppView}s are attached
         * to the same window.
         * @see android.view.WindowManager.LayoutParams.token
         *
         * @return The hash code of the window token. {@code null} if the window token is null.
         */
        @Hide
        @Nullable
        public Integer getMediaViewWindowTokenHashCode() {
            return mWindowToken == null ? null : mWindowToken.hashCode();
        }
        ...
        /**
         * Callback when stops an interactive app using its handle id.
         * @param handle the handle id of the interactive app.
         */
        @Hide
        public void onStopInteractiveApp(int handle) {
        }
        ...
        /**
         * Called when an interactive app streaming track is selected.
         *
         * @param type The type of the selected streaming track.
         * @param trackId The ID of the selected streaming track.
         */
        @Hide
        public void onSelectInteractiveAppStreamingTrack(int type, @Nullable String trackId) {
        }
        ...
        /**
         * Called when the window token of this session is cleared, which will happen when the
         * {@link TvInteractiveAppView} related to the session become not visible or detached from
         * window, or the session is released.
         */
        @Hide
        public void onWindowTokenCleared() {
        }
        ...
        /**
         * Called when interactive app focus is changed.
         *
         * @param focusLost Whether or not the focus is lost.
         */
        @Hide
        public void onInteractiveAppFocusChanged(boolean focusLost) {
        }
        ...
        /**
         * Called when the TV App sends the parental control approval result.
         *
         * @param requestId The ID to identify the request.
         * @param granted {@code true} if the parental control approval is granted.
         */
        @Hide
        public void onParentalControlApprovalResult(int requestId, boolean granted) {
        }

        /**
         * Called when the TV App sends the parental control PIN verification result.
         *
         * @param requestId The ID to identify the request.
         * @param result the PIN verification result.
         */
        @Hide
        public void onVerifyParentalControlPinResult(int requestId,
                @TvInteractiveAppManager.PinVerificationResult int result) {
        }

        /**
         * Called when the TV App sends the parental control PIN length.
         *
         * @param length the PIN length.
         */
        @Hide
        public void onParentalControlPinLength(int length) {
        }

        /**
         * Called when the TV App sends the parental control PIN set result.
         *
         * @param result the PIN verification result.
         */
        @Hide
        public void onSetParentalControlPinResult(
                @TvInteractiveAppManager.PinVerificationResult int result) {
        }
        ...
        /**
         * Called when Operator App start is requested.
         *
         * @param opAppInfo       The information of the Operator App service.
         ...
         */
        @Hide
        public void onStartOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo,
                @NonNull String startupLocation, @NonNull String launchLocation,
                @Nullable Bundle extra) {
        }

        /**
         * Called when Operator App stop is requested.
         *
         * @param opAppInfo The information of the Operator App service.
         */
        @Hide
        public void onStopOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo) {
        }

        /**
         * Called when Operator App state switch is requested.
         *
         * @param opAppInfo The information of the Operator App service.
         * @param state     The app state.
         */
        @Hide
        public void onSwitchOperatorAppState(@NonNull OperatorAppServiceInfo opAppInfo,
                @OperatorAppServiceInfo.OperatorAppState int state) {
        }

        /**
         * Called when Operator App supported status is requested.
         */
        @Hide
        public void onRequestOperatorAppSupportedStatus() {
        }
        ...
        /**
         * Requests parental control approval.
         *
         * @param requestId The ID to identify the request.
         * @param challengeUser {@code true} if the user should be challenged for
         *                      parental control approval.
         * @param keyValues The key-value pairs of the parental control approval.
         */
        @Hide
        @CallSuper
        public void requestParentalControlApproval(
                int requestId, boolean challengeUser, @NonNull Bundle keyValues) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "requestParentalControlApproval (requestId="
                                    + requestId + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onRequestParentalControlApproval(
                                    requestId, challengeUser, keyValues);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in requestParentalControlApproval", e);
                    }
                }
            });
        }
        ...
        /**
         * Cancels parental control approval.
         */
        @Hide
        @CallSuper
        public void cancelParentalControlApproval() {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "cancelParentalControlApproval");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onCancelParentalControlApproval();
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in cancelParentalControlApproval", e);
                    }
                }
            });
        }
        ...
        /**
         * Requests to verify parental control PIN.
         *
         * @param requestId The ID to identify the request.
        ...
        @Hide
        @CallSuper
        public void requestVerifyParentalControlPin(int requestId, @NonNull String pcPin) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "requestVerifyParentalControlPin (requestId="
                                    + requestId + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onRequestVerifyParentalControlPin(requestId, pcPin);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in requestVerifyParentalControlPin", e);
                    }
                }
            });
        }
        ...
        /**
         * Requests parental control PIN length.
         */
        @Hide
        @CallSuper
        public void requestParentalControlPinLength() {
            executeOrPostRunnableOnMainThread(new Runnable() {
                ...
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "requestParentalControlPinLength");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onRequestParentalControlPinLength();
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in requestParentalControlPinLength", e);
                    }
                }
            });
        }
        ...
        @Hide
        @CallSuper
        public void setParentalControlPin(@NonNull String oldPcPin, @NonNull String newPcPin) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "setParentalControlPin");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onSetParentalControlPin(oldPcPin, newPcPin);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in setParentalControlPin", e);
                    }
                }
            });
        }
        ...
        void requestOperatorAppSupportedStatus() {
            onRequestOperatorAppSupportedStatus();
        }

        void selectInteractiveAppStreamingTrack(int type, String trackId) {
            onSelectInteractiveAppStreamingTrack(type, trackId);
        }

        void startOperatorApp(OperatorAppServiceInfo opAppInfo, String startupLocation,
                String launchLocation, Bundle extra) {
            onStartOperatorApp(opAppInfo, startupLocation, launchLocation, extra);
        }

        void stopOperatorApp(OperatorAppServiceInfo opAppInfo) {
            onStopOperatorApp(opAppInfo);
        }

        void switchOperatorAppState(OperatorAppServiceInfo opAppInfo, int state) {
            onSwitchOperatorAppState(opAppInfo, state);
        }
        ...
        void notifyInteractiveAppFocusChanged(boolean focusLost) {
            if (DEBUG) {
                Log.d(TAG, "notifyInteractiveAppFocusChanged (focusLost= " + focusLost + ")");
            }
            onInteractiveAppFocusChanged(focusLost);
        }
        ...
        void sendParentalControlApprovalResult(int requestId, boolean granted) {
            if (DEBUG) {
                Log.d(TAG, "sendParentalControlApprovalResult (requestId= " + requestId
                        + ", granted= " + granted + ")");
            }
            onParentalControlApprovalResult(requestId, granted);
        }

        void sendVerifyParentalControlPinResult(int requestId, int result) {
            if (DEBUG) {
                Log.d(TAG, "sendVerifyParentalControlPinResult (requestId= " + requestId
                        + ", result= " + result + ")");
            }
            onVerifyParentalControlPinResult(requestId, result);
        }

        void sendParentalControlPinLength(int length) {
            if (DEBUG) {
                Log.d(TAG, "sendParentalControlPinLength (length= " + length + ")");
            }
            onParentalControlPinLength(length);
        }

        void sendSetParentalControlPinResult(int result) {
            if (DEBUG) {
                Log.d(TAG, "sendSetParentalControlPinResult (result= " + result + ")");
            }
            onSetParentalControlPinResult(result);
        }
        ...
        /**
         * Notifies when interactive app streaming tracks are changed.
         *
         * @param tracks the current streaming tracks.
         */
        @Hide
        @CallSuper
        public void notifyInteractiveAppStreamingTracksChanged(@NonNull List<TvTrackInfo> tracks) {
            ...
            executeOrPostRunnableOnMainThread(new Runnable() {
                ...
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyInteractiveAppStreamingTracksChanged (tracks="
                                    + tracksCopy + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onInteractiveAppStreamingTracksChanged(tracksCopy);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyInteractiveAppStreamingTracksChanged", e);
                    }
                }
            });
        }
        ...
        /**
         * Notifies when the interactive app streaming tracks are selected.
         *
         * @param type The type of the selected streaming track.
         * @param trackId The ID of the selected streaming track.
         */
        @Hide
        @CallSuper
        public void notifyInteractiveAppStreamingTrackSelected(int type, @Nullable String trackId) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyInteractiveAppStreamingTrackSelected (type=" + type
                                    + ", trackId=" + trackId + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onInteractiveAppStreamingTrackSelected(type, trackId);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyInteractiveAppStreamingTrackSelected", e);
                    }
                }
            });
        }
        ...
        /**
         * Sends the streaming playback status.
         *
         * @param status the current streaming playback status.
         */
        @Hide
        @CallSuper
        public void sendInteractiveAppStreamingPlaybackStatus(
                @TvInteractiveAppManager.StreamingPlaybackStatus int status) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "sendInteractiveAppStreamingPlaybackStatus (status="
                                    + status + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onStreamingPlaybackStatusChanged(status);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in sendInteractiveAppStreamingPlaybackStatus", e);
                    }
                }
            });
        }
        ...
        /**
         * Notifies when key set is changed.
         *
         * @param value     The key set value.
         * @param otherKeys The other keys.
         */
        @Hide
        public void notifyKeySetChanged(int value, @NonNull int[] otherKeys) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyKeySetChanged (value=" + value + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onKeySetChanged(value, otherKeys);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyKeySetChanged", e);
                    }
                }
            });
        }
        ...
        /**
         * Notifies when UI elements are replaced.
         *
         * @param elements The UI elements to be replaced.
         */
        @Hide
        public void notifyReplaceUiElements(@NonNull int[] elements) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyReplaceUiElements");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onReplaceUiElements(elements);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyReplaceUiElements", e);
                    }
                }
            });
        }
        ...
        /**
         * Notifies when Operator App supported status is changed.
         *
         * @param supported {@code true} if Operator App is supported.
         */
        @Hide
        public void notifyOperatorAppSupportedStatus(boolean supported) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyOperatorAppSupportedStatus (supported="
                                    + supported + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onOperatorAppSupportedStatus(supported);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyOperatorAppSupportedStatus", e);
                    }
                }
            });
        }
        ...
        /**
         * Notifies when Operator App update request is received.
         *
         * @param opAppInfo The information of the Operator App service.
         * @param immediate {@code true} if the update is immediate.
         */
        @Hide
        public void notifyOperatorAppUpdateRequest(@NonNull OperatorAppServiceInfo opAppInfo,
                boolean immediate) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyOperatorAppUpdateRequest (opAppInfo=" + opAppInfo
                                    + ", immediate=" + immediate + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onOperatorAppUpdateRequest(opAppInfo, immediate);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyOperatorAppUpdateRequest", e);
                    }
                }
            });
        }
        ...
        /**
         * Notifies when Operator App status is changed.
         *
         * @param opAppInfo The information of the Operator App service.
         ...
         */
        @Hide
        public void notifyOperatorAppStatusChanged(@NonNull OperatorAppServiceInfo opAppInfo,
                @OperatorAppServiceInfo.OperatorAppStatus int status,
                @Nullable Bundle extra) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyOperatorAppStatusChanged (status=" + status + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onOperatorAppStatusChanged(opAppInfo, status, extra);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyOperatorAppStatusChanged", e);
                    }
                }
            });
        }
        ...
        /**
         * Notifies when Operator App state is changed.
         *
         * @param opAppInfo The information of the Operator App service.
         * @param state     The app state.
         */
        @Hide
        public void notifyOperatorAppStateChanged(@NonNull OperatorAppServiceInfo opAppInfo,
                @OperatorAppServiceInfo.OperatorAppState int state) {
            executeOrPostRunnableOnMainThread(new Runnable() {
                @MainThread
                @Override
                public void run() {
                    try {
                        if (DEBUG) {
                            Log.d(TAG, "notifyOperatorAppStateChanged (state=" + state + ")");
                        }
                        if (mSessionCallback != null) {
                            mSessionCallback.onOperatorAppStateChanged(opAppInfo, state);
                        }
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyOperatorAppStateChanged", e);
                    }
                }
            });
        }
    ...
    private final class ServiceHandler extends Handler {
        ...
        private void broadcastOperatorAppPackageStatus(OperatorAppServiceInfo opAppInfo,
                int status, Bundle extra) {
            int n = mCallbacks.beginBroadcast();
            try {
                for (int i = 0; i < n; i++) {
                    try {
                        mCallbacks.getBroadcastItem(i).onOperatorAppPackageStatus(opAppInfo, status,
                                extra);
                    } catch (RemoteException e) {
                        Log.w(TAG, "error in notifyOperatorAppPackageStatus", e);
                    }
                }
            } finally {
                mCallbacks.finishBroadcast();
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case DO_CREATE_SESSION: {
                    ...
                    int subtype = (int) args.arg5;
                    args.recycle();
                    Session sessionImpl = null;
                    if (subtype == TvInteractiveAppServiceInfo.INTERACTIVE_APP_SUBTYPE_NONE) {
                        sessionImpl = onCreateSession(iAppServiceId, type);
                    } else {
                        sessionImpl = onCreateSession(iAppServiceId, type, subtype);
                    }
                    if (sessionImpl == null) {
                        try {
                ...
                case DO_NOTIFY_OPERATOR_APP_PACKAGE_STATUS: {
                    SomeArgs args = (SomeArgs) msg.obj;
                    OperatorAppServiceInfo opAppInfo = (OperatorAppServiceInfo) args.arg1;
                    int status = (int) args.arg2;
                    Bundle extra = (Bundle) args.arg3;
                    broadcastOperatorAppPackageStatus(opAppInfo, status, extra);
                    return;
                }
                default: {
