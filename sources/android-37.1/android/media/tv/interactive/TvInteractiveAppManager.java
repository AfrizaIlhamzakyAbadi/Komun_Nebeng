...
public final class TvInteractiveAppManager {
    ...
    @IntDef(flag = false, prefix = "ERROR_", value = {
            ...
            ERROR_INTERACTIVE_APP_NOT_FOUND,
            ERROR_FORBIDDEN_BY_TERMINAL,
            ERROR_FORBIDDEN_BY_USER,
    ...
    @Hide
    @Retention(RetentionPolicy.SOURCE)
    @IntDef(flag = false, prefix = "PIN_VERIFICATION_RESULT_", value = {
            PIN_VERIFICATION_RESULT_CORRECT,
            PIN_VERIFICATION_RESULT_INCORRECT,
            PIN_VERIFICATION_RESULT_LOCKED,
    })
    ...
    /**
     * PIN entry is locked because an invalid PIN has been entered too many times.
     */
    @Hide
    ...
    /**
     * Error code when the interactive app is not found.
     */
    @Hide
    public static final int ERROR_INTERACTIVE_APP_NOT_FOUND = 8;

    /**
     * Error code when the operation is rejected by the terminal.
     */
    @Hide
    public static final int ERROR_FORBIDDEN_BY_TERMINAL = 9;

    /**
     * Error code when operation is forbidden by a user action or setting.
     */
    @Hide
    public static final int ERROR_FORBIDDEN_BY_USER = 10;

    /**
     * Error code when an internal server error occurs.
     */
    @Hide
    public static final int ERROR_INTERNAL_SERVER_ERROR = 11;

    /**
     * Error code when service is unavailable.
     */
    @Hide
    ...
    @Hide
    @Retention(RetentionPolicy.SOURCE)
    @IntDef(flag = false, prefix = "STREAMING_PLAYBACK_STATUS_", value = {
            STREAMING_PLAYBACK_STATUS_PLAYING,
            STREAMING_PLAYBACK_STATUS_PAUSED,
            STREAMING_PLAYBACK_STATUS_STOPPED,
            STREAMING_PLAYBACK_STATUS_ERROR,
            STREAMING_PLAYBACK_STATUS_READY})
    public @interface StreamingPlaybackStatus {}

    /**
     * Streaming playback status: playing
     */
    @Hide
    public static final int STREAMING_PLAYBACK_STATUS_PLAYING = 1;

    /**
     * Streaming playback status: paused
     */
    @Hide
    public static final int STREAMING_PLAYBACK_STATUS_PAUSED = 2;

    /**
     * Streaming playback status: stopped
     */
    @Hide
    public static final int STREAMING_PLAYBACK_STATUS_STOPPED = 3;

    /**
     * Streaming playback status: error
     */
    @Hide
    public static final int STREAMING_PLAYBACK_STATUS_ERROR = 4;

    /**
     * Streaming playback status: ready
     */
    @Hide
    ...
    public TvInteractiveAppManager(ITvInteractiveAppManager service, int userId) {
        ...
        mClient = new ITvInteractiveAppClient.Stub() {
            ...
            @Override
            public void onRequestParentalControlApproval(
                    int requestId, boolean challengeUser, Bundle keyValues, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postRequestParentalControlApproval(requestId, challengeUser, keyValues);
                }
            }

            @Override
            public void onCancelParentalControlApproval(int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postCancelParentalControlApproval();
                }
            }

            @Override
            public void onRequestVerifyParentalControlPin(int requestId, String pcPin, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postVerifyParentalControlPinRequest(requestId, pcPin);
                }
            }

            @Override
            public void onRequestParentalControlPinLength(int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postParentalControlPinLengthRequest();
                }
            }

            @Override
            public void onSetParentalControlPin(String oldPcPin, String newPcPin, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postSetParentalControlPin(oldPcPin, newPcPin);
                }
            }
            ...
            @Override
            public void onInteractiveAppStreamingTracksChanged(List<TvTrackInfo> tracks, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    if (record.mSession.updateInteractiveAppStreamingTracks(tracks)) {
                        record.postInteractiveAppStreamingTracksChanged(tracks);
                    }
                }
            }

            @Override
            public void onKeySetChanged(int value, int[] otherKeys, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postKeySetChanged(value, otherKeys);
                }
            }

            @Override
            public void onReplaceUiElements(int[] elements, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postReplaceUiElements(elements);
                }
            }

            @Override
            public void onOperatorAppSupportedStatus(boolean supported, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postOperatorAppSupportedStatus(supported);
                }
            }

            @Override
            public void onOperatorAppUpdateRequest(OperatorAppServiceInfo opAppInfo,
                    boolean immediate, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postOperatorAppUpdateRequest(opAppInfo, immediate);
                }
            }

            @Override
            public void onOperatorAppStatusChanged(OperatorAppServiceInfo opAppInfo, int status,
                    Bundle extra, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postOperatorAppStatusChanged(opAppInfo, status, extra);
                }
            }

            @Override
            public void onOperatorAppStateChanged(OperatorAppServiceInfo opAppInfo, int state,
                    int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                    ...
                    record.postOperatorAppStateChanged(opAppInfo, state);
                }
            }

            @Override
            public void onInteractiveAppStreamingTrackSelected(int type, String trackId, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    if (record.mSession.updateInteractiveAppStreamingTrackSelection(
                            type, trackId)) {
                        record.postInteractiveAppStreamingTrackSelected(type, trackId);
                    }
                }
            }

            @Override
            public void onStreamingPlaybackStatusChanged(int status, int seq) {
                synchronized (mSessionCallbackRecordMap) {
                    SessionCallbackRecord record = mSessionCallbackRecordMap.get(seq);
                    if (record == null) {
                        Log.e(TAG, "Callback not found for seq " + seq);
                    ...
                    record.postStreamingPlaybackStatusChanged(status);
                }
            }
        };
        ITvInteractiveAppManagerCallback managerCallback =
            ...
            @Override
            public void onOperatorAppPackageStatus(String iAppServiceId,
                    OperatorAppServiceInfo opAppInfo, int status, Bundle extra) {
                synchronized (mLock) {
                    for (TvInteractiveAppCallbackRecord record : mCallbackRecords) {
                        record.postOperatorAppPackageStatus(iAppServiceId, opAppInfo,
                                status, extra);
                    }
                }
            }
    ...
    /**
     * Callback used to monitor status of the Operator App.
     */
    @Hide
    public abstract static class OperatorAppCallback extends TvInteractiveAppCallback {

        /**
         * Called when Operator App package status is changed.
         *
         * @param iAppServiceId The ID of the TV Interactive App service.
         * @param opAppInfo     The information of the Operator App service.
         * @param status        The package status.
         * @param extra         The extra information.
         */
        @Hide
        public void onOperatorAppPackageStatus(@NonNull String iAppServiceId,
                @NonNull OperatorAppServiceInfo opAppInfo,
                @OperatorAppServiceInfo.OperatorAppPackageStatus int status,
    ...
    private static final class TvInteractiveAppCallbackRecord {
        ...
        public void postOperatorAppPackageStatus(String iAppServiceId,
                OperatorAppServiceInfo opAppInfo, int status, Bundle extra) {
            mExecutor.execute(new Runnable() {
                @Override
                public void run() {
                    if (mCallback instanceof OperatorAppCallback) {
                        ((OperatorAppCallback) mCallback).onOperatorAppPackageStatus(
                                iAppServiceId, opAppInfo, status, extra);
                    } else {
                        Log.e(TAG, "mCallback is not instance of OperatorAppCallback");
                    }
                }
            });
        }
    ...
    /**
     * Creates a {@link Session} for a given TV interactive application with a designated subtype.
     *
     ...
     * @param subtype the subtype of the interactive application.
     * @param callback A callback used to receive the created session.
     * @param handler A {@link Handler} that the session creation will be delivered to.
     */
    @Hide
    public void createSession(@NonNull String iAppServiceId, int type, int subtype,
            @NonNull final SessionCallback callback, @NonNull Handler handler) {
        createSessionInternal(iAppServiceId, type, subtype, callback, handler);
    }

    private void createSessionInternal(String iAppServiceId, int type, int subtype,
        ...
        synchronized (mSessionCallbackRecordMap) {
            ...
            try {
                mService.createSession(mClient, iAppServiceId, type, subtype, seq, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
     ...
     */
    @Hide
    public void searchOperatorApp(@NonNull String iAppServiceId,
            @NonNull OperatorAppServiceInfo opAppInfo,
            @OperatorAppServiceInfo.OperatorAppSearchMethod int method,
            @Nullable Bundle extra) {
        try {
            mService.searchOperatorApp(iAppServiceId, opAppInfo, method, extra, mUserId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     */
    @Hide
    public void installOperatorApp(@NonNull String iAppServiceId,
            @NonNull OperatorAppServiceInfo opAppInfo) {
        try {
            mService.installOperatorApp(iAppServiceId, opAppInfo, mUserId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Uninstalls Operator App.
     *
     * @param iAppServiceId  The ID of the TV interactive app service.
     * @param opAppInfo      The information of the Operator App service.
     * @param method         The uninstall method.
     * @param launchLocation The launch location.
     */
    @Hide
    public void uninstallOperatorApp(@NonNull String iAppServiceId,
            @NonNull OperatorAppServiceInfo opAppInfo,
            @OperatorAppServiceInfo.OperatorAppUninstallMethod int method,
            @NonNull String launchLocation) {
        try {
            mService.uninstallOperatorApp(iAppServiceId, opAppInfo, method, launchLocation,
                    mUserId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    ...
    /**
     * Cancels an ongoing Operator App package operation.
     * supported operations: search/install/update
     *
     * @param iAppServiceId The ID of the TV interactive app service.
     * @param opAppInfo     The information of the Operator App service.
     */
    @Hide
    public void cancelOngoingOperatorAppPackageOperation(@NonNull String iAppServiceId,
            @NonNull OperatorAppServiceInfo opAppInfo) {
        try {
            mService.cancelOngoingOperatorAppPackageOperation(iAppServiceId, opAppInfo, mUserId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     */
    @Hide
    public void updateOperatorApp(@NonNull String iAppServiceId,
            @NonNull OperatorAppServiceInfo opAppInfo,
            @OperatorAppServiceInfo.OperatorAppSearchMethod int method,
            @Nullable Bundle extra) {
        try {
            mService.updateOperatorApp(iAppServiceId, opAppInfo, method, extra, mUserId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    /**
     * Requests Operator App package status.
     *
     * @param iAppServiceId The ID of the TV interactive app service.
     * @param opAppInfo     The information of the Operator App service.
     */
    @Hide
    public void requestOperatorAppPackageStatus(@NonNull String iAppServiceId,
            @NonNull OperatorAppServiceInfo opAppInfo) {
        try {
            mService.requestOperatorAppPackageStatus(iAppServiceId, opAppInfo, mUserId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public static final class Session {
        ...
        private final List<TvTrackInfo> mInteractiveAppStreamingAudioTracks = new ArrayList<>();
        // @GuardedBy("mMetadataLock")
        private final List<TvTrackInfo> mInteractiveAppStreamingVideoTracks = new ArrayList<>();
        // @GuardedBy("mMetadataLock")
        private final List<TvTrackInfo> mInteractiveAppStreamingSubtitleTracks = new ArrayList<>();
        // @GuardedBy("mMetadataLock")
        private String mSelectedInteractiveAppStreamingAudioTrackId = null;
        // @GuardedBy("mMetadataLock")
        private String mSelectedInteractiveAppStreamingVideoTrackId = null;
        // @GuardedBy("mMetadataLock")
        private String mSelectedInteractiveAppStreamingSubtitleTrackId = null;
        ...
        /**
         * Returns the list of interactive app streaming tracks for a given type. Returns
        ...
        @Hide
        @NonNull
        public List<TvTrackInfo> getInteractiveAppStreamingTracks(int type) {
            synchronized (mMetadataLock) {
                if (type == TvTrackInfo.TYPE_AUDIO) {
                    return new ArrayList<>(mInteractiveAppStreamingAudioTracks);
                } else if (type == TvTrackInfo.TYPE_VIDEO) {
                    return new ArrayList<>(mInteractiveAppStreamingVideoTracks);
                } else if (type == TvTrackInfo.TYPE_SUBTITLE) {
                    return new ArrayList<>(mInteractiveAppStreamingSubtitleTracks);
                }
            }
        ...
        /**
         * Returns the selected interactive app streaming track for a given type. Returns
         * {@code null} if the information is not available or any of the tracks for the given type
         * is not selected.
         *
        ...
        @Hide
        @Nullable
        public String getSelectedInteractiveAppStreamingTrack(int type) {
            synchronized (mMetadataLock) {
                if (type == TvTrackInfo.TYPE_AUDIO) {
                    return mSelectedInteractiveAppStreamingAudioTrackId;
                } else if (type == TvTrackInfo.TYPE_VIDEO) {
                    return mSelectedInteractiveAppStreamingVideoTrackId;
                } else if (type == TvTrackInfo.TYPE_SUBTITLE) {
                    return mSelectedInteractiveAppStreamingSubtitleTrackId;
                }
            }
            throw new IllegalArgumentException("invalid type: " + type);
        }
        ...
        /**
         * Selects an interactive app streaming track.
         *
         ...
         * @see #getInteractiveAppStreamingTracks(int)
         */
        public void selectInteractiveAppStreamingTrack(int type, @Nullable String trackId) {
            if (mToken == null) {
            ...
            synchronized (mMetadataLock) {
                if (type == TvTrackInfo.TYPE_AUDIO) {
                    if (trackId != null && !containsTrack(mInteractiveAppStreamingAudioTracks,
                            trackId)) {
                        Log.w(TAG, "Invalid audio trackId: " + trackId);
                        return;
                    }
                } else if (type == TvTrackInfo.TYPE_VIDEO) {
                    if (trackId != null && !containsTrack(mInteractiveAppStreamingVideoTracks,
                            trackId)) {
                        Log.w(TAG, "Invalid video trackId: " + trackId);
                        return;
                    }
                } else if (type == TvTrackInfo.TYPE_SUBTITLE) {
                    if (trackId != null && !containsTrack(mInteractiveAppStreamingSubtitleTracks,
                            trackId)) {
            ...
            try {
                mService.selectInteractiveAppStreamingTrack(mToken, type, trackId, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
        ...
        /**
         * Responds to onInteractiveAppStreamingTracksChanged() and updates the internal track
         * information.
         * @return true if the list of tracks after update is not empty.
         */
        boolean updateInteractiveAppStreamingTracks(List<TvTrackInfo> tracks) {
            synchronized (mMetadataLock) {
                mInteractiveAppStreamingAudioTracks.clear();
                mInteractiveAppStreamingVideoTracks.clear();
                mInteractiveAppStreamingSubtitleTracks.clear();
                if (tracks != null) {
                    for (TvTrackInfo track : tracks) {
                        if (track.getType() == TvTrackInfo.TYPE_AUDIO) {
                            mInteractiveAppStreamingAudioTracks.add(track);
                        } else if (track.getType() == TvTrackInfo.TYPE_VIDEO) {
                            mInteractiveAppStreamingVideoTracks.add(track);
                        } else if (track.getType() == TvTrackInfo.TYPE_SUBTITLE) {
                            mInteractiveAppStreamingSubtitleTracks.add(track);
                        }
                    }
                }
                return !mInteractiveAppStreamingAudioTracks.isEmpty()
                        || !mInteractiveAppStreamingVideoTracks.isEmpty()
                        || !mInteractiveAppStreamingSubtitleTracks.isEmpty();
            }
        }

        /**
         * Responds to onInteractiveAppStreamingTrackSelected() and updates the internal track
         * selection information.
         * @return true if there is an update.
         */
        boolean updateInteractiveAppStreamingTrackSelection(int type, String trackId) {
            synchronized (mMetadataLock) {
                if (type == TvTrackInfo.TYPE_AUDIO
                        && !TextUtils.equals(trackId,
                                mSelectedInteractiveAppStreamingAudioTrackId)) {
                    mSelectedInteractiveAppStreamingAudioTrackId = trackId;
                    return true;
                } else if (type == TvTrackInfo.TYPE_VIDEO
                        && !TextUtils.equals(trackId,
                                mSelectedInteractiveAppStreamingVideoTrackId)) {
                    mSelectedInteractiveAppStreamingVideoTrackId = trackId;
                    return true;
                } else if (type == TvTrackInfo.TYPE_SUBTITLE
                        && !TextUtils.equals(trackId,
                                mSelectedInteractiveAppStreamingSubtitleTrackId)) {
                    mSelectedInteractiveAppStreamingSubtitleTrackId = trackId;
                    return true;
                }
            }
            return false;
        ...
        void stopInteractiveApp(int handle) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.stopInteractiveAppWithHandle(mToken, mUserId, handle);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
        ...
        void sendParentalControlApprovalResult(int requestId, boolean granted) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.sendParentalControlApprovalResult(mToken, requestId, granted, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }

        void sendVerifyParentalControlPinResult(int requestId,
                @PinVerificationResult int result) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.sendVerifyParentalControlPinResult(mToken, requestId, result, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }

        void sendParentalControlPinLength(int length) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.sendParentalControlPinLength(mToken, length, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }

        void sendSetParentalControlPinResult(@PinVerificationResult int result) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.sendSetParentalControlPinResult(mToken, result, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
        ...
        private void clearMetadataLocked() {
            ...
            mInteractiveAppStreamingVideoTracks.clear();
            mInteractiveAppStreamingSubtitleTracks.clear();
            mSelectedInteractiveAppStreamingAudioTrackId = null;
            mSelectedInteractiveAppStreamingVideoTrackId = null;
            mSelectedInteractiveAppStreamingSubtitleTrackId = null;
        }
        ...
        /**
         * Notifies Interactive APP session when focus is changed.
         */
        @Hide
        public void notifyInteractiveAppFocusChanged(boolean focusLost) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.notifyInteractiveAppFocusChanged(mToken, focusLost, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
        ...
        /**
         * Requests the supported status of Operator App.
         */
        @Hide
        public void requestOperatorAppSupportedStatus() {
            ...
            try {
                mService.requestOperatorAppSupportedStatus(mToken, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
         ...
         *
         * @param opAppInfo       The information of the Operator App service.
         * @param startupLocation The startup location.
         * @param launchLocation  The launch location.
         * @param extra           The extra information.
         */
        public void startOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo,
                @NonNull String startupLocation, @NonNull String launchLocation,
                @Nullable Bundle extra) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            ...
            try {
                mService.startOperatorApp(mToken, opAppInfo, startupLocation, launchLocation, extra,
                        mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
         ...
         * @param opAppInfo The information of the Operator App service.
         */
        public void stopOperatorApp(@NonNull OperatorAppServiceInfo opAppInfo) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.stopOperatorApp(mToken, opAppInfo, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
        ...
        /**
         * Switches Operator App state.
         *
         * @param opAppInfo The information of the Operator App service.
         * @param state     The app state.
         */
        public void switchOperatorAppState(@NonNull OperatorAppServiceInfo opAppInfo,
                @OperatorAppServiceInfo.OperatorAppState int state) {
            if (mToken == null) {
                Log.w(TAG, "The session has been already released");
                return;
            }
            try {
                mService.switchOperatorAppState(mToken, opAppInfo, state, mUserId);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
    ...
    private static final class SessionCallbackRecord {
        ...
        void postRequestParentalControlApproval(final int requestId, final boolean challengeUser,
                final Bundle keyValues) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onRequestParentalControlApproval(
                            mSession, requestId, challengeUser, keyValues);
                }
            });
        }
        ...
        void postCancelParentalControlApproval() {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onCancelParentalControlApproval(mSession);
                }
            });
        }

        void postVerifyParentalControlPinRequest(final int requestId, final String pcPin) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onVerifyParentalControlPinRequest(mSession, requestId, pcPin);
                }
            });
        }
        ...
        void postParentalControlPinLengthRequest() {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onParentalControlPinLengthRequest(mSession);
                }
            });
        }

        void postSetParentalControlPin(final String oldPcPin, final String newPcPin) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onSetParentalControlPin(mSession, oldPcPin, newPcPin);
                }
            });
        }
        ...
        void postInteractiveAppStreamingTracksChanged(final List<TvTrackInfo> tracks) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onInteractiveAppStreamingTracksChanged(mSession, tracks);
                }
            });
        }

        void postKeySetChanged(final int value, final int[] otherKeys) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onKeySetChanged(mSession, value, otherKeys);
                }
            });
        }

        void postReplaceUiElements(final int[] elements) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onReplaceUiElements(mSession, elements);
                }
            });
        }

        void postOperatorAppSupportedStatus(final boolean supported) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onOperatorAppSupportedStatus(mSession, supported);
                }
            });
        }

        void postOperatorAppUpdateRequest(final OperatorAppServiceInfo opAppInfo,
                final boolean immediate) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onOperatorAppUpdateRequest(mSession, opAppInfo, immediate);
                }
            });
        }

        void postOperatorAppStatusChanged(final OperatorAppServiceInfo opAppInfo, final int status,
                final Bundle extra) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onOperatorAppStatusChanged(mSession, opAppInfo, status, extra);
                }
            });
        }

        void postOperatorAppStateChanged(final OperatorAppServiceInfo opAppInfo, final int state) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onOperatorAppStateChanged(mSession, opAppInfo, state);
                }
            });
        }

        void postInteractiveAppStreamingTrackSelected(final int type, final String trackId) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onInteractiveAppStreamingTrackSelected(
                            mSession, type, trackId);
                }
            });
        }

        void postStreamingPlaybackStatusChanged(final int status) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mSessionCallback.onStreamingPlaybackStatusChanged(mSession, status);
                }
            });
        }
    }
    ...
    public abstract static class SessionCallback {
        ...
        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#requestParentalControlApproval} is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this callback.
         * @param requestId The ID to identify the request.
         * @param challengeUser {@code true} if the user should be challenged for parental control
         *                      approval.
         * @param keyValues The key-value pairs of the parental control approval.
         */
        public void onRequestParentalControlApproval(
                Session session, int requestId, boolean challengeUser, Bundle keyValues) {
        }

        /**
         * This is called when {@link TvInteractiveAppService.Session#cancelParentalControlApproval}
         * is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this callback.
         */
        @Hide
        public void onCancelParentalControlApproval(@NonNull Session session) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#requestVerifyParentalControlPin} is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this callback.
         * @param requestId The ID to identify the request.
         * @param pcPin The parental control PIN.
         */
        @Hide
        public void onVerifyParentalControlPinRequest(@NonNull Session session, int requestId,
                @NonNull String pcPin) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#requestParentalControlPinLength} is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this callback.
         */
        @Hide
        public void onParentalControlPinLengthRequest(@NonNull Session session) {
        }

        /**
         * This is called when {@link TvInteractiveAppService.Session#setParentalControlPin}
         * is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this callback.
         * @param oldPcPin The old parental control PIN.
         * @param newPcPin The new parental control PIN.
         */
        @Hide
        public void onSetParentalControlPin(@NonNull Session session, @NonNull String oldPcPin,
                @NonNull String newPcPin) {
        }
        ...
        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#requestScheduleRecording(String, String,
         * Uri, Uri, Bundle)}
         * is called.
         *
         * @param session    A {@link TvInteractiveAppService.Session} associated with
         *                   this callback.
         * @param inputId    The ID of the TV input for the given channel.
         * @param channelUri The URI of a channel to be recorded.
         * @param programUri The URI of the TV program to be recorded.
         * @param params     Domain-specific data for this tune request. Keys
         *                   <em>must</em> be a scoped
         ...
         * @param inputId    The ID of the TV input for the given channel.
         * @param channelUri The URI of a channel to be recorded.
         * @param startTime  The start time of the recording in milliseconds since
         *                   epoch.
        ...
        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#requestSigning(String, String, String,
         * int, byte[])}
         * is called.
         *
         * @param session   A {@link TvInteractiveAppService.Session} associated with
         *                  this callback.
         * @param signingId the ID to identify the request.
         * @param algorithm the standard name of the signature algorithm requested, such
         *                  as
         *                  MD5withRSA, SHA256withDSA, etc.
        ...
        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#notifyInteractiveAppStreamingTracksChanged}
         * is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this callback.
         * @param tracks the current streaming tracks.
         */
        @Hide
        public void onInteractiveAppStreamingTracksChanged(@NonNull Session session,
                @NonNull List<TvTrackInfo> tracks) {
        }

        /**
         * This is called when {@link TvInteractiveAppService.Session#notifyKeySetChanged} is
         * called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this callback.
         * @param value The key set value.
         * @param otherKeys The other keys.
         */
        @Hide
        public void onKeySetChanged(@NonNull Session session, int value,
                @NonNull int[] otherKeys) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#notifyReplaceUiElements} is
         * called.
         *
         * @param session  A {@link TvInteractiveAppManager.Session} associated with
         *                 this callback.
         * @param elements The UI elements to be replaced.
         */
        @Hide
        public void onReplaceUiElements(@NonNull Session session, @NonNull int[] elements) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#notifyInteractiveAppStreamingTrackSelected}
         * is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this
         *                callback.
         * @param type    The type of the selected streaming track.
         * @param trackId The ID of the selected streaming track.
         */
        @Hide
        public void onInteractiveAppStreamingTrackSelected(@NonNull Session session, int type,
                @Nullable String trackId) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#sendInteractiveAppStreamingPlaybackStatus}
         * is called.
         *
         * @param session A {@link TvInteractiveAppManager.Session} associated with this
         *                callback.
         * @param status  the current streaming playback status.
         */
        @Hide
        public void onStreamingPlaybackStatusChanged(@NonNull Session session,
                @StreamingPlaybackStatus int status) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#notifyOperatorAppSupportedStatus}
         * is called.
         *
         * @param session   A {@link TvInteractiveAppManager.Session} associated with
         *                  this callback.
         * @param supported {@code true} if Operator App is supported.
         */
        @Hide
        public void onOperatorAppSupportedStatus(@NonNull Session session, boolean supported) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#notifyOperatorAppUpdateRequest}
         * is called.
         *
         * @param session   A {@link TvInteractiveAppManager.Session} associated with
         *                  this callback.
         * @param opAppInfo The information of the Operator App service.
         * @param immediate {@code true} if the update is immediate.
         */
        @Hide
        public void onOperatorAppUpdateRequest(@NonNull Session session,
                @NonNull OperatorAppServiceInfo opAppInfo, boolean immediate) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#notifyOperatorAppStatusChanged}
         * is called.
         *
         * @param session   A {@link TvInteractiveAppManager.Session} associated with
         *                  this callback.
         ...
         */
        @Hide
        public void onOperatorAppStatusChanged(@NonNull Session session,
                @NonNull OperatorAppServiceInfo opAppInfo,
                @OperatorAppServiceInfo.OperatorAppStatus int status, @Nullable Bundle extra) {
        }

        /**
         * This is called when
         * {@link TvInteractiveAppService.Session#notifyOperatorAppStateChanged}
         * is called.
         *
         * @param session   A {@link TvInteractiveAppManager.Session} associated with
         *                  this callback.
         ...
         */
        @Hide
        public void onOperatorAppStateChanged(@NonNull Session session,
                @NonNull OperatorAppServiceInfo opAppInfo,
                @OperatorAppServiceInfo.OperatorAppState int state) {
        }
    }
