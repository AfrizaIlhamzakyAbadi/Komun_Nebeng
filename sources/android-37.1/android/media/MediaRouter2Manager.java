...
import static android.media.MediaRouter2.ROUTING_TYPES_REMOTE;
...
public final class MediaRouter2Manager {
    ...
    public void registerScanRequest() {
        if (mScanRequestCount.getAndIncrement() == 0) {
            try {
                // MR2Manager doesn't support privileged routing types (only REMOTE). For privileged
                // routing support, clients should use MR2.
                int[] routingTypes =
                        Flags.enableMirroringInMediaRouter2() ? ROUTING_TYPES_REMOTE : new int[0];
                mMediaRouterService.updateScanningState(
                        mClient, SCANNING_STATE_WHILE_INTERACTIVE, routingTypes);
            } catch (RemoteException ex) {
                throw ex.rethrowFromSystemServer();
            }
        }
    ...
    public void unregisterScanRequest() {
        if (mScanRequestCount.updateAndGet(
            ...
            try {
                ...
                mMediaRouterService.updateScanningState(
                        mClient, SCANNING_STATE_NOT_SCANNING, /* routingTypes= */ new int[0]);
            } catch (RemoteException ex) {
                throw ex.rethrowFromSystemServer();
            }
        }
