...
import static android.media.MediaRoute2Info.RoutingType;
...
public final class MediaRouter2 {
    ...
    /* package */ static final int[] ROUTING_TYPES_REMOTE = new int[] {ROUTING_TYPE_REMOTE};
    ...
    private static IMediaRouterService sServiceInstrumentation;
    private static final Supplier<IMediaRouterService> sServiceSupplier =
            () -> {
                if (sServiceInstrumentation != null) {
                    return sServiceInstrumentation;
                }
                return IMediaRouterService.Stub.asInterface(
    ...
    /**
     * Stores the latest mutable collection of routes received from the system server, without any
     * filtering, sorting, or deduplication.
     *
    ...
    @GuardedBy("mLock")
    private int mStartScanActiveCount = 0;

    @GuardedBy("mLock")
    @Nullable
    private ScanToken mStartScanToken;
    ...
    @GuardedBy("mLock")
    private final Map<String, List<SuggestedDeviceInfo>> mSuggestedDeviceInfo = new HashMap<>();
    ...
    /**
     * Holds the latest immutable copy of exposed routes after filtering, sorting, and
     * deduplication. Can be accessed through {@link #getRoutes()}.
     *
     ...
     */
    @GuardedBy("mLock")
    private List<MediaRoute2Info> mFilteredRoutes = Collections.emptyList();
    ...
    public void startScan() {
        if (Flags.convergeMr2RequestScanAndStartScan()) {
            synchronized (mLock) {
                mStartScanActiveCount++;
                if (mStartScanActiveCount == 1) {
                    Preconditions.checkState(
                            mStartScanToken == null,
                            "startScan reached one, but scan token was non-null.");
                    mStartScanToken = requestScan(new ScanRequest.Builder().build());
                }
            }
        } else {
            mImpl.startScan();
    ...
    public void stopScan() {
        if (Flags.convergeMr2RequestScanAndStartScan()) {
            synchronized (mLock) {
                if (mStartScanActiveCount == 0) {
                    Log.w(TAG, "Ignoring call to stopScan() while startScan() count is 0.");
                    return;
                }
                mStartScanActiveCount--;
                if (mStartScanActiveCount == 0) {
                    Preconditions.checkState(
                            mStartScanToken != null,
                            "startScan count reached zero but scanToken is null.");
                    cancelScanRequest(mStartScanToken);
                    mStartScanToken = null;
                }
            }
        } else {
    ...
    public ScanToken requestScan(@NonNull ScanRequest scanRequest) {
        ...
        synchronized (mLock) {
            ScanningParams oldState =
                    getScanningStateLocked(/* tokenToIgnore= */ null, /* requestToInclude= */ null);
            ScanningParams newState =
                    getScanningStateLocked(
                            /* tokenToIgnore= */ null, /* requestToInclude= */ scanRequest);

            if (!oldState.equals(newState)) {
                try {
                    mImpl.updateScanningState(newState.scanningState, newState.routingTypes);
    ...
    public void cancelScanRequest(@NonNull ScanToken token) {
        ...
        synchronized (mLock) {
            if (!mScanRequestsMap.contains(token.mId)) {
            ...
            ScanningParams newState =
                    getScanningStateLocked(
                            /* tokenToIgnore= */ token, /* requestToInclude= */ null);
            if (!oldState.equals(newState)) {
                try {
                    mImpl.updateScanningState(newState.scanningState, newState.routingTypes);
                } catch (RemoteException ex) {
    ...
    private MediaRouter2(Context appContext) {
        ...
        mMediaRouterService = sServiceSupplier.get();
    ...
    public void setDeviceSuggestions(@NonNull List<SuggestedDeviceInfo> suggestedDeviceInfo) {
        if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
            updateDeviceSuggestions(
                    mContext.getPackageName(),
                    List.copyOf(suggestedDeviceInfo),
                    /* setWithService= */ true);
        } else {
    ...
    public void clearDeviceSuggestions() {
        if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
            updateDeviceSuggestions(mContext.getPackageName(), null, /* setWithService= */ true);
        } else {
    ...
    public Map<String, List<SuggestedDeviceInfo>> getDeviceSuggestions() {
        if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
            synchronized (mLock) {
                return getDeviceSuggestionsLocked();
            }
        } else {
    ...
    @GuardedBy("mLock")
    private Map<String, List<SuggestedDeviceInfo>> getDeviceSuggestionsLocked() {
        return mSuggestedDeviceInfo.entrySet().stream()
                .collect(
    ...
    void requestCreateController(
        ...
        if (stub != null) {
            try {
                ...
                if (managerRequestId == REQUEST_ID_NONE) {
    ...
    /**
     * Returns the {@link ScanningParams} for the current set of {@link #mScanRequestsMap scan
     * requests}.
     *
     * @param tokenToIgnore An optional {@link ScanToken} to ignore. This is useful to check the
     *     scanning state in preparation for a scan request cancellation.
     * @param requestToInclude An optional {@link ScanRequest} to include. This is useful to check
     *     the scanning state in preparation for a scan request.
     */
    @GuardedBy("mLock")
    private ScanningParams getScanningStateLocked(
            @Nullable ScanToken tokenToIgnore, @Nullable ScanRequest requestToInclude) {
        @ScanningState int state = SCANNING_STATE_NOT_SCANNING;
        Set<@RoutingType Integer> routingTypes = new HashSet<>();
        for (int i = 0; i < mScanRequestsMap.size(); i++) {
            if (tokenToIgnore != null && mScanRequestsMap.keyAt(i) == tokenToIgnore.mId) {
                continue;
            }
            ScanRequest request = mScanRequestsMap.valueAt(i);
            if (request.isScreenOffScan()) {
                state = SCANNING_STATE_SCANNING_FULL;
            } else if (state == SCANNING_STATE_NOT_SCANNING) {
                state = SCANNING_STATE_WHILE_INTERACTIVE;
            }
            if (Flags.enableMirroringInMediaRouter2()) {
                routingTypes.addAll(request.getRoutingTypes());
            }
        }

        if (requestToInclude != null) {
            if (requestToInclude.isScreenOffScan()) {
                state = SCANNING_STATE_SCANNING_FULL;
            } else if (state == SCANNING_STATE_NOT_SCANNING) {
                state = SCANNING_STATE_WHILE_INTERACTIVE;
            }
            if (Flags.enableMirroringInMediaRouter2()) {
                routingTypes.addAll(requestToInclude.getRoutingTypes());
            }
        }
        return new ScanningParams(state, routingTypes);
    }

    private record ScanningParams(
            @ScanningState int scanningState, Set<@RoutingType Integer> routingTypes) {}
    ...
    void updateRoutesOnHandler(List<MediaRoute2Info> newRoutes, List<String> missingPermissions) {
        List<MediaRoute2Info> filteredRoutesToDispatch;
        Map<String, MediaRoute2Info> currentRoutesSnapshot;
        synchronized (mLock) {
            mRoutes.clear();
            for (MediaRoute2Info route : newRoutes) {
                mRoutes.put(route.getId(), route);
            ...
            updateFilteredRoutesLocked();
            filteredRoutesToDispatch = mFilteredRoutes;
            currentRoutesSnapshot = mRoutes;
        }
        dispatchFilteredRoutesUpdatedOnHandler(filteredRoutesToDispatch);
        dispatchControllerUpdatedIfNeededOnHandler(currentRoutesSnapshot);
    }
    ...
    private List<MediaRoute2Info> filterRoutesWithCompositePreferenceLocked(
        ...
        Set<Integer> requestedRoutingTypes = Set.of();
        if (Flags.enableMirroringInMediaRouter2()) {
        ...
        for (MediaRoute2Info route :
            ...
            boolean isPrivilegedRoute =
                    Flags.enableMirroringInMediaRouter2() && route.supportsPrivilegedRoutingTypes();
            // We only filter privileged routes by routing type for backwards compatibility.
            // Non-privileged routes are filtered only by preferred features.
            // Historically, even while not actively scanning, proxy routers can see the
            // non-privileged routes that the target app sees.
            if (isPrivilegedRoute
                    && Collections.disjoint(
                            route.getSupportedRoutingTypes(), requestedRoutingTypes)) {
                continue;
            }
            if (!isPrivilegedRoute
            ...
            // We don't deduplicate privileged routes here. That must be done by the client.
            // The deduplication mechanism used by RouteDiscoveryPreference#shouldRemoveDuplicates
            // is managed by the local router, but we don't want the local router to be able to hide
            // privileged routes.
            if (!isPrivilegedRoute && mDiscoveryPreference.shouldRemoveDuplicates()) {
    ...
    public static final class ScanRequest {
        ...
        private final Set<@RoutingType Integer> mRoutingTypes;

        private ScanRequest(
                boolean isScreenOffScan, @NonNull Set<@RoutingType Integer> routingTypes) {
            mIsScreenOffScan = isScreenOffScan;
            mRoutingTypes = Set.copyOf(routingTypes);
        }
        ...
        /**
         * Returns the {@link MediaRoute2Info#getSupportedRoutingTypes() routing types} associated
         * with this scan request.
         *
         * <p>The default value is {@link MediaRoute2Info#ROUTING_TYPE_REMOTE}. Note that only
         * {@link MediaRouter2#getInstance(Context, String, Executor, Runnable) proxy routers}
         * support scanning for routing types other than {@link
         * MediaRoute2Info#ROUTING_TYPE_REMOTE}.
         *
         * @see Builder#setRoutingTypes(Set)
         * @see #requestScan(ScanRequest)
         */
        @NonNull
        @FlaggedApi(Flags.FLAG_ENABLE_MIRRORING_IN_MEDIA_ROUTER_2)
        public Set<@RoutingType Integer> getRoutingTypes() {
        ...
        public static final class Builder {
            ...
            private Set<@RoutingType Integer> mRoutingTypes;
             ...
             */
            public Builder() {
                mRoutingTypes = new ArraySet<>();
                mRoutingTypes.add(ROUTING_TYPE_REMOTE);
            }
            ...
            /**
             * Sets the {@link MediaRoute2Info#getSupportedRoutingTypes()} routing types} to scan
             * for.
             *
             * <p>The default value is {@link MediaRoute2Info#ROUTING_TYPE_REMOTE}. Only {@link
             * MediaRouter2#getInstance(Context, String, Executor, Runnable) proxy routers} are
             * allowed to scan for routing types other than {@link
             * MediaRoute2Info#ROUTING_TYPE_REMOTE}.
             *
             * @see #requestScan(ScanRequest)
             * @throws IllegalArgumentException If an empty set of routing types is provided.
             */
            @NonNull
            @FlaggedApi(Flags.FLAG_ENABLE_MIRRORING_IN_MEDIA_ROUTER_2)
            public Builder setRoutingTypes(@NonNull Set<@RoutingType Integer> routingTypes) {
                if (routingTypes.isEmpty()) {
                    throw new IllegalArgumentException("routing types must not be empty");
                }
                mRoutingTypes = new ArraySet<>(routingTypes);
                return this;
            }

            /**
             * Adds a {@link MediaRoute2Info#getSupportedRoutingTypes() routing type} to scan for.
             *
             * <p>The default value is {@link MediaRoute2Info#ROUTING_TYPE_REMOTE}. Only {@link
             * MediaRouter2#getInstance(Context, String, Executor, Runnable) proxy routers} are
             * allowed to scan for routing types other than {@link
            ...
            @NonNull
            @FlaggedApi(Flags.FLAG_ENABLE_MIRRORING_IN_MEDIA_ROUTER_2)
            public Builder addRoutingType(@RoutingType int routingType) {
                mRoutingTypes.add(routingType);
                return this;
            }
            ...
            public ScanRequest build() {
                return new ScanRequest(mIsScreenOffScan, mRoutingTypes);
            }
        }
    }
    ...
    class MediaRouter2Stub extends IMediaRouter2.Stub {
        ...
        public void notifyDeviceSuggestionsUpdated(
            ...
            mHandler.sendMessage(
                    obtainMessage(
                            MediaRouter2::updateDeviceSuggestions,
                            MediaRouter2.this,
                            suggestingPackageName,
                            suggestions,
    ...
    private class ProxyMediaRouter2Impl implements MediaRouter2Impl {
        ...
        @Override
        public void updateScanningState(int scanningState, Set<@RoutingType Integer> routingTypes)
                throws RemoteException {
            int[] routingTypesArray =
                    Flags.enableMirroringInMediaRouter2()
                                    && scanningState != SCANNING_STATE_NOT_SCANNING
                            ? routingTypes.stream().mapToInt(Integer::intValue).sorted().toArray()
                            : new int[0];
            mMediaRouterService.updateScanningState(mClient, scanningState, routingTypesArray);
        }
        ...
        public void stopScan() {
            if (mIsScanning.getAndSet(false)) {
                if (mScanRequestCount.updateAndGet(
                        ...
                        == 0) {
                    try {
                        // routingTypes are ignored while not scanning.
                        mMediaRouterService.updateScanningState(
                                mClient,
                                SCANNING_STATE_NOT_SCANNING,
                                /* routingTypes= */ new int[0]);
                    } catch (RemoteException ex) {
                        throw ex.rethrowFromSystemServer();
                    }
                }
            ...
            if (Flags.reduceTwoWayBinderCallsInMediaRouter2()) {
                ...
                ControllerCreationRequest request =
                        new ControllerCreationRequest(
                                requestId, REQUEST_ID_NONE, route, controller);
        ...
        private void onDiscoveryPreferenceChangedOnHandler(
            ...
            synchronized (mLock) {
                ...
                updateFilteredRoutesLocked();
                filteredRoutesToDispatch = mFilteredRoutes;
                currentRoutesSnapshot = new HashMap<>(mRoutes);
            }
        ...
        private void onDeviceSuggestionsChangeHandler(
            ...
            if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
                updateDeviceSuggestions(
                        suggestingPackageName, suggestedDeviceInfo, /* setWithService= */ false);
            } else {
                synchronized (mLock) {
                    if (Objects.equals(
                            mSuggestedDeviceInfo.get(suggestingPackageName), suggestedDeviceInfo)) {
    ...
    private class LocalMediaRouter2Impl implements MediaRouter2Impl {
        ...
        public void updateScanningState(int scanningState, Set<@RoutingType Integer> routingTypes)
            ...
            if (Flags.enableMirroringInMediaRouter2()) {
                if (scanningState != SCANNING_STATE_NOT_SCANNING
                        && (routingTypes.size() != 1
                                || routingTypes.iterator().next()
                                        != MediaRoute2Info.ROUTING_TYPE_REMOTE)) {
                    throw new IllegalArgumentException(
                            "Local routers only support ROUTING_TYPE_REMOTE.");
                }
            }
            if (scanningState != SCANNING_STATE_NOT_SCANNING) {
        ...
        public void setRouteListingPreference(@Nullable RouteListingPreference preference) {
            synchronized (mLock) {
                ...
                if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
                    validateRouteListingPreference(preference);
                    if (mStub != null) {
                        try {
                            mMediaRouterService.setRouteListingPreference(mStub, preference);
                        } catch (RemoteException ex) {
                            ex.rethrowFromSystemServer();
                        }
                    }
        ...
        public void setDeviceSuggestions(@Nullable List<SuggestedDeviceInfo> deviceSuggestions) {
            synchronized (mLock) {
                if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
                    try {
                        if (mStub != null) {
                            mMediaRouterService.setDeviceSuggestionsWithRouter2(
                                    mStub, deviceSuggestions);
                        }
                    } catch (RemoteException ex) {
                        ex.rethrowFromSystemServer();
                    }
        ...
        public void transferTo(
            ...
            if (!controller.tryTransferWithinProvider(route, routingChangeInfo)) {
                requestCreateController(controller, route, REQUEST_ID_NONE, routingChangeInfo);
            }
        }
        ...
        private void registerRouterStubIfNeededLocked() throws RemoteException {
            if (mStub == null) {
                ...
                if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
                    if (mSuggestedDeviceInfo.containsKey(mPackageName)) {
                        mMediaRouterService.setDeviceSuggestionsWithRouter2(
                                stub, mSuggestedDeviceInfo.get(mPackageName));
                    }
                    if (mRouteListingPreference != null) {
                        mMediaRouterService.setRouteListingPreference(
                                stub, mRouteListingPreference);
                    }
                }
                mStub = stub;
            }
        ...
        private void unregisterRouterStubIfNeededLocked(boolean isScanningStopping)
            ...
            if (mStub != null
                ...
                // Suggestions from other packages are no longer relevant when we unregister from
                // service because (1) no routes are available for transfer and (2) new suggestions
                // from other packages will not reach us. Suggestions from the same package are kept
                // though as they are maintained locally.
                if (Flags.disableRouterRegistrationsInLocalMediaRouter2()) {
                    mSuggestedDeviceInfo
                            .keySet()
                            .removeIf(
                                    suggestingPackage ->
                                            !suggestingPackage.equals(mContext.getPackageName()));
                }
                mStub = null;
            }
        }

        // TODO(b/504991701): Consolidate the shared validation logic between the client and the
        // service.
        private void validateRouteListingPreference(@Nullable RouteListingPreference preference) {
            if (preference == null) {
                return;
            }
            ComponentName linkedItemLandingComponent = preference.getLinkedItemComponentName();
            ...
            ComponentName resolveMissingPermissionsComponent =
                    preference.getMissingPermissionsComponentName();
            verifyActivityComponentName(
                    resolveMissingPermissionsComponent,
                    RouteListingPreference.ACTION_RESOLVE_MISSING_PERMISSIONS);
        }

        private void verifyActivityComponentName(ComponentName componentName, String action) {
            if (componentName == null) {
                return;
            }
            String packageName = componentName.getPackageName();
            if (TextUtils.isEmpty(packageName) || !packageName.equals(mContext.getPackageName())) {
                throw new IllegalArgumentException(
                        "packageName must be the same as the calling package and may not be empty");
            }
            if (!isValidActivityComponentName(componentName, action)) {
                throw new IllegalArgumentException(
                        "Unable to resolve "
                                + componentName
        ...
        private boolean isValidActivityComponentName(ComponentName componentName, String action) {
            Intent intent = new Intent(action);
            intent.setComponent(componentName);
            List<ResolveInfo> resolveInfos =
                    mContext.getPackageManager().queryIntentActivities(intent, /* flags= */ 0);
    ...
    /**
     * Sideloads a MediaRouter service reference for unit-testing purposes.
     *
     * <p>This method throws if called after any calls to {@link #getInstance}.
     *
     * <p>You must also call {@link #resetStateForTesting()} after a test to ensure two tests don't
     * share state with each other.
     */
    @Hide
    public static void instrument(@NonNull IMediaRouterService service) {
        Objects.requireNonNull(service);
        synchronized (sRouterLock) {
            if (sInstance != null) {
                throw new IllegalStateException(
                        "MR2 illegal instrumentation: local instance has already been created.");
            }
        }
        synchronized (sSystemRouterLock) {
            if (!sAppToProxyRouterMap.isEmpty()) {
                throw new IllegalStateException(
                        "MR2 illegal instrumentation: proxy instance has already been created.");
            }
        }
        sServiceInstrumentation = service;
    }

    /**
     * Clears all MediaRouter internal state, including any instrumentation set by {@link
     * #instrument}.
     *
     * <p>You must call this in-between tests to prevent them from sharing state.
     */
    @Hide
    public static void resetStateForTesting() {
        sServiceInstrumentation = null;
        synchronized (sRouterLock) {
            if (sInstance != null && sInstance.mStub != null) {
                throw new IllegalStateException("Test hasn't finished cleaning up.");
            }
            sInstance = null;
        }
        synchronized (sSystemRouterLock) {
            sAppToProxyRouterMap.clear();
        }
    }
}
