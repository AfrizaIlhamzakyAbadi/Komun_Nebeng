...
public class Chronometer extends TextView {
    ...
    private Duration mPausedDuration;
    ...
    public void setCountDown(boolean countDown) {
        ...
        if (mPausedDuration != null) {
            mBase = (mCountDown ? 1 : -1) * mPausedDuration.toMillis();
        }
        updateAll(/* dispatchTick= */ false);
    }
    ...
    public void setBase(@ElapsedRealtimeLong long base) {
        mBase = base;
        mBaseInstant = null;
        mPausedDuration = null;
        updateAll(/* dispatchTick= */ true);
    }
    ...
    /**
     * Puts the chronometer in a static display mode where it displays the given {@link Duration}
     * (which can be negative). In this mode, the chronometer cannot be "started".
     * <p>
     * In this mode a positive duration will be shown as positive (and negative as negative)
     * regardless of the value of {@link #setCountDown}.
     */
    @Hide
    @android.view.RemotableViewMethod
    public void setPausedDuration(@NonNull Duration duration) {
        mPausedDuration = requireNonNull(duration);
        mBase = (mCountDown ? 1 : -1) * mPausedDuration.toMillis();
        mBaseInstant = null;
        updateAll(/* dispatchTick= */ false);
    }
    ...
    public void setUseAdaptiveFormat(boolean useAdaptiveFormat) {
        if (mUseAdaptiveFormat == useAdaptiveFormat) {
            return;
        }
    ...
    public void updateText() {
        updateText(mPausedDuration != null ? 0 : mElapsedRealtimeClock.getAsLong());
    }
    ...
    /**
     * Updates the running state of the chronometer and posts a tick if necessary.
     *
     * Updates the text only if the chronometer is started by this call.
     */
    private void updateRunning() {
        boolean running = mVisible && mStarted && mPausedDuration == null && isShown();
        if (running != mRunning) {
            if (running) {
                updateText();
    ...
    /**
     * Updates the text AND running AND next tick if running.
     *
     * @param dispatchTick Whether to dispatch a tick event after updating the text, regardless of
     *     whether the chronometer is running or paused.
     */
    private void updateAll(boolean dispatchTick) {
        final boolean wasRunning = mRunning;
        updateRunning();
        if (!wasRunning && mRunning) {
            // updateRunning() already updated the text and the next tick
            return;
        }
        // In all cases we want to refresh the text for format or base changes.
        updateText();
        // Dispatch the tick if requested, even if not running.
        if (dispatchTick) {
            dispatchChronometerTick();
        }
        // If have been running and still are, cancel and reschedule the next tick
        if (mRunning) {
            // cancel the existing tick (in case it is at the wrong time) and reschedule a new one.
            removeCallbacks(mTickRunnable);
            postTickOnNextChange();
        }
    }
