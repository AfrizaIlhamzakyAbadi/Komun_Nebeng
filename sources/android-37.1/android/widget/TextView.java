...
import static android.widget.flags.Flags.enableContextMenuLookUp;
...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    // Accessibility action to look up selected text.
    private static final int ACCESSIBILITY_ACTION_LOOK_UP = 0x10010000;
    ...
    public void notifyContentCaptureTextChanged() {
        ...
        if (isLaidOut() && isImportantForContentCapture() && getNotifiedContentCaptureAppeared()) {
            ...
            if (cm != null && cm.isContentCaptureEnabled()) {
                ...
                if (session != null && session.isContentCaptureVersion1()) {
    ...
    public void onInitializeAccessibilityNodeInfoInternal(AccessibilityNodeInfo info) {
        ...
        if (isFocused()) {
            ...
            if (canLookUpText()) {
                info.addAction(
                        new AccessibilityNodeInfo.AccessibilityAction(
                                ACCESSIBILITY_ACTION_LOOK_UP,
                                getResources().getString(com.android.internal.R.string.look_up)));
            }
    ...
    public boolean performAccessibilityActionInternal(int action, Bundle arguments) {
        ...
        switch (action) {
            ...
            case ACCESSIBILITY_ACTION_LOOK_UP: {
                if (isFocused() && canLookUpText()) {
                    if (onTextContextMenuItem(ID_LOOK_UP)) {
                        return true;
                    }
                }
            } return false;
    ...
    static final int ID_LOOK_UP = android.R.id.lookUpText;
     ...
     * {@link android.R.id#lookUpText} (starting at API level 37) or
     ...
     * {@link android.R.id#pasteAsPlainText} (starting at API level 23),
     * {@link android.R.id#lookUpText} (starting at API level 37),
    ...
    boolean canLookUpText() {
        // Check prerequisite conditions for showing the Look Up item.
        // `canCopy` covers basic checks, such as presence of selection and passwords.
        if (!enableContextMenuLookUp()
                || !canCopy()
        ...
        Intent intent = createLookUpTextIntent(selectedText);
    ...
    private Intent createLookUpTextIntent(String selectedText) {
        Intent intent = new Intent(Intent.ACTION_LOOK_UP_TEXT);
        intent.setType("text/plain");
        intent.putExtra(Intent.EXTRA_TEXT, TextUtils.trimToParcelableSize(selectedText));
        return intent;
    }

    /**
     * Sends a LOOK_UP_TEXT intent with the selected text.
     * Does not check for preconditions such as whether the target activity is valid.
     * Use {@link #canLookUpText} to check if this call is valid or not.
     */
    private void lookUpSelectedText() {
        String selectedText = getSelectedText();
        if (!selectedText.isBlank()) {
            Intent intent = createLookUpTextIntent(selectedText);
            getContext().startActivity(intent);
