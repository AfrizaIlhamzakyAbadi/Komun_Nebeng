...
public final class RateLimiter {
    ...
    private final Map<Integer, Integer> mQuotaBucketToMaxRollingQuota = new ConcurrentHashMap<>();
    private final Map<String, Integer> mQuotaBucketToMaxMemoryQuota = new ConcurrentHashMap<>();
    ...
    private final Map<Integer, ConcurrentMap<Integer, Quota>> mUserIdToQuotasMap =
            new ConcurrentHashMap<>();
    ...
    /** Dumps the current state of the rate limiter. */
    public void dump(PrintWriter pw, IntFunction<String> uidToPackageNames) {
        pw.println("Health Connect Rate Limiter Status");
        pw.println("  Global Memory Quotas:");
        mQuotaBucketToAcrossAppsRemainingMemoryQuota.forEach(
                (bucket, quota) -> {
                    pw.printf(
                            Locale.ROOT,
                            "    %s: remaining=%.2f, lastUpdate=%d ms ago\n",
                            quotaBucketToString(bucket),
                            quota.getRemainingQuota(),
                            SystemClock.elapsedRealtime() - quota.getLastUpdatedTimeMillis());
                });

        pw.println("  Per-UID Quotas:");
        mUserIdToQuotasMap.forEach(
                (uid, bucketMap) -> {
                    pw.printf(Locale.ROOT, "    UID %d (%s):\n", uid, uidToPackageNames.apply(uid));
                    bucketMap.forEach(
                            (bucket, quota) -> {
                                pw.printf(
                                        Locale.ROOT,
    ...
    private Quota getQuota(int uid, @QuotaBucket.Type int quotaBucket) {
        ...
        if (!mUserIdToQuotasMap.containsKey(uid)) {
            mUserIdToQuotasMap.put(uid, new ConcurrentHashMap<>());
        }
