...
import android.health.connect.common.DeviceUtils;
import android.health.connect.common.DeviceUtilsImpl;
...
public class HealthServicesInitializer {
    ...
    private static boolean shouldReturnHealthConnectManager(Context context) {
        DeviceUtils deviceUtils = new DeviceUtilsImpl();

        if (deviceUtils.isWearPermissionsMode(context)) {
            return context.checkSelfPermission(HealthPermissions.MANAGE_HEALTH_PERMISSIONS)
                    == PackageManager.PERMISSION_GRANTED;
        }

        return deviceUtils.areHealthPermissionsAvailable(context);
    }
}
