...
public class HealthConnectManager {
     ...
     *
     * @deprecated See {@link #updateDataDownloadState}
     */
    @FlaggedApi(Flags.FLAG_DEPRECATE_DATA_DOWNLOAD_FLOW)
    @Deprecated
    @SystemApi
    ...
    @Nullable private static volatile Set<String> sSystemHealthPermissions;
    ...
    /**
     * Returns a list of apps that are considered health apps on the system. For wear devices, this
     * will entail apps that request permissions from {@link
     * HealthConnectManager#getSystemHealthPermissions(Context)}, while devices that have Health
     * Connect available will return compatible apps.
     *
     * <p>See {@link HealthConnectPermissionHelper#getHealthApps(UserHandle)} for more details on
     * the definition of compatible apps.
     *
     * @return a list of {@link HealthApp} objects representing compatible applications
     * @throws NullPointerException if any of the arguments is {@code null}.
     * @throws SecurityException if the caller doesn't possess {@code
     *     android.permission.MANAGE_HEALTH_PERMISSIONS}.
     */
    ...
    @FlaggedApi(FLAG_HEALTH_PERMISSION_READER_IMPROVEMENTS)
    @NonNull
    public List<HealthApp> getHealthApps() {
        try {
            return mService.getHealthApps(mContext.getUser());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    @UserHandleAware
    @FlaggedApi(FLAG_HEALTH_PERMISSION_READER_IMPROVEMENTS)
    public boolean isBodySensorSplitPermissionApp(@NonNull String packageName) {
        try {
            return mService.isBodySensorSplitPermissionApp(packageName, mContext.getUser());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     *     DataDownloadState}
     * @deprecated This feature is deprecated and has no effect. Staging remote data for restore can
     *     still be done using {@link #stageHealthConnectRemoteData(StageRemoteDataRequest,
     *     Executor, OutcomeReceiver)} without calling this API.
     */
    @SystemApi
    @UserHandleAware
    @RequiresPermission(Manifest.permission.STAGE_HEALTH_CONNECT_REMOTE_DATA)
    ...
    public void updateDataDownloadState(@DataDownloadState int downloadState) {
        if (Flags.deprecateDataDownloadFlow()) {
            // No-op: This feature is deprecated and has no effect.
            return;
        }
        try {
    ...
    /**
     * Returns an <b>immutable</b> set of health permissions that are usable <b>outside</b> Health
     * Connect, defined within the module and belonging to {@link
     * android.health.connect.HealthPermissions#HEALTH_PERMISSION_GROUP}.
     *
     * <p>This set of permissions gains significance on devices (such as Wear) to identify health
     * apps where Heath Connect is not available.
     *
     * <p>Compared to {@link #getHealthPermissions}, this method will only return permissions that
     * map to a custom app op instead of the generic {@link
     * AppOpsManager.OPSTR_READ_WRITE_HEALTH_DATA}, for example {@link
     * HealthPermissions.READ_HEART_RATE} which maps to {@link AppOpsManager.OPSTR_READ_HEART_RATE}.
     *
     * <p><b>Note:</b> If we, for some reason, fail to retrieve these, we return an empty set rather
     * than crashing the device. This means the health permissions infra will be inactive.
     */
    ...
    @NonNull
    @FlaggedApi(FLAG_HEALTH_PERMISSION_READER_IMPROVEMENTS)
    public static Set<String> getSystemHealthPermissions(@NonNull Context context) {
        if (sSystemHealthPermissions != null) {
            return sSystemHealthPermissions;
        }

        Set<String> result =
                getHealthPermissions(context).stream()
                        .filter(
                                healthPermission -> {
        ...
        sSystemHealthPermissions = result;
        return result;
    }
