...
import android.health.connect.common.PermissionUtils;
...
public final class HealthPermissions {
    ...
    /**
     * Represents the specific state of a health-related permission for an application.
     *
     * <p>The status determines how a permission is recognized by the system, specifically tracking
     * whether the permission is present in the application's manifest ("declared") and whether the
     * user or system has authorized it ("granted").
     */
    @Hide
    @Retention(RetentionPolicy.SOURCE)
    @IntDef({
        PERMISSION_STATUS_NONE,
        PERMISSION_STATUS_DECLARED,
        PERMISSION_STATUS_GRANTED_AND_DECLARED,
        PERMISSION_STATUS_GRANTED_NOT_DECLARED
    })
    public @interface PermissionStatus {}

    /**
     * The permission is neither declared nor granted.
     */
    @Hide
    public static final int PERMISSION_STATUS_NONE = 0;

    /**
     * The permission is in the manifest but not yet granted.
     */
    @Hide
    public static final int PERMISSION_STATUS_DECLARED = 1;

    /**
     * The permission is in the manifest and granted.
     */
    @Hide
    public static final int PERMISSION_STATUS_GRANTED_AND_DECLARED = 2;

    /**
     * The permission is granted but not declared in the manifest.
     *
     * <p>This status is possible for apps sharing the same UID as apps granting a permission but
     * not declaring the permission themselves.
     */
    @Hide
    public static final int PERMISSION_STATUS_GRANTED_NOT_DECLARED = 3;

    /**
     * Returns a string representation of the given {@code status}.
     */
    @Hide
    public static String permissionStatusToString(@PermissionStatus int status) {
        switch (status) {
            case PERMISSION_STATUS_DECLARED:
                return "DECLARED";
            case PERMISSION_STATUS_GRANTED_AND_DECLARED:
                return "GRANTED_AND_DECLARED";
            case PERMISSION_STATUS_GRANTED_NOT_DECLARED:
                return "GRANTED_NOT_DECLARED";
            case PERMISSION_STATUS_NONE:
            default:
                return "NONE";
        }
    }
    ...
    @Nullable private static Set<String> sAllMedicalPermissions;

    @Nullable private static Set<String> sAllFitnessPermissions;

    @Nullable private static Set<String> sAllAdditionalPermissions;

    @Nullable private static Set<String> sNormalHealthPermissions;
    ...
    public static Set<String> getAllMedicalPermissions() {
        if (AconfigFlagHelper.isHealthPermissionReaderImprovementsEnabled()) {
            if (sAllMedicalPermissions == null) {
                sAllMedicalPermissions =
                        Collections.unmodifiableSet(getAllMedicalPermissionsInternal());
            }
            return sAllMedicalPermissions;
        }
        return getAllMedicalPermissionsInternal();
    }

    private static Set<String> getAllMedicalPermissionsInternal() {
    ...
    /**
     * Returns all fitness permissions (read and write).
     */
    @Hide
    public static Set<String> getAllFitnessPermissions() {
        if (AconfigFlagHelper.isHealthPermissionReaderImprovementsEnabled()) {
            if (sAllFitnessPermissions == null) {
                sAllFitnessPermissions =
                        Collections.unmodifiableSet(getAllFitnessPermissionsInternal());
            }
            return sAllFitnessPermissions;
        }
        return getAllFitnessPermissionsInternal();
    }

    private static Set<String> getAllFitnessPermissionsInternal() {
        return HealthConnectMappings.getInstance().getAllFitnessPermissions().stream()
                .filter(HealthPermissions::isPermissionEnabled)
                .collect(Collectors.toSet());
    }

    /**
     * Returns all additional permissions.
     */
    @Hide
    public static Set<String> getAllAdditionalPermissions() {
        if (AconfigFlagHelper.isHealthPermissionReaderImprovementsEnabled()) {
            if (sAllAdditionalPermissions == null) {
                sAllAdditionalPermissions =
                        Collections.unmodifiableSet(getAllAdditionalPermissionsInternal());
            }
            return sAllAdditionalPermissions;
        }
        return getAllAdditionalPermissionsInternal();
    }

    private static Set<String> getAllAdditionalPermissionsInternal() {
        Set<String> permissions = new ArraySet<>();
        permissions.add(READ_EXERCISE_ROUTES);
        permissions.add(READ_HEALTH_DATA_IN_BACKGROUND);
        permissions.add(READ_HEALTH_DATA_HISTORY);
        return permissions;
    }

    /**
     * Returns a collection of all permissions that belong to {@link HEALTH_PERMISSION_GROUP} but
     * should not be exposed to the user when managing an app's permissions.
     */
    @Hide
    public static Set<String> getNormalHealthPermissions() {
        if (AconfigFlagHelper.isHealthPermissionReaderImprovementsEnabled()) {
            if (sNormalHealthPermissions == null) {
                sNormalHealthPermissions =
                        Collections.unmodifiableSet(getNormalHealthPermissionsInternal());
            }
            return sNormalHealthPermissions;
        }
        return getNormalHealthPermissionsInternal();
    }

    private static Set<String> getNormalHealthPermissionsInternal() {
        Set<String> permissions = new ArraySet<>();
        permissions.add(WRITE_DEVICE_UDI);
        return permissions;
    }
    ...
    public static Set<Integer> getDataCategoriesWithWritePermissionsForPackage(
        ...
        if (AconfigFlagHelper.isSharedUidSupportEnabled() && packageInfo.packageName != null) {
            List<String> grantedPermissions =
                    PermissionUtils.getGrantedHealthPermissionsForUid(
                            context.getPackageManager(), packageInfo.packageName, context);
            for (String currPerm : grantedPermissions) {
                int dataCategory =
                        HealthConnectMappings.getInstance()
                                .getHealthDataCategoryForWritePermission(currPerm);
                if (dataCategory >= 0) {
