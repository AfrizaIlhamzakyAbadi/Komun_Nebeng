...
public class UceController {
    ...
    /**
     * Setup the capability event callback synchronously.
     */
    public void setupCapabilityEventCallback(RcsFeatureManager manager) {
        if (!Flags.synchronousRcsUceCallbackRegistration()) {
            return;
        }
        logi("setupCapabilityEventCallback");
        mRcsConnectedState = RCS_STATE_CONNECTING;
        mRcsFeatureManager = manager;
        if (mRcsFeatureManager != null) {
            mRcsFeatureManager.addCapabilityEventCallback(mCapabilityEventListener);
        } else {
            logw("setupCapabilityEventCallback: RcsFeatureManager is null");
        }
    }
    ...
    /**
     * Remove the capability event callback synchronously.
     */
    public void removeCapabilityEventCallback() {
        if (!Flags.synchronousRcsUceCallbackRegistration()) {
            return;
        }
        logi("removeCapabilityEventCallback");
        mRcsConnectedState = RCS_STATE_DISCONNECTED;
        if (mRcsFeatureManager != null) {
