...
public class ImsManager implements FeatureUpdates {
    ...
    /**
     * Indicates whether VoNR is provisioned on this slot.
     */
    private boolean isVonrProvisionedOnDevice() {
        if (!Flags.enableVonrProvisioningStatus()) {
            return true;
        }
        if (isMmTelProvisioningRequired(CAPABILITY_TYPE_VOICE, REGISTRATION_TECH_NR)) {
            return isVonrProvisioned();
        }
    ...
    private void reevaluateCapabilities() throws ImsException {
        ...
        boolean isImsNeededForRtt =
                updateRttConfigValue()
                        && isActiveSubscriptionPresent()
                        && isImsVoiceNeeded(request);
    ...
    /**
     * @return {@code true} if IMS needs to be turned on for voice in the request.
     */
    private boolean isImsVoiceNeeded(CapabilityChangeRequest request) {
        if (request == null || request.getCapabilitiesToEnable() == null) {
            return false;
        }
        for (CapabilityChangeRequest.CapabilityPair c : request.getCapabilitiesToEnable()) {
            if (c.getCapability() == MmTelFeature.MmTelCapabilities.CAPABILITY_TYPE_VOICE) {
                return true;
            }
        }
        return false;
    ...
    private void updateVoiceCellFeatureValue(CapabilityChangeRequest request, boolean isNonTty) {
        ...
        if (voLteFeatureOn && voNrAvailable && isVonrProvisionedOnDevice()) {
            request.addCapabilitiesToEnableForTech(
                    MmTelFeature.MmTelCapabilities.CAPABILITY_TYPE_VOICE,
    ...
    private boolean isVonrProvisioned() {
        return getImsProvisionedBoolNoException(CAPABILITY_TYPE_VOICE, REGISTRATION_TECH_NR);
    }
    ...
    public boolean shouldOverrideWfcRoamingModeWhileUsingNTN() {
        ...
        if (!isInCarrierRoamingNtnMode()) {
            log("shouldOverrideWfcRoamingModeWhileUsingNTN: not in carrier roaming ntn mode");
            return false;
        }

        boolean overrideWfcRoamingMode = getBooleanCarrierConfig(
                CarrierConfigManager.KEY_OVERRIDE_WFC_ROAMING_MODE_WHILE_USING_NTN_BOOL);
        log("shouldOverrideWfcRoamingModeWhileUsingNTN: overrideWfcRoamingMode="
    ...
    protected boolean isInCarrierRoamingNtnMode() {
        ...
        if (satelliteManager == null) {
            log("isInCarrierRoamingNtnMode: satelliteManager is null");
            return false;
        }

        try {
            boolean isInCarrierRoamingNtnMode = satelliteManager.isInCarrierRoamingNtnMode(
                    SubscriptionManager.getSubscriptionId(mPhoneId));
            log("isInCarrierRoamingNtnMode: " + isInCarrierRoamingNtnMode);
            return isInCarrierRoamingNtnMode;
        } catch (IllegalStateException | IllegalArgumentException ex) {
            loge("isInCarrierRoamingNtnMode: ex=" + ex);
        }

        log("isInCarrierRoamingNtnMode: returning false");
        return false;
    }
    ...
    private boolean getOverrideWfcRoamingModeWhileUsingNtnDeviceConfig() {
        ...
        try {
            ...
            log("getOverrideWfcRoamingModeWhileUsingNtnDeviceConfig: " + overrideWfcRoamingMode);
        } catch (Resources.NotFoundException ex) {
            loge("getOverrideWfcRoamingModeWhileUsingNtnDeviceConfig: ex=" + ex);
        }
        log("getOverrideWfcRoamingModeWhileUsingNtnDeviceConfig: returning "
                + overrideWfcRoamingMode);
        return overrideWfcRoamingMode;
    }
}
