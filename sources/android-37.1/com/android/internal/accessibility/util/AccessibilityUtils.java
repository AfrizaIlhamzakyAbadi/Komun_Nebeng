...
public final class AccessibilityUtils {
    ...
    /**
     * Loads the label of a component from a {@link PackageItemInfo}, optionally making it safe for
     * presentation based on flag.
     */
    public static CharSequence loadLabel(PackageManager pm, PackageItemInfo packageItemInfo) {
        if (packageItemInfo == null || pm == null) {
            return "";
        }
        CharSequence label = packageItemInfo.loadLabel(pm);
        if (android.view.accessibility.Flags.dialogForceSafeLabel()) {
            return makeSafeLabel(label);
        }
        return label;
    }

    /**
     * Loads the label of a component from a {@link ResolveInfo}, optionally making it safe for
     * presentation based on flag.
     */
    public static CharSequence loadLabel(PackageManager pm, ResolveInfo resolveInfo) {
        if (resolveInfo == null || pm == null) {
            return "";
        }
    ...
    private static CharSequence makeSafeLabel(CharSequence label) {
        if (label != null) {
            return android.text.TextUtils.makeSafeForPresentation(
                    label.toString(),
                    PackageItemInfo.MAX_SAFE_LABEL_LENGTH,
                    /* ellipsizeDip= */ 0,
                    android.text.TextUtils.SAFE_STRING_FLAG_TRIM
                            | android.text.TextUtils.SAFE_STRING_FLAG_FIRST_LINE);
        }
        return "";
    }
    ...
    public static @AccessibilityFragmentType int getAccessibilityServiceFragmentType(
        ...
        final boolean requireShortcut = android.view.accessibility.Flags.newShortcutLifecycle()
                && (accessibilityServiceInfo.flags & AccessibilityServiceInfo.FLAG_REQUIRE_SHORTCUT)
                != 0;

        if (targetSdk <= Build.VERSION_CODES.Q) {
            return AccessibilityFragmentType.VOLUME_SHORTCUT_TOGGLE;
        }
        return (requestA11yButton || requireShortcut)
