...
public class FrameTracker implements SurfaceControl.OnJankDataListener {
    ...
    /**
     * Sets whether to copy the monitored {@link SurfaceControl} to prevent crashes if the owner
     * releases the original surface before monitoring completes. This only takes effect when the
     * tracker is initialized in surface-only mode.
     */
    public static void setCopySurfaceControlEnabled(boolean enabled) {
        sCopySurfaceControlEnabled = enabled;
    }
    ...
    private static class JankInfo {
        ...
        JankInfo update(SurfaceControl.JankData jankStat) {
            ...
            this.actualPresentDelta = jankStat.getActualPresentDeltaNanos();
    ...
    public FrameTracker(@NonNull Configuration config,
        ...
        if (mSurfaceOnly) {
            if (sCopySurfaceControlEnabled) {
                mSurfaceControl = new SurfaceControl(config.getSurfaceControl(), "FrameTracker");
                mIsSurfaceControlCopied = true;
            } else {
    ...
    private void finishTraced() {
        ...
        for (int i = 0; i < mJankInfos.size(); i++) {
            ...
            if ((jankType & (JANK_APPLICATION | JANK_COMPOSER)) != 0) {
                totalAnimationTime += info.actualPresentDelta;
        ...
        if (Trace.isTagEnabled(TRACE_TAG_APP)) {
            ...
            Trace.traceCounter(TRACE_TAG_APP, name + "#weightedJank",
                    (int) (totalWeightedJank / totalAnimationTime * NANOS_IN_SECOND * 1000));
    ...
    public void removeObservers() {
        ...
        if (mIsSurfaceControlCopied) {
            mSurfaceControl.release();
            mIsSurfaceControlCopied = false;
        }
    }
