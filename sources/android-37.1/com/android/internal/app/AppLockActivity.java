...
public class AppLockActivity extends Activity {
    ...
    private void showSetupScreenLockDialog(String packageName) {
        ...
        if (mScrollView != null) {
            ...
            Slog.w(TAG, "mScrollView for add screen lock dialog is null, cannot toggle"
                    + " divider visibility.");
        }
        ...
        final BiometricPrompt.AuthenticationCallback authenticationCallback =
                new BiometricPrompt.AuthenticationCallback() {
                    ...
                    public void onAuthenticationSucceeded(
                        ...
                        if (!showSettingsPromoIfNeeded(success, newAppLockEnabled)) {
                            if (DEBUG) {
                                Slog.d(TAG, "Finishing, promo was not shown");
                            }
                            finish();
                        }
                    }
    ...
    /**
     * Checks the current discovery state and displays the promotion dialog if the user has enabled
     * App Lock for enough apps to meet the
     * {@link Settings.Secure#APP_LOCK_SETTINGS_DISCOVERED_STATE_THRESHOLD}.
     *
     * @return {@code true} if the settings promotion dialog was shown, {@code false} otherwise. If
     *         {@code true}, the caller must not finish the activity to allow user interaction with
     *         the dialog.
     */
    private boolean showSettingsPromoIfNeeded(boolean success, boolean newAppLockEnabled) {
        if (DEBUG) {
            Slog.d(TAG, "showSettingsPromoIfNeeded() called with success=" + success
                    + ", newAppLockEnabled=" + newAppLockEnabled);
        }
        // Only proceed if the App Lock enablement was successful
        ...
        final int discoveredState = Settings.Secure.getIntForUser(contentResolver,
                Settings.Secure.APP_LOCK_SETTINGS_DISCOVERED,
                Settings.Secure.APP_LOCK_SETTINGS_DISCOVERED_STATE_INITIAL, userId);

        // Stop if the user is already aware of the Settings page
        if (discoveredState == Settings.Secure.APP_LOCK_SETTINGS_DISCOVERED_STATE_DISCOVERED) {
            if (DEBUG) {
                Slog.d(TAG, "App Lock settings already discovered. Not showing promo dialog.");
            }
            return false;
        }

        final int nextState = discoveredState + 1;
        if (nextState >= Settings.Secure.APP_LOCK_SETTINGS_DISCOVERED_STATE_THRESHOLD) {
            if (DEBUG) {
                Slog.d(TAG, "Enablement count threshold reached, showing settings promo dialog.");
            }
            Settings.Secure.putIntForUser(contentResolver,
                    Settings.Secure.APP_LOCK_SETTINGS_DISCOVERED,
                    Settings.Secure.APP_LOCK_SETTINGS_DISCOVERED_STATE_DISCOVERED, userId);
            showSettingsPromoDialog();
            return true;
        } else {
            // Increment enablement count
            if (DEBUG) {
                Slog.d(TAG, "Incrementing APP_LOCK_SETTINGS_DISCOVERED state to "
                        + (nextState));
            }
            Settings.Secure.putIntForUser(contentResolver,
                    Settings.Secure.APP_LOCK_SETTINGS_DISCOVERED, nextState, userId);
            return false;
        }
    }

    /**
     * Inflates the bottom-sheet promotion dialog UI into the current activity.
     */
    private void showSettingsPromoDialog() {
        // Apply a slide-up transition for the promotion dialog to ensure a smooth visual transition
        // after the biometric authentication prompt is dismissed.
        // Disable the transition in test environment.
        ...
        setContentView(R.layout.app_lock_settings_promo_dialog);

        // Dismiss App Lock settings promotion dialog when the transparent background is tapped.
        final View rootLayout = findViewById(R.id.app_lock_promo_activity_layout);
        if (rootLayout instanceof ResolverDrawerLayout) {
            ((ResolverDrawerLayout) rootLayout).setOnDismissListener(this::onBackPressed);
        } else {
            rootLayout.setOnClickListener(view -> onBackPressed());
        }

        // Show the divider only when content is scrollable.
        mScrollView = findViewById(R.id.app_lock_promo_dialog_scroll_view);
        if (mScrollView != null) {
            mGlobalLayoutListener = () -> {
                final boolean canScroll = mScrollView.canScrollVertically(/* direction= */ 1);
                findViewById(R.id.app_lock_promo_dialog_bottom_sheet_divider)
                        .setVisibility(canScroll ? View.VISIBLE : View.GONE);
            };
            mScrollView.getViewTreeObserver().addOnGlobalLayoutListener(mGlobalLayoutListener);
        } else {
            Slog.w(TAG, "mScrollView for promo dialog is null, cannot toggle divider visibility.");
        }

        findViewById(R.id.app_lock_settings_promo_dialog_negative_button)
                .setOnClickListener(view -> {
                    finish();
                });
        findViewById(R.id.app_lock_settings_promo_dialog_positive_button)
                .setOnClickListener(view -> {
                    if (DEBUG) {
                        Slog.d(TAG, "Launching App Lock settings page.");
                    }
                    final Intent intent = new Intent(Settings.ACTION_APP_LOCK_SETTINGS);
