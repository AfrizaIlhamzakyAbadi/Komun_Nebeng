...
import static android.app.supervision.SupervisionManager.SUPERVISION_SYSTEM_ENTITY;
...
public class SuspendedAppActivity extends AlertActivity
    ...
    private static final String ACTION_MANAGE_SUPERVISION_APP_LIMITS =
            "android.settings.action.MANAGE_SUPERVISION_APP_LIMITS";
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (mSuppliedDialogInfo != null && !isSystemEntity(mSuspendingPackage)) {
            try {
    ...
    public void onClick(DialogInterface dialog, int which) {
        ...
        if (mSuspendingPackage != null && !isSystemEntity(mSuspendingPackage)) {
    ...
    private Intent getFallbackIntent() {
        if (!isSystemEntity(mSuspendingPackage)) {
            return null;
        }
        final Intent supervisionSettingsIntent = new Intent(ACTION_MANAGE_SUPERVISION_APP_LIMITS);
        final ResolveInfo resolvedInfo =
                mPm.resolveActivityAsUser(
                        supervisionSettingsIntent,
                        MATCH_DIRECT_BOOT_UNAWARE | MATCH_DIRECT_BOOT_AWARE,
                        mUserId);
        if (resolvedInfo != null
                && resolvedInfo.activityInfo != null
                && resolvedInfo.activityInfo.applicationInfo != null
                && resolvedInfo.activityInfo.applicationInfo.isSystemApp()) {
            supervisionSettingsIntent.setComponent(
                    new ComponentName(
                            resolvedInfo.activityInfo.packageName, resolvedInfo.activityInfo.name));
            return supervisionSettingsIntent;
        }
        return null;
    }

    private static boolean isSystemEntity(String suspendingPackage) {
        return SUPERVISION_SYSTEM_ENTITY.equals(suspendingPackage);
    }
}
