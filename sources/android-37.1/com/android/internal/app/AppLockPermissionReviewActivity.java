...
public class AppLockPermissionReviewActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mScrollView = findViewById(R.id.app_lock_permission_review_scroll_view);
        if (mScrollView != null) {
            mGlobalLayoutListener = () -> {
                final boolean canScrollParent = mScrollView.canScrollVertically(/* direction= */ 1)
                        || mScrollView.canScrollVertically(/* direction= */ -1);
                final boolean canScrollRecycler =
                        recyclerView.canScrollVertically(/* direction= */ 1)
                        || recyclerView.canScrollVertically(/* direction= */ -1);

                recyclerView.setVerticalScrollBarEnabled(!canScrollParent);
                findViewById(R.id.app_lock_permission_review_bottom_sheet_divider).setVisibility(
                        canScrollParent || canScrollRecycler ? View.VISIBLE : View.GONE);
            };
