 ...
 * authentication lifecycle, including requiring the Keyguard to be dismissed if active, showing the
 * {@link BiometricPrompt}, processing the result, and performing the requested action upon success.
 *
 ...
 *         identified by {@link Intent#EXTRA_INTENT} being non {@code null} and
 *         {@link #EXTRA_ACTIVITY_MODE} being set to {@code null}.
 ...
 *         {@link Intent#EXTRA_INTENT} and {@link #EXTRA_ACTIVITY_MODE} being set to {@code null}.
 ...
 *         identified by {@link #EXTRA_ACTIVITY_MODE} being set to {@link ActivityMode#UNINSTALL}.
 *     <li><b>Clear Storage Mode:</b> The activity acts as a transparent overlay that intercepts the
 *         clear storage attempt of an App Lock enabled app. It immediately shows a
 *         {@link BiometricPrompt} and upon successful authentication, it sets the result to
 *         {@link android.app.Activity#RESULT_OK} and finishes, allowing the caller to proceed with
 *         clearing the app storage. This mode is identified by {@link #EXTRA_ACTIVITY_MODE}
 *         being set to {@link ActivityMode#CLEAR_STORAGE}.
 * </ul>
 *
 ...
 *         while the activity is visible (this does not apply to uninstall and clear storage modes,
 *         which always requires user confirmation).</li>
...
public final class LockedAppActivity extends Activity {
    ...
    public static final String EXTRA_ACTIVITY_MODE = "com.android.internal.app.extra.ACTIVITY_MODE";
    ...
    protected enum ActivityMode {
        ...
        @Nullable
        static ActivityMode fromInt(int value) {
    ...
    private final BiometricPrompt.AuthenticationCallback mAuthenticationCallback =
            new BiometricPrompt.AuthenticationCallback() {
                ...
                public void onAuthenticationSucceeded(BiometricPrompt.AuthenticationResult result) {
                    ...
                    mIsAuthFlowActive = false;
                    if (isClearStorageMode()) {
                        setResult(RESULT_OK);
                        finish();
                        return;
                    }
                ...
                public void onAuthenticationError(int errorCode, CharSequence errString) {
                    ...
                    // In intercept and clear storage mode, finish the activity to unblock the UI.
                    if (isOneShotAuthMode() || isInterceptMode()) {
                        finish();
                    }
                }
            };
    ...
    /**
     * Tracks whether an authentication flow is currently active. This covers the entire
     * authentication sequence, including the Keyguard dismissal bouncer (if active) and the
    ...
    public static Intent createLockedAppActivityUninstallIntent(
        ...
        return createLockedAppActivityIntent(
                ...
                .putExtra(EXTRA_ACTIVITY_MODE, ActivityMode.UNINSTALL.ordinal())
    ...
    /**
     * Creates an {@link Intent} to launch {@link LockedAppActivity} for the purpose of
     * authenticating a clear app storage request.
     *
     * <p>Upon successful authentication, the activity will set the result to
     * {@link android.app.Activity#RESULT_OK} and finish, allowing the caller to proceed with
     * clearing the app storage.
     *
     * @param packageName the name of the package to unlock.
     * @param userId      the user ID for which the package is locked.
     * @return a configured {@link Intent} to start {@link LockedAppActivity}.
     */
    public static Intent createClearStorageAuthIntent(String packageName, int userId) {
        return createLockedAppActivityIntent(packageName, userId, /* target= */ null)
                .putExtra(EXTRA_ACTIVITY_MODE, ActivityMode.CLEAR_STORAGE.ordinal());
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // In uninstall and clear storage mode, we always want to confirm with the user.
        if (!isOneShotAuthMode() && finishIfUnlocked(mPackageName, mUserId)) {
            return;
        }
        ...
        if (!isOneShotAuthMode()) {
            // Register locked state listener. This is unnecessary for uninstall and clear storage
            // modes because we do not unlock the package upon successful authentication.
        ...
        if (isInterceptMode() || isOneShotAuthMode()) {
            // In intercept, uninstall, and clear storage mode, show the BiometricPrompt
            // immediately.
            showBiometricPrompt();
        } else {
    ...
    private void initStateFromIntent() {
        ...
        ActivityMode implicitMode = mTarget != null ? ActivityMode.INTERCEPT
                : ActivityMode.LOCKED_TASK;
        int modeInt = intent.getIntExtra(EXTRA_ACTIVITY_MODE, implicitMode.ordinal());
        mActivityMode = ActivityMode.fromInt(modeInt);

        if (mActivityMode == null) {
            Slog.e(TAG, "Invalid activity mode provided: " + modeInt + ", finishing.");
            finish();
            return;
        }
        ...
        if (DEBUG) {
            Slog.d(TAG, "In " + mActivityMode.name() + " mode");
        }
    }
     ...
     * <ul>
     *     <li>In intercept, uninstall and clear storage mode, a transparent panel theme is used to
     *         overlay the locked app.</li>
     *     <li>In locked task mode, a standard theme with no action bar is used.</li>
     * </ul>
     */
    private void applyTheme() {
        mInjector.setTheme(this, (isInterceptMode() || isOneShotAuthMode())
    ...
    /**
     * Configures the user interface based on the current mode (intercept, uninstall, clear storage,
     * or locked task). This must be called after {@code super.onCreate()} and in
    ...
    private boolean setupUi() {
        if (isInterceptMode() || isOneShotAuthMode()) {
            // In intercept, uninstall, and clear storage modes, ensure the activity is translucent.
    ...
    private boolean isInterceptMode() {
        return mActivityMode == ActivityMode.INTERCEPT;
    }

    private boolean isLockedTaskMode() {
        return mActivityMode == ActivityMode.LOCKED_TASK;
    }

    private boolean isUninstallMode() {
        return mActivityMode == ActivityMode.UNINSTALL;
    }

    private boolean isClearStorageMode() {
        return mActivityMode == ActivityMode.CLEAR_STORAGE;
    }

    private boolean isOneShotAuthMode() {
        return isUninstallMode() || isClearStorageMode();
    }
    ...
    public void onResume() {
        super.onResume();
        if (DEBUG) {
            Slog.d(TAG, "onResume: " + mActivityMode.name() + " mode");
        }
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        // In uninstall and clear storage modes, always require user confirmation. Otherwise, finish
        // if the package is no longer locked upon focus change. For other modes, the
        // BiometricPrompt should only be shown if the package is locked and the activity has window
        // focus.
        if (mPackageName == null) {
    ...
    private void maybeShowBiometricPromptForLockedTask() {
        if (!isLockedTaskMode()) {
            if (DEBUG) {
                Slog.d(TAG, "maybeShowBiometricPromptForLockedTask: " + mActivityMode.name()
                        + " mode, returning");
            }
            return;
        }
        ...
        if (mIsAuthFlowActive) {
            if (DEBUG) {
                Slog.d(TAG, "maybeShowBiometricPromptForLockedTask: auth flow is already active,"
                        + " returning");
            }
            return;
        }
    ...
    private void showBiometricPrompt() {
        final KeyguardManager km = mInjector.getKeyguardManager(this);
        if (km != null && km.isKeyguardLocked()) {
            mIsAuthFlowActive = true;
            km.requestDismissKeyguard(this, new AppLockKeyguardDismissCallback());
            return;
        }
        ...
        biometricPrompt.authenticateUser(mCancellationSignal, getMainExecutor(),
                mAuthenticationCallback, mUserId);
        mIsAuthFlowActive = true;
    }

    private void onKeyguardDismissFailed(String errorMsg) {
        if (DEBUG) {
            Slog.d(TAG, "Keyguard dismiss failed: " + errorMsg);
        }
        mIsAuthFlowActive = false;
        if (isUninstallMode()) {
            sendUninstallFailure(PackageInstaller.STATUS_FAILURE_ABORTED, errorMsg);
            finish();
            return;
        }
        // In intercept and clear storage mode, finish the activity to unblock the UI.
        // In locked task mode, we do not finish so that the activity remains active as an overlay
        // blocking access to the task.
        if (isOneShotAuthMode() || isInterceptMode()) {
            finish();
        }
    }
    ...
    private class AppLockKeyguardDismissCallback extends KeyguardManager.KeyguardDismissCallback {
        @Override
        public void onDismissSucceeded() {
            if (isFinishing() || isDestroyed()) {
                if (DEBUG) {
                    Slog.d(TAG, "Keyguard dismiss succeeded, but the activity is finishing or "
                            + "destroyed, returning");
                }
                return;
            }
            if (DEBUG) {
                Slog.d(TAG, "Keyguard dismiss succeeded, showing biometric prompt");
            }
            // Reset mIsAuthFlowActive so that showBiometricPrompt() can proceed to
            // authenticate and correctly track the active auth flow state.
            mIsAuthFlowActive = false;
            showBiometricPrompt();
        }
        ...
        public void onDismissCancelled() {
            onKeyguardDismissFailed("Keyguard dismiss cancelled");
        }

        @Override
        public void onDismissError() {
            onKeyguardDismissFailed("Keyguard dismiss error");
        }
    }
    ...
    public static class Injector {
        ...
        /**
         * Returns the {@link KeyguardManager} for the given activity.
         *
         * @param activity the activity to get the keyguard manager from.
         * @return the keyguard manager, or {@code null} if not available.
         */
        @Nullable
        public KeyguardManager getKeyguardManager(Activity activity) {
