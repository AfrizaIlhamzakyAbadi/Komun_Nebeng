...
public class NotificationRowIconView extends CachingIconView {
    ...
    /**
     * Represents the themed launcher icon with a transparent background. The foregound coloring
     * depends on the device theme.
     */
                ...
                case ICON_TYPE_BRIDGED_ICON -> {
                    ...
                    if (bridgedIcon == null) {
                        Log.w(TAG, "Bridged notification metadata missing icon, falling back to"
                                + " launcher icon");
                        yield mIconProvider.getLauncherIcon(/* transparent= */ false);
                    }
                    yield bridgedIcon;
                }
                case ICON_TYPE_LAUNCHER_ICON -> {
                    yield mIconProvider.getLauncherIcon(/* transparent= */ false);
                }
                case ICON_TYPE_TRANSPARENT_LAUNCHER_ICON -> {
                    yield mIconProvider.getLauncherIcon(/* transparent= */ true);
                }
                default -> null;
            };
        } else {
            if (mIconProvider != null && mIconProvider.getIconType() == ICON_TYPE_LAUNCHER_ICON) {
                return mIconProvider.getLauncherIcon(/* transparent= */ false);
            }
            return null;
        }
    }
    ...
    /**
     * Manually set which type of icon should be used for the View. See IconType for possible
     * icon types.
    ...
    public interface NotificationIconProvider {
         ...
         */
        @Nullable Drawable getLauncherIcon(boolean transparent);
    }
}
