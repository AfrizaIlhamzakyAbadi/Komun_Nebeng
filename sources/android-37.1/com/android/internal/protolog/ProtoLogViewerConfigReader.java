...
import static com.android.internal.tracing.winscope.protos.Protolog.ProtoLogViewerConfig.MessageData.MESSAGE_ID;
...
public class ProtoLogViewerConfigReader {
    ...
    @NonNull
    private volatile LongSparseArray<String> mLogMessageMap = new LongSparseArray<>();
    ...
    public synchronized void loadViewerConfig(@NonNull String[] groups, @NonNull ILogger logger) {
        LongSparseArray<String> newLogMessageMap = mLogMessageMap.clone();
        ...
        for (String group : groups) {
            ...
            try {
                ...
                for (Long key : mappings.keySet()) {
                    newLogMessageMap.put(key, mappings.get(key));
                }
                modified = true;
                logger.log("Loaded " + mappings.size() + " log definitions for group " + group);
            } catch (IOException e) {
        ...
        if (modified) {
            mLogMessageMap = newLogMessageMap;
        }
    }
