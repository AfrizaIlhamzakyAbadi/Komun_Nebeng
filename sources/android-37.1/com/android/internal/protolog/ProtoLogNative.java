...
public final class ProtoLogNative {
    ...
    private static final String LOG_TAG = "ProtoLogNative";
    ...
    public static synchronized void init() {
        init(PERFETTO_BACKEND_SYSTEM);
    }

    /**
     * Initialize the native implementation with a specific backend mask.
     *
     * @param backends The backend mask to use for initialization.
     */
    public static synchronized void init(int backends) {
        if (sInitialized) {
            Log.w(LOG_TAG, "ProtoLogNative already initialized in this process.");
            return;
        }
        sInitialized = true;
        nativeInit(backends);
    ...
    private static native void nativeInit(int backends);
    ...
    /**
     * Set whether logging calls on the current thread should route to the test datasource.
     *
     * @param active True to route current thread to test datasource.
     */
    public static native void setThreadLocalTestDataSourceActive(boolean active);

    /**
     * Listener for native data source events.
     */
    public interface NativeDataSourceListener {
        /**
         * Called when a tracing instance starts.
         *
         * @param dsName      The name of the DataSource.
         * @param instanceIdx The instance index.
         * @param config      The tracing configuration.
         */
        void onTracingInstanceStart(String dsName, int instanceIdx, byte[] config);

        /**
         * Called when a tracing instance stops.
         *
         * @param dsName      The name of the DataSource.
         * @param instanceIdx The instance index.
         * @param config      The tracing configuration.
         */
        void onTracingInstanceStop(String dsName, int instanceIdx, byte[] config);
    }

    private static final Set<NativeDataSourceListener> sListeners = new CopyOnWriteArraySet<>();

    /**
     * Register a DataSource event listener.
     * @param listener The listener.
     */
    public static void registerListener(NativeDataSourceListener listener) {
        sListeners.add(listener);
    }

    /**
     * Unregister a DataSource event listener.
     * @param listener The listener.
     */
    public static void unregisterListener(NativeDataSourceListener listener) {
        sListeners.remove(listener);
    }

    /**
     * Called from JNI when a tracing instance starts.
     *
     * @param dsName      The name of the DataSource.
     * @param instanceIdx The instance index.
     * @param config      The tracing configuration.
     */
    public static void onTracingInstanceStart(String dsName, int instanceIdx, byte[] config) {
        for (NativeDataSourceListener listener : sListeners) {
            listener.onTracingInstanceStart(dsName, instanceIdx, config);
        }
    }

    /**
     * Called from JNI when a tracing instance stops.
     *
     * @param dsName      The name of the DataSource.
     * @param instanceIdx The instance index.
     * @param config      The tracing configuration.
     */
    public static void onTracingInstanceStop(String dsName, int instanceIdx, byte[] config) {
        for (NativeDataSourceListener listener : sListeners) {
            listener.onTracingInstanceStop(dsName, instanceIdx, config);
        }
    }
}
