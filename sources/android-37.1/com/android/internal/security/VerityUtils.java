...
public abstract class VerityUtils {
    ...
    /**
     * Computes the expected fs-verity file digest from a standardized .fsv_meta sidecar container.
     * To match the Linux kernel's measurement (FS_IOC_MEASURE_VERITY), this reads the entire
     * 256-byte fsverity_descriptor structure and hashes it with SHA-256.
     *
     * See packages/modules/Virtualization/guest/authfs/src/fsverity/metadata/metadata.hpp
     * for more details about the .fsv_meta layout.
     */
    public static @Nullable byte[] getFsverityDigestFromFsvMeta(@NonNull String metaFilePath) {
        try (java.io.RandomAccessFile raf = new java.io.RandomAccessFile(metaFilePath, "r")) {
            // Minimum size to hold the 4-byte version header + 256-byte descriptor
            if (raf.length() < 260) return null;

            // Skip the 4-byte format version header (at offset 0x0000)
            raf.seek(4);

            // Read the entire 256-byte fsverity_descriptor
            byte[] descriptor = new byte[256];
            raf.readFully(descriptor);

            // Return the exact measurement the kernel produces: SHA256(descriptor)
            return java.security.MessageDigest.getInstance("SHA-256").digest(descriptor);
        } catch (Exception e) {
            return null;
        }
    }
