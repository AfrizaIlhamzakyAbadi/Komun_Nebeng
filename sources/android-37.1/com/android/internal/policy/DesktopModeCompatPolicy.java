...
public class DesktopModeCompatPolicy {
    ...
    /**
     * @see #isTopActivityExemptFromDesktopWindowing(WindowingExemptionsInfo)
     */
    public boolean isTopActivityExemptFromDesktopWindowing(@NonNull TaskInfo task) {
        return isTopActivityExemptFromDesktopWindowing(WindowingExemptionsInfo.fromTaskInfo(task));
    }
     ...
     * environment.
     */
    public boolean isTopActivityExemptFromDesktopWindowing(@NonNull WindowingExemptionsInfo info) {
        if (isHeadlessSystemUserMode() && info.getUserId() == USER_SYSTEM) {
            // An activity for system user in HSUM should not activate desktop windowing.
            return true;
        }
        final String packageName = info.getBaseActivity() != null
                ? info.getBaseActivity().getPackageName() : null;
        final String className = info.getBaseActivity() != null
                ? info.getBaseActivity().getClassName() : null;
        if (packageName == null) {
            return false;
        }
        // If activity is not being displayed, window mode change has no visual affect so leave
        // unchanged.
        if (info.isTopActivityNoDisplay()) {
            return false;
        }
        // Dream and assistant activities should be fullscreen and thus should be forced out of
        // desktop.
        if (info.getTopActivityType() == ACTIVITY_TYPE_DREAM
                || info.getTopActivityType() == ACTIVITY_TYPE_ASSISTANT) {
            return true;
        }
        // If activity belongs to package exempt via device config, force out of desktop.
        if (isPackageExemptViaConfig(packageName) && !info.isActivityStackTransparent()) {
            return true;
        }
        ...
        if (isPartOfDefaultHomePackageOrNoHomeAvailable(packageName, info.getUserId(), className)) {
            return true;
        }
        ...
        return isTransparentTask(info.isActivityStackTransparent(), info.getNumActivities())
                && (hasFullscreenTransparentPermission(packageName, info.getUserId(),
                        info.getTopActivityInfo())
                    || hasPlatformSignature(info.getTopActivityInfo())
                    || (Flags.enablePrivilegedAppTransparentWindowingExemptions()
                        && isPrivilegedApp(info.getTopActivityInfo())))
                && !mConfigTransparentExemptionIgnoreList.contains(packageName);
    }

    /**
     * @see #shouldDisableDesktopEntryPoints(WindowingExemptionsInfo)
     */
    public boolean shouldDisableDesktopEntryPoints(@NonNull TaskInfo task) {
        return shouldDisableDesktopEntryPoints(WindowingExemptionsInfo.fromTaskInfo(task));
    }
     ...
     */
    public boolean shouldDisableDesktopEntryPoints(@NonNull WindowingExemptionsInfo info) {
        final String packageName = info.getBaseActivity() != null
                ? info.getBaseActivity().getPackageName() : null;
        final String className = info.getBaseActivity() != null
                ? info.getBaseActivity().getClassName() : null;
    ...
    public boolean isPackageLaunchInFullscreen(@Nullable ActivityInfo activityInfo) {
        if (!Flags.launchGamesInFullscreenByAllowlist()) return false;
        if (activityInfo == null) return false;
        if (activityInfo.isChangeEnabled(ActivityInfo.FORCE_LAUNCH_FULLSCREEN_FROM_DESKTOP)) {
            return true;
        }
