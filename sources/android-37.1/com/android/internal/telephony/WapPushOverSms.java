...
public class WapPushOverSms implements ServiceConnection {
    ...
    private DecodedResult decodeWapPdu(byte[] pdu, InboundSmsHandler handler) {
        ...
        try {
            ...
            long rawHeaderLength = pduDecoder.getValue32();
            index += pduDecoder.getDecodedDataLength();

            int headerStartIndex = index;

            if (rawHeaderLength < 0 || rawHeaderLength > Integer.MAX_VALUE
                    || headerStartIndex + rawHeaderLength > pdu.length) {
                if (DBG) Rlog.w(TAG, "Received PDU. Invalid header length: " + rawHeaderLength);
                result.statusCode = Intents.RESULT_SMS_GENERIC_ERROR;
                return result;
            }
