...
public class ServiceStateTracker extends Handler {
    ...
    private CarrierDisplayNameData getCarrierDisplayNameLegacy() {
        ...
        if (!TextUtils.isEmpty(satellitePlmn)) {
            ...
            showSpn = true;
            showPlmn = false;
        } else if (!TextUtils.isEmpty(wfcVoiceSpnFormat)) {
            // Show PLMN + Wi-Fi Calling if there is no valid SPN in the above case
            String originalPlmn = TextUtils.emptyIfNull(plmn).trim();

            if (mIccRecords != null && mCarrierConfig.getBoolean(
                    CarrierConfigManager.KEY_WFC_CARRIER_NAME_OVERRIDE_BY_PNN_BOOL)) {
                String pnnName = mIccRecords.getPnnHomeName();
                if (!TextUtils.isEmpty(pnnName)) {
                    originalPlmn = pnnName;
                }
            }

            if (!TextUtils.isEmpty(originalPlmn) || !wfcVoiceSpnFormat.contains("%s")) {
    ...
    private void updateSatelliteDisplayOverride() {
        ...
        if (satelliteModemStateListener != null
                ...
                || (!TextUtils.isEmpty(operator)
                        && sc != null && sc.isInCarrierRoamingNbIotNtn(mPhone))) {
    ...
    public boolean showOperatorName() {
        ...
        if (mFeatureFlags.updateCarrierNameAfterSimReady()) {
            // If the flag is already set, or if records are loaded currently, allow the name.
            boolean recordsLoaded = mIccRecords != null && mIccRecords.getRecordsLoaded();
            if (!mIsSimReadyForDisplay && !recordsLoaded) {
                return false; // Only discard if records are truly not ready.
            }
        }
