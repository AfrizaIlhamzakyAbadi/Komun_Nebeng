...
public abstract class ValueParser {
    ...
    /**
     * Retrieves cell broadcast service data coding scheme from the Text COMPREHENSION-TLV object.
     *
     * @param ctlv A Text COMPREHENSION-TLV object
     ...
     */
    public static byte retrieveCellBroadcastCodingScheme(ComprehensionTlv ctlv)
            throws ResultException {
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();

        try {
            int dcs = rawValue[valueIndex] & 0xFF;
            int codingGroup = dcs >> 4;

            if (codingGroup == 0x00) {
                return 0x00;  // GSM 7-bit packed
            } else if (codingGroup == 0x01) {
                int alphabet = dcs & 0x0F;
                if (alphabet == 0x00) {
                    return 0x00;  // GSM 7-bit packed
                } else if (alphabet == 0x01) {
                    return 0x08; // UCS2
                }
            } else if (codingGroup == 0x02) {
                int language = dcs & 0x0F;
                if (language >= 0x01 && language <= 0x03) {
                    return 0x08; // UCS2
                }
            } else if (codingGroup >= 0x04 && codingGroup <= 0x07) {
                if ((dcs & 0x20) != 0) {
                    throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD,
                            "Doesn't support compression");
                }
                int alphabet = (dcs >> 2) & 0x03;
                if (alphabet == 0) {
                    return 0x00;  // GSM 7-bit packed
                } else if (alphabet == 1) {
                    return 0x04; // 8-bit data
                } else if (alphabet == 2) {
                    return 0x08; // UCS2
                }
            } else if (codingGroup == 0x09) {
                throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD,
                        "Unsupported message type for UDH");
            } else if (codingGroup == 0x0F) {
                int alphabet = (dcs >> 2) & 0x01;
                if (alphabet == 0) {
                    return 0x00;  // GSM 7-bit packed
                } else if (alphabet == 1) {
                    return 0x04; // 8-bit data
            ...
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD, "Unsupported DCS");
        } catch (IndexOutOfBoundsException e) {
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
        }
    }
    ...
    /**
     * Retrieves text from the USSD STRING COMPREHENSION-TLV object, and decodes it
     * into a String.
     *
     * @param ctlv A USSD_STRING COMPREHENSION-TLV object
     * @param codingScheme The coding scheme to be used for the decoding
     * @return A String object decoded from the USSD STRING command
     * @throws ResultException if the format is not supported or invalid
     */
    static String retrieveUssdString(ComprehensionTlv ctlv, byte codingScheme)
            throws ResultException {
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();
        String text = null;
        ...
        try {
            ...
            if (codingScheme == 0x00) { // GSM 7-bit packed
                text = GsmAlphabet.gsm7BitPackedToString(rawValue,
                        valueIndex + 1, (textLen * 8) / 7);
                if (codingGroup == 0x01 && (dcs & 0x0F) == 0x00 && text != null) {
                    text = text.length() >= 3 ? text.substring(3) : "";
                }
            } else if (codingScheme == 0x04) { // 8-bit data
                text = GsmAlphabet.gsm8BitUnpackedToString(rawValue,
                        valueIndex + 1, textLen);
            } else if (codingScheme == 0x08) { // UCS2
                int payloadIndex = valueIndex + 1;
                int payloadLen = textLen;
                if (codingGroup == 0x01) {
                    payloadIndex += 2;
                    payloadLen -= 2;
                }
                text = payloadLen > 0
                        ? new String(rawValue, payloadIndex, payloadLen, "UTF-16")
                        : "";
            } else {
                throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
            }
