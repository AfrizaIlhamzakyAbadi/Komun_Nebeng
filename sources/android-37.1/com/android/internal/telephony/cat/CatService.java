...
public class CatService extends Handler implements AppInterface {
    ...
    private boolean mSupportStkSmsTpMrHandling = false;
    ...
    static final int MSG_ID_NOTIFY_SEND_USSD_RESULT  = 12;
    ...
    private SendUssdCommandHandler mSendUssdHandler = null;
    ...
    private CatService(CommandsInterface ci, UiccCardApplication ca, IccRecords ir,
        ...
        try {
            mSupportStkSmsTpMrHandling = mContext.getResources().getBoolean(
                    com.android.internal.R.bool.config_stk_sms_tp_mr_handling);
        } catch (NotFoundException e) {
            CatLog.e(this, "config_stk_sms_tp_mr_handling resource is not found");
        }
        ...
        switch (cmdParams.getCommandType()) {
            ...
            case SEND_SMS:
                ...
                if (cmdParams instanceof SendSMSParams) {
                    ...
                    // Execution must continue to broadcast the intent to the STK application to
                    // display the alpha identifier popup to the user.
                } else {
    ...
    public void sendStkSms(SendSMSParams cmdParams) {
        ...
        int pendingIntentFlags = Flags.stkSmsPacking()
        ...
        if (mFeatureFlags.stkSmsTpMrHandling() && mSupportStkSmsTpMrHandling) {
            Phone phone = PhoneFactory.getPhone(mSlotId);
            // TODO(b/517547666): Create an API to get the next message reference.
            SmsDispatchersController sdc = phone != null
                    && phone.getIccSmsInterfaceManager() != null
                    ? phone.getIccSmsInterfaceManager().mDispatchersController : null;
            if (sdc != null && sdc.is3gppFormatAndSmsSubmitOrCommandPdu(cmdParams.mRawTpdu)
                    && sdc.getGsmDispatcher() != null) {
                cmdParams.mRawTpdu[1] = (byte) sdc.getGsmDispatcher().nextMessageRef();
            }
        }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case MSG_ID_NOTIFY_COMMAND_RESULT:
                ...
                mCurrntCmd = null;
                break;
            case MSG_ID_NOTIFY_SEND_USSD_RESULT: {
                if (msg.obj instanceof SendUssdCommandHandler.UssdResult) {
                    SendUssdCommandHandler.UssdResult result =
                            (SendUssdCommandHandler.UssdResult) msg.obj;
                    sendTerminalResponse(
                            result.cmdDet, result.resultCode, false, 0x00, result.resp);
                }
                mSendUssdHandler = null;
                mCurrntCmd = null;
                break;
            }
            default:
    ...
    public void sendUssd(CommandDetails cmdDet, String request, byte codingScheme) {
        ...
        if (mSendUssdHandler != null) {
            CatLog.e(this, "Already handling another command");
            sendTerminalResponse(cmdDet, ResultCode.TERMINAL_CRNTLY_UNABLE_TO_PROCESS,
                    false, 0x00, null);
            return;
        }

        mSendUssdHandler = new SendUssdCommandHandler(
                getLooper(), this, mContext, subInfo.getSubscriptionId(), mSlotId);
        mSendUssdHandler.start(cmdDet, request, codingScheme);
    }
}
