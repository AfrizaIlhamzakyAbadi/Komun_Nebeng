...
public class CommandParamsFactory extends Handler {
    ...
    // Constants for SMS packing
    private static final int SEND_SMS_QUALIFIER_PACKING_REQUIRED = 0x01;
    private static final int TP_DCS_7_BIT_DEFAULT = 0xF0;
    private static final int TP_VPF_MASK = 0x18;
    private static final int TP_VPF_RELATIVE = 2;
    private static final int TP_VPF_ENHANCED = 1;
    private static final int TP_VPF_ABSOLUTE = 3;
    ...
    public boolean processSendUssd(CommandDetails cmdDet,
        ...
        byte codingScheme = parser.parseMandatory(ComprehensionTlvTag.USSD_STRING,
                (ctlv) -> ValueParser.retrieveCellBroadcastCodingScheme(ctlv), false);
        String ussdString = parser.parseMandatory(ComprehensionTlvTag.USSD_STRING,
                (ctlv) -> ValueParser.retrieveUssdString(ctlv, codingScheme));
    ...
    public boolean processSMSEventNotify(CommandDetails cmdDet,
        ...
        if (smsMessage != null) {
            ...
            if (Flags.stkSmsPacking()
                    && (cmdDet.commandQualifier & SEND_SMS_QUALIFIER_PACKING_REQUIRED) != 0
                    && rawTpdu != null && rawTpdu.length > 2) {
                rawTpdu = packSmsTpdu(rawTpdu);
            }
    ...
    /**
     * Helper method to pack an unpacked 8-bit raw SMS T-PDU payload into GSM 7-bit default format.
     *
     * Performs a series of parsing stages to disassemble the T-PDU header and extract the Message
     * Body (User Data). The content is then repacked into GSM 7-bit septets (including any User
     * Data Header), and finally reconstructed into a complete T-PDU payload with updated Data
     * Coding Scheme (DCS) and User Data Length (UDL).
     *
     * @param rawTpdu The raw unpacked 8-bit SMS T-PDU byte array received from UICC.
     * @return The fully reconstructed and packed GSM 7-bit byte array, or the original array
     *         if packing fails or an exception occurs.
     */
    @VisibleForTesting
    public byte[] packSmsTpdu(byte[] rawTpdu) {
        try {
            int daLen = rawTpdu[2] & 0xff;
            int daBytes = 1 + (daLen + 1) / 2;
            int pidIndex = 3 + daBytes;
            int dcsIndex = pidIndex + 1;
            int vpf = (rawTpdu[0] & TP_VPF_MASK) >> 3;
            int vpLen = 0;
            if (vpf == TP_VPF_RELATIVE) {
                vpLen = 1;
            } else if (vpf == TP_VPF_ENHANCED || vpf == TP_VPF_ABSOLUTE) {
                vpLen = 7;
            }
            int udlIndex = dcsIndex + 1 + vpLen;
            int udIndex = udlIndex + 1;

            // Parse the User Data Header (UDH), if present (indicated by TP-UDHI set in byte 0).
            int msgIndex = udIndex;
            int headerLen = 0;
            boolean hasUserDataHeader = (rawTpdu[0] & 0x40) != 0;
            byte[] header = null;
            if (hasUserDataHeader) {
                headerLen = (rawTpdu[udIndex] & 0xff) + 1;
                header = new byte[headerLen - 1];
                System.arraycopy(rawTpdu, udIndex + 1, header, 0, header.length);
                msgIndex += headerLen;
            }

            if (msgIndex <= rawTpdu.length) {
                // Unpack the 8-bit payload into a text string, omitting the pre-parsed headers.
                String unpackedString = GsmAlphabet.gsm8BitUnpackedToString(
                        rawTpdu, msgIndex, rawTpdu.length - msgIndex);

                // Re-pack the extracted text using the GSM 7-bit alphabet, including the UDH.
                byte[] packedSeptets = GsmAlphabet.stringToGsm7BitPackedWithHeader(
                        unpackedString, header, 0, 0);

                byte[] packedTpdu = new byte[udlIndex + packedSeptets.length];

                // Reconstruct the T-PDU: copy original headers, set 7-bit DCS, append septets.
                System.arraycopy(rawTpdu, 0, packedTpdu, 0, udlIndex);
                packedTpdu[dcsIndex] = (byte) TP_DCS_7_BIT_DEFAULT;
                System.arraycopy(packedSeptets, 0, packedTpdu, udlIndex, packedSeptets.length);

                return packedTpdu;
            }
        } catch (Exception e) {
            CatLog.d(this, "packSmsTpdu: Error packing rawTpdu: " + e);
        }
        return rawTpdu;
    }
}
