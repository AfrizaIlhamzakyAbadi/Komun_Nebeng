...
public class PhoneConfigurationManager {
     ...
     */
    public static void enablePhone(Phone phone, boolean enable, Message result) {
        if (phone == null) {
     ...
     */
    public static void getPhoneStatusFromModem(Phone phone, Message result) {
        if (phone == null) {
    ...
    public boolean getPhoneStatusFromCache(int phoneId) throws NoSuchElementException {
        Boolean status = mPhoneStatusMap.get(phoneId);
        if (status != null) {
            return status;
        } else {
