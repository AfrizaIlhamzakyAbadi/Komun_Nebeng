...
public final class SmsApplication {
    ...
    public static void grantPermissionsToSystemApps(Context context) {
        ...
        assignExclusiveSmsPermissionsToSystemApp(context, packageManager, appOps,
                cbrPackageName, false);
        assignReadRestrictedAppOpToSystemApp(packageManager, appOps, cbrPackageName);
    ...
    /**
     * Assign READ_RESTRICTED_MESSAGES app op to a system package.
     *
     ...
     */
    private static void assignReadRestrictedAppOpToSystemApp(PackageManager packageManager,
            AppOpsManager appOps, String packageName) {
        if (Flags.secureAccessToRestrictedRcsMessages() && packageName != null) {
            try {
                PackageInfo info = packageManager.getPackageInfo(packageName, 0);
                appOps.setUidMode(AppOpsManager.OPSTR_READ_RESTRICTED_MESSAGES,
                        info.applicationInfo.uid, AppOpsManager.MODE_ALLOWED);
    ...
    /**
     * Helper to check if the context has the necessary permissions to perform cross-user package
     * lookups and user iteration.
     */
    private static boolean hasUserPermissions(Context context) {
        return context.checkSelfPermission(android.Manifest.permission.INTERACT_ACROSS_USERS)
                        == PackageManager.PERMISSION_GRANTED
                && (context.checkSelfPermission(android.Manifest.permission.MANAGE_USERS)
                                == PackageManager.PERMISSION_GRANTED
                        || context.checkSelfPermission(android.Manifest.permission.CREATE_USERS)
                                == PackageManager.PERMISSION_GRANTED);
    }

    /**
     * Iterate over all users to ensure every instance of the system app gets the necessary
     * AppOps.Assign WRITE_SMS AppOps permission.
     *
     * @param context The context
     * @param packageManager The package manager instance
     * @param appOps The AppOps manager instance
     * @param packageName The package name of the system app
     */
    @RequiresPermission(allOf = {permission.INTERACT_ACROSS_USERS},
            anyOf = {permission.MANAGE_USERS, permission.CREATE_USERS})
    private static void assignExclusiveSmsPermissionsToSystemAppForAllUsers(
            Context context,
            PackageManager packageManager,
            AppOpsManager appOps,
            String packageName) {
        List<UserHandle> users = context.getSystemService(UserManager.class).getUserHandles(true);

        for (UserHandle user : users) {
            try {
                PackageInfo info =
                        packageManager.getPackageInfoAsUser(packageName, 0, user.getIdentifier());
                    ...
                    if (mode != AppOpsManager.MODE_ALLOWED) {
                        Log.w(
                                ...
                                packageName
                                        ...
                                        + " (UID "
                                        + uid
                                        + ") does not have OP_WRITE_SMS:  (fixing)");
                        setExclusiveAppops(packageName, appOps, uid, AppOpsManager.MODE_ALLOWED);
                    }
                }
            } catch (NameNotFoundException e) {
    ...
    private static void assignExclusiveSmsPermissionsToSystemApp(
        ...
        if (sigatureMatch) {
            final int result =
                    packageManager.checkSignatures(context.getPackageName(), packageName);
        ...
        if (hasUserPermissions(context)) {
            assignExclusiveSmsPermissionsToSystemAppForAllUsers(
                    context, packageManager, appOps, packageName);
        } else {
            try {
                PackageInfo info = packageManager.getPackageInfo(packageName, 0);
