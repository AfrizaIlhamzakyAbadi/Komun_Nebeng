...
public class TelephonyAdminReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (shouldDisallow2g != mDisallowCellular2gRestriction) {
            ...
            if (shouldDisallow2g) {
                long currentlyAllowedNetworkTypes = mPhone.getAllowedNetworkTypes(
                        TelephonyManager.ALLOWED_NETWORK_TYPES_REASON_ENABLE_2G);
                if ((currentlyAllowedNetworkTypes & TelephonyManager.NETWORK_CLASS_BITMASK_2G)
                        !=  0) {
                    mPhone.setAllowedNetworkTypes(
                            TelephonyManager.ALLOWED_NETWORK_TYPES_REASON_ENABLE_2G,
                            currentlyAllowedNetworkTypes
                            & ~TelephonyManager.NETWORK_CLASS_BITMASK_2G, null);
                    Rlog.i(TAG, "2G is disabled by User Restriction.");
                }
            }
            mPhone.updateUsageSetting();
        } else {
