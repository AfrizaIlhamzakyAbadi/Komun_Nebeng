...
public class GsmCdmaPhone extends Phone {
    ...
    public Connection dial(String dialString, @NonNull DialArgs dialArgs,
        ...
        boolean isPsOnlyTech = false;
        if (mSST != null) {
            // Check if the fix flag is enabled
            if (mFeatureFlags.fixCsFallbackSsOnVowifi()) {
                NetworkRegistrationInfo wwanPsNri = mSST.mSS.getNetworkRegistrationInfo(
                        NetworkRegistrationInfo.DOMAIN_PS,
                        AccessNetworkConstants.TRANSPORT_TYPE_WWAN);

                boolean isWwanPsInService = wwanPsNri != null && wwanPsNri.isInService();
                if (isWwanPsInService) {
                    int wwanDataRat = wwanPsNri.getAccessNetworkTechnology();
                    int wwanRilRat = ServiceState.networkTypeToRilRadioTechnology(wwanDataRat);
                    isPsOnlyTech = ServiceState.isPsOnlyTech(wwanRilRat);
                }
            } else {
                // Fallback to the old logic if the flag is disabled
                isPsOnlyTech = mSST.mSS.getDataRegistrationState() == ServiceState.STATE_IN_SERVICE
                        && ServiceState.isPsOnlyTech(mSST.mSS.getRilDataRadioTechnology());
            }
        }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_SET_ALLOWED_NETWORK_TYPES_FOR_2G_DISABLED_DONE:
                ...
                break;
            case EVENT_TIMEOUT_EMERGENCY_CALLS:
                logd("EVENT_TIMEOUT_EMERGENCY_CALLS");
                mUse2gForEmergencyCalls = false;
                if (mIsAllowedNetworkTypesLoadedFromDb) {
                    disable2gNetworkType(null);
                }
                break;
    ...
    private void updateDefaultEnable2gSettings(@NonNull PersistableBundle config) {
        ...
        if (RadioInterfaceCapabilityController.getInstance().getCapabilities().contains(
            ...
            if (is2gProtectionDefaultEnabled && !is2gProtectionDefaultUpdated()) {
                ...
                disable2gNetworkType(
                        obtainMessage(EVENT_SET_ALLOWED_NETWORK_TYPES_FOR_2G_DISABLED_DONE));
            }
        }
    }

    private void disable2gNetworkType(Message message) {
