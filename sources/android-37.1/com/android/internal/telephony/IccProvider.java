...
public class IccProvider extends ContentProvider {
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        while (--n >= 0) {
            ...
            if (DBG) log("parsing '" + Rlog.pii(TAG, param) + "'");

            String[] pair = param.split("=", 2);

            if (pair.length != 2) {
                Rlog.e(TAG, "resolve: bad whereClause parameter: " + Rlog.pii(TAG, param));
                continue;
            }
        ...
        if (DBG) log("delete mvalues= " + Rlog.pii(TAG, values));
        boolean success = updateIccRecordInEf(efType, values, pin2, subId);
        if (!success) {
            return 0;
        }
    ...
    updateIccRecordInEf(int efType, ContentValues values, String pin2, int subId) {
        ...
        if (DBG) log("updateIccRecordInEf: efType=" + efType +
                ", values: [ "+ Rlog.pii(TAG, values) + "  ], subId:" + subId);
        try {
            IIccPhoneBook iccIpb = IIccPhoneBook.Stub.asInterface(
                    TelephonyFrameworkInitializer
                            .getTelephonyServiceManager()
    ...
    private void loadRecord(AdnRecord record, MatrixCursor cursor, int id) {
        if (!record.isEmpty()) {
            ...
            if (DBG) log("loadRecord: " + Rlog.pii(TAG, alphaTag) + ", " + Rlog.pii(TAG, number));
            contact[0] = alphaTag;
            contact[1] = number;
            ...
            if (anrs != null) {
                ...
                for (String anr : anrs) {
                    if (DBG) log("Adding anr:" + Rlog.pii(TAG, anr));
