...
public class PinStorage extends Handler {
    ...
    private static final int SUPPLY_PLATFORM_MANAGED_PIN_COMPLETE = 8;
    ...
    public PinStorage(Context context, @NonNull Looper looper) {
        super(looper);
        mContext = context;
    ...
    public synchronized String getPin(int slotId, String iccid) {
        return getPin(slotId, iccid, /* includePlatformManagedPins= */true);
    }

    /**
     * A variant of {@link #getPin(int, String)} that allows the caller to control whether
     * platform-managed PINs may be returned from the method or not.
     */
    public synchronized String getPin(int slotId, String iccid,
            boolean includePlatformManagedPins) {
        ...
        if (Flags.autoSimPinManagement() && includePlatformManagedPins) {
    ...
    private synchronized void makePlatformManagedPinsAvailableAndUsablePostReboot() {
        ...
        for (int slotId = 0; slotId < slotCount; slotId++) {
            ...
            // In case the slot is currently locked, provide it with the PIN.
            supplyPlatformManagedPinToSlot(slotId, platformPin.pin);
        }
        boolean commitResult = editor.commit();
        logd("Result of storing platform-managed PINs as reboot-ready? " + commitResult);
    }

    private synchronized void supplyPlatformManagedPinToSlot(int slotId, String pin) {
        UiccProfile profile = UiccController.getInstance().getUiccProfileForPhone(slotId);
        if (profile == null) {
            return;
        }

        if (profile.getState() == IccCardConstants.State.PIN_REQUIRED) {
            logd("Supplying PIN to slot " + slotId);
            Message onComplete = obtainMessage(SUPPLY_PLATFORM_MANAGED_PIN_COMPLETE);
            onComplete.arg2 = slotId;  // arg1 is the number of remaining attempts in the response
            profile.supplyPin(pin, onComplete);
        }
    }

    private synchronized void removePlatformManagedPinIfWrong(
            int slotId, int attemptsLeft, boolean pinWrong) {
        logd("Result of providing PIN: to slot " + slotId + ": " + attemptsLeft
                + " pin wrong? " + pinWrong);
        if (pinWrong) {
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case DEVICE_UNLOCKED_EVENT:
                ...
                break;
            case SUPPLY_PLATFORM_MANAGED_PIN_COMPLETE:
                removePlatformManagedPinIfWrong(/* slotId= */ msg.arg2,
                        /* attemptsLeft= */ msg.arg1, isPinWrong(msg));
                break;
            default:
                // Nothing to do
                break;
    ...
    private boolean isPinWrong(Message msg) {
        ...
        return exception.getCommandError() == CommandException.Error.PASSWORD_INCORRECT;
    }
