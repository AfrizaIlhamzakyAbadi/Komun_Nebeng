...
public class UiccProfile extends IccCard {
    ...
    private void updateCarrierNameForSubscription(int subId, int nameSource) {
        ...
        // Removed the !newCarrierName.equals(oldSubName) check to allow
        // SubscriptionManagerService to update the source priority even
        // if the display name string remains the same.
        if (!TextUtils.isEmpty(newCarrierName)) {
    ...
    public void updateExternalState() {
        CardState cardState;
        if (mFlags.refactorUiccProfileToUseCorrectCardState()) {
            UiccPort port = mUiccCard.getUiccPortForPhone(mPhoneId);
            cardState =
                    (port != null) ? port.getCardState() : IccCardStatus.CardState.CARDSTATE_ABSENT;
        } else {
            cardState = mUiccCard.getCardState();
        }

        if (DBG) {
            log("updateExternalState: cardState = " + cardState);
        }
        // First check if card state is IO_ERROR or RESTRICTED
        if (cardState == IccCardStatus.CardState.CARDSTATE_ERROR) {
            setExternalState(IccCardConstants.State.CARD_IO_ERROR);
            return;
        }

        if (cardState == IccCardStatus.CardState.CARDSTATE_RESTRICTED) {
        ...
        if (cardLocked) {
            if (mIccRecords != null && (mIccRecords.getLockedRecordsLoaded()
                ...
                if (lockedState == IccCardConstants.State.PIN_REQUIRED) {
                    // Try to get the PIN from the PIN storage, excluding platform-managed PINs.
                    // Since this flow is called when a SIM card is inserted, platform-managed
                    // PINs should not be provided during that flow. Instead, the PIN will be
                    // provided to the SIM card after the user authenticates.
                    String pin = mPinStorage.getPin(mPhoneId, mIccRecords.getFullIccId(),
                            /* includePlatformManagedPins= */ false);
                    if (!pin.isEmpty()) {
    ...
    public boolean hasIccCard() {
        ...
        if (cardState != IccCardStatus.CardState.CARDSTATE_ABSENT) {
            return true;
        }
        loge(
                "hasIccCard: UiccProfile is not null but UiccCard/UiccPort is null or card state is"
                        + " ABSENT");
        return false;
    }
    ...
    public boolean resetAppWithAid(String aid, boolean reset) {
        synchronized (mLock) {
            ...
            for (int i = 0; i < mUiccApplications.length; i++) {
                UiccCardApplication app = mUiccApplications[i];
                if (app != null && (TextUtils.isEmpty(aid) || aid.equals(app.getAid()))) {
                    ...
                    if (!TextUtils.isEmpty(aid)
                            && app.getType() == AppType.APPTYPE_ISIM) {
                        isIsimRefresh = true;
                    }

                    if (mFlags.fixInconsistentRefreshState()) {
                        cleanupApplicationReferences(app);
                    }
    ...
    /**
     * Clears references and unregisters event handlers for a disposed application.
     */
    @GuardedBy("mLock")
    private void cleanupApplicationReferences(@NonNull UiccCardApplication app) {
        // Unregister handlers to prevent stale events
        app.unregisterForReady(mHandler);
        IccRecords ir = app.getIccRecords();
        if (ir != null) {
            ir.unregisterForRecordsLoaded(mHandler);
        ...
        // If we are removing the current active application, clear the references
        if (app == mUiccApplication) {
            log("cleanupApplicationReferences: clearing current application references");
            unregisterCurrAppEvents();
            mUiccApplication = null;
            mIccRecords = null;
        }
