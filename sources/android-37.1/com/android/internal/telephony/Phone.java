...
import static android.telephony.TelephonyManager.ACTION_2G_ENABLED_FOR_EMERGENCY_CALLING;
...
public abstract class Phone extends Handler implements PhoneInternalInterface {
    ...
    // Max duration (5 min) to override admin restrictions and enable 2G specifically for emergency
    // call support.
    private static final int EMERGENCY_CALL_TIMER_MAX_MS = 300000;
    /* Event Constants */
    ...
    protected static final int EVENT_TIMEOUT_EMERGENCY_CALLS = 78;
    protected static final int EVENT_2G_ENABLED_EMERGENCY_CALL_NOTIFY = 79;
    protected static final int EVENT_LAST = EVENT_2G_ENABLED_EMERGENCY_CALL_NOTIFY;
    ...
    protected boolean mUse2gForEmergencyCalls = false;
    ...
    public void handleMessage(Message msg) {
        ...
        switch(msg.what) {
            ...
            case EVENT_SET_USAGE_SETTING_DONE:
                ...
                break;
            case EVENT_2G_ENABLED_EMERGENCY_CALL_NOTIFY:
                logd("EVENT_2G_ENABLED_EMERGENCY_CALL_NOTIFY");
                ar = (AsyncResult) msg.obj;
                if (ar.userObj != null) {
                ...
                if (ar.exception == null) {
                    // Send a notification that 2G has been enabled for emergency calling.
                    sendNotification2gEnabledForEmergencyCalling();
                }
                break;
            default:
    ...
    */
     ...
     */
    public void requestSignalStrengthUpdate(Message resp, WorkSource workSource) {
        SignalStrengthController ssc = getSignalStrengthController();
        if (ssc == null) {
            ...
            ssc.requestSignalStrengthUpdate(resp, workSource);
        }
    }
    ...
    private @TelephonyManager.NetworkTypeBitMask long getEffectiveAllowedNetworkTypes() {
        ...
        if (m2gAdminUpdater.isCellular2gDisabled() && !mUse2gForEmergencyCalls) {
            logd("SubId " + getSubId()
    ...
    /**
     * Requests to set the allowed emergency network types for a specific reason
     *
     * @param reason reason to configure allowed network type
     * @param networkTypes one of the network types
     * @param response callback Message
     */
    public void setAllowedEmergencyNetworkTypes(
            @TelephonyManager.AllowedNetworkTypesReason int reason,
            @TelephonyManager.NetworkTypeBitMask long networkTypes, @Nullable Message response) {
        if (!mFeatureFlags.key2gToggleEmergency()) {
            return;
        }
        if (m2gAdminUpdater.isCellular2gDisabled()) {
            mUse2gForEmergencyCalls = true;
            sendMessageDelayed(obtainMessage(EVENT_TIMEOUT_EMERGENCY_CALLS),
                    EMERGENCY_CALL_TIMER_MAX_MS);
            setAllowedNetworkTypes(reason, networkTypes, response);
        } else {
            setAllowedNetworkTypes(reason, networkTypes,
                    obtainMessage(EVENT_2G_ENABLED_EMERGENCY_CALL_NOTIFY, response));
        }
    }

    private void sendNotification2gEnabledForEmergencyCalling() {
        ...
        if (componentName != null) {
            ...
            intent.setAction(ACTION_2G_ENABLED_FOR_EMERGENCY_CALLING);
            intent.setComponent(componentName);
            SubscriptionManager.putSubscriptionIdExtra(intent, getSubId());
            if (mContext.getPackageManager().queryBroadcastReceivers(intent, 0).size() > 0) {
                mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
            }
        } else {
            loge("sendNotification2gEnabledForEmergencyCalling: componentName is null, cannot send "
                    + "broadcast.");
        }
    }

    @VisibleForTesting
    public boolean isUse2gForEmergencyCalls() {
        return mUse2gForEmergencyCalls;
    }
    ...
    public void sendDialerSpecialCode(String code) {
        if (!TextUtils.isEmpty(code)) {
            ...
            mContext.sendBroadcastAsUser(secrectCodeIntent, UserHandle.ALL);
        }
    }
    ...
    public void holdActiveCall() throws CallStateException {
        throw new UnsupportedOperationException("holdActiveCall");
    }

    @Override
    public void unholdHeldCall() throws CallStateException {
        throw new UnsupportedOperationException("unholdHeldCall");
    }

    /**
     * Hold the specified connection.
     * @param c the connection to hold.
     * @throws CallStateException if the connection cannot be held.
     */
    @Override
    public void hold(Connection c) throws CallStateException {
        throw new UnsupportedOperationException("hold");
    }

    /**
     * Unhold the specified connection.
     * @param c the connection to unhold.
     * @throws CallStateException if the connection cannot be unheld.
     */
    @Override
    public void unhold(Connection c) throws CallStateException {
        throw new UnsupportedOperationException("unhold");
    }
}
