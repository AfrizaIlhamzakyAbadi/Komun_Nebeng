    ...
    public static void makeDefaultPhone(Context context, @NonNull FeatureFlags featureFlags) {
        synchronized (sLockProxyPhones) {
            if (!sMadeDefaults) {
                ...
                // Move the permission verification logic and monitor initialization to a background
                // thread to prevent blocking the main thread during boot.
                AsyncTask.THREAD_POOL_EXECUTOR.execute(() -> {
                    ComponentName componentName =
                            SmsApplication.getDefaultSmsApplication(context,
                                    true /* updateIfNeeded */);
