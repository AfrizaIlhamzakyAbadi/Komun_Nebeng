...
public class EuiccController extends IEuiccController.Stub {
    ...
    protected Set<Integer> getCurrentEmbeddedSubscriptionIds(int cardId) {
    ...
    public boolean isPrivilegedFromCarrierIdentifier(
            DownloadableSubscription subscription, String callingPackage) {
        if (subscription.getCarrierIdentifier() == null) {
            Log.i(TAG, "DownloadableSubscription does not include CarrierIdentifier");
            return false;
        }
        ...
        Log.i(TAG, "Parent CarrierId from CarrierIdentifier=" + parentCarrierId);

        if (parentCarrierId == TelephonyManager.UNKNOWN_CARRIER_ID) {
            Log.i(TAG, "Cannot gain CarrierId-based Privileges on UNKNOWN_CARRIER_ID.");
            return false;
        }
        ...
        for (Phone phone : PhoneFactory.getPhones()) {
            if (phone.getCarrierId() == parentCarrierId) {
                ...
                if (cpt != null
                    ...
                    Log.i(TAG, "isPrivilegedFromCarrierIdentifier=true for package="
                            + callingPackage + ", phoneId=" + phone.getPhoneId());
                    return true;
                } else {
                    Log.e(TAG, "CarrierIds Matched but Requesting Package Lacks Carrier Privs"
                            + " phoneId=" + phone.getPhoneId());
                }
            } else  {
                Log.i(TAG, "Download not privileged from CarrierId=" + phone.getCarrierId()
                        + " phoneId=" + phone.getPhoneId());
            }
        }
    ...
    protected boolean callerCanManageDevicePolicyManagedSubscriptions(String callingPackage,
      long callingToken) {
