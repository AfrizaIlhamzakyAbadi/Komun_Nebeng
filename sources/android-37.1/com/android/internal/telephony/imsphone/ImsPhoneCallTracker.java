...
public class ImsPhoneCallTracker extends CallTracker implements ImsPullCall {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (TelecomManager.ACTION_DEFAULT_DIALER_CHANGED.equals(intent.getAction())) {
            ...
            } else if (TelephonyIntents.ACTION_EMERGENCY_CALLBACK_MODE_CHANGED.equals(
                    intent.getAction())) {
                ...
                if (!isInEcm) {
                    ...
                    toNotify.addAll(mHandoverCall.getConnections());
                    if (mPendingMO != null) {
    ...
    public void startListeningForCalls(int subId) throws ImsException {
        ...
        try {
            ...
            logw("Failed to add emergency callback (subId=" + subId + "): " + e.getMessage());
        }
    ...
    private boolean prepareForDialing(ImsPhone.ImsDialArgs dialArgs) throws CallStateException {
        ...
        // if there is.
        checkForDialIssues(dialArgs.isEmergency);
     ...
     */
    public void hold(Connection connection) throws CallStateException {
        if (!(connection instanceof ImsPhoneConnection imsConn)) {
            return;
        }

        if (imsConn.getState() != ImsPhoneCall.State.ACTIVE) {
            Log.i(LOG_TAG, "Requested to hold an IMS connection that is not ACTIVE; "
                    + "ImsPhoneConnection = " + imsConn);
            return;
        }
        ImsCall imsCall = imsConn.getImsCall();
        if (imsCall == null) {
            return;
        }
        ...
        logHoldSwapState("hold(targeted)");
        boolean switched = false;
        if (mForegroundCall.hasConnection(imsConn)) {
            mForegroundCall.switchWith(mBackgroundCall);
            switched = true;
        }
        ...
        try {
            ...
            mHoldSwitchingState = oldHoldState;
            logHoldSwapState("hold(targeted) - fail");
            throw new CallStateException(e.getMessage());
        }
    }
     ...
     * @param connection the connection to unhold.
     * @throws CallStateException if the connection cannot be unheld.
     */
    public void unhold(Connection connection) throws CallStateException {
        if (!(connection instanceof ImsPhoneConnection imsConn)) {
            return;
        }

        if (imsConn.getState() != ImsPhoneCall.State.HOLDING) {
            Log.i(LOG_TAG, "Requested to unhold an IMS connection that is not HOLDING; "
                    + "ImsPhoneConnection = " + imsConn);
            return;
        }
        ImsCall imsCall = imsConn.getImsCall();
        ...
        logHoldSwapState("unhold(targeted)");
        boolean switched = false;
        if (mBackgroundCall.hasConnection(imsConn)) {
            mForegroundCall.switchWith(mBackgroundCall);
            switched = true;
        }
        ...
        try {
            ...
            mHoldSwitchingState = oldHoldState;
            logHoldSwapState("unhold(targeted) - fail");
            throw new CallStateException(e.getMessage());
        }
    }
    ...
    public void unholdHeldCall() throws CallStateException {
        ...
        if (imsCall == null && mForegroundCall.getState() == ImsPhoneCall.State.HOLDING) {
    ...
    public void checkForDialIssues(boolean isEmergencyCall) throws CallStateException {
        ...
        if (mFeatureFlags.preventDialingDuringResume()) {
            if (!isEmergencyCall && (mHoldSwitchingState == HoldSwapState.PENDING_SINGLE_CALL_UNHOLD
                    || mHoldSwitchingState == HoldSwapState.SWAPPING_ACTIVE_AND_HELD
                    || mHoldSwitchingState == HoldSwapState.PENDING_RESUME_FOREGROUND_AFTER_FAILURE
                    || mHoldSwitchingState == HoldSwapState.PENDING_RESUME_FOREGROUND_AFTER_HOLD)) {
                throw new CallStateException(CallStateException.ERROR_INVALID,
                        "Cannot dial a SIM call while a call resume is in progress.");
            }
        }
    }
    ...
    public void hangup (ImsPhoneCall call, /*@android.telecom.Call.RejectReason*/ int rejectReason)
        ...
        if (imsCall != null && imsCall.isCallSessionMergePending()) {
            ...
            if (conn != null) {
                conn.onConnectionEvent(android.telecom.Connection.EVENT_DISCONNECT_FAILED, null);
            }
            ...
            case EVENT_EXIT_ECBM_BEFORE_PENDINGMO:
                if (mPendingMO != null) {
                    if (DomainSelectionResolver.getInstance().isDomainSelectionSupported()) {
                        ...
                        try {
                            getEcbmInterface().exitEmergencyCallbackMode();
                            mPhone.setOnEcbModeExitResponse(this,
                            ...
                            e.printStackTrace();
                            mPendingMO.setDisconnectCause(DisconnectCause.ERROR_UNSPECIFIED);
                            sendEmptyMessageDelayed(EVENT_HANGUP_PENDINGMO,
                                    TIMEOUT_HANGUP_PENDINGMO);
                        }
                    }
                }
                break;
    ...
    public class CarrierRoamingNtnListener extends TelephonyCallback
        ...
        public void onCarrierRoamingNtnModeChanged(boolean active) {
            ...
            if (mTelephonyManager != null) {
                ...
                mImsManager.setWfcRoamingSettingInternal(
                        mImsManager.isWfcEnabledByUser() && mImsManager.isWfcRoamingEnabledByUser());
