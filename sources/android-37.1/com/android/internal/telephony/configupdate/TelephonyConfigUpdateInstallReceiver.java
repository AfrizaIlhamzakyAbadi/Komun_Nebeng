...
public class TelephonyConfigUpdateInstallReceiver extends ConfigUpdateInstallReceiver implements
        ...
        for (int carrierId : carrierIds) {
            ...
            for (String plmn : plmns) {
                ...
                if (plmnConfig != null) {
                    ...
                    if (!isValidNtnEmergencyCallToSatelliteHandover(
                            plmnConfig.getNtnEmergencyCallToSatelliteHandover())) {
                        Log.e(TAG, "found invalid ntnEmergencyCallToSatelliteHandover : "
                                + plmnConfig.getNtnEmergencyCallToSatelliteHandover());
                        mConfigUpdaterMetricsStats.reportCarrierConfigError(SatelliteConstants
                                .CONFIG_UPDATE_RESULT_CARRIER_DATA_INVALID_HANDOVER_TYPE);
                        return false;
                    }
                }
            }

            Integer connectHysteresisSec = satelliteConfig
                    .getConnectHysteresisSecByCarrierId(carrierId);
            if (connectHysteresisSec != null) {
                if (connectHysteresisSec < 0) {
                    Log.e(TAG, "found invalid connectHysteresisSec : " + connectHysteresisSec);
                    mConfigUpdaterMetricsStats.reportCarrierConfigError(SatelliteConstants
                            .CONFIG_UPDATE_RESULT_CARRIER_DATA_INVALID_CONNECT_HYSTERESIS);
                    return false;
                }
            }
    ...
    private boolean isValidNtnConnectType(int ntnConnectType) {
        if (ntnConnectType < CARRIER_ROAMING_NTN_CONNECT_UNKNOWN
    ...
    private boolean isValidNtnEmergencyCallToSatelliteHandover(int handoverType) {
        if (handoverType < SatelliteManager.EMERGENCY_CALL_TO_SATELLITE_HANDOVER_TYPE_UNKNOWN
                || handoverType > SatelliteManager.EMERGENCY_CALL_TO_SATELLITE_HANDOVER_TYPE_T911) {
            Log.e(TAG, "isValidNtnEmergencyCallToSatelliteHandover: handoverType="
                    + handoverType + " is not valid");
            return false;
        }
        return true;
    }
