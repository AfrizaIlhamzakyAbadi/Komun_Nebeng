...
public class CarrierDisplayNameResolver {
    ...
    private CarrierDisplayNameData getOutOfServiceDisplayName(CarrierDisplayNameData data) {
        ...
        boolean showSpn = data.shouldShowSpn();
        if (ss.getState() == ServiceState.STATE_POWER_OFF && !forceDisplayNoService && !isEmcOnly) {
            plmn = null;
            showSpn = false;
    ...
    private static final class WfcCarrierNameFormatter {
        ...
        public String formatVoiceName(String name) {
            String trimmedName = TextUtils.emptyIfNull(name).trim();
            if (TextUtils.isEmpty(trimmedName) && mVoiceFormat.contains("%s")) {
                return name;
            }
            return String.format(mVoiceFormat, trimmedName);
        }
        ...
        public String formatDataName(String name) {
            String trimmedName = TextUtils.emptyIfNull(name).trim();
            if (TextUtils.isEmpty(trimmedName) && mDataFormat.contains("%s")) {
                return name;
            }
            return String.format(mDataFormat, trimmedName);
        }
    }
}
