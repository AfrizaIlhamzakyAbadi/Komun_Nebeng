...
public class CarrierKeyDownloadManager extends Handler {
    ...
    @VisibleForTesting
    public volatile long mDownloadId = -1;
    ...
    private boolean downloadKey() {
        ...
        try {
            // Cancel any pending/stuck download for this carrier before starting a new one.
            // This prevents concurrent download conflicts and server-side stalls.
            if (mDownloadId != -1) {
                logd("Removing previous download: " + mDownloadId);
                mDownloadManager.remove(mDownloadId);
                mDownloadId = -1; // Reset immediately to avoid stale ID if enqueue fails
                try {
                    mContext.unregisterReceiver(mDownloadReceiver);
                } catch (IllegalArgumentException e) {
                    // Ignore if receiver was not registered
                    logd("Receiver was not registered: " + e.getMessage());
                }
            }

            // Register receiver for the new download
