...
public class SmsDispatchersController extends Handler {
    ...
    private static final int PDU_MTI_MASK = 0x03;
    private static final int PDU_MTI_SMS_SUBMIT = 0x01;
    private static final int PDU_MTI_SMS_COMMAND = 0x02;
    ...
    public void sendText(String destAddr, String scAddr, String text, PendingIntent sentIntent,
        ...
        if (SatelliteController.getInstance().shouldSendSmsToDatagramDispatcher(mPhone)) {
            ...
            return;
        } else if (SatelliteController.getInstance().isInCarrierRoamingNbIotNtn(mPhone)) {
     ...
     */
    public synchronized int incrementMessageReference() {
        mMessageRef = (mMessageRef + 1) % 256;
        return mMessageRef;
    }

    /**
     * Returns the SMS Dispatcher for CS domain.
     */
    public SMSDispatcher getGsmDispatcher() {
        return mGsmDispatcher;
    }

    /**
     * Returns true if 3GPP format is used for MO SMS and the PDU is a SUBMIT or COMMAND.
     */
    public boolean is3gppFormatAndSmsSubmitOrCommandPdu(byte[] pdu) {
        if (isCdmaMo() || pdu == null || pdu.length <= 1) {
            return false;
        }
        return (pdu[0] & PDU_MTI_MASK) == PDU_MTI_SMS_SUBMIT
                || (pdu[0] & PDU_MTI_MASK) == PDU_MTI_SMS_COMMAND;
    }
