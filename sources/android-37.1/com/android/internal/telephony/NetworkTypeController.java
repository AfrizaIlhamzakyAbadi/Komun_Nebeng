...
public class NetworkTypeController extends StateMachine {
    ...
    private static final ModemDisplayNetworkType DEFAULT_MODEM_NETWORK_TYPE =
            new ModemDisplayNetworkType(TelephonyDisplayInfo.OVERRIDE_NETWORK_TYPE_NONE);
    ...
    private boolean mIsTimerResetEnabledForNrAdvancedRrcIdle;
    ...
    private record ModemDisplayNetworkType(
        ...
        public boolean isOverridden() {
            return overrideNetworkType != TelephonyDisplayInfo.OVERRIDE_NETWORK_TYPE_NONE;
        }
    }
    ...
    private void parseCarrierConfigs() {
        ...
        mIsTimerResetEnabledForNrAdvancedRrcIdle =
                config.getBoolean(
                        CarrierConfigManager.KEY_NR_TIMERS_RESET_IF_NR_ADVANCED_AND_RRC_IDLE_BOOL);
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case EVENT_MODEM_DISPLAY_NETWORK_TYPE_OVERRIDE:
                    ...
                    ar = (AsyncResult) msg.obj;
                    boolean isNrAdvancedFromModem =
                            (((int) ar.result) == DisplayNetworkType.NR_ADVANCED);

                    // Override radio's opinion, which is considered as higher priority.
                    if (isNrAdvancedFromModem) {
                        mModemOverrideNetworkType = new ModemDisplayNetworkType(
                                TelephonyDisplayInfo.OVERRIDE_NETWORK_TYPE_NR_ADVANCED);
                        mDisplayInfoController.updateTelephonyDisplayInfo();
                    } else {
                        mModemOverrideNetworkType = DEFAULT_MODEM_NETWORK_TYPE;

                        // If the modem explicitly revokes NR_ADVANCED, synchronize the framework.
                        log("Modem override is NOT NR_ADVANCED. Clearing ratcheted PCC fields "
                                + "and recalculating framework override type.");

                        // Clear the stale bands
                        mRatchetedNrBands.clear();
                        mRatchetedNrBandwidths.clear();

                        updatePhysicalChannelConfigs(mPhysicalChannelConfigs);
                        mDisplayInfoController.updateTelephonyDisplayInfo();

                        // update mOverrideNetworkType to the correct base 5G icon.
                        transitionToCurrentState();
                    }
                    break;
    ...
    private void updatePhysicalChannelConfigs(List<PhysicalChannelConfig> physicalChannelConfigs) {
        ...
        if (isPccListEmpty && isUsingPhysicalChannelConfigForRrcDetection()) {
            ...
            // Clear the stale NR bands and bandwidths so isNrAdvanced() returns false,
            // but only if the carrier has not explicitly requested to ratchet these fields
            // during idle.
        ...
        if (anchorNrCellId == PhysicalChannelConfig.PHYSICAL_CELL_ID_UNKNOWN) {
            ...
            if (mDoesPccListIndicateIdle
                    && isPccListDormant(physicalChannelConfigs)
    ...
    private void transitionWithTimerTo(IState destState) {
        ...
        if (mIsTimerResetEnabledForNrAdvancedRrcIdle
                && mPrimaryTimerState.equals(STATE_CONNECTED_NR_ADVANCED)
                && (destName.equals(STATE_CONNECTED_RRC_IDLE)
                        || destName.equals(STATE_NOT_RESTRICTED_RRC_IDLE))) {
            if (DBG) log("transitionWithTimerTo: Reset NR advanced timers at NR idle state");
            resetAllTimers();
        }
    ...
    private void updateTimers() {
        ...
        if (mIsPrimaryTimerActive || mIsSecondaryTimerActive) {
            if (currentState.equals(STATE_CONNECTED_NR_ADVANCED)) {
                ...
                resetAllTimers();
            } else if (mIsTimerResetEnabledForNrAdvancedRrcIdle
                    && mPrimaryTimerState.equals(STATE_CONNECTED_NR_ADVANCED)
                    && (currentState.equals(STATE_CONNECTED_RRC_IDLE)
                            || currentState.equals(STATE_NOT_RESTRICTED_RRC_IDLE))) {
    ...
    private boolean isPccListDormant(List<PhysicalChannelConfig> physicalChannelConfigs) {
        if (physicalChannelConfigs == null || physicalChannelConfigs.isEmpty()) {
            return true;
        }

        for (PhysicalChannelConfig config : physicalChannelConfigs) {
            if (config.getContextIds().length > 0) {
                if (DBG) {
                    log("isPccListDormant: found active context IDs "
                                    + Arrays.toString(config.getContextIds()));
                }
                return false;
            }
        }
