...
public class DataProfileManager extends Handler {
    ...
    private void onInternetDataNetworkConnected(@NonNull Set<DataNetwork> internetNetworks) {
        ...
        if (defaultProfile == null
                || DataUtils.areDataProfilesEqual(
                        defaultProfile,
                        mPreferredDataProfile,
                        mPhone.getHalVersion(TelephonyManager.HAL_SERVICE_DATA))) {
            return;
        }
    ...
    public DataProfile getDataProfileForNetworkRequest(
        ...
        return getDataProfileForNetworkRequest(networkRequest, networkType, isNtn,
                isEsimBootstrapProvisioning, ignorePermanentFailure, null);
    }

    /**
     * Get the data profile for the given network request.
     *
     * @param networkRequest The network request.
     * @param networkType The current network type.
     * @param isNtn {@code true} if the network is a non-terrestrial network.
     * @param isEsimBootstrapProvisioning {@code true} if eSIM bootstrap provisioning is in
     * progress.
     * @param ignorePermanentFailure {@code true} if the evaluation is due to environment changes,
     * then we should ignore the permanent failure reached earlier.
     * This should be set to true for condition-based retry/setup.
     * @param candidateFilter An optional supplementary predicate to further filter candidate data
     * profiles. Only profiles for which the predicate returns {@code true} will be considered.
     * <b>Note:</b> This filter must only be used for transient, environmental constraints (such
     * as throttling or temporary exclusion lists) and must NOT be used to override the core
     * capability matching logic (which is strictly governed by the {@code networkRequest}).
     * @return The data profile. {@code null} if can't find any satisfiable data profile.
     */
    @Nullable
    public DataProfile getDataProfileForNetworkRequest(
            @NonNull TelephonyNetworkRequest networkRequest, @NetworkType int networkType,
            boolean isNtn, boolean isEsimBootstrapProvisioning, boolean ignorePermanentFailure,
            @Nullable Predicate<DataProfile> candidateFilter) {
        ...
        if (networkRequest.hasAttribute(TelephonyNetworkRequest
            ...
            apnSetting = getApnSettingForNetworkRequest(networkRequest, networkType, isNtn,
                    isEsimBootstrapProvisioning, ignorePermanentFailure, candidateFilter);
        }
    ...
    private ApnSetting getApnSettingForNetworkRequest(
        ...
        if (!isEsimBootstrapProvisioning) {
                ...
                if (mPreferredDataProfile != null
                        && (candidateFilter == null || candidateFilter.test(mPreferredDataProfile))
                        && networkRequest.canBeSatisfiedBy(mPreferredDataProfile)
                        && isDataProfileSuitable(mPreferredDataProfile, networkType, isNtn,
                                isEsimBootstrapProvisioning, true /* ignorePermanentFailure */,
                                false /* checkPreferredApnSetId */)) {
        ...
        List<DataProfile> dataProfiles = mAllDataProfiles.stream()
                ...
                .filter(dp -> candidateFilter == null || candidateFilter.test(dp))
        ...
        dataProfiles = dataProfiles.stream()
                .filter(dp -> isDataProfileSuitable(dp, networkType, isNtn,
                        isEsimBootstrapProvisioning, ignorePermanentFailure,
                        true /* checkPreferredApnSetId */))
                .collect(Collectors.toList());
        if (dataProfiles.isEmpty()) {
            log("Can't find any suitable data profile for network type "
                    + TelephonyManager.getNetworkTypeName(networkType) + ", infrastructure for "
                    + NetworkRegistrationInfo.isNonTerrestrialNetworkToString(isNtn)
                    + ", and preferred set ID " + mPreferredDataProfileSetId);
            return null;
        }
    ...
    /**
     * Check if the data profile is suitable for the current environment.
     *
     * @param dp The data profile.
     * @param networkType The current data network type.
     * @param isNtn {@code true} if the device is currently attached to non-terrestrial network.
     * @param isEsimBootstrapProvisioning {@code true} if esim bootstrap provisioning is in
     *                                    progress.
     * @param ignorePermanentFailure {@code true} to ignore
     *                               {@link ApnSetting#getPermanentFailed()}.
     * @param checkPreferredApnSetId {@code true} to check if the APN set ID matches the preferred
     *                               one.
     * @return {@code true} if the data profile is suitable.
     */
    private boolean isDataProfileSuitable(@NonNull DataProfile dp,
            @NetworkType int networkType, boolean isNtn, boolean isEsimBootstrapProvisioning,
            boolean ignorePermanentFailure, boolean checkPreferredApnSetId) {
        ApnSetting apnSetting = dp.getApnSetting();
        if (apnSetting == null) return false;
        if (!ignorePermanentFailure && apnSetting.getPermanentFailed()) return false;
        if (!apnSetting.canSupportNetworkType(networkType)) return false;
        if (isEsimBootstrapProvisioning != apnSetting.isEsimBootstrapProvisioning()) return false;
        if (isNtn) {
            if (!apnSetting.isForInfrastructure(ApnSetting.INFRASTRUCTURE_SATELLITE)) return false;
        } else {
            if (!apnSetting.isForInfrastructure(ApnSetting.INFRASTRUCTURE_CELLULAR)) return false;
        }
        if (checkPreferredApnSetId) {
            if (apnSetting.getApnSetId() != Telephony.Carriers.MATCH_ALL_APN_SET_ID
                    && apnSetting.getApnSetId() != mPreferredDataProfileSetId) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Get the number of suitable data profiles that can satisfy the given network request.
     *
     * @param networkRequest The network request.
     * @return The number of suitable data profiles.
     */
    public int getSuitableDataProfileCount(@NonNull TelephonyNetworkRequest networkRequest,
            @NetworkType int networkType, boolean isNtn) {
        boolean isEsimBootstrapProvisioning = mDataNetworkController
                .isEsimBootStrapProvisioningActivated();

        int count = (int) mAllDataProfiles.stream()
                .filter(networkRequest::canBeSatisfiedBy)
                .filter(dp -> isDataProfileSuitable(dp, networkType, isNtn,
                        isEsimBootstrapProvisioning, false /* ignorePermanentFailure */,
                        true /* checkPreferredApnSetId */))
                .count();
        log("getSuitableDataProfileCount: Found " + count + " suitable data profiles for "
                + networkRequest);
        return count;
    }
