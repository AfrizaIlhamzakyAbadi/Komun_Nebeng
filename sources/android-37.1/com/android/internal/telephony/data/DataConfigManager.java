...
public class DataConfigManager extends Handler {
    ...
    /** The default timeout in ms for data network stuck in a handover state. */
    private static final int DEFAULT_NETWORK_HANDOVER_TIMEOUT_MS = 90000;
     ...
     * falling back to the framework's own retry rules (DataSetupRetryRule) to schedule
     * proactive retries with other suitable data profiles.
     */
    @NonNull
    private volatile Set<Integer> mIgnoreNetworkSuggestedRetryCauses = Collections.emptySet();
    ...
    /** The metered network capabilities for home network */
    @NonNull
    @NetCapability
    private final Set<Integer> mMeteredCapabilities = new HashSet<>();
    /** The metered network capabilities for roaming network */
    @NonNull
    @NetCapability
    private final Set<Integer> mRoamingMeteredCapabilities = new HashSet<>();
    ...
    public DataConfigManager(@NonNull Phone phone, @NonNull Looper looper,
        ...
        mNetworkHandoverTimeout = DEFAULT_NETWORK_HANDOVER_TIMEOUT_MS;
    ...
    private void updateCarrierConfig() {
        ...
        updateMeteredApnTypesAndCapabilities();
    ...
    public int getNetworkCapabilityPriority(@NetCapability int capability) {
        // Safe because ConcurrentHashMap does not allow a null value for a non-null key.
        return mNetworkCapabilityPriorityMap.getOrDefault(capability, 0);
    }
    ...
    private void updateDataRetryRules() {
        synchronized (this) {
            ...
            Set<Integer> causesSet = Collections.emptySet();
            int[] causes = mCarrierConfig.getIntArray(
                    CarrierConfigManager
                      .KEY_TELEPHONY_DATA_SETUP_IGNORE_NETWORK_SUGGESTED_RETRY_CAUSES_INT_ARRAY);
            if (causes != null) {
                causesSet = Arrays.stream(causes).boxed().collect(Collectors.toUnmodifiableSet());
            }
            mIgnoreNetworkSuggestedRetryCauses = causesSet;
        }
    }
    ...
    /**
     * Check if the framework should ignore the network-suggested retry timer for the failure cause
     * and instead fall back to the framework's own retry rules (DataSetupRetryRule).
     *
     * When ignored, the failed data profile is still throttled for the network-suggested duration
     * to avoid infinite retry loops on that profile, while the framework proactively schedules
     * retry setup attempts with other suitable profiles based on its own retry rules.
     * Note that if the cause is ignored but no framework retry rule is configured for it,
     * no retry will be scheduled.
     *
     * @param cause Data failure cause
     * @return {@code true} if the network-suggested retry timer should be ignored and the framework
     * should fall back to its own retry rules for scheduling attempts on other profiles.
     */
    public boolean shouldIgnoreNetworkSuggestedRetry(@DataFailureCause int cause) {
        return mFeatureFlags.ignoreNetworkSuggestedRetryFallback()
                && mIgnoreNetworkSuggestedRetryCauses.contains(cause);
    }
    ...
    /**
     * Update the home and roaming metered APN types and network capabilities from the carrier
     * config.
     */
    private void updateMeteredApnTypesAndCapabilities() {
        synchronized (this) {
            ...
            mMeteredCapabilities.clear();
            String[] meteredCaps = mCarrierConfig.getStringArray(
                    CarrierConfigManager.KEY_CARRIER_METERED_NETWORK_CAPABILITIES_STRINGS);
            if (meteredCaps != null) {
                Arrays.stream(meteredCaps)
                        .map(DataUtils::getNetworkCapabilityFromString)
                        .filter(cap -> cap >= 0)
                        .forEach(mMeteredCapabilities::add);
            }

            mRoamingMeteredCapabilities.clear();
            String[] roamingMeteredCaps = mCarrierConfig.getStringArray(
                    CarrierConfigManager.KEY_CARRIER_METERED_ROAMING_NETWORK_CAPABILITIES_STRINGS);
            if (roamingMeteredCaps != null) {
                Arrays.stream(roamingMeteredCaps)
                        .map(DataUtils::getNetworkCapabilityFromString)
                        .filter(cap -> cap >= 0)
                        .forEach(mRoamingMeteredCapabilities::add);
            }
        }
    }
    ...
    public Set<Integer> getMeteredNetworkCapabilities(boolean isRoaming) {
        Set<Integer> meteredCapabilities = new HashSet<>();
        synchronized (this) {
            Set<Integer> meteredApnTypes = isRoaming ? mRoamingMeteredApnTypes : mMeteredApnTypes;
            meteredApnTypes.stream()
                    .map(DataUtils::apnTypeToNetworkCapability)
                    .filter(cap -> cap >= 0)
                    .forEach(meteredCapabilities::add);

            if (mFeatureFlags.makeMeteredUnmeteredConfigurableForSliceNetwork()) {
                meteredCapabilities.addAll(isRoaming
                        ? mRoamingMeteredCapabilities : mMeteredCapabilities);
            }
        }
        ...
        } else if (!mFeatureFlags.makeMeteredUnmeteredConfigurableForSliceNetwork()) {
