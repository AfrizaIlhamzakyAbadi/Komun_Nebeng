...
public class AutoDataSwitchController extends Handler {
    ...
    /**
     * Defines the priority of transports for auto data switch evaluation.
     * A higher priority transport (like Wi-Fi) will suspend cellular
     * auto switch evaluations.
     */
    private static final int TRANSPORT_PRIORITY_SATELLITE = 0;
    private static final int TRANSPORT_PRIORITY_CELLULAR = 1;
    private static final int TRANSPORT_PRIORITY_NON_CELLULAR = 2; // Wi-Fi, etc.
    ...
    /**
     * Determines the priority of the transport for auto data switch evaluation.
     */
    private int getTransportPriority(@NonNull NetworkCapabilities nc) {
        if (nc.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR)) {
            return TRANSPORT_PRIORITY_CELLULAR;
        }
        if (nc.hasTransport(NetworkCapabilities.TRANSPORT_SATELLITE)) {
            return TRANSPORT_PRIORITY_SATELLITE;
        }
        // Treat all other non-cellular transports as high priority
        // to save power and avoid unnecessary cellular evaluations.
        return TRANSPORT_PRIORITY_NON_CELLULAR;
    }
    ...
    public void updateDefaultNetworkCapabilities(
        ...
        if (networkCapabilities != null) {
            ...
            // A network is "preferred non-cellular" if its transport priority is higher
            // than cellular.
            mDefaultNetworkIsOnNonCellular =
                    getTransportPriority(networkCapabilities) > TRANSPORT_PRIORITY_CELLULAR;
