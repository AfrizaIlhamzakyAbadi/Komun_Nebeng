...
public class DataRetryManager extends Handler {
    ...
    public static class DataThrottlingEntry {
        ...
        public DataThrottlingEntry(@NonNull DataProfile dataProfile,
                ...
                @RetryType int retryType, @DataFailureCause int cause,
                @ElapsedRealtimeLong long expirationTimeMillis) {
    ...
    /**
     * Check if proactive APN fallback is applicable for the given network request list.
     * Proactive APN fallback is only applicable for the default data profile (INTERNET)
     * when there are multiple suitable data profiles available.
     *
     * @param requestList The network request list.
     * @return {@code true} if proactive APN fallback is applicable.
     */
    private boolean isProactiveApnFallbackApplicable(@NonNull NetworkRequestList requestList,
            @TransportType int transport) {
        if (!mFlags.ignoreNetworkSuggestedRetryFallback()) return false;
        if (requestList.isEmpty()) return false;

        // Proactive APN fallback is only applicable for the default data profile (INTERNET).
        TelephonyNetworkRequest internetRequest = requestList.stream()
                .filter(r -> r.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET))
                .findFirst()
                .orElse(null);
        if (internetRequest == null) {
            logv("isProactiveApnFallbackApplicable: Not default data. Skip.");
            return false;
        }

        // Proactive fallback is only applicable when there are multiple suitable data profiles
        // to fall back to.
        NetworkRegistrationInfo nri = mPhone.getServiceState().getNetworkRegistrationInfo(
                NetworkRegistrationInfo.DOMAIN_PS, transport);
        int networkType = nri != null
                ? nri.getAccessNetworkTechnology() : TelephonyManager.NETWORK_TYPE_UNKNOWN;
        boolean isNtn = mPhone.getServiceState().isUsingNonTerrestrialNetwork();

        int count = mDataProfileManager.getSuitableDataProfileCount(internetRequest,
                networkType, isNtn);
        log("isProactiveApnFallbackApplicable: Found " + count + " suitable data profiles for "
                + "default data. Fallback " + (count > 1 ? "is" : "is not") + " applicable.");
        return count > 1;
    }
    ...
    private void onEvaluateDataSetupRetry(@NonNull DataProfile dataProfile,
        ...
        if (retryDelayMillis == Long.MAX_VALUE || retryDelayMillis == Integer.MAX_VALUE) {
            ...
            throttleDataProfile(dataProfile, requestList, null,
                    ThrottleStatus.RETRY_TYPE_NEW_CONNECTION, transport, cause, Long.MAX_VALUE);
            return;
        } else if (retryDelayMillis != DataCallResponse.RETRY_DURATION_UNDEFINED) {
            if (mDataConfigManager.shouldIgnoreNetworkSuggestedRetry(cause)
                    && isProactiveApnFallbackApplicable(requestList, transport)) {
                logl("Network suggested retry for " + dataProfile + " with delay "
                        + retryDelayMillis + "ms, but cause " + DataFailCause.toString(cause)
                        + " is in the ignore list and multiple APNs are available. "
                        + "Performing proactive APN fallback: Throttling failed profile for "
                        + retryDelayMillis
                        + "ms and scheduling next attempt using framework rules.");

                // Throttling the failed profile ensures the framework tries other APNs (fallback)
                // instead of looping on this one.
                throttleDataProfile(dataProfile, requestList, null,
                        ThrottleStatus.RETRY_TYPE_NEW_CONNECTION, transport, cause,
                        SystemClock.elapsedRealtime() + retryDelayMillis);

                // Use the framework's configured retry rules for scheduling the next attempt.
                // This allows the framework to pick another data profile immediately or
                // based on its own backoff logic.
                retryDelayMillis = DataCallResponse.RETRY_DURATION_UNDEFINED;
            } else {
                // Network specifically asks retry the previous data profile again.
                // This path is taken if the cause is not ignored, if only one APN is available,
                // or if the request is not for the default data profile.
        ...
        if (!retryScheduled) {
            if (mDataConfigManager.shouldIgnoreNetworkSuggestedRetry(cause)) {
                loge("onEvaluateDataSetupRetry: Network-suggested retry was ignored for cause "
                        + DataFailCause.toString(cause) + ", but no matching framework retry rule "
                        + "was found in mDataSetupRetryRuleList. Telephony will NOT retry "
                        + "this request! Please ensure a matching DataSetupRetryRule is configured "
                        + "for this cause.");
            } else {
    ...
    private void onEvaluateDataHandoverRetry(@NonNull DataNetwork dataNetwork,
        ...
        if (retryDelayMillis == Long.MAX_VALUE || retryDelayMillis == Integer.MAX_VALUE) {
            ...
            throttleDataProfile(dataNetwork.getDataProfile(),
                    ...
                    ThrottleStatus.RETRY_TYPE_HANDOVER, targetTransport, cause, Long.MAX_VALUE);
        } else if (retryDelayMillis != DataCallResponse.RETRY_DURATION_UNDEFINED) {
            // Network specifically asks retry the previous data profile again.
            ...
            throttleDataProfile(dataNetwork.getDataProfile(),
                    dataNetwork.getAttachedNetworkRequestList(), dataNetwork,
                    ThrottleStatus.RETRY_TYPE_HANDOVER, targetTransport, cause,
    ...
    private void throttleDataProfile(@NonNull DataProfile dataProfile,
            ...
            @TransportType int transport, @DataFailureCause int cause,
            @ElapsedRealtimeLong long expirationTime) {
        DataThrottlingEntry entry = new DataThrottlingEntry(dataProfile, networkRequestList,
                dataNetwork, transport, retryType, cause, expirationTime);
    ...
    private void onDataProfileUnthrottled(@Nullable DataProfile dataProfile, @Nullable String apn,
        ...
        if (dataProfile != null) {
            ...
            stream = stream.filter(entry -> DataUtils.areTrafficDescriptorsEqual(
                    entry.dataProfile.getTrafficDescriptor(),   // storedTd
                    dataProfile.getTrafficDescriptor(),         // requestTd
                    mRil.getHalVersion(TelephonyManager.HAL_SERVICE_DATA)));
    ...
    /**
     * Check if the data profile is throttled specifically due to the causes that framework should
     * ignore for network-suggested retry.
     *
     * @param dataProfile The data profile to check.
     * @param transport The transport.
     * @return {@code true} if the data profile is throttled by ignored causes.
     */
    public boolean isDataProfileThrottledByIgnoredCauses(@NonNull DataProfile dataProfile,
            @TransportType int transport) {
        long now = SystemClock.elapsedRealtime();
        return mDataThrottlingEntries.stream().anyMatch(
                entry -> entry.dataProfile.equals(dataProfile) && entry.expirationTimeMillis > now
                        && entry.transport == transport
                        && mDataConfigManager.shouldIgnoreNetworkSuggestedRetry(entry.cause));
    }
