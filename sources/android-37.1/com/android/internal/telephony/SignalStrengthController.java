...
public class SignalStrengthController extends Handler {
    ...
    /** Required magnitude change between unsolicited SignalStrength reports for NTN. */
    private static final int NTN_REPORTING_HYSTERESIS_DB = 2;
    ...
    private static final int EVENT_NR_RSRP_BOOST_CHANGED                    = 10;
    private static final int EVENT_LTE_RSRP_BOOST_CHANGED                   = 11;

    private static final int EXPECTED_NUM_SIGNAL_THRESHOLD_SIZE = 4;
    ...
    private final AtomicBoolean mIsCarrierRoamingNtnModeActive = new AtomicBoolean(false);
    // Track the active NR RSRP boost for dynamic thresholding
    private volatile int mActiveNrRsrpBoost = 0;
    // Track the pending NR RSRP boost that has been queued for execution
    private int mPendingNrRsrpBoost = 0;
    // Track the active LTE RSRP boost for dynamic thresholding
    private volatile int mActiveLteRsrpBoost = 0;
    // Track the pending LTE RSRP boost that has been queued for execution
    private int mPendingLteRsrpBoost = 0;
    // Cache of the last consolidated thresholds sent to the modem
    @Nullable
    private List<SignalThresholdInfo> mLastConsolidatedThresholdInfos = null;
    // Flag to force update the modem reporting criteria (e.g. after modem restart)
    private boolean mForceUpdateModemCriteria = false;
    // Cache of the boosted carrier config to prevent GC churn
    @Nullable
    private PersistableBundle mBoostedCarrierConfig = null;

    private final CarrierRoamingNtnListener mCarrierRoamingNtnListener =
            new CarrierRoamingNtnListener();
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_NR_RSRP_BOOST_CHANGED: {
                mActiveNrRsrpBoost = msg.arg1;
                mBoostedCarrierConfig = null; // Invalidate cache safely on handler thread
                localLog("EVENT_NR_RSRP_BOOST_CHANGED: NR boost changed to " + mActiveNrRsrpBoost
                        + " dB, re-evaluating thresholds");
                updateReportingCriteria();
            ...
            case EVENT_LTE_RSRP_BOOST_CHANGED: {
                mActiveLteRsrpBoost = msg.arg1;
                mBoostedCarrierConfig = null; // Invalidate cache safely on handler thread
                localLog("EVENT_LTE_RSRP_BOOST_CHANGED: LTE boost changed to " + mActiveLteRsrpBoost
                        + " dB, re-evaluating thresholds");
                updateReportingCriteria();
                if (mSignalStrength != null) {
                    updateSignalStrength(new SignalStrength(mSignalStrength));
    ...
    private void onReset() {
        mForceUpdateModemCriteria = true;
    ...
    private void onSignalStrengthResult(@NonNull AsyncResult ar) {
        ...
        if (ar.result instanceof SignalStrength) {
        ...
        if (ar.userObj instanceof Message) {
            ...
            loge("onSignalStrengthResult() Unexpected user obj: " + ar.userObj);
        }
    ...
    private void updateSignalStrength(@NonNull SignalStrength signalStrength) {
        ...
        if (serviceStateTracker != null) {
            PersistableBundle config = getCarrierConfigForSignalStrengthLevel();
            mSignalStrength.updateLevel(config, serviceStateTracker.mSS,
                    isInNtnCarrierRoamingMode());
        } else {
    ...
    /**
     * Get the carrier config to be used for AP-side signal strength level calculation.
     * Overrides the standard thresholds with boosted ones if an NR boost is active and configured.
     * Caches the result to prevent GC churn on frequent signal updates.
     */
    @NonNull
    private PersistableBundle getCarrierConfigForSignalStrengthLevel() {
        if (!isInNtnCarrierRoamingMode() && (mActiveNrRsrpBoost > 0 || mActiveLteRsrpBoost > 0)) {
            if (mBoostedCarrierConfig == null) {
                mBoostedCarrierConfig = new PersistableBundle(mCarrierConfig);
                boolean modified = false;

                if (mActiveNrRsrpBoost > 0) {
                    modified |= overrideBoostedThresholds(mBoostedCarrierConfig,
                            CarrierConfigManager.KEY_5G_NR_SSRSRP_BOOSTED_THRESHOLDS_INT_ARRAY,
                            CarrierConfigManager.KEY_5G_NR_SSRSRP_THRESHOLDS_INT_ARRAY,
                            "NR");
                }

                if (mActiveLteRsrpBoost > 0) {
                    modified |= overrideBoostedThresholds(mBoostedCarrierConfig,
                            CarrierConfigManager.KEY_LTE_RSRP_BOOSTED_THRESHOLDS_INT_ARRAY,
                            CarrierConfigManager.KEY_LTE_RSRP_THRESHOLDS_INT_ARRAY,
                            "LTE");
                }

                if (!modified) {
                    mBoostedCarrierConfig = mCarrierConfig;
                }
            }
            return mBoostedCarrierConfig;
        }
    ...
    public SignalStrength getSignalStrength() {
        // TODO(b/465185506): Remove the magical behavior of refreshing signal strength when polled
        // on a slot with an opportunistic profile.
    ...
    /**
     * Requests the current signal strength as SignalStrength and returns either the
     * SignalStrength object or detailed error info from the HAL.
     *
     * <p>If this request is rate-limited (called within the minimum refresh threshold), the
     * cached {@link SignalStrength} is returned immediately without querying the modem.
     */
    public void requestSignalStrengthUpdate(
            @Nullable Message response, @Nullable WorkSource workSource) {
        if (!isRefreshAllowed()) {
            AsyncResult.forMessage(response, mSignalStrength, null);
            response.sendToTarget();
            return;
        }

        mCi.getSignalStrength(
                obtainMessage(EVENT_POLL_SIGNAL_STRENGTH_DONE, response),
                workSource);
    }

    private boolean isRefreshAllowed() {
        final long curTime = System.currentTimeMillis();
        // If last signal strength is older than the minimum interval,or somehow if curTime is
        // smaller than mSignalStrengthUpdatedTime (system time update), it's considered stale.
        return (mSignalStrengthUpdatedTime > curTime)
                || (curTime - mSignalStrengthUpdatedTime > SIGNAL_STRENGTH_REFRESH_THRESHOLD_IN_MS);
    }

    private boolean shouldRefreshSignalStrength() {
        if (!isRefreshAllowed()) return false;
    ...
    public void updateReportingCriteria() {
        localLog("updateReportingCriteria: isInNtnCarrierRoamingMode="
                + isInNtnCarrierRoamingMode());
        ...
        int lteMeasurementEnabled = mCarrierConfig.getInt(isInNtnCarrierRoamingMode()
        ...
        int[] lteRsrpThresholds = mCarrierConfig.getIntArray(isInNtnCarrierRoamingMode()
                ? CarrierConfigManager.KEY_NTN_LTE_RSRP_THRESHOLDS_INT_ARRAY
                : CarrierConfigManager.KEY_LTE_RSRP_THRESHOLDS_INT_ARRAY);
        if (!isInNtnCarrierRoamingMode() && mActiveLteRsrpBoost > 0) {
            int[] rawThresholds = getRawRegisteredThresholds(
                    CarrierConfigManager.KEY_LTE_RSRP_BOOSTED_THRESHOLDS_INT_ARRAY,
                    mActiveLteRsrpBoost,
                    SignalThresholdInfo.SIGNAL_RSRP_MIN_VALUE,
                    SignalThresholdInfo.SIGNAL_RSRP_MAX_VALUE,
                    "LTE");
            if (rawThresholds != null) {
                lteRsrpThresholds = rawThresholds;
            }
        }
        ...
        if (mPhone.getHalVersion(HAL_SERVICE_NETWORK).greaterOrEqual(RIL.RADIO_HAL_VERSION_1_5)) {
            int[] lteRsrqThresholds = mCarrierConfig.getIntArray(isInNtnCarrierRoamingMode()
            ...
            int[] lteRssnrThresholds = mCarrierConfig.getIntArray(isInNtnCarrierRoamingMode()
            ...
            int nrMeasurementEnabled = mCarrierConfig.getInt(isInNtnCarrierRoamingMode()
            ...
            int[] nrSsrsrpThresholds = mCarrierConfig.getIntArray(isInNtnCarrierRoamingMode()
                    ? CarrierConfigManager.KEY_NTN_5G_NR_SSRSRP_THRESHOLDS_INT_ARRAY :
                    CarrierConfigManager.KEY_5G_NR_SSRSRP_THRESHOLDS_INT_ARRAY);
            if (!isInNtnCarrierRoamingMode() && mActiveNrRsrpBoost > 0) {
                int[] rawThresholds = getRawRegisteredThresholds(
                        CarrierConfigManager.KEY_5G_NR_SSRSRP_BOOSTED_THRESHOLDS_INT_ARRAY,
                        mActiveNrRsrpBoost,
                        SignalThresholdInfo.SIGNAL_SSRSRP_MIN_VALUE,
                        SignalThresholdInfo.SIGNAL_SSRSRP_MAX_VALUE,
                        "NR");
                if (rawThresholds != null) {
                    nrSsrsrpThresholds = rawThresholds;
                }
            }
            ...
            int[] nrSsrsrqThresholds = mCarrierConfig.getIntArray(isInNtnCarrierRoamingMode()
            ...
            int[] nrSssinrThresholds = mCarrierConfig.getIntArray(isInNtnCarrierRoamingMode()
    ...
    private void consolidatedAndSetReportingCriteria(
        ...
        for (SignalThresholdInfo signalThresholdInfo : signalThresholdInfos) {
            ...
            int hysteresisMs = getHysteresisMsFromCarrierConfig(ran, measurementType);
            consolidatedSignalThresholdInfos.add(
                    new SignalThresholdInfo.Builder()
                            .setRadioAccessNetworkType(ran)
                            .setSignalMeasurementType(measurementType)
                            .setHysteresisMs(hysteresisMs)
        ...
        if (!mForceUpdateModemCriteria && mLastConsolidatedThresholdInfos != null
                && mLastConsolidatedThresholdInfos.equals(consolidatedSignalThresholdInfos)) {
            localLog("consolidatedAndSetReportingCriteria: duplicate criteria. "
                    + "Skipping modem update.");
            return;
        }
        mForceUpdateModemCriteria = false;
        mLastConsolidatedThresholdInfos = consolidatedSignalThresholdInfos;
     ...
     * Note: Based on the current use case, there does not exist multiple App signal threshold info
     * requests with hysteresis db value, so this logic picks the latest hysteresis db value set.
     *
     * TODO(b/262655157): Support Multiple App Hysteresis DB value customization
     */
    ...
    private int getHysteresisDbFromCarrierConfig(int ran, int measurement) {
        int defaultHysteresisDb = (Flags.newSatelliteIcon()
                && isInNtnCarrierRoamingMode())
                ? NTN_REPORTING_HYSTERESIS_DB : REPORTING_HYSTERESIS_DB;
        ...
        if (Flags.newSatelliteIcon()
            ...
            if (ran == AccessNetworkConstants.AccessNetworkType.EUTRAN) {
                if (measurement == SignalThresholdInfo.SIGNAL_MEASUREMENT_TYPE_RSRP) {
                    configKey = CarrierConfigManager.KEY_NTN_EUTRAN_RSRP_HYSTERESIS_DB_INT;
                } else if (measurement == SignalThresholdInfo.SIGNAL_MEASUREMENT_TYPE_RSRQ) {
                    configKey = CarrierConfigManager.KEY_NTN_EUTRAN_RSRQ_HYSTERESIS_DB_INT;
                } else if (measurement == SignalThresholdInfo.SIGNAL_MEASUREMENT_TYPE_RSSNR) {
                    configKey = CarrierConfigManager.KEY_NTN_EUTRAN_RSSNR_HYSTERESIS_DB_INT;
                }
            } else if (ran == AccessNetworkConstants.AccessNetworkType.NGRAN) {
                if (measurement == SignalThresholdInfo.SIGNAL_MEASUREMENT_TYPE_SSRSRP) {
                    configKey = CarrierConfigManager.KEY_NTN_NGRAN_SSRSRP_HYSTERESIS_DB_INT;
                } else if (measurement == SignalThresholdInfo.SIGNAL_MEASUREMENT_TYPE_SSRSRQ) {
                    configKey = CarrierConfigManager.KEY_NTN_NGRAN_SSRSRQ_HYSTERESIS_DB_INT;
                } else if (measurement == SignalThresholdInfo.SIGNAL_MEASUREMENT_TYPE_SSSINR) {
                    configKey = CarrierConfigManager.KEY_NTN_NGRAN_SSSINR_HYSTERESIS_DB_INT;
                }
            }
        }
        ...
        int configHysteresisDb = defaultHysteresisDb;
        if (configKey != null) {
            configHysteresisDb = mCarrierConfig.getInt(configKey, defaultHysteresisDb);
        }
        return configHysteresisDb >= SignalThresholdInfo.HYSTERESIS_DB_MINIMUM
                ? configHysteresisDb : defaultHysteresisDb;
    }

    private int getHysteresisMsFromCarrierConfig(int ran, int measurement) {
        int configHysteresisMs = REPORTING_HYSTERESIS_MILLIS;
        String configKey = null;

        if (Flags.newSatelliteIcon()
                && isInNtnCarrierRoamingMode()) {
            if (ran == AccessNetworkConstants.AccessNetworkType.EUTRAN) {
                configKey = CarrierConfigManager.KEY_NTN_EUTRAN_SIGNAL_REPORTING_HYSTERESIS_MS_INT;
            } else if (ran == AccessNetworkConstants.AccessNetworkType.NGRAN) {
                configKey = CarrierConfigManager.KEY_NTN_NGRAN_SIGNAL_REPORTING_HYSTERESIS_MS_INT;
            }
        }

        if (configKey != null) {
            configHysteresisMs = mCarrierConfig.getInt(configKey, REPORTING_HYSTERESIS_MILLIS);
        }

        return Math.max(0, configHysteresisMs);
    }
    ...
    void notifySignalStrength() {
        ...
        if (!mSignalStrength.equals(mLastSignalStrength) || subId != mLastSubId) {
            try {
                ...
                loge("notifySignalStrength() Phone already destroyed: " + ex
    ...
    void updateServiceStateArfcnRsrpBoost(@NonNull ServiceState serviceState,
            ...
            switch (cellIdentity.getType()) {
                case CellInfo.TYPE_LTE:
                    ...
                    if (arfcn != INVALID_ARFCN) {
                        int index = containsEarfcnInEarfcnRange(mEarfcnPairListForRsrpBoost,
                                arfcn);
                        if (index != -1) {
                            rsrpBoost = mLteRsrpBoost;
                            lteRsrpBoost = rsrpBoost;
                        }
                        localLog("updateServiceStateArfcnRsrpBoost: LTE EARFCN=" + arfcn
                                + ", index=" + index + ", boost=" + rsrpBoost + " dB");
                    }
                    break;
                case CellInfo.TYPE_NR:
                    ...
                    if (arfcn != INVALID_ARFCN) {
                        ...
                        if (index != -1 && mNrRsrpBoost != null) {
                            ...
                            nrRsrpBoost = rsrpBoost;
                        }
                        localLog("updateServiceStateArfcnRsrpBoost: NR ARFCN=" + arfcn
                                + ", index=" + index + ", boost=" + nrRsrpBoost + " dB");
                    }
                    break;
                default:
                    break;
        ...
        if (mPendingNrRsrpBoost != nrRsrpBoost) {
            localLog("Pending NR RSRP Boost changed from " + mPendingNrRsrpBoost
                    + " to " + nrRsrpBoost);
            mPendingNrRsrpBoost = nrRsrpBoost;
            removeMessages(EVENT_NR_RSRP_BOOST_CHANGED);
            sendMessage(obtainMessage(EVENT_NR_RSRP_BOOST_CHANGED, nrRsrpBoost, 0));
        }

        if (mPendingLteRsrpBoost != lteRsrpBoost) {
            localLog("Pending LTE RSRP Boost changed from " + mPendingLteRsrpBoost
                    + " to " + lteRsrpBoost);
            mPendingLteRsrpBoost = lteRsrpBoost;
            removeMessages(EVENT_LTE_RSRP_BOOST_CHANGED);
            sendMessage(obtainMessage(EVENT_LTE_RSRP_BOOST_CHANGED, lteRsrpBoost, 0));
        }
    }
    ...
    private void onCarrierConfigurationChanged(int slotIndex) {
        ...
        mCarrierConfig = getCarrierConfig();
        mBoostedCarrierConfig = null; // Invalidate cache
    ...
    private boolean isInNtnCarrierRoamingMode() {
        return mIsCarrierRoamingNtnModeActive.get();
    }
    ...
    /**
     * Overrides standard thresholds in the bundle with boosted ones if configured.
     * @return true if the thresholds were successfully overridden.
     */
    private boolean overrideBoostedThresholds(@NonNull PersistableBundle bundle,
            @NonNull String boostedKey, @NonNull String standardKey, @NonNull String label) {
        int[] boostedThresholds = mCarrierConfig.getIntArray(boostedKey);
        if (boostedThresholds != null) {
            if (boostedThresholds.length == EXPECTED_NUM_SIGNAL_THRESHOLD_SIZE) {
                bundle.putIntArray(standardKey, boostedThresholds);
                localLog("Active " + label
                        + " Boost. Overriding AP thresholds in config with boosted: "
                        + Arrays.toString(boostedThresholds));
                return true;
            } else {
                loge(boostedKey + " has invalid length: " + boostedThresholds.length
                        + ", expected " + EXPECTED_NUM_SIGNAL_THRESHOLD_SIZE + ".");
            }
        }
        return false;
    }
    ...
    /**
     * Calculates raw thresholds by subtracting active boost from boosted thresholds,
     * clamping to valid RSRP bounds.
     */
    @Nullable
    private int[] getRawRegisteredThresholds(@NonNull String boostedKey, int activeBoost,
            int minValue, int maxValue, @NonNull String label) {
        int[] boostedThresholds = mCarrierConfig.getIntArray(boostedKey);
        if (boostedThresholds != null) {
            if (boostedThresholds.length == EXPECTED_NUM_SIGNAL_THRESHOLD_SIZE) {
                int[] rawRegisteredThresholds = new int[boostedThresholds.length];
                for (int i = 0; i < boostedThresholds.length; i++) {
                    rawRegisteredThresholds[i] = Math.max(minValue, Math.min(maxValue,
                            boostedThresholds[i] - activeBoost));
                }
                localLog("Active " + label + " Boost=" + activeBoost
                        + " dB. Overriding registered modem " + label
                        + " thresholds from boosted: "
                        + Arrays.toString(boostedThresholds) + " -> raw: "
                        + Arrays.toString(rawRegisteredThresholds));
                return rawRegisteredThresholds;
            } else {
                loge(boostedKey + " has invalid length: " + boostedThresholds.length
                        + ", expected " + EXPECTED_NUM_SIGNAL_THRESHOLD_SIZE + ".");
            }
    ...
    private class CarrierRoamingNtnListener extends TelephonyCallback
        ...
        public void onCarrierRoamingNtnModeChanged(boolean active) {
            localLog("onCarrierRoamingNtnModeChanged: active=" + active);
            mIsCarrierRoamingNtnModeActive.set(active);
            updateReportingCriteria();
            if (mSignalStrength != null) {
