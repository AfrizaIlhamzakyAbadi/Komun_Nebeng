...
public class BatteryStatsHistory {
    ...
    public static class TraceDelegate {
        ...
        private final ArrayMap<String, Integer> mPendingProperties = new ArrayMap<>();
        private final Runnable mWriteRunnable = this::writePendingProperties;
        ...
        public void traceCounter(@NonNull String name, int value) {
            ...
            if (mShouldSetProperty) {
                synchronized (mPendingProperties) {
                    boolean wasEmpty = mPendingProperties.isEmpty();
                    mPendingProperties.put(name, value);
                    if (wasEmpty) {
                        BackgroundThread.getHandler().post(mWriteRunnable);
                    }
                }
            }
        }
        ...
        private void writePendingProperties() {
            ArrayMap<String, Integer> propertiesToWrite;
            synchronized (mPendingProperties) {
                propertiesToWrite = new ArrayMap<>(mPendingProperties);
                mPendingProperties.clear();
            }
            for (int i = 0; i < propertiesToWrite.size(); i++) {
                String key = propertiesToWrite.keyAt(i);
                int val = propertiesToWrite.valueAt(i);
                try {
                    SystemProperties.set("debug.tracing." + key, Integer.toString(val));
                } catch (RuntimeException e) {
                    Slog.e(TAG, "Failed to set debug.tracing." + key, e);
                }
            }
        }
