...
public class ApplicationSharedMemory implements AutoCloseable {
    ...
    // LINT.IfChange(max_flags)
    public static final int MAX_FLAGS = 32;
    // LINT.ThenChange(core/jni/com_android_internal_os_ApplicationSharedMemory.cpp:max_flags)
    ...
    /**
     * Sets a configuration bit flag in the shared memory bank.
     *
     * @param flagIndex the index of the flag (must be between 0 and MAX_FLAGS - 1).
     * @param val the boolean value to set.
     * @throws IllegalArgumentException if the flag index is out of bounds.
     */
    public void setBitFlag(int flagIndex, boolean val) {
        checkMutable();
        if (flagIndex < 0 || flagIndex >= MAX_FLAGS) {
            throw new IllegalArgumentException("flagIndex out of bounds: " + flagIndex);
        }
        nativeSetBitFlag(mPtr, flagIndex, val);
    }

    /**
     * Returns a configuration bit flag from the shared memory bank.
     *
     * @param flagIndex the index of the flag (must be between 0 and MAX_FLAGS - 1).
     * @return the boolean value of the flag.
     * @throws IllegalArgumentException if the flag index is out of bounds.
     */
    public boolean getBitFlag(int flagIndex) {
        checkMapped();
        if (flagIndex < 0 || flagIndex >= MAX_FLAGS) {
            throw new IllegalArgumentException("flagIndex out of bounds: " + flagIndex);
        }
        return nativeGetBitFlag(mPtr, flagIndex);
    }
    ...
    @CriticalNative
    private static native void nativeSetBitFlag(long ptr, int flagIndex, boolean val);

    @CriticalNative
    private static native boolean nativeGetBitFlag(long ptr, int flagIndex);
