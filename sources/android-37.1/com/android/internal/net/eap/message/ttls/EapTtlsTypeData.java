...
public class EapTtlsTypeData {
    ...
    /**
     * Size limit for a TLS message allowing for worst-case protection overhead according to section
     * 6.2.3. "Payload Protection" of RFC 5246 TLS 1.2.
     *
     * <p>Formula: 4 * (TLS_MAX_FRAGMENT_LEN + 2048) where TLS_MAX_FRAGMENT_LEN = 16384 (16KB
     * plaintext limit). Worst-case record size = 16384 + 2048 = 18432 bytes. Maximum of 4 records
     * in a single flight = 18432 * 4 = 73728 bytes (72KB).
     */
    static final int MAX_LEN_BYTES = 72 * 1024;
    ...
    private static void validateMessageLength(int messageLength) throws EapTtlsParsingException {
        if (messageLength < 0 || messageLength > MAX_LEN_BYTES) {
            throw new EapTtlsParsingException("Invalid message length: " + messageLength);
        }
    }
