...
final class IkeEncryptedPayloadBody {
    ...
    static byte[] combinedModeDecrypt(
            ...
            byte[] decryptionKey,
            byte[] iv)
            throws AEADBadTagException, BadPaddingException {
    ...
    private static byte[] stripPadding(byte[] paddedPlaintext) throws BadPaddingException {
        if (paddedPlaintext.length < PAD_LEN_LEN) {
            // Reject plaintext shorter than the minimum required length (PAD_LEN_LEN) to prevent
            // out-of-bounds array read during padding parsing.
            throw new BadPaddingException("Decrypted plaintext is too short");
        }
        ...
        if (decryptedDataLen < 0) {
            // Prevent negative array size allocation to mitigate potential out-of-bounds array
            // access (ArrayIndexOutOfBoundsException/IllegalArgumentException) which could cause a
            // Denial of Service (DoS).
            throw new BadPaddingException(
                    "Decrypted data length is negative: " + decryptedDataLen);
        }
