 ...
 *
 * <p>PRFs accept keys of any length, but have a preferred key size. The preferred key size MUST be
 * used as the length of SK_d, SK_pi, and SK_pr.
 * <ul>
 *   <li>For PRFs based on the HMAC construction, the preferred key size is equal to the length of
 *       the output of the underlying hash function (RFC 7296). HMAC by its nature will accept keys
 *       of any length. Thus, HMAC-based PRF does not need to do additional modification to the
 *       keys.
 *   <li>PRF-AES-XCBC's preferred key size is 128 bits (RFC 4434). PRF-AES-XCBC needs to modify the
 *       user-provided keys following the rules in RFC 4434.
 * </ul>
 *
