...
//    @DataClass.ParcelWith(ParsingUtils.StringPairListParceler.class)
    ...
    @NonNull
    private final List<MemoryBudget> memoryBudgets = new ArrayList<>();
    @NonNull
    @DataClass.ParcelWith(Parcelling.BuiltIn.ForInternedStringSet.class)
    ...
    @NonNull
    private ArrayMap<String, SparseArray<ArraySet<ParsedMainComponent>>>
            mProcessToComponentMapping = new ArrayMap<>();

    @NonNull
    @Override
    public ArrayMap<String, SparseArray<ArraySet<ParsedMainComponent>>>
            getProcessToComponentMapping() {
        return mProcessToComponentMapping;
    }

    private void addComponentToProcessMapping(ParsedMainComponent component, int componentType) {
        final String processName = component.getProcessName();
        if (processName == null) {
            return;
        }

        SparseArray<ArraySet<ParsedMainComponent>> components =
                mProcessToComponentMapping.get(processName);
        if (components == null) {
            components = new SparseArray<>();
            mProcessToComponentMapping.put(processName, components);
        }
        ArraySet<ParsedMainComponent> typeComponents = components.get(componentType);
        if (typeComponents == null) {
            typeComponents = new ArraySet<>();
            components.put(componentType, typeComponents);
        }
    ...
    public PackageImpl addActivity(ParsedActivity parsedActivity) {
        ...
        addComponentToProcessMapping(parsedActivity, PackageManager.TYPE_ACTIVITY);
        return this;
    }
    ...
    public PackageImpl addProvider(ParsedProvider parsedProvider) {
        ...
        addComponentToProcessMapping(parsedProvider, PackageManager.TYPE_PROVIDER);
        return this;
    }
    ...
    public PackageImpl addReceiver(ParsedActivity parsedReceiver) {
        ...
        addComponentToProcessMapping(parsedReceiver, PackageManager.TYPE_RECEIVER);
        return this;
    }
    ...
    public PackageImpl addService(ParsedService parsedService) {
        ...
        addComponentToProcessMapping(parsedService, PackageManager.TYPE_SERVICE);
        return this;
    }
...
//        appInfo.hiddenUntilInstalled
        ...
        appInfo.memoryBudgets.clear();
        appInfo.memoryBudgets.addAll(memoryBudgets);
...
//        appInfo.uid
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeTypedList(this.memoryBudgets);
    }
    ...
    public PackageImpl(@NonNull Parcel in, @Nullable ParsingPackageUtils.Callback callback) {
        ...
        in.readTypedList(this.memoryBudgets, MemoryBudget.CREATOR);
        assignDerivedFields();
        assignDerivedFields2();
    ...
    @Override
    public PackageImpl addMemoryBudget(MemoryBudget budget) {
        // This method will throw if the incoming budget is incompatible with the budgets that are
        // already in the list.
        MemoryBudget.addBudget(memoryBudgets, budget);
        return this;
    }

    @NonNull
    @Override
    public List<MemoryBudget> getMemoryBudgets() {
        return Collections.unmodifiableList(memoryBudgets);
    }

    @Override
    public boolean hasProcessMemoryBudget() {
        for (ParsedProcess p : processes.values()) {
            if (!p.getMemoryBudgets().isEmpty()) {
                return true;
            }
        }
        return false;
