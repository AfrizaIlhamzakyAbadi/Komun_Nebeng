...
import android.util.apk.InitAppScanMetrics;
...
import com.android.internal.pm.pkg.component.MemoryBudgetParseUtils;
...
public class ParsingPackageUtils {
    ...
    private ParseResult<ParsingPackage> parseClusterPackage(ParseInput input, File packageDir,
        ...
        try {
            ...
            ParseResult<ParsingPackage> validateResult =
                    validateSingleUserProcess(input, result.getResult());
            if (validateResult.isError()) {
                return input.error(validateResult);
            }
            return input.success(pkg);
    ...
    private ParseResult<ParsingPackage> validateSingleUserProcess(ParseInput input,
            ParsingPackage pkg) {
        if (!android.multiuser.Flags.singletonProcessPermissionFeature()) {
            return input.success(pkg);
        }
        for (ParsedProcess process : pkg.getProcesses().values()) {
            if (!process.isSingleUser()) {
                continue;
            }
            final String processName = process.getName();
            // Each component type needs to be checked against flags declared by implementation of
            // ParsedMainComponent. Activities and receivers are implemented by the same class,
            // however receivers flags can be singleUser or systemUserOnly, where as activities are
            // only systemUserOnly.
            final SparseArray<ArraySet<ParsedMainComponent>> components =
                    pkg.getProcessToComponentMapping().get(processName);
            if (components == null) {
                continue;
            }

            for (int i = 0; i < components.size(); i++) {
                final int componentType = components.keyAt(i);
                final ArraySet<ParsedMainComponent> componentList = components.valueAt(i);
                final int flags;
                final String componentTypeName;
                switch (componentType) {
                    case PackageManager.TYPE_ACTIVITY -> {
                        // Check if the activity is in the single user process and is not a system
                        // user only activity. Single user activities are not supported.
                        flags = ActivityInfo.FLAG_SYSTEM_USER_ONLY;
                        componentTypeName = "Activity";
                    }
                    case PackageManager.TYPE_SERVICE -> {
                        // Check if the service is in the single user process and is not a system
                        // user only service or a single user service.
                        flags = ServiceInfo.FLAG_SINGLE_USER | ServiceInfo.FLAG_SYSTEM_USER_ONLY;
                        componentTypeName = "Service";
                    }
                    case PackageManager.TYPE_PROVIDER -> {
                        // Check if the provider is in the single user process and is not a system
                        // user only provider or a single user provider.
                        flags = ProviderInfo.FLAG_SINGLE_USER | ProviderInfo.FLAG_SYSTEM_USER_ONLY;
                        componentTypeName = "Provider";
                    }
                    case PackageManager.TYPE_RECEIVER -> {
                        // Check if the receiver is in the single user process and is not a system
                        // user only receiver or a single user receiver.
                        flags = ActivityInfo.FLAG_SINGLE_USER | ActivityInfo.FLAG_SYSTEM_USER_ONLY;
                        componentTypeName = "Receiver";
                    }
                    default -> {
                        continue;
                    }
                ...
                ParseResult<ParsingPackage> result =
                        validateSingleUserComponents(input, componentList, flags,
                            componentTypeName, processName);
                if (result != null) {
                    return result;
                }
            }
    ...
    private ParseResult<ParsingPackage> validateSingleUserComponents(
            ParseInput input,
            ArraySet<ParsedMainComponent> components,
            int flags,
            String componentTypeName,
            String processName) {
        if (components != null) {
            for (ParsedMainComponent component : components) {
                if ((component.getFlags() & flags) == 0) {
                    return input.error(
                            INSTALL_PARSE_FAILED_MANIFEST_MALFORMED,
                            componentTypeName
                                    + " "
                                    + component.getName()
                                    + " is part of a singleUser process "
                                    + processName
                                    + " but doesn't have necessary attribute"
                                    + " systemUserOnly=true or singleUser=true`");
                }
            }
        }
        return null;
    ...
    private ParseResult parseBaseAppChildTag(ParseInput input, String tag, ParsingPackage pkg,
        ...
        switch (tag) {
            ...
            case "memory-budget":
                if (Flags.memoryBudgetManifest()) {
                    final ParseResult<MemoryBudget> memoryBudgetResult =
                            MemoryBudgetParseUtils.parseMemoryBudget(input, pkg, res, parser);
                    if (memoryBudgetResult.isSuccess()) {
                        MemoryBudget budget = memoryBudgetResult.getResult();
                        if (budget != null) {
                            pkg.addMemoryBudget(budget);
                        }
                        return input.success(pkg);
                    } else {
                        return input.error(memoryBudgetResult.getErrorMessage());
                    }
                } else {
                    return ParsingUtils.unknownTag("<application>", pkg, parser, input);
                }
    ...
    public static ParseResult<SigningDetails> getSigningDetails(ParseInput input,
        ...
        return getSigningDetails(input, pkg.getBaseApkPath(), pkg.isStaticSharedLibrary(),
                pkg.getTargetSdkVersion(), pkg.getSplitCodePaths(), skipVerify, fsvMetaSupplier);
    }
     ...
     * all APK contents are signed correctly and consistently.
     *
     * This overload does not specify a fsv_meta supplier for fs-verity fast-tracking.
     */
    @CheckResult
    public static ParseResult<SigningDetails> getSigningDetails(ParseInput input,
            ParsedPackage pkg, boolean skipVerify) {
        return getSigningDetails(input, pkg, skipVerify, null /* fsvMetaSupplier */,
                null /* metrics */);
    }
    ...
    @CheckResult
    public static ParseResult<SigningDetails> getSigningDetails(ParseInput input,
            ParsedPackage pkg, boolean skipVerify, Supplier<File> fsvMetaSupplier,
            InitAppScanMetrics metrics) {
        return getSigningDetails(input, pkg.getBaseApkPath(), pkg.isStaticSharedLibrary(),
                pkg.getTargetSdkVersion(), pkg.getSplitCodePaths(), skipVerify, fsvMetaSupplier,
                metrics);
    }
    ...
    public static ParseResult<SigningDetails> getSigningDetails(ParseInput input,
        ...
        return getSigningDetails(input, baseApkPath, isStaticSharedLibrary, targetSdkVersion,
                splitCodePaths, skipVerify, fsvMetaSupplier, null /* metrics */);
    }
    ...
    /**
     * Verifies the signatures of a specific APK path against existing signing details.
     *
     * This overload does not specify a fsv_meta supplier for fs-verity fast-tracking.
     */
    @CheckResult
    public static ParseResult<SigningDetails> getSigningDetails(ParseInput input,
        ...
        return getSigningDetails(input, baseCodePath, skipVerify, isStaticSharedLibrary,
                existingSigningDetails, targetSdk, null /* fsvMetaSupplier */);
    }

    /**
     * Verifies the signatures of a specific APK path against existing signing details,
     * optionally fast-tracking verification using the provided fsv_meta supplier.
     */
    @CheckResult
    public static ParseResult<SigningDetails> getSigningDetails(ParseInput input,
            String baseCodePath, boolean skipVerify, boolean isStaticSharedLibrary,
            ...
            Supplier<File> fsvMetaSupplier) {
        return getSigningDetails(input, baseCodePath, skipVerify, isStaticSharedLibrary,
                existingSigningDetails, targetSdk, fsvMetaSupplier, null /* metrics */);
    }

    /**
     * Verifies the signatures of a specific APK path against existing signing details,
     * optionally fast-tracking verification using the provided fsv_meta supplier and
     * metrics.
     */
    @CheckResult
    public static ParseResult<SigningDetails> getSigningDetails(ParseInput input,
